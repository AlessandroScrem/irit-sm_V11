...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Given a surface - compute its unnormalized normal vector field surface, as M
* the cross product if its partial derivatives.                              M
*                                                                            *
* PARAMETERS:                                                                M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Given a planar surface - compute its Jacobian scalar field surface, as     M
* the cross product if its partial derivatives.                              M
*                                                                            *
* PARAMETERS:                                                                M
*   Srf:       To compute the Jacobian scalar field for.	             M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdSrfStruct *:   A scalar field representing the Jacobain of the given M
*                      planar Srf. 		                             M
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbSrfNormalSrf, MvarCalculateTVJacobian                                M
*                                                                            *
* KEYWORDS:                                                                  M
*   Symb2DSrfJacobian, normal, symbolic computation                          M
*****************************************************************************/
CagdSrfStruct *Symb2DSrfJacobian(const CagdSrfStruct *Srf)
{
	...
	*PSrfZ = NULL;
    ...
    SymbSrfSplitScalar(DuSrf, &Srf1W, &Srf1X, &Srf1Y, &Srf1Z);
    SymbSrfSplitScalar(DvSrf, &Srf2W, &Srf2X, &Srf2Y, &Srf2Z);
    CagdSrfFree(DuSrf);
    CagdSrfFree(DvSrf);
    ...
    if (!CagdMakeSrfsCompatible(&PSrfW, &PSrfZ, TRUE, TRUE, TRUE, TRUE))
	SYMB_FATAL_ERROR(SYMB_ERR_SRF_FAIL_CMPT);

    CrossProdSrf = SymbSrfMergeScalar(PSrfW, PSrfZ, NULL, NULL);
    CagdSrfFree(PSrfZ);
    CagdSrfFree(PSrfW);
