...
#define SYMB_SELF_INTER_REL_EPS		100
#define SYMB_HAUSDORFF_DIST_PTS_EPS	1e-9
...
static CagdPType *SymbHausDistSampleCrv(const CagdCrvStruct *Crv, int Samples);
static CagdPType *SymbHausDistSampleSrf(const CagdSrfStruct *Srf, int Samples);
				     ...
				     CagdBType SelfInter)
{
	...
	*XPts = Srf -> Points[1];
	...
	for (j = 0; j < VLength; j++) {
		...
		if (SelfInter) {
		    CagdRType
			SelfInterEps = Epsilon * SYMB_SELF_INTER_REL_EPS;
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Sample uniform samples from the given Crv.	                             *
*                                                                            *
* PARAMETERS:                                                                *
*   Crv:     To sample uniformly.                                            *
*   Samples: Number of samples.                                              *
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdRType *:   Vector of E3 points samples of size Samples, allocated    *
*                  dynamically.					             *
*****************************************************************************/
static CagdPType *SymbHausDistSampleCrv(const CagdCrvStruct *Crv, int Samples)
{
    CagdRType TMin, TMax, TMax1, Dt, t;
    CagdPType *Vec, *d;

    if (Samples < 2)
        return NULL;

    Vec = IritMalloc(Samples * sizeof(CagdPType));

    CagdCrvDomain(Crv, &TMin, &TMax);
    Dt = (TMax - TMin - SYMB_HAUSDORFF_DIST_PTS_EPS) / (Samples - 1);
    TMax1 = TMax + Dt * 0.5;
    for (t = TMin, d = Vec; t < TMax1; t += Dt, d++) {
        CagdRType
	    *R = CagdCrvEval(Crv, t);

	CagdCoerceToE3(*d, &R, -1, Crv -> PType);
    }
    assert(&Vec[Samples] == d && IRIT_APX_EQ(t, TMax + Dt));
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Sample uniform samples from the given Srf.                               *
*                                                                            *
* PARAMETERS:                                                                *
*   Srf:     To sample uniformly.                                            *
*   Samples: Number of samples as (Samples x Samples).                       *
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdRType *:   Vector of E3 points of size (Samples X samples),	     *
*                  allocated dynamically.				     *
*****************************************************************************/
static CagdPType *SymbHausDistSampleSrf(const CagdSrfStruct *Srf, int Samples)
{
    CagdRType UMin, UMax, UMax1, Du, u, VMin, VMax, VMax1, Dv, v;
    CagdPType *Vec, *d;

    if (Samples < 2)
        return NULL;

    Vec = IritMalloc(IRIT_SQR(Samples) * sizeof(CagdPType));

    CagdSrfDomain(Srf, &UMin, &UMax, &VMin, &VMax);
    Du = (UMax - UMin - SYMB_HAUSDORFF_DIST_PTS_EPS) / (Samples - 1);
    UMax1 = UMax + Du * 0.5;
    Dv = (VMax - VMin - SYMB_HAUSDORFF_DIST_PTS_EPS) / (Samples - 1);
    VMax1 = VMax + Dv * 0.5;
    for (v = VMin, d = Vec; v < VMax1; v += Dv) {
        for (u = UMin; u < UMax1; u += Du, d++) {
	    CagdRType
	        *R = CagdSrfEval(Srf, u, v);

	    CagdCoerceToE3(*d, &R, -1, Srf -> PType);
	}
    }
    assert(&Vec[IRIT_SQR(Samples)] == d &&
	   IRIT_APX_EQ(u, UMax + Du) &&
	   IRIT_APX_EQ(v, VMax + Dv));
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Compute the Hausdorff Distance (HD) between two vectors of points.       M
*                                                                            *
* PARAMETERS:                                                                M
*   V1, V2:      The two vectors to consider.                                M
*   V1Size, V2Size:    Lengths of vectors V1 and V2.                         M
*   HDistSide:   1 for HD from V1 to V2, 2 for HD from V2 to V1, 3 for a     M
*                symmetric HD.                                               M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdRType:   Computed HD.                                                M
*                                                                            *
* SEE ALSO:                                                                  M
*    SymbHausDistBySamplesCrvCrv                                             M
*                                                                            *
* KEYWORDS:                                                                  M
*   SymbHausDistOfSamplefPts                                                 M
*****************************************************************************/
				   ...
				   int HDistSide)
{
    ...
    if (HDistSide & 0x01) {
		...
		if ((d = IRIT_PT_PT_DIST_SQR(R1, R2)) < MinDist)
		    MinDist = d;
	    }

	    HD = IRIT_MAX(HD, MinDist);
	}
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Given two curves, compute an Hausdorff distance bound by sampling two      M
* dense sets of sampled points and estimating the Hausdorff distance between M
* the two curves by the sampled points.					     M
*                                                                            *
* PARAMETERS:                                                                M
*   Crv1, Crv2:  The two curves, Crv1(u) and Crv2(v), to estimate their      M
*                Hausdorff distance.					     M
*   Samples:     Number of samples to take, uniformly in parametric space.   M
*   HDistSide:   1 for HD from V1 to V2, 2 for HD from V2 to V1, 3 for a     M
*                symmetric HD.                                               M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdRType:   An upper bound on the Hausdorff distance.                   M
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbHausDistOfSamplefPts						     M
*                                                                            *
* KEYWORDS:                                                                  M
*   SymbHausDistBySamplesCrvCrv, curve curve distance                        M
*****************************************************************************/
...
{
    CagdPType
        *V1 = SymbHausDistSampleCrv(Crv1, Samples),
        *V2 = SymbHausDistSampleCrv(Crv2, Samples);
    CagdRType
        HD = SymbHausDistOfSamplefPts(V1, V2, Samples, Samples, HDistSide);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Given a curve and a surface, compute an Hausdorff distance bound by        M
* sampling two dense sets of sampled points and estimating the Hausdorff     M
* distance between the two shapes by the sampled points.		     M
*                                                                            *
* PARAMETERS:                                                                M
*   Crv1, Srf2:  The two curves and surfaces, Crv(t) and Srf2(u, v), to      M
*                estimate their Hausdorff distance.			     M
*   Samples:     Number of samples to take, uniformly in parametric space,   M
*                as (Samples x Samples) samples.			     M
*   HDistSide:   1 for HD from V1 to V2, 2 for HD from V2 to V1, 3 for a     M
*                symmetric HD.                                               M
*                                                                            *
* RETURN VALUE:                                                              M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbHausDistOfSamplefPts, SymbHausDistBySamplesSrfSrf		     M
*                                                                            *
* KEYWORDS:                                                                  M
*   SymbHausDistBySamplesCrvSrf, curve surface distance		             M
*****************************************************************************/
				      ...
				      int HDistSide)
{
    CagdPType
        *V1 = SymbHausDistSampleCrv(Crv1, Samples),
        *V2 = SymbHausDistSampleSrf(Srf2, Samples);
    CagdRType
        HD = SymbHausDistOfSamplefPts(V1, V2, Samples,
				      IRIT_SQR(Samples), HDistSide);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Given two surfaces, compute an Hausdorff distance bound by sampling two    M
* dense sets of sampled points and estimating the Hausdorff distance between M
* the two surfaces by the sampled points.				     M
*                                                                            *
* PARAMETERS:                                                                M
*   Srf1, Srf2:  The two surfaces, Srf1(u) and Srf2(v), to estimate their    M
*                Hausdorff distance.					     M
*   Samples:     Number of samples to take, uniformly in parametric space,   M
*                as (Samples x Samples) samples.			     M
*   HDistSide:   1 for HD from V1 to V2, 2 for HD from V2 to V1, 3 for a     M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   SymbHausDistBySamplesSrfSrf, surface surface distance	             M
*****************************************************************************/
...
{
    CagdPType
        *V1 = SymbHausDistSampleSrf(Srf1, Samples),
        *V2 = SymbHausDistSampleSrf(Srf2, Samples);
    CagdRType
        HD = SymbHausDistOfSamplefPts(V1, V2, IRIT_SQR(Samples),
				      IRIT_SQR(Samples), HDistSide);
