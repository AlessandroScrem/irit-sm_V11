...
*   SubdivTol:    Accuracy of computation.  0.001 will be a good start.	     M
				   ...
				   CagdBType Euclidean)
{
	...
	*ProjNrmlSrf = SymbSrfVecDotProd(NrmlSrf, VDir);
    ...
    Cntrs = UserCntrSrfWithPlane(ProjNrmlSrf, Plane, SubdivTol, TRUE);
...
* VDir.	 Equal to < S(u, v) x N(u, v), VDir > = 0.			     M
*                                                                            *
* PARAMETERS:                                                                M
					...
					CagdBType Euclidean)
{
	...
	*ProjPlrNrmlSrf = SymbSrfVecDotProd(PlrNrmlSrf, VDir);
    ...
    Cntrs = UserCntrSrfWithPlane(ProjPlrNrmlSrf, Plane, SubdivTol, TRUE);
				 ...
				 CagdBType Euclidean)
{
	...
	*TSrf3;
    ...
    if (IRIT_APX_EQ(Theta, 90.0))
        return SymbSrfSilhouette(Srf, VDirNormalized, SubdivTol, Euclidean);
    ...
    Cntrs = UserCntrSrfWithPlane(TSrf3, Plane, SubdivTol, TRUE);
