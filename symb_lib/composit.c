/******************************************************************************
* Composit.c - crv-crv and crv-srf Composition computation (symbolic).        *
*******************************************************************************
...
*   SymbComposeSrfCrv, MvarComposeTVSrf					     M
*                                                                            *
* KEYWORDS:                                                                  M
					   ...
					   const CagdCrvStruct *Crv2)
{
	...
	*CmpsCrv, *CmpsCrvA, *CmpsCrvB;
	...
	ExtremeSet = SymbCrvZeroSet(TCrv, 1, SYMB_CC_COMPOSE_EPS, TRUE);
	CagdCrvFree(TCrv);
	if (ExtremeSet == NULL) {
	    int l = Crv2 -> Length;
	    CagdRType
...
*                                                                            *
* PARAMETERS:                                                                M
*   CCrv1, CCrv2: The two curves to compose together.                        M
					 ...
					 CagdRType Epsilon)
{
    ...
    CagdPtStruct
	*BndryPts = NULL;
    CagdCrvStruct *Crv1, *Crv2, *TCrv2, *Crvs2, *CrvComp, *TCrv, *RetCrv;
    CagdBBoxStruct BBox;

    if (CAGD_IS_BSPLINE_CRV(CCrv1) && !BspCrvHasOpenEC(CCrv1))
	Crv1 = CagdCnvrtFloat2OpenCrv(CCrv1);
    else
	Crv1 = CagdCrvCopy(CCrv1);
    if (CAGD_IS_BSPLINE_CRV(CCrv2) && !BspCrvHasOpenEC(CCrv2))
	Crv2 = CagdCnvrtFloat2OpenCrv(CCrv2);
    else
	...
	/* Split Crv2 at all wrapping locations. */
        Crvs2 = CagdCrvSubdivAtParams(Crv2, BndryPts, SYMB_PER_COMP_EPS * 10,
				      &Proximity);
	CagdPtFreeList(BndryPts);

	RetCrv = NULL;
	for (TCrv2 = Crvs2; TCrv2 != NULL; TCrv2 = TCrv2 -> Pnext) {
	    CrvComp = SymbComposePeriodicCrvCrv(Crv1, TCrv2, Epsilon);
	    if (RetCrv == NULL)
	        RetCrv = CrvComp;
	    else {
	        TCrv = CagdMergeCrvCrv(RetCrv, CrvComp, TRUE);
		CagdCrvFree(RetCrv);
		CagdCrvFree(CrvComp);
		RetCrv = TCrv;
	    }
	...
	if (t < TMin1 || t > TMax1) {
	    TCrv2 = CagdCrvCopy(Crv2);
	    Trans[0] = t < TMin1 ? TMax1 - TMin1 : TMin1 - TMax1;
	    CagdCrvTransform(TCrv2, Trans, 1.0);

	    RetCrv = SymbComposePeriodicCrvCrv(Crv1, TCrv2, Epsilon);
	    CagdCrvFree(TCrv2);
	}
	else
...
{
    CagdSrfStruct
	*CacheSrfs = (CagdSrfStruct *) AttrGetRefPtrAttrib(Srf -> Attr, "_csc");
					    ...
					    const CagdCrvStruct *Crv)
{
	...
	*CmpsCrv, *TCrv, *CmpsTCrv;
	...
	if (GlblComposeSrfCrvCache &&
	    (Srf1 = (CagdSrfStruct *) AttrGetRefPtrAttrib(Srf -> Attr,
						          "_csc")) != NULL) {
	    Srf2 = Srf1 -> Pnext;
	}
	else {
	    ...
	    if (GlblComposeSrfCrvCache)
	        AttrSetRefPtrAttrib(&((CagdSrfStruct *) Srf) -> Attr,
			            "_csc", Srf1);
	}
					 ...
					 CagdRType Epsilon)
{
	...
	*BndryPts = NULL;
	...
	/* Lets see if the end points are almost the same. */
	CagdCrvDomain(Crv1c, &TMin, &TMax);
	R = CagdCrvEval(Crv1c, TMax);
	CagdCoerceToE3(Pt1, &R, -1, Crv1c -> PType);
	CagdCrvDomain(Crv2c, &TMin, &TMax);
	R = CagdCrvEval(Crv2c, TMin);
	CagdCoerceToE3(Pt2, &R, -1, Crv2c -> PType);
	if (IRIT_PT_APX_EQ_EPS(Pt1, Pt2, Epsilon * 10.0)) {
