...
#define SYMB_BIARC_MAX_RADIUS		1e4
#define SYMB_BIARC_INFLECTION_TOL	1e-6

IRIT_STATIC_DATA CagdRType
    GlblMaxArcRadius = SYMB_BIARC_MAX_RADIUS;
IRIT_STATIC_DATA CagdCrvStruct
    *GlblDCrv = NULL;

static SymbArcStruct *SymbCrvBiArcApproxC1(const CagdCrvStruct *CCrv,
					   CagdRType Tolerance,
					   CagdRType MaxAngle);
...
* 1. The curve is split at all C^1 discontinuities.			     M
* 2. The curve is split at inflection points, if any.			     M
* 3. Each convex/concave curve region:					     M
...
*	and recursively invoke step 2 on both halves.			     M
*                                                                            *
* PARAMETERS:                                                                M
*   Crv:        2D Curve to approximate using piecewise biarcs.              M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbCrv2DInflectionPts, SymbCrvCubicApprox, SymbCrvBiArcApproxC1         M
*                                                                            *
* KEYWORDS:                                                                  M
				  ...
				  CagdRType MaxAngle)
{
	...
	*KVC1DiscPotential = BspKnotAllC1Discont(Crv -> KnotVector,
	...
	if (BspCrvMeshC1Continuous(Crv, Idx, NULL))
	    break;
    }
	...
	RetVal = (SymbArcStruct *)
	    CagdListAppend(SymbCrvBiArcApprox(Crvs, Tolerance,
					      MaxAngle),
			   SymbCrvBiArcApprox(Crvs -> Pnext, Tolerance,
					      MaxAngle));
	...
	return RetVal;	
	    ...
	    RetVal = SymbCrvBiArcApproxC1(Crv, Tolerance, MaxAngle);
	}
    }

#ifdef DEBUG_DUMP_ARCS
    {
        int Odd = TRUE;
	SymbArcStruct *Arc;

        for (Arc = RetVal; Arc != NULL; Arc = Arc -> Pnext) {
	    CagdCrvStruct *Crv;
	    IPObjectStruct *PCrv;
		...
		Start.Pt[2] = Center.Pt[2] = End.Pt[2] = 0.0;
	        ...
	        Crv = CagdMergePtPt2(Arc -> Pt1, Arc -> Pt2);

	    }

	    PCrv = IPGenCRVObject(Crv);
	    AttrSetObjectRGBColor(PCrv, Odd ? 255 : 0, Odd ? 0 : 255, 0);
	    IPStderrObject(PCrv);
	    IPFreeObject(PCrv);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes a piecewise biarc approximation to given C^1 planar curve.      M
*   The following steps are performed during this approximation process:     M
				    ...
				    CagdRType MaxAngle)
{
	...
	*AllArcs = NULL;
    ...
    /* Maximal radius to consider for the arcs. */
    GlblMaxArcRadius = CagdCrvArcLenPoly(CCrv) * SYMB_BIARC_MAX_RADIUS;
	...
	CagdRType
	    *KV = Crv -> KnotVector;
	int i1, i2, 
	    Len = Crv -> Order + Crv -> Length;
	CagdCrvStruct *Crvs;

	if (IRIT_APX_EQ(Pt -> Pt[0], TMin) ||
	    IRIT_APX_EQ(Pt -> Pt[0], TMax))
	    continue;
	...
	Arcs = SymbCrvBiArcApproxAux2(Crvs, Tolerance);
    ...
    Arcs = SymbCrvBiArcApproxAux2(Crv, Tolerance);
    AllArcs = (SymbArcStruct *) CagdListAppend(AllArcs, Arcs);
...
*   Let:								     *
* + curve end points' be P1 and P3, with (unnormalized) tangents V1 and V3.  *
					     ...
					     CagdRType Tolerance)
{
	...
	Split = FALSE;
    CagdRType a, *R, TMin, TMax, LenV31, LenT31, CosBeta, A, B, C, Disc,
        LenTan1, LenTan3, t, Rad1, Rad2;
    CagdPType Pt1, Pt2, Pt3, Pt12, Pt23, Cntr1, Cntr2;
    CagdVType Tan1, Tan3, Tan2, V31, T13;
    CagdCrvStruct *Crvs;
	...
	/* Domain is too small - the best we can do is add line in between. */
	Arcs1 = SymbArcNew(FALSE);
	IRIT_PT_COPY(Arcs1 -> Pt1, Pt1);
	IRIT_PT_RESET(Arcs1 -> Cntr);
	IRIT_PT_COPY(Arcs1 -> Pt2, Pt3);
	return Arcs1;
    }
