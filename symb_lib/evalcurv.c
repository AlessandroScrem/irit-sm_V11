...
void SymbEvalCrvCurvPrep(CagdCrvStruct *Crv, CagdBType Init)
{
	...
	AttrSetRefPtrAttrib(&Crv -> Attr, "_EvalCurv", DCrvs);
    }
    else {
        int i;

      	if ((DCrvs = (CagdCrvStruct **) AttrGetRefPtrAttrib(Crv -> Attr,
							 "_EvalCurv")) == NULL)
	    return;
...
void SymbEvalSrfCurvPrep(CagdSrfStruct *Srf, CagdBType Init)
{
	...
	AttrSetRefPtrAttrib(&Srf -> Attr, "_EvalCurv", DSrfs);
    }
    else {
        int i;

      	if ((DSrfs = (CagdSrfStruct **) AttrGetRefPtrAttrib(Srf -> Attr,
							 "_EvalCurv")) == NULL)
	    return;
