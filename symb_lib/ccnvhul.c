...
IRIT_STATIC_DATA IrtPlnType
    GlblPlane = { 1.0, 0.0, 0.0, 1e-10 };      /* Used to contour surfaces. */
...
static CagdPtStruct *SymbCHTestOpenPoints(CagdPtStruct *Pts,
					  ...
					  CagdPtStruct *PtOrderedList,
					  CagdCrvStruct *Crvs);
static CagdCrvStruct *SymbCrvListCnvxHullVaDir(CagdCrvStruct *Crvs,
					       CagdRType SubdivTol,
					       int NDir);
...
*   SubdivTol: Of numeric search for the zero set (for surface subdivision). M
*	       A positive value (0.01 is a good start).			     M
*                                                                            *
* RETURN VALUE:                                                              M
...
#   endif /* DUMP_DIAMETER_SYMBOLIC_FUNC */
	...
	Cntrs = UserCntrSrfWithPlane(Srf, GlblPlane, SubdivTol, TRUE);
...
*   SubdivTol: Of numeric search for the zero set (for surface subdivision). *
...
#   endif /* TEST_PERF_CH2_PREPROC */
	...
	Srf = SymbPtCnvxHullF3Srf(Pt, NCrv, VCrv);
    	...
    	CntrEnd -> Pnext = UserCntrSrfWithPlane(Srf, GlblPlane, SubdivTol,
						TRUE);
...
#   endif /* DEBUG_CH_TEST */
    ...
    Cntrs = SymbCHTestZeroSetAux(Crvs, Lines.Pnext, SubdivTol);
...
*   NDir:     The number of  directions for trivial Rejection (8,16 is good).*
*                                                                            *
* RETURN VALUE:                                                              *
...
#	endif /* TEST_PERF_CH_PREPROC */
	...
	ConnectDomainEps(&PtList, ADJUST_PARAM_EPS);
	DeleteSmallDomainEps(&PtList, IRIT_EPS);
	Cntrs = SymbCHTestZeroSetWithExtremLine(Crvs, Pa, Pb, SubdivTol, PtList);
	ProcessContours(Cntrs, &PtList);
	IPFreePolygonList(Cntrs);
...
#	endif /* TEST_PERF_CH_PREPROC */
	...
	Cntrs = SymbCHTestZeroSet(Crvs, SubdivTol, InOrderedCHDomain);
...
#   ifndef NO_OPEN_END_PT_TEST
    ...
    OpenPt = SymbCHTestOpenPoints(OpenPts.Pnext,
				  SubdivTol, InOrderedCHDomain, Crvs);
    CagdPtFreeList(InOrderedCHDomain);
	...
	while (Cnt--) {
	    ...
	    Top = CagdPtCopy(PtQ);
	    Top -> Pnext = NULL;
            OpenPt -> Pnext = SymbCHTestOpenPoints(Top, SubdivTol,
...
{
    return SymbCrvListCnvxHullVaDir(Crvs, SubdivTol, 8);
}
...
*                                                                            M
* Note the above equation yields a scalar value since C(t) is planar. The    M
* resulting set in t contains all the sub-domain in C(t) that is on the      M
* convex hull of C(t).  Connecting these pieces with straight lines yields   M
* the final convex hull curve.						     M
*                                                                            *
* PARAMETERS:                                                                M
...
CagdCrvStruct *SymbCrvCnvxHull(const CagdCrvStruct *Crv, CagdRType SubdivTol)
{
    ...
    RetCrv =  SymbCrvListCnvxHullVaDir(TCrv, SubdivTol, 8);
