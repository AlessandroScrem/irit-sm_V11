...
#define SYMB_CRV_POLY_EPS	  1e-7
#define SYMB_CRV_POLY_OUTREACH	  1e-4

#define SYMB_CRV_OPTI_TOL_PT_COORD(WPoints, Points, Axis, Inx) \
    (WPoints == NULL || WPoints[Inx] == 0.0 ? Points[Axis][Inx] \
					    : Points[Axis][Inx] / WPoints[Inx]);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   If the given curve is huge, simply select Samples points out if its      M
* control polygon.                                                           M
*                                                                            *
* PARAMETERS:                                                                M
						     ...
						     CagdRType Tolerance)
{
	...
	if (Params != NULL)
	    AttrSetRefPtrAttrib(&Poly -> Attr, "SaveParamVals", Params);
