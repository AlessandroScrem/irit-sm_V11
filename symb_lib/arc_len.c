...
#define ARCLEN_CONST_SET_IRIT_EPS 		1e-10
#define SYMB_ARC_LEN_LST_SQR_FIT_FACTOR		10
				 ...
				 CagdRType Epsilon)
{
	...
	*Crv = CAGD_IS_BEZIER_CRV(OrigCrv) ? CagdCnvrtBzr2BspCrv(OrigCrv)
    ...
    Dt = TMax - TMin - IRIT_UEPS;

    if (CAGD_IS_PERIODIC_CRV(Crv)) {
        CagdCrvStruct
	    *NewCrv = CagdCnvrtPeriodic2FloatCrv(Crv);
    ...
    for (i = 0; i < MAX_ALEN_IMPROVE_ITERS; i++) {
        CagdCrvStruct *ScalarCrv2, *ScalarCrvSqr, *ErrorCrv, *ErrorCrvFit;
	CagdRType Min, Max, *SCPoints, *PtKnots,
	    *Points = Crv -> Points[1],
	...
	/* Compute a direct approximation to the inverse function. */
	...
	/* Compute a least squares fit to the inverse function. */
	n = ScalarCrv -> Length * SYMB_ARC_LEN_LST_SQR_FIT_FACTOR;
	PtKnots = IritMalloc(n * sizeof(CagdRType));
	for (t = TMin, j = 0; t <= TMax; t += Dt / (n - 1)) {
	    CagdRType
	        *R = CagdCrvEval(Crv, t),
	        V = IsRational ? R[1] / R[0] : R[1];

	    if (CtlPtList == NULL)
	        CtlPtList = CtlPt = CagdCtlPtNew(CAGD_PT_E1_TYPE);
	    else {
	        CtlPt -> Pnext = CagdCtlPtNew(CAGD_PT_E1_TYPE);
		CtlPt = CtlPt -> Pnext;
	    }
	    CtlPt -> Coords[1] = V >= 0.0 ? sqrt(V) : 0.0;
	    PtKnots[j++] = t;
	}

	ScalarCrv2 = BspCrvInterpolate(CtlPtList, PtKnots,
				       ScalarCrv -> KnotVector,
				       ScalarCrv -> Length,
				       ScalarCrv -> Order, FALSE);
	//CagdCrvSetDomain(ScalarCrv2, TMin, TMax);
	CagdCtlPtFreeList(CtlPtList);
	IritFree(PtKnots);

	/* Compare the two to test the error. */
	ScalarCrvSqr = SymbCrvMult(ScalarCrv2, ScalarCrv2);
	ErrorCrvFit = SymbCrvSub(ScalarCrvSqr, Crv);
	CagdCrvFree(ScalarCrvSqr);

	CagdCrvMinMax(ErrorCrvFit, 1, &Min, &Max);
	CagdCrvFree(ErrorCrvFit);

	if (Min > -Epsilon && Max < Epsilon) {
	    CagdCrvFree(ScalarCrv);
	    ScalarCrv = ScalarCrv2;
	    break;
	}
	else {
	    /* Refine in regions where the error is too large. */
...
* Arc length is estimated by computing the square of Crv's first derivative  M
...
CagdCrvStruct *SymbCrvArcLenSclrCrv(const CagdCrvStruct *Crv,
				    CagdRType Epsilon)
{
    CagdCrvStruct *DerivCrv, *DerivMagSqrCrv, *DerivMagCrv, *ArcLenCrv ,
        *NewCrv = NULL;

    if (CAGD_IS_BSPLINE_CRV(Crv) && Crv -> Periodic) {
...
*   Approximation is achieved by least square fitting of points sampled      M
				...
				int Order)
{
	...
	*PtList = NULL;
    ...
    /* Constructs an open-end knot sequence. */
    ...
    LstSqrCrv = BspCrvInterpolate(PtList, Params, KV,
				  NumCtlPts, Order, FALSE);
    if (Crv -> Periodic) {
	int Len1 = LstSqrCrv -> Length - 1;

	/* Make first and last points the same. */
	for (i = 1; i <= NumDims; i++) {
	    LstSqrCrv -> Points[i][0] =
		LstSqrCrv -> Points[i][Len1] =
		    (LstSqrCrv -> Points[i][0] +
		     LstSqrCrv -> Points[i][Len1]) * 0.5;
	}
    }
