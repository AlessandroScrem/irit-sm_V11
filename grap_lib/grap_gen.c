				  ...
				  int FreeCtlMesh)
{
    ...
    if ((PrepSils = AttrGetObjectRefPtrAttrib(PObj, "_silh")) != NULL) {
...
void IGActiveFreeNamedAttribute(IPObjectStruct *PObj, char *Name)
{
	...
	(SphCones = AttrGetObjectRefPtrAttrib(PObjAttr, "_SphCones")) != NULL)
...
IPObjectStruct *IGGenPolygonSketches(IPObjectStruct *PObj, IrtRType FineNess)
{
	...
	AttrSetObjectRefPtrAttrib(PPtsObj1, "_SphCones",
...
void IGDrawPolySilhouette(IPObjectStruct *PObj)
{
	...
	if ((PrepSils = AttrGetObjectRefPtrAttrib(PObj, "_silh")) == NULL) {
	    ...
	    if (IGGlblCacheGeom)
	        AttrSetObjectRefPtrAttrib(PObj, "_silh", PrepSils);
	}
...
IPObjectStruct *IGGetObjIsoLines(IPObjectStruct *PObj)
{
	...
	if ((PObjPolylines = AttrGetObjectObjAttrib(PObj, "_IsolinesLoRes"))
		...
		AttrGetObjectRefPtrAttrib(PObj, "_IsolinesLoRes");
    }
    else
...
IPObjectStruct *IGGetObjPolygons(IPObjectStruct *PObj)
{
	...
	if ((PObjPolygons = AttrGetObjectObjAttrib(PObj, "_PolygonsLoRes"))
		...
		AttrGetObjectRefPtrAttrib(PObj, "_PolygonsLoRes");
    }
    else
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IGInitSrfMovie, IGClearObjTextureMovieAttr                               M
*                                                                            *
* KEYWORDS:                                                                  M
*   IGInitSrfTexture                                                         M
*****************************************************************************/
int IGInitSrfTexture(IPObjectStruct *PObj)
{
    ...
    if ((Image = AttrGetObjectRefPtrAttrib(PObj, "_ImageTexture")) != NULL)
	return TRUE;

    if (!IrtImgParsePTextureString2(Texture, TextureFName,
				   ScaleXYZ, &FlipXYImage, &NewImage,
    ...
    if (NewImage)
        Image = IrtImgReadImage3(TextureFName, &MaxX, &MaxY, &Alpha);
    else
        Image = IrtImgReadImage2(TextureFName, &MaxX, &MaxY, &Alpha);
	...
	if (FlipHorizontal) {
	    Image = IrtImgFlipHorizontallyImage(Image, MaxX, MaxY, Alpha);
	}
	if (FlipVertical) {
	    Image = IrtImgFlipVerticallyImage(Image, MaxX, MaxY, Alpha);
	}

	/* Images are cached globally so only keep a reference in attr: */
	AttrSetObjectRefPtrAttrib(PObj, "_ImageTexture", Image);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Reads in a texture mapping movie if object has "pmovie" attribute and    M
* set up the texture movie for further processing.                           M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:      Object to extract its texture mapping movie if has one.       M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:       TRUE if object has texture mapping movie, FALSE otherwise.    M
*                                                                            *
* SEE ALSO:                                                                  M
*   IGInitSrfTexture, IGClearObjTextureMovieAttr                             M
*                                                                            *
* KEYWORDS:                                                                  M
*   IGInitSrfMovie                                                           M
*****************************************************************************/
int IGInitSrfMovie(IPObjectStruct *PObj)
{
	...
	Alpha = TRUE,				    /* Get alpha if you can. */
	FlipXYImage = FALSE,
	FlipHorizonalImage = FALSE,
	FlipVerticalImage = FALSE,
        RestartMovie = FALSE,
	HasXYZScale = FALSE,
        NewMovie = TRUE;
    char MovieFName[IRIT_LINE_LEN_LONG];
    const char
        *Movie = AttrGetObjectStrAttrib(PObj, "pmovie");
    IrtImgPixelStruct **Images;
    IrtRType ScaleXYZ[3], TimeSetup[3];

    if (Movie == NULL)
	return FALSE;

    if ((Images = AttrGetObjectRefPtrAttrib(PObj, "_MovieTexture")) != NULL)
	return TRUE;

    if (!IrtMovieParsePMovieString(Movie, MovieFName,
				   ScaleXYZ, &NewMovie,
				   &FlipXYImage, &RestartMovie,
				   TimeSetup, &FlipHorizonalImage,
				   &FlipVerticalImage)) {
        IRIT_WARNING_MSG_PRINTF("Failed to parse texture mapping movie file \"%s\".  Movie ignored\n",
				Movie);
        return FALSE;
    }
    HasXYZScale = !IRIT_APX_EQ(ScaleXYZ[2], IRIT_INFNTY);
    ...
    if (NewMovie)
        Images = IrtMovieReadMovie3(MovieFName, &MaxX, &MaxY, &Alpha);
    else
        Images = IrtMovieReadMovie2(MovieFName, &MaxX, &MaxY, &Alpha);
	...
	if (FlipXYImage) {
	    if ((Height & 0x03) != 0) {
	        IRIT_WARNING_MSG_PRINTF("Image height no aligned 4 - flipping movie of object \"%s\" ignored\n",
					PObj -> ObjName);

	    }
	    else {
	        for (i = 0; Images[i] != NULL; i++)
		    Images[i] = IrtImgFlipXYImage(Images[i],
						  MaxX, MaxY, Alpha);
		IRIT_SWAP(int, Width, Height);
	    }
	}
	...
	if (FlipHorizonalImage) {
	    for (i = 0; Images[i] != NULL; i++)
		Images[i] = IrtImgFlipHorizontallyImage(Images[i],
							MaxX, MaxY, Alpha);
	}

	if (FlipVerticalImage) {
	    for (i = 0; Images[i] != NULL; i++)
		Images[i] = IrtImgFlipVerticallyImage(Images[i],
						      MaxX, MaxY, Alpha);
	}

	AttrSetObjectRefPtrAttrib(PObj, "_MovieTexture", Images);
	AttrSetObjectIntAttrib(PObj, "_MovieWidth", Width);
	AttrSetObjectIntAttrib(PObj, "_MovieHeight", Height);
	AttrSetObjectIntAttrib(PObj, "_MovieAlpha", Alpha);
	AttrSetObjectIntAttrib(PObj, "_MovieRestart", RestartMovie);
	if (!IRIT_PT_EQ_ZERO(TimeSetup))
	    AttrSetObjectObjAttrib(PObj, "_MovieTimes",
				   IPGenPTObject(&TimeSetup[0],
						 &TimeSetup[1],
						 &TimeSetup[2]), FALSE);
	AttrSetObjectRealAttrib(PObj, "_MovieScaleX", ScaleXYZ[0]);
	AttrSetObjectRealAttrib(PObj, "_MovieScaleY", ScaleXYZ[1]);
	...
	return TRUE;
        ...
        IRIT_WARNING_MSG_PRINTF("Failed to read texture mapping movie file \"%s\".  Movie ignored\n",
				Movie);

	/* Remove the "ptexture" attribute so we would not try again... */
        AttrFreeObjectAttribute(PObj, "pmovie");
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Clears all texture/movie attributes, if any from PObj.		     M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:      Object to free all texture/movie attributes.		     M
*                                                                            *
* RETURN VALUE:                                                              M
*   void								     M
*                                                                            *
* SEE ALSO:                                                                  M
*   IGInitSrfTexture, IGInitSrfMovie                                         M
*                                                                            *
* KEYWORDS:                                                                  M
*   IGClearObjTextureMovieAttr                                               M
*****************************************************************************/
void IGClearObjTextureMovieAttr(IPObjectStruct *PObj)
{
    AttrFreeObjectAttribute(PObj, "ptexture");
    AttrFreeObjectAttribute(PObj, "_ImageTexture");
    AttrFreeObjectAttribute(PObj, "_ImageWidth");
    AttrFreeObjectAttribute(PObj, "_ImageHeight");
    AttrFreeObjectAttribute(PObj, "_ImageAlpha");
    AttrFreeObjectAttribute(PObj, "_ImageScaleX");
    AttrFreeObjectAttribute(PObj, "_ImageScaleY");
    AttrFreeObjectAttribute(PObj, "_ImageTexID");

    AttrFreeObjectAttribute(PObj, "pmovie");
    AttrFreeObjectAttribute(PObj, "_MovieTexture");
    AttrFreeObjectAttribute(PObj, "_MovieWidth");
    AttrFreeObjectAttribute(PObj, "_MovieHeight");
    AttrFreeObjectAttribute(PObj, "_MovieAlpha");
    AttrFreeObjectAttribute(PObj, "_MovieRestart");
    AttrFreeObjectAttribute(PObj, "_MovieTimes");
    AttrFreeObjectAttribute(PObj, "_MovieScaleX");
    AttrFreeObjectAttribute(PObj, "_MovieScaleY");
    AttrFreeObjectAttribute(PObj, "_MovieTexIDs");
}
