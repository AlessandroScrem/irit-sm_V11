...
void IGDrawSurfaceGenPolygons(IPObjectStruct *PObj)
{
	...
	if (IRIT_APX_EQ(IGGlblRelLowresFineNess, 1.0)) {
	    AttrSetObjectRefPtrAttrib(PObj, "_PolygonsLoRes", PObjPolygons);
	}
	else {
					    ...
					    IrtRType FineNess)
{
	...
	PPolygons = IPSurface2Polygons(Srf, IGGlblFourPerFlat,
				       ...
				       HasTexture ||
				           HasMovie ||
				           IGGlblAlwaysGenUV,
				       TRUE, IGGlblPolygonOptiApprox);

	if (HasTexture || HasMovie) {
	    CagdRType UMin, UMax, VMin, VMax, UScale, VScale;
		...
		VScale = AttrGetObjectRealAttrib(PObj, "_ImageScaleY");
	        ...
	        UScale = AttrGetObjectRealAttrib(PObj, "_MovieScaleX"),
		VScale = AttrGetObjectRealAttrib(PObj, "_MovieScaleY");
	    }
	    else {
	        assert(0);
		UScale = VScale = 1.0;
...
static void IGDrawSurfaceGenPolylines(IPObjectStruct *PObj)
{
	...
	if (IRIT_APX_EQ(IGGlblRelLowresFineNess, 1.0)) {
	    AttrSetObjectRefPtrAttrib(PObj, "_IsolinesLoRes", PObjPolylines);
	}
	else {
