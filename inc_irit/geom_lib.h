...
#define GM_REF_UPDATE_INTRR_VRTX(V, V1, V2) { \
    IRIT_PT_BLEND((V) -> Coord, (V1) -> Coord, (V2) -> Coord, 0.5); \
    if (IP_HAS_NORMAL_VRTX((V1)) && IP_HAS_NORMAL_VRTX((V2))) { \
	IRIT_VEC_BLEND((V) -> Normal, (V1) -> Normal, (V2) -> Normal, 0.5); \
	IP_SET_NORMAL_VRTX((V)); \
    }; \
    DeformVrtxFctrFunc(V); \
}
...
typedef IrtRType *(*GMPointDeformVrtxDirFuncType)(const IPVertexStruct *V);
typedef IrtRType (*GMPointDeformVrtxFctrFuncType)(IPVertexStruct *V);
...
IRIT_GLOBAL_DATA_HEADER IrtVecType
