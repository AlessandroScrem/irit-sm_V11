...
#define USER_DDM_DEFAULT_POLY_FINENESS  11
...
IRIT_STATIC_DATA const TrimSrfStruct *GlblTrimmedSrf;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Maps the given DDM texture defined in Texture over the trimmed surface   M
* TSrf, duplicating it (UDup x VDup) times over the parametric domain in     M
* non trimmed regions only.						     M
*                                                                            *
* PARAMETERS:                                                                M
*   TSrf:    Trimmed surface to derive a DDM mapping for.                    M
...
*	     employ the UV coordinates of the surface TSrf.		     M
*   Random:  If TRUE, the placement is made at random over the trimmed srf.  M
*                                                                            *
* RETURN VALUE:                                                              M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   UserDDMPolysOverSrf, UserDDMPolysOverPolys                               M
*                                                                            *
* KEYWORDS:                                                                  M
*   UserDDMPolysOverTrimmedSrf                                               M
*****************************************************************************/
					   ...
					   int Random)
{
    ...
    RetVal = UserDDMPolysOverSrf(TSrf -> Srf, Texture, UDup, VDup,
				 LclUV, Random);
...
*   Random:  If TRUE, the placement is made at random over the surface.      M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  A set of objects that derive the DDM surface          M
*	above Srf.							     M
				    ...
				    int Random)
{
    ...
    LclTexture = IPCopyObject(NULL, Texture, FALSE);
    if (!IP_IS_POLY_OBJ(Texture)) {
	IPFreeformConvStateStruct
	    State = IPFFCState;

	State.FineNess = USER_DDM_DEFAULT_POLY_FINENESS;
	State.OptimalPolygons = SYMB_CRV_APPROX_UNIFORM;
	State.DumpObjsAsPolylines = FALSE;
	State.DrawFFGeom = TRUE;
	State.DrawFFMesh = FALSE;
	State.ComputeNrml = TRUE;

        LclTexture = IPConvertFreeForm(LclTexture, &State);    /* In place. */
    }
    ...
    if (Random) {
	    ...
	    n = (int) (UDup * VDup);
        CagdUVType
	    *RandomUVs = SymbUniformAprxPtOnSrfDistrib(Srf, FALSE, n, NULL);

	for (i = 0; i < n; i++) {
	    if (GlblTrimmedSrf != NULL &&
		!TrimIsPointInsideTrimSrf(GlblTrimmedSrf, RandomUVs[i])) {
	        /* UV location is trimmed away - skip it. */
	        continue;
	    }
	    ...
	    IPTraverseObjListHierarchy(LclTexture, Mat, UserOneDDMOverSrf);
	}
	...
	for (GlblU = UMin;
	     GlblU < UMax - GlblDu * 0.500301060;
	     GlblU += GlblDu) {
	    for (GlblV = VMin;
		 GlblV < VMax - GlblDv * 0.500301060;
		 GlblV += GlblDv) {
	        CagdUVType UV;
		...
		UV[1] = GlblV;
		if (GlblTrimmedSrf != NULL &&
		    !TrimIsPointInsideTrimSrf(GlblTrimmedSrf, UV)) {
		    /* UV location is trimmed away - skip it. */
		    continue;
		}

		IPTraverseObjListHierarchy(LclTexture, Mat, 
					   UserOneDDMOverSrf);
	    }
	}
    }
...
*                                                                            *
* PARAMETERS:                                                                *
*   PObj:   Polygonal object to process.  A leaf in the geometry tree.       *
...
static void UserOneDDMOverSrf(IPObjectStruct *PObj, IrtHmgnMatType Mat)
{
	...
	AttrSetObjectUVAttrib(CpObj, "uvvals", GlblU, GlblV);
				      ...
				      int LclUV)
{
	...
	AttrSetRefPtrAttrib(&Pl -> Attr, "_uvExtrm", UVMinMax);
