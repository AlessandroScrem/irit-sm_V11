...
*   SubdivTol:     Of Cone - normal surface intersection, for subdivision    M
					  ...
					  CagdRType Euclidean)
{
	...
	*CntrObjList = NULL;
	...
	if (HasPosVals && HasNegVals) {
	    ...
	    Cntrs = UserCntrSrfWithPlane(SclrSrf, Plane, SubdivTol, TRUE);
...
*   SubdivTol:  Accuracy of parabolic piecewise linear approximation.        M
*                                                                            *
* RETURN VALUE:                                                              M
					...
					CagdRType SubdivTol)
{
	...
	*GaussSrf = SymbSrfGaussCurvature(PSrf, TRUE);
    IPPolygonStruct *Pl, *Plls2,
      *Plls = UserCntrSrfWithPlane(GaussSrf, GaussPlane, SubdivTol, TRUE);
...
*   SubdivTol: Of computation.  If negative the function whose zero set is   M
				...
				CagdRType Angle)
{
    ...
    Plls = UserCntrSrfWithPlane(ProdSrf, XPlane, IRIT_ABS(SubdivTol), TRUE);
			       ...
			       CagdBType TrimInvisible)
{
	...
	MVPts = MvarMVsZeros0D(Finals, Constraints, 2, fabs(SubTol), NumTol);
	...
	Plls = UserCntrSrfWithPlane(ProdSrf, XPlane, IRIT_ABS(NumTol), TRUE);
...
*	       deviation from VDir, FALSE for less than.		     M
*   SubdivTol: Subdivision tolerance of freeforms silhouette approx.         M
*                                                                            *
* RETURN VALUE:                                                              M
				       ...
				       CagdRType SubdivTol)
{
    ...
    IPPolygonStruct
	*Pls = SymbSrfIsocline(Srf, VDir, Theta, SubdivTol, FALSE);
    TrimSrfStruct
				       ...
				       CagdRType SubdivTol)
{
	...
	*RuledList = NULL;
	...
	AttrSetRefPtrAttrib(&Pl1 -> Attr, "E3Poly", E3Poly);
	...
	if ((Pl2 = UserMoldMatchPolyline(Pl1, &Pls)) == NULL) {
	    E3Poly = (IPPolygonStruct *) AttrGetRefPtrAttrib(Pl1 -> Attr,
							     "E3Poly");
	...
	if ((Reparam = CagdMatchingTwoCurves(Crv1, Crv2,
					     (int) IRIT_MAX(Len *
						        USER_MOLD_MATCH_REDUCE,
						       3),
					     Len, 3, FALSE, TRUE, TRUE, FALSE,
					     CagdMatchRuled2Norm)) == NULL) {
	    UserFatalError(USER_ERR_INVALID_CURVE);
	    return NULL;
	}
	...
	IPFreePolygon(E3Poly);
	IPFreePolygon(Pl1);
	E3Poly = (IPPolygonStruct *) AttrGetRefPtrAttrib(Pl2 -> Attr,
							 "E3Poly");
	IPFreePolygon(E3Poly);
	IPFreePolygon(Pl2);
					      ...
					      IPPolygonStruct **Pls)
{
    ...
    IPPolygonStruct *Pl2,
	*ClosestPl = NULL,
	*E3Poly = (IPPolygonStruct *) AttrGetRefPtrAttrib(Pl -> Attr,
							  "E3Poly");
	...
	IPPolygonStruct *MinPl,
	    *E3Poly2 = (IPPolygonStruct *) AttrGetRefPtrAttrib(Pl2 -> Attr,
							       "E3Poly");
...
*   Reparam:       Reparameterization function.				     *
				       ...
				       int Reparam2)
{
	...
	*VList = NULL;
	...
	if (!GMPointFrom3Planes(Pln1, Pln2, Pln3, Pt1)) {
	    /* Singularity could occur if isoclines are really far. */
