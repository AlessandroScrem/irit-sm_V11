...
#define USER_NCCTP_MAX_MITER		1.5
...
#define UTIL_NCPTP_CRV_TOLERANCE      (Units == IP_NC_GCODE_UNIT_MM ? 0.1 \
								    : 0.004)
#define UTIL_NCPTP_REF_TOLERANCE      (Units == IP_NC_GCODE_UNIT_MM ? 1 \
								    : 0.04)
...
IRIT_STATIC_DATA IrtRType
    ...
    GlblUserNCCTPMiter = USER_NCCTP_MAX_MITER;
...
static IPPolygonStruct *UserNCRefinePlToTol(IPPolygonStruct *Pl, IrtRType Tol);
static IPPolygonStruct *UserNCContourSlowOnPlunge(IRndrZBuffer1DPtrType
						                  NCCTPZBuf,
						  int MainFlip,
				      ...
				      IPNCGCodeUnitType Units)
{
	...
	*BBoxStr = AttrGetObjectStrAttrib(PObj, "NCCntrBBox");
    int OldCopyRefCount,
        PreciseClip = AttrGetObjectIntAttrib(PObj, "NCCntrPreciseClip"),
        Flip = FALSE;
    IrtRType y,
        SlowOnPlunge = AttrGetObjectRealAttrib(PObj, "NCCntrSlowOnPlunge"),
        MaxDepthStep = AttrGetObjectRealAttrib(PObj, "NCCntrMaxDepthStep"),
        MaxMiterScale = AttrGetObjectRealAttrib(PObj, "NCCntrMaxMiterScale");
	...
	*TPath = NULL;
    ...
    if (IP_ATTR_IS_BAD_INT(PreciseClip))
        PreciseClip = TRUE;

    if (IP_ATTR_IS_BAD_REAL(MaxMiterScale))
        GlblUserNCCTPMiter = USER_NCCTP_MAX_MITER;
    else
        GlblUserNCCTPMiter = MaxMiterScale;

    if (IP_ATTR_IS_BAD_REAL(MaxDepthStep))
        MaxDepthStep = IRIT_INFNTY;
    else if (!IP_ATTR_IS_BAD_REAL(SlowOnPlunge)) {
        /* Cannot set both "NCCntrSlowOnPlunge" and "NCCntrMaxDepthStep". */
        MaxDepthStep = IRIT_INFNTY;
	IRIT_WARNING_MSG_PRINTF("Warning: Cannot set both \"NCCntrSlowOnPlunge\" and \"NCCntrMaxDepthStep\". \"MaxDepthStep\" disabled\n");
    }
    ...
    /* Stage 1 & 2 - tessellate geometry, verify normals and offset. */
    ...
    for (y = BBox.Min[1]; y <= BBox.Max[1] + IRIT_EPS; y += TPathSpace) {
	    ...
	    if (SlowOnPlunge != IRIT_INFNTY && LastPl != NULL) {
	        Pl = UserNCRefinePlToTol(Pl, UTIL_NCPTP_REF_TOLERANCE);
	        TPath = UserNCContourSlowOnPlunge(NCCTPZBuf, Flip, Pl, LastPl,
						  TPath, &BBox, Offset,
						  SlowOnPlunge);
	    }
	    else if (MaxDepthStep != IRIT_INFNTY && LastPl != NULL) {
	        Pl = UserNCRefinePlToTol(Pl, UTIL_NCPTP_REF_TOLERANCE);
    ...
    /* Stage 4 - Move the toolpath down by Offset amount but make sure it   */
    ...
    /* Clip the toolpath to be inside ClipObj in the XY plane. */
	...
	for (Pl = TPath; Pl != NULL; Pl = Pl -> Pnext) {
		...
		*PrevV = NULL,
	    ...
	    if (!GMPolygonPointInclusion(ClipPl, V -> Coord)) {
	        ...
	        if (PreciseClip) {
		    /* Add a start vertex at the clipping polygon boundary. */
		    if (Pl -> PVertex != NULL &&
			PrevV != NULL &&
			GMIsInterLinePolygon2D(ClipPl -> PVertex, 
					       Pl -> PVertex -> Coord, 
					       PrevV -> Coord, &t)) {
		        VTmp = IPAllocVertex2(Pl -> PVertex);
			IRIT_PT_BLEND(VTmp -> Coord,
				      PrevV -> Coord, 
				      Pl -> PVertex -> Coord, t);
			Pl -> PVertex = VTmp;
		    }
		    else if (Pl -> PVertex != NULL && PrevV != NULL) {
		        IRIT_WARNING_MSG_PRINTF("Warning: Failed to precisely clip to clipping polygon.\n");
		    }
		}
	    ...
	    for (PrevV = NULL, V = Pl -> PVertex -> Pnext;
		 V != NULL && GMPolygonPointInclusion(ClipPl, V -> Coord);
		 PrevV = V, V = V -> Pnext);

	    if (PrevV != NULL && V != NULL) {
	        if (PreciseClip) {
		    /* V is out. Add an end vertex at clipping polygon boundary. */
		    if (GMIsInterLinePolygon2D(ClipPl -> PVertex, 
					       PrevV -> Coord, 
					       V -> Coord, &t)) {
			PrevV -> Pnext = IPAllocVertex2(PrevV -> Pnext);
			IRIT_PT_BLEND(PrevV -> Pnext -> Coord, 
				      V -> Coord, PrevV -> Coord, t);
			PrevV = PrevV -> Pnext;
		    }
		    else {
		        IRIT_WARNING_MSG_PRINTF("Warning: Failed to precisely clip to clipping polygon.\n");
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Refine, in place the given polyline so two adjacent vertices are at most *
* Tol appart.                                                                *
*                                                                            *
* PARAMETERS:                                                                *
*   Pl:     Polyline to refine, in place.                                    *
*   Tol:    Maximal distance between adjacent vertices.                      *
*                                                                            *
* RETURN VALUE:                                                              *
*   IPPolygonStruct *:   Refined polyline.                                   *
*****************************************************************************/
static IPPolygonStruct *UserNCRefinePlToTol(IPPolygonStruct *Pl, IrtRType Tol)
{
    IPVertexStruct *V;

    Tol *= Tol;  /* We compare distances squares. */
    ...
    for (V = Pl -> PVertex; V -> Pnext != NULL; ) {
	    ...
	    *VNext = V -> Pnext;
        IrtRType
	    DSqr = IRIT_PT_PT_DIST_SQR(V -> Coord, VNext -> Coord);

	if (DSqr > Tol) {
	    V -> Pnext = IPAllocVertex2(VNext);
	    IRIT_PT_BLEND(V -> Pnext -> Coord, V -> Coord, VNext -> Coord, 0.5);
	}
	else
	    V = V -> Pnext;
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Divides the given NewPl if it dives down more than SlowOnPlunge and set  *
* "RelFeedRate" attributes to slow it down on the divided polylines.         *
*                                                                            *
* PARAMETERS:                                                                *
*   NCCTPZBuf:    The 1D zbuffer we use.                                     *
*   Flip:	  TRUE to reverse NewPl, FALSE to add as is.                 *
*   NewPl:        New contour to add to TPath.	Used in place.		     *
...
*   SlowOnPlunge: If set, deep dives more than this value are slowed down by *
*                 setting relative feedrates, "RelFeedRate", attributes that *
*                 are less than one.					     *
*                                                                            *
* RETURN VALUE:                                                              *
						  ...
						  IrtRType SlowOnPlunge)
{
    ...
    IPVertexStruct *V, *V2, *StartV,
	*NewV = NewPl -> PVertex,
        *LastV = LastPl -> PVertex;
    IPPolygonStruct *Pl,
        *RetList = NULL;

    /* Store the Z differences in the X component of the normal: */
    for (V = NewV; V != NULL; V = V -> Pnext)
        V -> Normal[0] = -IRIT_INFNTY;

    /* Sample the data at the same resolution as we used for the Zbuffer. */
    for (i = 0, x = BBox -> Min[0];
	 i < USER_NCCTP_CNTR_ZBUF_SIZE;
	 i++, x += Dx) {
	if (LastV != NULL && NewV != NULL) {
	    /* Make sure x is between this and next vertex for New/Last Pls. */
	    if (LastV -> Pnext != NULL && x > LastV -> Pnext -> Coord[0])
		LastV = LastV -> Pnext;
	    if (NewV -> Pnext != NULL && x > NewV -> Pnext -> Coord[0])
		NewV = NewV -> Pnext;
		...
		LastZ = IRIT_BOUND(LastZ, BBox -> Min[2], BBox -> Max[2]);
		NewZ = UserNCGetContourZLevel(x, NewV);
		NewZ = IRIT_BOUND(NewZ, BBox -> Min[2], BBox -> Max[2]);

		NewV -> Normal[0] = IRIT_MAX(NewV -> Normal[0], LastZ - NewZ);

		if (NewV -> Pnext != NULL && NewV -> Pnext -> Pnext == NULL)
		    NewV -> Pnext -> Normal[0] = LastZ - NewZ;
	    }
	}
    }

    for (NewV = StartV = NewPl -> PVertex; NewV != NULL; NewV = NewV -> Pnext) {
        assert(NewV -> Normal[0] != -IRIT_INFNTY);   /* We tested all vrtcs. */

	if (NewV -> Normal[0] > SlowOnPlunge) {
	    IrtRType MaxPlunge;

	    /* Found a region which plunges too much - divide poly here. */
	    for (MaxPlunge = 0.0, V = NewV;
		 V != NULL && V -> Normal[0] > SlowOnPlunge;
		 V = V -> Pnext) {
	        MaxPlunge = IRIT_MAX(MaxPlunge, NewV -> Normal[0]);
	    }
	    if (NewV -> Pnext != V) {          /* Ignore plunges too short. */
	        /* Split at NewV and V and mark this poly as too deep. */
	        if (StartV != NewV) {
		    for (V2 = StartV; V2 -> Pnext != NewV; V2 = V2 -> Pnext);
		    V2 -> Pnext = IPCopyVertex(NewV);
		...
		if (V != NULL) {
		    V2 = V -> Pnext;
		    V -> Pnext = V2 == NULL ? NULL : IPCopyVertex(V2);
		}
		else
		    V2 = NULL;
		Pl = IPAllocPolygon(0, NewV, NULL);
		AttrSetRealAttrib(&Pl -> Attr, "RelFeedRate",
				  SlowOnPlunge / MaxPlunge);
		AttrSetRGBColor(&Pl -> Attr, 255, 10, 10);
		IRIT_LIST_PUSH(Pl, RetList);

		if ((NewV = StartV = V2) == NULL)
		    break;
	    }	    
	}
    }

    if (StartV != NULL) {                      /* Add last polyline, if any. */
	if (StartV -> Pnext != NULL) {
	    Pl = IPAllocPolygon(0, StartV, NULL);
	    AttrSetRGBColor(&Pl -> Attr, 255, 255, 10);
	    IRIT_LIST_PUSH(Pl, RetList);
	}
    ...
    if (MainFlip) {
        /* Poly list is already reversed - just reverse the vertices. */
        for (Pl = RetList; Pl != NULL; Pl = Pl -> Pnext)
	    Pl -> PVertex = IPReverseVrtxList2(Pl -> PVertex);

        RetList = IPReversePlList(RetList);
    }

    TPath = IPAppendPolyLists(RetList, TPath);
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Interpolate intermediate plunging steps if the plunge from previous      *
* contour is larger than MaxDepthStep.  The interpolated steps are also      *
						 ...
						 IrtRType MaxDepthStep)
{
    ...
    if ((TooDeepPls = UserNC2ContourZDiff(NewPl, LastPl, BBox,
					  MaxDepthStep)) != NULL) {
...
*   MaxDepthStep:  Maximal depth to allow plunging from last step without    *
*                  intermediate interpolation steps.                         *
*   SlowOnPlunge:  If TRUE, deep dives are slowed down by setting relative   *
*                  feedrates,  "RelFeedRate", attributes that are less than  *
*                  one.							     *
*                                                                            *
* RETURN VALUE:                                                              *
...
static void UserNCCTPSOffsetPolys(const IPObjectStruct *PObj)
{
    ...
    PTmp = IPGenPOLYObject(GMPolyOffset3D(PTriObj -> U.Pl,
					  GlblUserNCCTPOffset,
					  TRUE, GlblUserNCCTPMiter, NULL));

    /* Sanity check that we do have reasonable mesh - non-oriented         */
    /* properly mesh will yield huge offsets due to wrong miter joints'    */
    /* computations.  Scream a warning message in such a case.             */
    {
        int i;
        GMBBBboxStruct BBoxIn, BBoxOut;

	BBoxIn = *GMBBComputeBboxObject(PTriObj);
	BBoxOut = *GMBBComputeBboxObject(PTmp);

	for (i = 0; i < 3; i++) {
	    if (IRIT_ABS(BBoxOut.Max[i] - BBoxIn.Max[i]) > GlblUserNCCTPOffset * 10 ||
		IRIT_ABS(BBoxOut.Min[i] - BBoxIn.Min[i]) > GlblUserNCCTPOffset * 10)
	        break;
	}
	if (i < 3) {
	    IRIT_WARNING_MSG_PRINTF("Warning: Input mesh might be unoriented - offset approximation might be wrong.\n");
	}
    }
					   ...
					   IrtHmgnMatType Mat)
{
	...
	/* Make sure all segments are C^1 continuous. */
					     ...
					     CagdBType TrimSelfInters)
{
	...
	*CrvOffList = NULL;
	...
	for (TCrv = CrvOff; TCrv != NULL; TCrv = TCrv -> Pnext)
	    AttrSetRefPtrAttrib(&TCrv -> Attr, "_OrigCrv", Crv);
    ...
    for (i = 0; i < n; i++) {
        CagdCrvStruct
	    *CrvOrig1 = AttrGetRefPtrAttrib(CrvOffVec[i] -> Attr, "_OrigCrv"),
	    *CrvOrig2 = AttrGetRefPtrAttrib(CrvOffVec[(i + 1) % n] -> Attr,
					    "_OrigCrv");
	...
	AttrSetRefPtrAttrib(&CrvOffVec[i] -> Attr, "_OrigCrv", CrvOrig1);
	AttrSetRefPtrAttrib(&CrvOffVec[(i + 1) % n] -> Attr, "_OrigCrv",
			    CrvOrig2);
    }
