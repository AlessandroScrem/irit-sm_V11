...
static IrtRType UserCrvArngmntFindPlane(CagdCrvStruct *Crvs,
					...
					IrtPlnType Pln);
static CagdCrvStruct *UserCrvArngmntProcessIntersections(
...
*                   USER_CA_SPLIT_C1DISCONT_PTS to split at parameteric C1   V
*                       discontinuities,  according to the knotsequence.     V
*                   USER_CA_SPLIT_REAL_C1DISCONT_PTS to split at actual C1   V
*                       disconts, examined in Euclidean space.		     V
...
*       Params[1] = A list object of pts to examine and split if near them.  V
				     ...
				     const void *Params[])
{
	...
	case USER_CA_OPER_BREAK_INTER:
	    ...
	    TCrvs = NewCA -> CagdCrvs;
	    NewCA -> CagdCrvs = UserCrvArngmntProcessIntersections(TCrvs,
								   InterTol);
						    ...
						    int FitSize)
{
	...
	*NewCrvs = NULL;
	...
	if (Crv -> Order == 2 &&
	    ...
	    (NewCrv = BspCrvFitLstSqr(Crv, 3, NewLength,
					...
					IrtPlnType Pln)
{
	...
	*V = NULL;
    ...
    if (IPVrtxListLen(Pl -> PVertex) < 3) {/* One linear curve was inserted.*/
	IPVertexStruct
	    *V2 = Pl -> PVertex;

	if (IRIT_APX_EQ(V2 -> Coord[2], 0.0) &&
	    IRIT_APX_EQ(V2 -> Pnext -> Coord[2], 0.0)) {
	    /* Use the Z = 0 plane. */
	    Pln[0] = Pln[1] = Pln[3] = 0.0;
	    Pln[2] = 1.0;
	    return IRIT_MAX(V2 -> Coord[2], V2 -> Pnext -> Coord[2]);
	}
	else {
	    int MinAxis = 0;
	    IrtRType MinVal;
	    ...
	    IRIT_VEC_SUB(Vec, V2 -> Pnext -> Coord, V2 -> Coord);

	    /* Add a third point not on the line. */
	    for (MinVal = IRIT_ABS(Vec[0]), i = 1; i < 3; i++) {
		if (IRIT_ABS(Vec[i]) < MinVal) {
		    MinVal = IRIT_ABS(Vec[i]);
		    MinAxis = i;
		}		
	    }

	    /* Add a third point in the min axis direction. */
	    V = IPAllocVertex2(NULL);
	    IRIT_PT_COPY(V -> Coord, V2 -> Coord);
	    Vec[0] = MinAxis == 0;
	    Vec[1] = MinAxis == 1;
	    Vec[2] = MinAxis == 2;
	    IRIT_PT_ADD(V -> Coord, V -> Coord, Vec);
	    IRIT_LIST_PUSH(V, Pl -> PVertex);
	}
    }
...
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdCrvStruct *:  Same list after the splits at intersections.	     *
*****************************************************************************/
static CagdCrvStruct *UserCrvArngmntProcessIntersections(
						   const CagdCrvStruct *Crvs,
						   CagdRType Tolerance)
{
    return CagdCrvCrvInterArrangment(Crvs, TRUE, Tolerance);
}
						      ...
						      UserCASplitType CrvSplit)
{
    ...
    if (CrvSplit & (USER_CA_SPLIT_C1DISCONT_PTS |
		    USER_CA_SPLIT_REAL_C1DISCONT_PTS)) {
        NewCrvs = NULL;
	...
	while (Crvs != NULL) {
	    ...
	    TCrvs = CagdCrvSubdivAtAllC1Discont(Crv,
				CrvSplit & USER_CA_SPLIT_REAL_C1DISCONT_PTS,
			        Tolerance);
...
static int UserCrvArngmntPrepEval(UserCrvArngmntStruct *CA)
{
    ...
    CA -> Regions = (UserCARegionStruct **)
              IritMalloc(sizeof(UserCARegionStruct *) * CA -> AllocSizeCrvs);
    CA -> NumOfRegions = 0;
...
*                                                                            *
* PARAMETERS:                                                                *
*   Crv:  Curve to create a reference structure for.  Used in-place.         *
*                                                                            *
* RETURN VALUE:                                                              *
				 ...
				 IrtRType ZOffset)
{
	...
	AttrSetObjectIntAttrib(PCrv, "RgnID", i + 1);
...
*   Tolerance:    Tolerance of intersection locations' computation.	     M
*   ZOffset:      if positive, offset the n'th region in Z by n*ZOffset.     M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:         TRUE if successful, FALSE otherwise.                        M
