...
*   SubdivTol:  Accuracy of subdivision approximation. 0.01 is a good start. M
	...
	*GaussSrf = SymbSrfGaussCurvature(Srf, TRUE);
	...
	for ( ; V != NULL && PolyObj -> U.Pl != NULL; V = V -> Pnext) {
		...
		if ((NewPolyObj = FFKrnlBooleanAND(PolyObj,
			...
			IRIT_PT_BLEND(Pos2, V -> Coord, V -> Pnext -> Coord,
				      0.5);
				      ...
				      int DecompSrfs)
{
    ...
    MvarPolylineStruct
        *MVPlls = MvarMVsZeros1D(&GaussMV, NULL, 1, Step,
...
#	    ifdef DEBUG
	    {
		IRIT_SET_IF_DEBUG_ON_PARAMETER(_DebugParabLnsTrimmedSrfs,
					       FALSE) {
...
#	    endif /* DEBUG */
	    ...
	    assert(GaussSrf -> PType == CAGD_PT_E1_TYPE);

	    for (TSrf = DSrfs; TSrf != NULL; TSrf = TSrf -> Pnext) {
	        CagdRType
		    *InsideUV = TrimPointInsideTrimmedCrvs(TSrf -> TrimCrvList,
							   TSrf),
		    *R = CagdSrfEval(GaussSrf, InsideUV[0], InsideUV[1]);

		AttrSetIntAttrib(&TSrf -> Attr, "K", IRIT_SIGN(R[1]));
	    }
