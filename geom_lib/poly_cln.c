...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Routine to make sure all polys given are circular/linear.  Update in     M
* place.								     M
*                                                                            *
* PARAMETERS:                                                                M
*   Pls:    List of polys to make sure are circular/linear, in place.        M
...
*   GMVrtxListToCircOrLinDup, IPOpenPolysToClosed, IPClosedPolysToOpen	     M
*                                                                            *
* KEYWORDS:                                                                  M
...
* SEE ALSO:                                                                  M
*   GMCleanUpPolylineList, GMCleanUpPolygonList, GMFilterInteriorVertices    M
*   GMVrtxListToCircOrLin, IPOpenPolysToClosed, IPClosedPolysToOpen						     M
*                                                                            *
* KEYWORDS:                                                                  M
					 ...
					 int n)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrAngleID = ATTRIB_NAME_BAD_NUMBER;
    int l;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAngleID, "_Angle");
	...
	for ( ; V -> Pnext != NULL; V = V -> Pnext) {
	    ...
	    IRIT_VEC_NORMALIZE(V2);
	    AttrSetRealAttrib2(&V -> Attr, AttrAngleID,
	...
	for (V = VHead; V -> Pnext -> Pnext != NULL; ) {
	    if (AttrGetRealAttrib2(V -> Pnext -> Attr,
				   AttrAngleID) < RemoveAngle) {
		IPVertexStruct
		    *VNext = V -> Pnext;
