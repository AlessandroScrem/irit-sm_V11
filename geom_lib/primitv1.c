...
*	        	       1 - surface primitive.                        M
*		               2 - model primitive.                          M
*                              3 - trivariate volumetric primitive.          M
*                                                                            *
* RETURN VALUE:                                                              M
				 ...
				 IrtRType WidthZ)
{
        ...
        case GM_GEN_PRIM_MDLS:
	{
        ...
        case GM_GEN_PRIM_TVS:
	{
	    TrivTVStruct
	        *TVs = TrivNSPrimBox(Pt[0], Pt[1], Pt[2],
				     Pt[0] + WidthX,
				     Pt[1] + WidthY,
				     Pt[2] + WidthZ);

	    return IPGenTRIVARObject(TVs);
	}
        default:
				  ...
				  int Bases)
{
    ...
    GMGenTransMatrixZ2Dir(Mat, Pt, Dir, 1.0);
        ...
        case GM_GEN_PRIM_SRFS:
	{
		...
		*Srf = CagdPrimConeSrf(GlblOrigin, R, IRIT_PT_LENGTH(Dir),
	    ...
	    return GMTransformObjectInPlace(IPGenSRFObject(Srf), Mat);
	}
        case GM_GEN_PRIM_MDLS:
	{
	    ...
	    return GMTransformObjectInPlace(IPGenMODELObject(Mdl), Mat);
	}
        case GM_GEN_PRIM_TVS:
	{
	    TrivTVStruct
	        *TVs = TrivNSPrimCone(GlblOrigin, R,
				      IRIT_PT_LENGTH(Dir),
				      GlblSurfaceRational,
				      R * 0.65);

	    return GMTransformObjectInPlace(IPGenTRIVARObject(TVs), Mat);
	}
        default:
        case GM_GEN_PRIM_POLYS:
	    break;
				   ...
				   int Bases)
{
	...
	{
	    TrivTVStruct
		*TVs = TrivNSPrimCone2(GlblOrigin, R1, R2,
				       IRIT_PT_LENGTH(Dir),
				       GlblSurfaceRational,
				       IRIT_MIN(R1, R2) * 0.65);
				   ...
				   int Bases)
{
	...
	{
	    TrivTVStruct
	        *TVs = TrivNSPrimCylinder(GlblOrigin, R,
					  IRIT_PT_LENGTH(Dir),
					  GlblSurfaceRational,
					  R * 0.65);
...
IPObjectStruct *PrimGenSPHEREObject(const IrtVecType Center, IrtRType R)
{
	...
	{
	    TrivTVStruct
	        *TVs = TrivNSPrimSphere(Center, R, GlblSurfaceRational,
					R * 0.65);
				   ...
				   IrtRType Rminor)
{
	...
	{
	    TrivTVStruct
	        *TVs = TrivNSPrimTorus(GlblOrigin, Rmajor, Rminor,
				       GlblSurfaceRational,
				       Rminor * 0.5);
...
IPObjectStruct *PrimGenPOLYGONObject(IPObjectStruct *PObjList, int IsPolyline)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrNormalID = ATTRIB_NAME_BAD_NUMBER;
    int i,
	NumVertices = 0;
	...
	*VTail = NULL;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrNormalID, "Normal");
	...
	if (IP_IS_POLY_OBJ(PObj)) {
		...
		IPObjectStruct
		    *PVec = IPCoerceObjectPtTypeTo(PObj, IP_OBJ_VECTOR);
		...
		if ((PNormalStr = AttrGetStrAttrib2(V -> Attr,
						    AttrNormalID)) != NULL) {
		    IrtVecType N;
...
*                                                                            *
* SEE ALSO:                                                                  M
*   PrimGenPolygon3Vrtx, PrimGenPolygon4Vrtx2, GMGenPolyline2Vrtx            M
*                                                                            *
* KEYWORDS:                                                                  M
				     ...
				     IPPolygonStruct *Pnext)
{
    ...
    return PrimGenPolygon4Vrtx2(V1, V2, V3, V4, Vin,
				VrtcsRvrsd, &Singular, Pnext);
}
...
*   VrtcsRvrsd:        Set to TRUE if has Vin and order of vertices is       M
*		       oriented in reverse.				     M
*   Singular:          -1 if regular, index of singular vertex if this       M
*                      rectangle is singular and a triangle is returned.     M
*   Pnext:             Next is chain of polygons, in linked list.            M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPPolygonStruct *: The constructed polygon.                              M
*                                                                            *
* SEE ALSO:                                                                  M
*   PrimGenPolygon3Vrtx, PrimGenPolygon4Vrtx, GMGenPolyline2Vrtx             M
*                                                                            *
* KEYWORDS:                                                                  M
*   PrimGenPolygon4Vrtx2                                                     M
*****************************************************************************/
				     ...
				     IPPolygonStruct *Pnext)
{
	...
	IRIT_PT_APX_EQ(V2, V3) ||
        ...
        IRIT_WARNING_MSG("Constructed a degenerated triangle polygon.\n");
#endif /* DEBUG */
