...
*   MiterEdges:  Positive to properly handle dihedral angle, in which case   M
*                sets the maximal scale allowed.  Zero to disable.	     M            
*   AmountFunc:  Scale the offset amount according to this function. A NULL  M
*		 here will use a constant scaling factor of one.	     M
*                                                                            *
* RETURN VALUE:                                                              M
				...
				GMPolyOffsetAmountFuncType AmountFunc)
{
    int HasVNrmls = TRUE;
    IrtRType
	MiterEdges1 = MiterEdges > 0.0 ? 1.0 / MiterEdges : 0.0;
