			       ...
			       int EstimateNrmls)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrK1CurvID = ATTRIB_NAME_BAD_NUMBER,
        AttrK2CurvID = ATTRIB_NAME_BAD_NUMBER,
        AttrKCurvID = ATTRIB_NAME_BAD_NUMBER,
        AttrHCurvID = ATTRIB_NAME_BAD_NUMBER,
        AttrD1ID = ATTRIB_NAME_BAD_NUMBER,
        AttrD2ID = ATTRIB_NAME_BAD_NUMBER,
	...
	**Vertices = PVIdx -> Vertices;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrK1CurvID, "K1Curv");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrK2CurvID, "K2Curv");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrKCurvID, "KCurv");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrHCurvID, "HCurv");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrD1ID, "D1");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrD2ID, "D2");
	...
	if (!IP_HAS_NORMAL_VRTX(V)) {
	    ...
	    AttrSetRealAttrib2(&V -> Attr, AttrKCurvID, 0.0);
	    AttrSetRealAttrib2(&V -> Attr, AttrHCurvID, 0.0);
	    AttrSetRealAttrib2(&V -> Attr, AttrK1CurvID, 0.0);
	    AttrSetRealAttrib2(&V -> Attr, AttrK2CurvID, 0.0);
	    AttrSetStrAttrib2(&V -> Attr, AttrD1ID, "0,0,0");
	    AttrSetStrAttrib2(&V -> Attr, AttrD2ID, "0,0,0");
	...
	AttrSetRealAttrib2(&V -> Attr, AttrKCurvID, GMPL_BOUND_CRVTR(K));
	AttrSetRealAttrib2(&V -> Attr, AttrHCurvID, GMPL_BOUND_CRVTR(H));
	...
	AttrSetRealAttrib2(&V -> Attr, AttrK1CurvID, GMPL_BOUND_CRVTR(k1));
	AttrSetRealAttrib2(&V -> Attr, AttrK2CurvID, GMPL_BOUND_CRVTR(k2));
	...
	AttrSetStrAttrib2(&V -> Attr, AttrD1ID, SDirection);

	IRIT_SWAP(IrtRType, Dir[0], Dir[1]);
	Dir[0] = -Dir[0];
	MatMultVecby4by4(E3Dir, Dir, InvMat);
	sprintf(SDirection, "%g, %g, %g", E3Dir[0], E3Dir[1], E3Dir[2]);
	AttrSetStrAttrib2(&V -> Attr, AttrD2ID, SDirection);
    }
	...
	do {
	    int VIndex = AttrGetIntAttrib2(V -> Attr, AttrVIdxID);
