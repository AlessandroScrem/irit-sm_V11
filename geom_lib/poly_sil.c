				      ...
				      IrtHmgnMatType ViewMat)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrWasVisitedID = ATTRIB_NAME_BAD_NUMBER;
    int i;
    IrtRType Dot1, Dot2;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrWasVisitedID, "_Was_Visited");
    ...
    for (Pl = PObjReg -> U.Pl; Pl != NULL; Pl = Pl -> Pnext)
        AttrSetIntAttrib2(&(Pl -> Attr), AttrWasVisitedID, FALSE);
    ...
    for (Pl = PObjReg -> U.Pl; Pl != NULL; Pl = Pl -> Pnext) {
	    ...
	    if (i == 0)
	        i++;
	    if ((V -> PAdj) &&
		!AttrGetIntAttrib2(V -> PAdj -> Attr, AttrWasVisitedID)) {
        ...
        AttrSetIntAttrib2(&(Pl -> Attr), AttrWasVisitedID, TRUE);        
    }
...
static GMTableStruct *GMPolyhedraToGS(IPObjectStruct *PObjReg)
{
    ...
    PCurr = PObjReg -> U.Pl;
    while (PCurr != NULL) {
        AttrSetIntAttrib2(&(PCurr->Attr), AttrWasVisitedID, FALSE);
        PCurr = PCurr -> Pnext;
    }
    j=0;
    PCurr = PObjReg -> U.Pl;
    while (PCurr != NULL) {
	    ...
	    i++;
	    if ((VCurr -> PAdj) &&
		!AttrGetIntAttrib2(VCurr -> PAdj -> Attr, AttrWasVisitedID)) {
        ...
        AttrSetIntAttrib2(&(PCurr->Attr), AttrWasVisitedID, TRUE);
        PCurr = PCurr -> Pnext;
        j++;
    }
    return GSTable;
