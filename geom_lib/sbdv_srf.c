				 ...
				 GMCreateRefineObjFuncType CreateRefineObjFunc)
{
	...
	*CCObj = NULL;
    ...
    /* Filters out 2-pts polygons, zero-length edges etc. */
    GMCleanUpPolygonList(&PObjNGons -> U.Pl, IRIT_EPS);
				 ...
				 GMSubSrfsVrtcsStruct *GMSubCCVrtcs)
{
	...
	for (j = 0; VrtxVec[j] != -1; j++) {
	    ...
	    OtherPolygon = IPCnvFindAdjacentPoly(PVIdx, V, VNext);
	...
	for (PolyPtr = PolyArray; 
		...
		if (IRIT_PT_APX_EQ (VNext -> Coord, VOld -> Coord)) {
		    ...
		    IRIT_PT_SCALE(MidPtOldV, 0.5);
		     ...
		     /* Sum of surround edge midpoints. */
		    IRIT_PT_ADD(MidPtEdge, MidPtEdge, MidPtOldV);
		    OtherPolygon = IPCnvFindAdjacentPoly(PVIdx, VNext,
						      VNext -> Pnext);
		    if (OtherPolygon == NULL) {
			/* Extra ordinary vertices average. */
			IRIT_PT_ADD(HoleVAvg, HoleVAvg, MidPtOldV);
				     ...
				     GMSubSrfsVrtcsStruct *GMSubLoopVrtcs)
{
	...
	if (NumOfNeighbors >= 3 && NumOfNeighbors == nfaces) {
		...
		OtherPolygon1 = IPCnvFindAdjacentPoly(PVIdx, VOld, Vadj);
		OtherPolygon2 = IPCnvFindAdjacentPoly(PVIdx, Vadj, VOld);
					...
					GMSubSrfsVrtcsStruct *GMSubBtrflyVrtcs)
{
	...
	*VrtxVec, *Nbrs;
	...
	for (j = 0; VrtxVec[j] != -1; j++) {
		...
		OtherPolygon_1 = IPCnvFindAdjacentPoly(PVIdx, VOther, V);
		OtherPolygon_2 = IPCnvFindAdjacentPoly(PVIdx, VNext, VOther);
		/* Adjacent triangle is (VNext, V, VAdjOther).		     */
		OtherPolygon_3 = IPCnvFindAdjacentPoly(PVIdx, V, VAdjOther);
		OtherPolygon_4 = IPCnvFindAdjacentPoly(PVIdx, VAdjOther, VNext);
