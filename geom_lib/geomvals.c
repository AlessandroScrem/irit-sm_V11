...
IRIT_STATIC_DATA int
    GlblGMPolySignedArea = FALSE;

static IrtRType PolygonXYArea(const IPVertexStruct *VHead);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Set the computed area of polys to be signed or not. If signed, clockwise M
* (CW) polygons in the XY plane will have positive sign, & negative if CCW.  M
*   For polygon is in general position, CW and CCW are not well defined.     M
*                                                                            *
* PARAMETERS:                                                                M
*   SignedArea:    TRUE for signed area, FALSE for always positive area.     M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:       Previous state of signed area.                                M
*                                                                            *
* SEE ALSO:                                                                  M
*   GMPolyObjectArea, GMPolyOnePolyArea                                      M
*                                                                            *
* KEYWORDS:                                                                  M
*   GMPolyObjectAreaSetSigned                                                M
*****************************************************************************/
int GMPolyObjectAreaSetSigned(int SignedArea)
{
    int OldVal = GlblGMPolySignedArea;

    GlblGMPolySignedArea = SignedArea;
...
*                                                                            *
* SEE ALSO:                                                                  M
*   GMPolyOnePolyArea, GMPolyObjectAreaSetSigned                             M
*                                                                            *
* KEYWORDS:                                                                  M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   GMPolyObjectArea, GMPolyObjectAreaSetSigned                              M
*                                                                            *
* KEYWORDS:                                                                  M
*   GMPolyOnePolyArea                                                        M
*****************************************************************************/
double GMPolyOnePolyArea(const IPPolygonStruct *Pl)
{
	...
	return 0.0;
    else if (GlblGMPolySignedArea &&
	     IRIT_APX_EQ(Pl -> Plane[0], 0.0) &&
	     IRIT_APX_EQ(Pl -> Plane[1], 0.0)) { /* XY Plane to begin with. */
	ObjectArea = PolygonXYArea(Pl -> PVertex);
    }
    else {
        IPVertexStruct
	    *VHead = IPCopyVertexList(Pl -> PVertex),   /* Dont trans orig. */
	    *V = VHead;

        /* Create trans matrix to map polygon to the XY parallel plane. */
...
static IrtRType PolygonXYArea(const IPVertexStruct *VHead)
{
	...
	*V = VHead;
    ...
    return (GlblGMPolySignedArea ? PolygonArea : IRIT_FABS(PolygonArea)) * 0.5;
}
