...
static int CmpTwoVertices(VoidPtr VP1, VoidPtr VP2)
{
	...
	*Coord2 = V2 -> Coord;
	...
	int VIndex1 = AttrGetIntAttrib2(V1 -> Attr, AttrVidxID),
	    VIndex2 = AttrGetIntAttrib2(V2 -> Attr, AttrVidxID);

	if (!IP_ATTR_IS_BAD_INT(VIndex1))
	    AttrSetIntAttrib2(&V2 -> Attr, AttrVidxID, VIndex1);
	else if (!IP_ATTR_IS_BAD_INT(VIndex2))
	    AttrSetIntAttrib2(&V1 -> Attr, AttrVidxID, VIndex2);
	else
...
VoidPtr GMPolyAdjacncyGen(IPObjectStruct *PObj, IrtRType EqlEps)
{
	...
	*Pl = PObj -> U.Pl;
    ...
    for (Pl = PObj -> U.Pl, VIndex = 0; Pl != NULL; Pl = Pl -> Pnext) {
		...
		AttrSetIntAttrib2(&V -> Attr, AttrVidxID, VIndex++);
	    }
	...
	V = Pl -> PVertex;
	VIdx1 = AttrGetIntAttrib2(V -> Attr, AttrVidxID);

        do {
	    IPVertexStruct
		*Vnext = V -> Pnext ? V -> Pnext : Pl -> PVertex;
	    VIdx2 = AttrGetIntAttrib2(Vnext -> Attr, AttrVidxID);
...
#   endif /* DEBUG */
    ...
    VIdx = AttrGetIntAttrib2(V -> Attr, AttrVidxID);
