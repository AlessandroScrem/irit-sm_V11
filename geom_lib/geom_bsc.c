...
IrtRType GMVecVecAngle(const IrtVecType V1, const IrtVecType V2, int Normalize)
{
    ...
    if (Normalize) {
        ...
        IRIT_VEC_COPY(D1, V1);
        IRIT_VEC_COPY(D2, V2);
	IRIT_VEC_SAFE_NORMALIZE(D1);
	IRIT_VEC_SAFE_NORMALIZE(D2);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Reflects a 3D vector through a plane with normal PlaneNormal	     M
*                                                                            *
* PARAMETERS:                                                                M
*   Dst:	  Reflected vector.					     M
*   Src:	  Input Vector to reflect.				     M
*   PlaneNormal:  Normal of plane (through origin) to reflect Src through.   M
*                                                                            *
* RETURN VALUE:                                                              M
*   void								     M
*                                                                            *
...
void GMVecReflectPlane(IrtVecType Dst, IrtVecType Src, IrtVecType PlaneNormal)
{
    ...
    IRIT_VEC_COPY(M, PlaneNormal);
    IRIT_VEC_SAFE_NORMALIZE(M);
    Size = IRIT_DOT_PROD(Src, M);
    IRIT_VEC_SCALE(M, 2 * Size);
    IRIT_VEC_SUB(Dst, Src, M);
}
...
*                                                                            *
* RETURN VALUE:                                                              M
*   IrtRType:   Parameter along the line where if the closest point is Pl    M
*               equal zero and if closest point is Pl + Vl equal one.        M
*                 In other words, the closest point is on the finite line    M
*               segment for return value in [0, 1].			     M
*                                                                            *
* SEE ALSO:                                                                  M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the extreme distance between point Pt and polygons/lines Poly.  M
* The extreme distance is computed on the vertices of Pl only.               M
*                                                                            *
* PARAMETERS:                                                                M
*   Pl:   Poly to examine its extreme distance from point Pt.                M
*   Pt:   Point to examine its extreme distance from poly Pl.                M
*   ExtremeV:  Will be set to the vertex at the extreme distance.            M
*              If NULL it is ignored.					     M
*   MaxDist:   TRUE to compute the maximum distance, FALSE for the minimum.  M
*                                                                            *
* RETURN VALUE:                                                              M
*   IrtRType:   Extreme distance, -1.0 if error.                             M
*                                                                            *
* SEE ALSO:                                                                  M
*   GMDistPolyPoly, GMDistPolyPt2                                            M
*                                                                            *
* KEYWORDS:                                                                  M
*   GMDistPolyPt                                                             M
*****************************************************************************/
...
{
    IrtRType
        ExtremeD = MaxDist ? 0.0 : IRIT_INFNTY;
    IPVertexStruct
        *ExtV = NULL;
    ...
    return ExtV == NULL ? -1.0 : sqrt(ExtremeD);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the extreme distance between point Pt and polygons/lines Poly.  M
* The extreme distance is computed on the vertices and edges of Pl only.     M
*                                                                            *
* PARAMETERS:                                                                M
*   Pl:         Poly to examine its extreme distance from point Pt.          M
*   IsPolyline: TRUE if Pl holds polylines, FALSE if polygons.               M
*   Pt:         Point to examine its extreme distance from poly Pl.          M
*   ExtremePt:  Will be set to the location at the extreme distance.         M
*               If NULL it is ignored.					     M
*   MaxDist:    TRUE to compute the maximum distance, FALSE for the minimum. M
*                                                                            *
* RETURN VALUE:                                                              M
		       ...
		       int MaxDist)
{
	...
	do {
	    ...
	    if (MaxDist) {
	        /* Maximum distance can only occur on the vertices. */
	        if (ExtremeD < d) {
		    ExtremeD = d;
		    ExtV = V;
		    IRIT_PT_COPY(ExtPt, V -> Coord);
		}
	    }
	    else {
	        IrtRType t;
		...
		IrtVecType Dir;
	        ...
	        /* Minimal distance can occur at vertices. */
	        if (ExtremeD > d) {
		    ExtremeD = d;
		    ExtV = V;
		    IRIT_PT_COPY(ExtPt, V -> Coord);
		...
		/* Minimal distance can occur in the middle of edges. */
		if ((VNext = V -> Pnext) != NULL || !IsPolyline) {
		    if (VNext == NULL)
		        VNext = Pl -> PVertex;

		    IRIT_VEC_SUB(Dir, VNext -> Coord, V -> Coord);
		    t = GMPointFromPointLine(Pt, V -> Coord, Dir,
					     ClosestPoint);
		    if (t >= 0.0 && t <= 1.0) {
		        d = IRIT_PT_PT_DIST_SQR(Pt, ClosestPoint);
			if (ExtremeD > d) {
			    ExtremeD = d;
			    ExtV = V;
			    IRIT_PT_COPY(ExtPt, ClosestPoint);
			}
		    }
		}
	    }
...
*   TagIgnoreV: If non zero, vertices that own this tag are ignored.         M
*                                                                            *
* RETURN VALUE:								     M
			...
			int TagIgnoreV)
{
    ...
    *V1 = *V2 = NULL;
    ...
    do {
        if ((Vrtx1 -> Tags & TagIgnoreV) == 0) {
	    IPVertexStruct
	        *Vrtx2 = Pl2 -> PVertex;

	    do {
	        if ((Vrtx2 -> Tags & TagIgnoreV) == 0) {
		    DistSqr = IRIT_PT_PT_DIST_SQR(Vrtx1 -> Coord,
						  Vrtx2 -> Coord);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Routine to check if a point is inside a polyhedra in 3D.                 M
...
*   AllInters:    OUT - if not NULL, will be updated with a vector of all    M
*                 intersction locations.  First entry must hold number of    M
*                 elements in this vector.				     M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:       Number of intersections of ray with the polygon.              M
*                                                                            *
...
int GMPolygonRayInter2(const IPPolygonStruct *Pl,
		       ...
		       IrtRType *AllInters)
{
    int NewState, RayOtherAxes,
        MaxNumOfInter = AllInters != NULL ? (int) AllInters[0] : 0,
	Quit = FALSE,
	NumOfInter = 0;
    GMGeomRayRelationType AlgState;
	...
	*VLast = NULL;
	...
	if ((NewState = GMPointRayRelation(V -> Coord, PtRay, RayOtherAxes))
		...
		if (MaxNumOfInter > NumOfInter++) {
		    assert(AllInters != NULL);
		    AllInters[NumOfInter - 1] = Inter;
		}
			      ...
			      int Axes)
{
    if (IRIT_APX_UEQ(PtRay[Axes], Pt[Axes]))
        return GM_GEOM_ON_RAY;
    else if (PtRay[Axes] < Pt[Axes])
        return GM_GEOM_ABOVE_RAY;
    else
	return GM_GEOM_BELOW_RAY;
}
					    ...
					    IPPolygonStruct *Islands)
{
	...
	if (GMPolygonRayInter2(Root, MaxV -> Coord, 0,
			       &FirstInterV, &FirstInterP, NULL) == 0) {
	    IPFreePolygon(Root);
			    ...
			    const IrtPtType Pt)
{
    ...
    /* Singular cases (Pt is on one of the three input points). */
    if (IRIT_PT_APX_EQ_ZERO_EPS(PtV1, GMBasicEps)) {
	RetVal[0] = 1.0;
	RetVal[1] = RetVal[2] = 0.0;
	return RetVal;
    }
    else if (IRIT_PT_APX_EQ_ZERO_EPS(PtV2, GMBasicEps)) {
	RetVal[0] = RetVal[2] = 0.0;
	RetVal[1] = 1.0;
	return RetVal;
    }
    else if (IRIT_PT_APX_EQ_ZERO_EPS(PtV3, GMBasicEps)) {
	RetVal[0] = RetVal[1] = 0.0;
	RetVal[2] = 1.0;
	return RetVal;
    }
