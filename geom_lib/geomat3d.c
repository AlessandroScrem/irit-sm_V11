...
*                                                                            *
* KEYWORDS:                                                                  M
*   GMTransformObjectInPlace, transformations                                M
*****************************************************************************/
					    ...
					    int AnimUpdate)
{
    ...
    int AnimUpdateChildrens, i;
    IPObjectStruct *NewPObj, *PTmp, *PAnim;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAnimationID, "animation");

    PAnim = AttrGetObjectObjAttrib2(PObj, AttrAnimationID);
    AnimUpdateChildrens = AnimUpdate && (PAnim == NULL);
	...
	IPObjectStruct
	    *TmpObj = IPCoerceObjectPtTypeTo(PObj, CAGD_PT_P3_TYPE);
	IrtRType R[4];
	...
	IRIT_PT_COPY(&TmpObj -> U.CtlPt.Coords[1], R);
    	...
    	NewPObj = IPCoerceObjectPtTypeTo(TmpObj, PObj -> U.CtlPt.PtType);
    ...
    if (PAnim != NULL) {
	    ...
	    AttrSetObjectObjAttrib2(NewPObj, AttrAnimationID, PAnim, FALSE);
	}
	else {
	    AttrSetObjectObjAttrib2(NewPObj, AttrAnimationID,
		       ...
		       const IrtVecType V2)
{
	...
	if (IRIT_DOT_PROD(V1Copy, V2Copy) > 0.0) {        /* Same vectors. */
	    ...
	    if (!GMOrthogonalVector(V1Copy, V, FALSE)) {
