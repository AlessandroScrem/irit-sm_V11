			      ...
			      const IPPolygonStruct *OriginalPl)
{
	...
	AttrIDID = ATTRIB_NAME_BAD_NUMBER,
	AttrUVValsID = ATTRIB_NAME_BAD_NUMBER;
    int R, G, B, ID, DoUV,
	DoRgb = AttrGetRGBColor(OriginalPl -> PVertex -> Attr, &R, &G, &B);
    IPVertexStruct *V, *VHead;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrIDID, "ID");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrUVValsID, "uvvals");

    ID = AttrGetIntAttrib2(OriginalPl -> Attr, AttrIDID);
    DoUV = AttrGetUVAttrib2(OriginalPl -> PVertex -> Attr,
			    AttrUVValsID) != NULL;
	...
	if (!IP_ATTR_IS_BAD_INT(ID))
	    AttrSetIntAttrib2(&PlList -> Attr, AttrIDID, ID);
			 ...
			 IrtRType *Wgt)
{
    ...
    if (Pl == NULL || Pl -> PVertex == NULL || Pl -> PVertex -> Pnext == NULL)
	return FALSE;

    /* Find out the triangle inside Pl that holds V. */
    for (i = 1, VFirst = Pl -> PVertex, V = VFirst -> Pnext;
			     ...
			     const IPVertexStruct *V2)
{
    ...
    if ((UV1 = AttrGetUVAttrib2(V1 -> Attr, AttrUVValsID)) != NULL &&
	(UV2 = AttrGetUVAttrib2(V2 -> Attr, AttrUVValsID)) != NULL) {
        IrtRType t1, t2;
	IrtVecType Vec1, Vec2;
...
int GMInterpVrtxUVFromPl(IPVertexStruct *Vrtx, const IPPolygonStruct *Pl)
{
    ...
    if (GMEvalWeightsVFromPl(Vrtx -> Coord, Pl, Wgt)) {
	    ...
	    if ((UVTmp = AttrGetUVAttrib2(V -> Attr, AttrUVValsID)) == NULL) {
	        IritFree(Wgt);
	        return FALSE;
	    }
    ...
    AttrSetUVAttrib2(&Vrtx -> Attr, AttrUVValsID, UV[0], UV[1]);
...
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
	AttrvrtxIdxID = ATTRIB_NAME_BAD_NUMBER;
    IPVertexStruct
	...
	*Coord2 = V2 -> Coord;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrvrtxIdxID, "_vrtxIdx");

    if (IRIT_PT_APX_EQ_EPS(Coord1, Coord2, NRML_SAME_VRTX_EPS)) {
	/* Update cross references so we could recover the averaged normal. */
        int Idx,
	    Idx1 = AttrGetIntAttrib2(V1 -> Attr, AttrvrtxIdxID),
	    Idx2 = AttrGetIntAttrib2(V2 -> Attr, AttrvrtxIdxID);

	if (!IP_ATTR_IS_BAD_INT(Idx1)) {
	    AttrSetIntAttrib2(&V2 -> Attr, AttrvrtxIdxID, Idx1);
	    Idx = Idx1;
	}
	else if (!IP_ATTR_IS_BAD_INT(Idx2)) {
	    AttrSetIntAttrib2(&V1 -> Attr, AttrvrtxIdxID, Idx2);
	    Idx = Idx2;
	}
	else {
			      ...
			      IrtRType MaxAngle)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrvrtxIdxID = ATTRIB_NAME_BAD_NUMBER,
        AttrCosNrmlMaxDevID = ATTRIB_NAME_BAD_NUMBER;
    int i, VIndex,
	TotalVertices = 0;
    IrtRType Min, Max, R,
	...
	*BBox = GMBBComputePolyListBbox(PlList);
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrvrtxIdxID, "_vrtxIdx");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrCosNrmlMaxDevID, "_CosNrmlMaxDeviation");
	...
	GlblCrntPlBlendNormal = Pl;
		...
		GlblVertices[VIndex].Count = 1;
	        AttrSetIntAttrib2(&V -> Attr, AttrvrtxIdxID, VIndex++);
	    }
    ...
    for (Pl = PlList; Pl != NULL; Pl = Pl -> Pnext) {
        V = Pl -> PVertex;
        do {
	    i = AttrGetIntAttrib2(V -> Attr, AttrvrtxIdxID);
    ...
    for (Pl = PlList; Pl != NULL; Pl = Pl -> Pnext) {
	    ...
	    AttrSetRealAttrib2(&V -> Attr, AttrCosNrmlMaxDevID,
...
void GMFixOrientationOfPolyModel(IPPolygonStruct *Pls)
{
    ...
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrOrientDisjointID = ATTRIB_NAME_BAD_NUMBER;
    IRIT_STATIC_DATA IPPolygonStruct
	...
	*PDummy = IPGenPOLYObject(Pls);
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrOrientDisjointID, "_OrientDisjoint");
	...
	if (!GM_IS_REORIENTED_POLY(Pl)) {
		...
		*UnMarkedPls = NULL;
	    ...
	    AttrSetIntAttrib2(&UnMarkedPls -> Attr, AttrOrientDisjointID,
			      TRUE);
