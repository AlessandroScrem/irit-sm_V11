...
void GMPlSilImportanceAttr(IPPolygonStruct *PolyList)
{
    ...
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrSilImpID = ATTRIB_NAME_BAD_NUMBER,
        AttrSilImpRangeID = ATTRIB_NAME_BAD_NUMBER,
	...
	**PPolys = PVIdx -> PPolys;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrSilImpID, "SilImp");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrSilImpRangeID, "SilImpRange");
	...
	SilImp /= Weights;
	AttrSetRealAttrib2(&V1 -> Attr, AttrSilImpID, SilImp);
    }
	...
	const int
	    *Nbrs = IPCnvPolyVrtxNeighbors(PVIdx, i, 1);
	IrtRType
	    SilImp = AttrGetRealAttrib2(V1 -> Attr, AttrSilImpID);

	for (j = 0; Nbrs[j] >= 0; j++) {
	    IPVertexStruct
		*V2 = Vertices[Nbrs[j]];

	    if (AttrGetRealAttrib2(V2 -> Attr, AttrSilImpID) > SilImp)
		NumHigherNeighbors++;
	}

	if (NumHigherNeighbors < 3)
	    AttrSetRealAttrib2(&V1 -> Attr, AttrSilImpRangeID, TRUE);
    }
...
#   ifdef DEBUG
    {
		...
		do {
		    ...
		    IrtRType
		        SilImp = AttrGetRealAttrib2(V -> Attr, AttrSilImpID);
...
IPPolygonStruct *GMPlSilImportanceRange(IPPolygonStruct *PolyList)
{
	...
	*PlRange = NULL;
	...
	do {
	    if (AttrGetRealAttrib2(V -> Attr, AttrSilImpRangeID) == TRUE &&
		AttrGetRealAttrib2(VNext -> Attr, AttrSilImpRangeID) == TRUE) {
		int i,
		    DMaxIdx = 0;
		IrtRType Dp[3];
