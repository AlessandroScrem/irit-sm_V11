				 ...
				 unsigned int NumOfChecks)	
{
	...
	for (i = 0; i < PolyVrtxIdxPtr -> NumVrtcs; ++i) {
	    ...
	    FitData[i][3] = AttrGetRealAttrib2(Vertex -> Attr, AttrK1CurvID);
	    FitData[i][4] = AttrGetRealAttrib2(Vertex -> Attr, AttrK2CurvID);
	    
	    StrPtr = AttrGetStrAttrib2(Vertex -> Attr, AttrD1ID);
	    if ((StrPtr == NULL ) ||
	    ...
	    StrPtr = AttrGetStrAttrib2(Vertex -> Attr, AttrD2ID);
	    if ((StrPtr == NULL ) ||
		(sscanf(StrPtr, "%lf, %lf, %lf", 
			&FitData[i][8],
			&FitData[i][9],
...
static int ValidateCurvatureAnalysisAux(IPPolygonStruct *PPoly)
{
    ...
    for (CurPoly = PPoly;
	 ...
	 CurPoly = CurPoly -> Pnext) {
	    ...
	    if (AttrGetStrAttrib2(CurVertex -> Attr, AttrK1CurvID) == NULL ||
		AttrGetStrAttrib2(CurVertex -> Attr, AttrK2CurvID) == NULL ||
		AttrGetStrAttrib2(CurVertex -> Attr, AttrD1ID) == NULL ||
		AttrGetStrAttrib2(CurVertex -> Attr, AttrD2ID) == NULL)
	      return FALSE;
	}
    }
    return TRUE;
