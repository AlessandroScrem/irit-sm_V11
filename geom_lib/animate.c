...
static int GMAnimAffineTransAnimationOneAux1(const IPObjectStruct *PObj);
static int GMAnimAffineTransAnimationOneAux2(const IPObjectStruct *PObj,
					     IrtRType Trans,
					     IrtRType Scale);
static void GMAnimAffineTransAnimCrv(IPObjectStruct *AnimCrv,
				     IrtRType Trans,
				     IrtRType Scale);
...
int GMAnimHasAnimationOne(const IPObjectStruct *PObj)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrAnimationID = ATTRIB_NAME_BAD_NUMBER;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAnimationID, "animation");

    if (AttrGetObjectObjAttrib2(PObj, AttrAnimationID) != NULL)
        return TRUE;
...
/*****************************************************************************
* DESCRIPTION:								     M
*   Affine transform the animation domain in the given object(s), in place.  M
*   Domain "D" is mapped to domain "D * Scale + Trans".                      M
*									     *
* PARAMETERS:								     M
*   PObjs:     Objects to update animation domain.			     M
*   Trans:     Translation amount.					     M
*   Scale:     Scale amount.						     M
*									     *
* RETURN VALUE:								     M
*   int:	TRUE if there are animation attributes. FALSE otherwise.     M
*                                                                            *
...
*                                                                            *
* KEYWORDS:                                                                  M
*   GMAnimAffineTransAnimation, animation                                    M
*****************************************************************************/
			       ...
			       IrtRType Scale)
{
    ...
    for (PObj = PObjs; PObj != NULL; PObj = PObj -> Pnext) {
      if (!GMAnimAffineTransAnimationOne(PObj, Trans, Scale))
	    return FALSE;
    }
...
/*****************************************************************************
* DESCRIPTION:								     M
*   Affine transform the animation domain in the given object, in place.     M
*   Domain "D" is mapped to domain "D * Scale + Trans".                      M
*									     *
* PARAMETERS:								     M
*   PObj:      Object to update animation domain.			     M
*   Trans:     Translation amount.					     M
*   Scale:     Scale amount.						     M
*									     *
* RETURN VALUE:								     M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   GMAnimAffineTransAnimationOne, animation                                 M
*****************************************************************************/
int GMAnimAffineTransAnimationOne(const IPObjectStruct *PObj,
				  IrtRType Trans,
				  IrtRType Scale)
{
    /* Must two two passes as we have reference counts here. */
    return GMAnimAffineTransAnimationOneAux1(PObj) &&
           GMAnimAffineTransAnimationOneAux2(PObj, Trans, Scale);
}

/*****************************************************************************
* DESCRIPTION:								     *
*   First pass - just mark all curves to be processed.			     *
*									     *
* PARAMETERS:								     *
...
static int GMAnimAffineTransAnimationOneAux1(const IPObjectStruct *PObj)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
	AttrAnimationID = ATTRIB_NAME_BAD_NUMBER,
	AttrAreParamID = ATTRIB_NAME_BAD_NUMBER;
    IPObjectStruct *ObjPtr, *AnimationP;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAnimationID, "animation");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAreParamID, "_areparam");
	...
	while ((PTmp = IPListObjectGet(PObj, i++)) != NULL) {
	    GMAnimAffineTransAnimationOneAux1(PTmp);
	}
    }
    else if ((AnimationP = AttrGetObjectObjAttrib2(PObj,
						   AttrAnimationID)) != NULL) {
	if (IP_IS_OLST_OBJ(AnimationP)) {
	    int i = 0;

	    while ((ObjPtr = IPListObjectGet(AnimationP, i++)) != NULL) {
		if (IP_IS_CRV_OBJ(ObjPtr)) {
		    AttrSetObjectIntAttrib2(ObjPtr, AttrAreParamID, TRUE);
		}
	    }
	}
	else if (IP_IS_CRV_OBJ(AnimationP)) {
	    AttrSetObjectIntAttrib2(AnimationP, AttrAreParamID, TRUE);
	}
    }
...
/*****************************************************************************
* DESCRIPTION:								     *
*   First pass - reparam all curves not processed yet.			     *
*									     *
* PARAMETERS:								     *
*   PObj:      Object to update animation domain.			     *
					     ...
					     IrtRType Scale)
{
	...
	while ((PTmp = IPListObjectGet(PObj, i++)) != NULL) {
	    GMAnimAffineTransAnimationOneAux2(PTmp, Trans, Scale);
	}
    }
    else if ((AnimationP = AttrGetObjectObjAttrib2(PObj,
						   AttrAnimationID)) != NULL) {
	if (IP_IS_OLST_OBJ(AnimationP)) {
	    ...
	    while ((ObjPtr = IPListObjectGet(AnimationP, i++)) != NULL) {
	        if (IP_IS_CRV_OBJ(ObjPtr) &&
		    AttrGetObjectIntAttrib2(ObjPtr, AttrAreParamID) == TRUE) {
		    GMAnimAffineTransAnimCrv(ObjPtr, Trans, Scale);
		}
	    }
	}
	else if (IP_IS_CRV_OBJ(AnimationP) &&
		 AttrGetObjectIntAttrib2(AnimationP, AttrAreParamID) == TRUE) {
	    GMAnimAffineTransAnimCrv(AnimationP, Trans, Scale);
	}
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Update one animation curve following the prescribed Scale and Trans vals.*
*                                                                            *
* PARAMETERS:                                                                *
*   AnimCrv:   A curve anumation object update its animation domain.         *
*   Trans:     Translation amount.					     *
*   Scale:     Scale amount.						     *
*                                                                            *
* RETURN VALUE:                                                              *
				     ...
				     IrtRType Scale)
{
    ...
    AttrFreeObjectAttribute(AnimCrv, "_areparam");

    if (CAGD_IS_BEZIER_CRV(AnimCrv -> U.Crvs)) {
        /* Convert to a Bspline curve. */
        Crv = CagdCnvrtBzr2BspCrv(AnimCrv -> U.Crvs);
	CagdCrvFree(AnimCrv -> U.Crvs);
	AnimCrv -> U.Crvs = Crv;
    }
    else
        Crv = AnimCrv -> U.Crvs;
    ...
    BspKnotAffineTrans(Crv -> KnotVector,
		       ...
		       Trans- Crv -> KnotVector[0] * (1.0 - Scale),
		       Scale);
}
...
* DESCRIPTION:								     M
*   Affine transform the animation domain in the given object(s), in place.  M
*   Animation domain is mapped to "Min, Max".		                     M
*									     *
* PARAMETERS:								     M
*   PObjs:     Objects to update animation domain.			     M
*   Min, Max:  Desired time domain of animation.			     M
*									     *
* RETURN VALUE:								     M
*   int:	TRUE if there are animation attributes. FALSE otherwise.     M
*                                                                            *
...
*                                                                            *
* KEYWORDS:                                                                  M
*   GMAnimAffineTransAnimation2, animation                                   M
*****************************************************************************/
				...
				IrtRType Max)
{
    ...
    GMAnimFindAnimationTime(&Anim, PObjs);

    if (Anim.StartT != IRIT_INFNTY) {
        Scale = (Max - Min) / (Anim.FinalT - Anim.StartT);
        return GMAnimAffineTransAnimation(PObjs, Min, Scale);
    }
    else
        return FALSE;
}
...
* DESCRIPTION:								     M
*   Affine transform the animation domain in the given object, in place.     M
*   Amination domain is mapped to "Min, Max".		                     M
*									     *
* PARAMETERS:								     M
*   PObj:      Object to update animation domain.			     M
*   Min, Max:  Desired time domain of animation.			     M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   GMAnimAffineTransAnimationOne2, animation                                M
*****************************************************************************/
				   ...
				   IrtRType Max)
{
    ...
    GMAnimFindAnimationTimeOne(&Anim, PObj);

    if (Anim.StartT != IRIT_INFNTY) {
        Scale = (Max - Min) / (Anim.FinalT - Anim.StartT);
        return GMAnimAffineTransAnimationOne(PObj, Min, Scale);
    }
    else
        return FALSE;
}
				...
				IrtHmgnMatType ObjMat)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrAnimationID = ATTRIB_NAME_BAD_NUMBER,
        AttrAnimTimeID = ATTRIB_NAME_BAD_NUMBER,
        AttrISvisibleID = ATTRIB_NAME_BAD_NUMBER;
    IPObjectStruct *AnimationP;
    int WasAnim = FALSE;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAnimationID, "animation");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAnimTimeID, "animtime");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrISvisibleID, "_isvisible");

    if ((AnimationP = AttrGetObjectObjAttrib2(PObj,
					      AttrAnimationID)) != NULL) {
        IrtRType t,
	    Time = Anim -> RunTime;

	if ((t = AttrGetObjectRealAttrib2(PObj, AttrAnimTimeID)) !=
							    IP_ATTR_BAD_REAL)
	    Time = t;
	...
	if (Time != GM_ANIM_NO_DEFAULT_TIME) {
	    ...
	    if (Vis >= 0.0)
	        AttrSetObjectRealAttrib2(PObj, AttrISvisibleID, Vis);
	}
...
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrAnimationMatID = ATTRIB_NAME_BAD_NUMBER;
    IPObjectStruct *OldMatObj;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAnimationMatID, "_animation_mat");

    /* Copy over existing matrix if has one - faster and thread-safer. */
    if ((OldMatObj = AttrGetObjectObjAttrib2(PObj,
					     AttrAnimationMatID)) != NULL) {
        IRIT_HMGN_MAT_COPY(OldMatObj -> U.Mat, ObjMat);
    }
    else {
        AttrSetObjectObjAttrib2(PObj, AttrAnimationMatID,
