		       ...
		       IrtBType ErrorDiffusion)
{
    ...
    IrtImgPixelStruct *Line,
        *Img = IrtImgReadImage(InputImage, &MaxX, &MaxY, &Alpha);
    IrtBType *DtrImg;
    ...
    if (OutputImage != NULL &&
	((p = strrchr(OutputImage, '.')) != NULL))
        IrtImgWriteSetType(p + 1);

    IrtImgWriteOpenFile(NULL, OutputImage, FALSE, DtrXSize, DtrYSize);
