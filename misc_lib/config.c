		       ...
		       int NumVar)
{
    ...
    int n;
    char CfgName2[IRIT_LINE_LEN_VLONG],	CfgName3[IRIT_LINE_LEN_VLONG], 
	CfgName4[IRIT_LINE_LEN_VLONG];
    FILE *f2, *f3, *f4;
	...
	*Cptr = 0;					 /* Delete old type. */
...
#if defined(__WINNT__) && defined(_WIN64)
    /* For 64 bit machines, try first xxx64.cfg and xxxD64.cfg config files  */
    /* and if fails, prepare versions try without postfix '64' and without   */
    /* postfix 'D'.							     */
    n = (int) strlen(CfgName);
    strcpy(CfgName2, CfgName);
    if (n > 2 && strcmp(&CfgName[n - 2], "64") == 0) {
	CfgName2[n - 2] = 0;
	strcat(CfgName2, ".cfg");
    }
    else
	CfgName2[0] = 0;
    strcpy(CfgName3, CfgName);
    strcpy(CfgName4, CfgName);
    if (n > 3 && strcmp(&CfgName[n - 3], "D64") == 0) {
	CfgName3[n - 3] = 0;
	CfgName4[n - 3] = 0;
	strcat(CfgName3, ".cfg");
	strcat(CfgName4, "64.cfg");
    }
    else {
	CfgName3[0] = 0;
	CfgName4[0] = 0;
	...
	(f2 = FindFile(PrgmName, CfgName2, &FoundName)) == NULL &&
	(f4 = FindFile(PrgmName, CfgName4, &FoundName)) == NULL &&
	(f3 = FindFile(PrgmName, CfgName3, &FoundName)) == NULL) {
#else
    strcat(CfgName, ".cfg");		    /* And add the config file type. */
...
#if defined(__WINNT__) && defined(_WIN64)
    if (f == NULL && f2 != NULL) {
	IritInformationMsgPrintf("File \"%s\" is used as configuration file",
			         CfgName2);
	f = f2;
    }
    if (f == NULL && f3 != NULL) {
