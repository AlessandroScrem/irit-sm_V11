...
IRIT_STATIC_DATA int
    IritPseudoRandomSeed = 301150; // 301060;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Routine to convert all lower case chars into upper case, in place.	     M
*                                                                            *
* PARAMETERS:                                                                M
*   s:        String to convert to upper case.                               M
*                                                                            *
* RETURN VALUE:                                                              M
*   char *:   Reference to s.                                                M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritStrUpper, upper case, lower case                                     M
*****************************************************************************/
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Routine to convert all upper case chars into lower case, in place.	     M
*                                                                            *
* PARAMETERS:                                                                M
*   s:        String to convert to lower case.                               M
*                                                                            *
* RETURN VALUE:                                                              M
*   char *:   Reference to s.                                                M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritStrLower, lower case, upper case                                     M
*****************************************************************************/
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IritRandomInit, IritPseudoRandom	                                     M
*                                                                            *
* KEYWORDS:                                                                  M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   A seed generator for a pseudo random number generator using the large    M
* Bezier evaluation table of the cagd library...			     M
*                                                                            *
* PARAMETERS:                                                                M
*   Seed:   New seed for the randomization process.                          M
*                                                                            *
* RETURN VALUE:                                                              M
*   void			                                             M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdIChooseKTable, IritPseudoRandom, IritRandom                          M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritPseudoRandomInit                                                     M
*****************************************************************************/
void IritPseudoRandomInit(unsigned int Seed)
{
    IritPseudoRandomSeed = Seed + 30101960;
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes a pseudo random number generator using the large Bezier         M
* evaluation table of the cagd library...				     M
*                                                                            *
* PARAMETERS:                                                                M
*   None                                                                     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IrtRType:     A pseudo random number between zero and one.               M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdIChooseKTable, IritPseudoRandomInit, IritRandom                      M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritPseudoRandom                                                         M
*****************************************************************************/
...
{
    unsigned
	Sx = 13 + (IritPseudoRandomSeed / 3) %
                                           (CAGD_MAX_BEZIER_CACHE_ORDER - 13),
	Sy = 10 + (IritPseudoRandomSeed / 5) %
                                           (CAGD_MAX_BEZIER_CACHE_ORDER - 10);
    double RandVal;

    assert(Sx >= 10 &&
	   Sx <= CAGD_MAX_BEZIER_CACHE_ORDER &&
	   Sy >= 10 &&
	   Sy <= CAGD_MAX_BEZIER_CACHE_ORDER);

    if (Sx < Sy)
	IRIT_SWAP(unsigned int, Sx, Sy);

    RandVal = CagdIChooseKTable[Sx][Sy];
    IritPseudoRandomSeed += CAGD_MAX_BEZIER_CACHE_ORDER + 13;
    if (IritPseudoRandomSeed < 301060)
        IritPseudoRandomSeed = 301060;

    do {
        RandVal = RandVal / M_PI;
    } while (RandVal > 10.0);

    RandVal = RandVal - floor(RandVal);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Routine to compute the cpu time, in second, of the running process.        M
*                                                                            *
* PARAMETERS:                                                                M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Routine to search for a given file name.				     M
*                                                                            *
* PARAMETERS:                                                                M
...
const char *searchpath(const char *Name)
{
    ...
    if ((p = getenv("IRIT_PATH64")) != NULL ||
	(p = getenv("IRIT_PATH")) != NULL) {
#else
