...
IPObjectStruct *ExtremumControlPointVals(IPObjectStruct *Obj, CagdRType *Min)
{
	...
	**Points = NULL;
    ...
    if (IP_IS_TRIVAR_OBJ(Obj)) {
	Points = Obj -> U.Trivars -> Points;
	Len = Obj -> U.Trivars -> ULength *
	      Obj -> U.Trivars -> VLength *
	      Obj -> U.Trivars -> WLength;
	PType = Obj -> U.Trivars -> PType;
    }
    else if (IP_IS_SRF_OBJ(Obj)) {
	...
	IRIT_NON_FATAL_ERROR("Extremum allowed on curves/surfaces/trivars only");
	return NULL;
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Routine to return parameter locations of local extrema values of given     M
* scalar freeform (a curve, surface, or trivariate).		 	     M
*                                                                            *
* PARAMETERS:                                                                M
*   Obj:      A scalar curve, surface, or trivariate to derive its extrema.  M
*             If not scalar, only first axis is considered.                  M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:   A list of extrema parameter locations.	             M
*                                                                            *
* KEYWORDS:                                                                  M
*   ExtremaOfFreeforms                                                       M
*****************************************************************************/
IPObjectStruct *ExtremaOfFreeforms(IPObjectStruct *PObj, IrtRType *Euclidean)
{
    MvarMVStruct
        *MV = NULL;
    MvarPtStruct *ExtremaPts;
    IPObjectStruct *ObjPtList;

    switch (PObj -> ObjType) {
        case IP_OBJ_CURVE:
	    MV = MvarCrvToMV(PObj -> U.Crvs);
	    break;
	case IP_OBJ_SURFACE:
	    MV = MvarSrfToMV(PObj -> U.Srfs);
	    break;
	case IP_OBJ_TRIVAR:
    ...
    ExtremaPts = MvarCalculateExtremePoints(MV);

    if (IRIT_REAL_PTR_TO_INT(Euclidean)) {
	CagdRType *R;
	CagdPType Pt;
	MvarPtStruct *EPt;
	IPObjectStruct *ObjPt;

	ObjPtList = IPGenLISTObject(NULL);
        for (EPt = ExtremaPts; EPt != NULL; EPt = EPt -> Pnext) {
	    int ExtType = AttrGetIntAttrib(EPt -> Attr, "extreme_type");

	    R = MvarMVEval(MV, EPt -> Pt);
	    CagdCoerceToE3(Pt, &R, -1, MV -> PType);
	    ObjPt = IPGenPTObject(&Pt[0], &Pt[1], &Pt[2]);
	    if (!IP_ATTR_IS_BAD_INT(ExtType))
		AttrSetObjectIntAttrib(ObjPt, "extreme_type", ExtType);
	    IPListObjectAppend(ObjPtList, ObjPt);
	}
    }
    else
        ObjPtList = MvarCnvrtMVPtsToCtlPts(ExtremaPts, -1);
...
IPObjectStruct *GenBoolOneSrfObject(IPObjectStruct *BndryCrv)
{
    ...
    CagdSrfStruct
        *BoolSumSrf = CagdSrfFromNBndryCrvs(BndryCrv -> U.Crvs, FALSE);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Affine reparametrization, effectively changing the domain of animation.    M
*                                                                            *
* PARAMETERS:                                                                M
*   Obj:         An object to change its animation domain.                   M
*   TMin, TMax:  New domain of animation.				     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  An object identical to Obj but with animation domain  M
*                      from TMin to TMax.                                    M
*                                                                            *
* KEYWORDS:                                                                  M
*   AnimReparametrization                                                    M
*****************************************************************************/
IPObjectStruct *AnimReparametrization(IPObjectStruct *Obj,
				      IrtRType *TMin,
				      IrtRType *TMax)
{
    Obj = IPCopyObject(NULL, Obj, TRUE);

    if (!GMAnimAffineTransAnimationOne2(Obj, *TMin, *TMax)) {
        IPFreeObject(Obj);
	return NULL;
    }
    else
