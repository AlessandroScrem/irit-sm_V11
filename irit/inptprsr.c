...
/* Operator preceding parser stack, and stack pointer: */
...
IRIT_GLOBAL_DATA int
    InptPrsrLastToken = IP_TKN_END;
IRIT_GLOBAL_DATA InptPrsrEvalErrType
    IPGlblParseError = IPE_NO_ERR;

#ifdef DEBUG1
    IRIT_GLOBAL_DATA int MaxStackPointer = 0; /* Measure max depth of stack. */
#endif /* DEBUG1 */
...
*   Algorithm: Using operator precedence with the following grammar:         M
...
ParseTree *InptPrsrGenInputParseTree(void)
{
    ...
    InptPrsrLastToken = IP_TKN_END;    /* Holds last token read from stream. */
    IPGlblParseError = IPE_NO_ERR;		     /* No errors so far ... */

    Root = OperatorPrecedence();

    if (IPGlblParseError) {
	/* Free partially allocated tree. */
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Same as InptPrsrGenInputParseTree but aslo return error code if any.     M
*                                                                            *
* PARAMETERS:                                                                M
*   Error:   Error code if any or IPE_NO_ERR if none                         M
*                                                                            *
* RETURN VALUE:                                                              M
*   ParseTree *:   Constructed parsed tree.                                  M
*                                                                            *
...
*   InptPrsrGenInputParseTree                                                M
*****************************************************************************/
ParseTree *InptPrsrGenInputParseTree2(int *Error)
{
    ParseTree
        *PT = InptPrsrGenInputParseTree();

    *Error = IPGlblParseError;
