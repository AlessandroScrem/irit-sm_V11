...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Fetches the size of the Db stack (number of objects in DB).              M
*                                                                            *
* PARAMETERS:                                                                M
*   None		                                                     M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:  Size of DB stack.                                                  M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritDBGetStackSize                                                       M
*****************************************************************************/
int IritDBGetStackSize()
{
    return IritDBStackSize;
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Fetch one object from DB stack.					     M
*                                                                            *
* PARAMETERS:                                                                M
*   Idx:  Index of object to fetch from Stack. Between 0 & IritDBStackSize.  M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  Fetched object from DB.                               M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritDBGetStackObject                                                     M
*****************************************************************************/
IPObjectStruct *IritDBGetStackObject(int Idx)
{
    return IritDBObj[Idx];
}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Inserts the given hierarchy of this object, if any, into DB.		     M
* Note the root is already assumed in the DB and is NOT inserted.            M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:     Object to insert to the DB.                                    M
*   ByReference:  TRUE to insert the objects, FALSE to insert a new copy.    M
*                                                                            *
* RETURN VALUE:                                                              M
*   void                                                                     M
*                                                                            *
* SEE ALSO:                                                                  M
*   IritDBInsertObject, IritDBInsertObjectLast                               M
*                                                                            *
* KEYWORDS:                                                                  M
*   IritDBInsertHierarchy                                                    M
*****************************************************************************/
void IritDBInsertHierarchy(IPObjectStruct *PObj, int ByReference)
{
    ...
    while ((PTmp = IPListObjectGet(PObj, i++)) != NULL) {
        assert(PTmp -> ObjName != NULL && strlen(PTmp -> ObjName) > 0);

	if (IritDBGetObjByName(PTmp -> ObjName) == NULL)
	    IritDBInsertObject(ByReference ? PTmp
					   : IPCopyObject(NULL, PTmp, TRUE),
			       FALSE);

	IritDBInsertHierarchy(PTmp, ByReference);
    }
}
