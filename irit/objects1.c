...
double GetObjectSize(IPObjectStruct *Obj)
{
	...
	case IP_OBJ_TRISRF:
	    if (Obj -> U.TriSrfs -> Pnext != NULL)
	        return (double) CagdListLength(Obj -> U.TriSrfs);
	    else
	        return (double) TRNG_LENGTH_MESH_SIZE(Obj -> U.TriSrfs
						                 -> Length);
	case IP_OBJ_TRIVAR:
	...
	*CoordPObj = NULL;
	...
	case IP_OBJ_TRIMSRF:
	    if (PObj -> U.TrimSrfs == NULL)
	        break;
	    if (PObj -> U.TrimSrfs -> Pnext != NULL) {
		TrimSrfStruct *TSrf;

		/* Extract a single trimmed surface from the list. */
		for (TSrf = PObj -> U.TrimSrfs;
		     TSrf != NULL && Index > 0;
		     TSrf = TSrf -> Pnext, Index--);
		if (TSrf != NULL)
		    CoordPObj = IPGenTRIMSRFObject(TrimSrfCopy(TSrf));
	    }
	    else {
		CagdSrfStruct
		...
		/* Extract a ctlpt from the surface. */
		CagdEditSingleSrfPt(Srf, &CtlPt,
				    Index % Srf -> ULength,
				    Index / Srf -> ULength,
				    FALSE);
		CoordPObj = IPGenCTLPTObject(Srf -> PType, CtlPt.Coords);
	    }
	    break;
	case IP_OBJ_TRIVAR:
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the area of the given entity.		                     M
*                                                                            *
* PARAMETERS:                                                                M
*   Obj:       To compute area for. 		                             M
*                                                                            *
* RETURN VALUE:                                                              M
*   double:   Area of Obj.				                     M
*                                                                            *
* SEE ALSO:                                                                  M
*   GMPolyObjectArea, SymbCrvEnclosedAreaEval                                M
*                                                                            *
* KEYWORDS:                                                                  M
*   CrvAreaIntergal                                                          M
*****************************************************************************/
double GetObjectArea(const IPObjectStruct *Obj)
{
    if (IP_IS_CRV_OBJ(Obj))
	return SymbCrvEnclosedAreaEval(Obj -> U.Crvs);
    else if (IP_IS_POLY_OBJ(Obj))
        return GMPolyObjectArea(Obj);
    else
	return 0.0;
}
