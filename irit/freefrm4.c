...
#define IRIT_BITANGENT_EPS	1e-10
				  ...
				  IrtRType *RHasTopLvlTrim)
{
	...
	*Crvs = NULL;
	...
	while ((CrvObj = IPListObjectGet(TrimmedCrvsObj, NumCrvs++)) != NULL) {
		...
		IRIT_NON_FATAL_ERROR("GenTSrf: Non curve object as a trimmed curve");
		return NULL;
	    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Evaluate the volume enclosed by a trivariate.			     M
*                                                                            *
* PARAMETERS:                                                                M
*   TVObj:    Trivariate to evaluate its enclosed volume.                    M
*   VolType:  TRUE to intergrate the surfaces with respect to the XY plane,  M
*             FALSE to intergrate the surfaces with respect to the origin,   M
*                                                                            *
* RETURN VALUE:                                                              M
*   IrtRType:  The volume (can be negative if TV reversed). 	             M
*                                                                            *
* KEYWORDS:                                                                  M
*   EvalTrivarVolume                                                         M
*****************************************************************************/
IrtRType EvalTrivarVolume(IPObjectStruct *TVObj, IrtRType *VolType)
{
    return TrivTVVolume(TVObj -> U.Trivars, IRIT_REAL_PTR_TO_INT(VolType));
}
...
IPObjectStruct *ComputeCrvMoments(IPObjectStruct *CrvObj, IrtRType *RMoment)
{
    ...
    CagdCrvFirstMoments(CrvObj -> U.Crvs, 1000, Pt, Vec);
...
IPObjectStruct *GenTVPREVObject(IPObjectStruct *Cross)
{
	...
	int IsRational = AttrGetObjectIntAttrib(Cross, "Rational");
        IrtRType
	    StartAngle = AttrGetObjectRealAttrib(Cross, "StartAngle"),
	    EndAngle = AttrGetObjectRealAttrib(Cross, "EndAngle");

	if (IP_ATTR_IS_BAD_INT(IsRational))
	    IsRational = FALSE;

	if (!IP_ATTR_IS_BAD_REAL(StartAngle) &&
	    !IP_ATTR_IS_BAD_REAL(EndAngle)) {
	    return IPGenTRIVARObject(TrivTVOfRev2(Cross -> U.Srfs,
						  !IsRational,
						  StartAngle,
						  EndAngle));
	}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the jacobian of a given trivariate.			     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:        Trivariate to compute its Jacobian.		             M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  A scalar trivarate representing the determinant of    M
*                      the Jacobian of TV.				     M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivComputeJacobian                                                      M
*****************************************************************************/
IPObjectStruct *TrivComputeJacobian(IPObjectStruct *Tv)
{
    MvarMVStruct
        *JMV = MvarCalculateTVJacobian(Tv -> U.Trivars);
    TrivTVStruct *JTV;
    ...
    return IPGenTRIVARObject(JTV);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Attempts to make the Jacobian of the given trivariate more uniform/fair. M
*                                                                            *
* PARAMETERS:                                                                M
*   Tv:        Trivariate to improve its Jacobian.		             M
*   StepSize:  Step size of the numeric marching.			     M
*   RIters:    Number of numeric improvement iterations to perform.          M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  Piecewise linear polygonal approximation of result.   M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivImproveJacobian                                                      M
*****************************************************************************/
IPObjectStruct *TrivImproveJacobian(IPObjectStruct *Tv,
				    IrtRType *StepSize,
				    IrtRType *RIters)
{
    TrivTVStruct
        *ImpJTv = TrivTVCopy(Tv -> U.Trivars);

    MvarTrivJacobianImprove(ImpJTv, *StepSize, IRIT_REAL_PTR_TO_INT(RIters));

    return IPGenTRIVARObject(ImpJTv);
}
			  ...
			  IrtRType *RMode)
{
	...
	case 1:
		...
		if (IP_IS_NUM_OBJ(PNum1) && IP_IS_NUM_OBJ(PNum2)) {
		    IPObjectStruct *ObjPllnsList;
		    MvarPolylineStruct
		        *Pllns = MVarCrvKernelSilhouette(Crv -> U.Crvs,
							 *RGamma,
							 PNum1 -> U.R,
							 PNum2 -> U.R);

		    ObjPllnsList = MvarCnvrtMVPolysToIritPolys(Pllns);
	
		    MvarPolylineFreeList(Pllns);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Creates a blending surface that blends the given two surfaces, while     M
* interpolating Srf1 at UMin and Srf2 at UMax.  Continuity at interpolated   M
* locations is governed by the BlendDegree (2 for C*0, 4 for C^1).	     M
*                                                                            *
* PARAMETERS:                                                                M
*   Srf1:         Starting surface.                                          M
*   Srf2:         Terminaing surface.                                        M
*   BlendDegree:  Governs the continuity (2 for C*0, 4 for C^1).	     M
*   TanScale:     If C^1, controls the tangent fields scale.		     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  The blended surface.				     M
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbBlendTwoSurfaces                                                     M
*                                                                            *
* KEYWORDS:                                                                  M
*   GenBlendTwoSurface		                                             M
*****************************************************************************/
IPObjectStruct *GenBlendTwoSurface(IPObjectStruct *Srf1,
				   IPObjectStruct *Srf2,
				   IrtRType *BlendDegree,
				   IrtRType *TanScale)
{
    CagdSrfStruct
        *Srf = CagdBlendTwoSurfaces(Srf1 -> U.Srfs,
			            Srf2 -> U.Srfs,
			            IRIT_REAL_PTR_TO_INT(BlendDegree),
			            *TanScale);
...
*   RMaxError:     TRUE for minimizing maximal error, FALSE to minimize the  M
*                  error's sum over the entire domain.			     M
*                                                                            *
* RETURN VALUE:                                                              M
			       ...
			       IrtRType *RMaxError)
{
	...
	MaxError = IRIT_REAL_PTR_TO_INT(RMaxError);
    ...
    Crv = CagdMatchingTwoCurves(PCrv1 -> U.Crvs, PCrv2 -> U.Crvs,
				...
				Rotate, AllowNegativeNorm, FALSE,
				MaxError, MatchNorm);
...
*   PSubdivTol: Subdivision tolerance of the approximation.		     M
*              If >1, assumes tesselates object (if necessary) and uses the  M
*              Boolean obj-plane intersections.			             M
*	       If <1 and a freeform shape, uses the multivariate solver.     M
				...
				IPObjectStruct *PValidationObj)
{
	...
	Cntrs = UserCntrSrfWithPlane(PObj -> U.Srfs, Plane, *PSubdivTol,
				     *PSubdivTol < 1.0);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Tests for inclusion of Pt in a given trivariate.		             M
*   This function is optimized for many pt-in-trivar queries.                M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:       Trivariate to test for point inclusion.                      M
*   Pt:         Point position.		                                     M
*   RSampling:  Trivar sampling rate in each parametric direction:           M
*               If positive, sets sampling rates along the TV edge and prep. M
*               some auxiliary data structures.			             M
*               If negative, prep. aux. data is freed.	                     M
*               If zero, ignored (during actual inclusion queries).          M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  TRUE if Pt inside trivar, FALSE otherwise.	     M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivarPtInclusion                                                        M
*****************************************************************************/
IPObjectStruct *TrivarPtInclusion(IPObjectStruct *PObj,
				  IrtPtType Pt,
				  IrtRType *RSampling)
{
    int Sampling = IRIT_REAL_PTR_TO_INT(RSampling);

    if (Sampling != 0) {
	if (Sampling > 0)
	    TrivTVPointInclusionPrep(PObj -> U.Trivars, Sampling);
	else
	    TrivTVPointInclusionFree(PObj -> U.Trivars);
	return NULL;
    }
    else
        return IPGenNUMValObject(TrivTVPointInclusion(PObj -> U.Trivars, Pt));
}
...
IPObjectStruct *FreeFormGeomType(IPObjectStruct *FreeForm)
{
	...
	case TRNG_TRISRF_BEZIER_TYPE:
	    return IPGenNUMValObject(IP_COERCE_BEZIER_TYPE);
	...
	case TRNG_TRISRF_BSPLINE_TYPE:
	    return IPGenNUMValObject(IP_COERCE_BSPLINE_TYPE);

	case CAGD_CPOWER_TYPE:
	case CAGD_SPOWER_TYPE:
	case TRIV_TVPOWER_TYPE:
	case MVAR_POWER_TYPE:
	    return IPGenNUMValObject(IP_COERCE_POWER_TYPE);
...
*                                                                            *
* PARAMETERS:                                                                M
*   Crv:         Curve(s) to compute mutual tangent lines.                   M
*                  If only on curve is provided - its self bi-tangents are   M
*                computed.  otherwise the bi-tangent to the given two curves M
*                are derived.						     M
*   FineNess:    The accuracy of the numeric computation.                    M
*                                                                            *
* RETURN VALUE:                                                              M
...
*   CrvTwoTangents                                                           M
*****************************************************************************/
IPObjectStruct *CrvTwoTangents(IPObjectStruct *Crvs, IrtRType *FineNess)
{
    int Count = 0;
    IrtRType
	R = 0.0;
    ...
    if (IP_IS_CRV_OBJ(Crvs)) {
        Pts = MvarMVBiTangentLine(Crvs -> U.Crvs, Crvs -> U.Crvs, 
				  *FineNess, IRIT_BITANGENT_EPS);
    }
    else if (IP_IS_OLST_OBJ(Crvs)) {
	IPObjectStruct
	    *PCrv1 = IPListObjectGet(Crvs, 0),
	    *PCrv2 = IPListObjectGet(Crvs, 1);

	if (IP_IS_CRV_OBJ(PCrv1) && IP_IS_CRV_OBJ(PCrv2)) {
	    Pts = MvarMVBiTangentLine(PCrv1 -> U.Crvs, PCrv2 -> U.Crvs, 
				      *FineNess, IRIT_BITANGENT_EPS);
	}
	else {
	    IRIT_NON_FATAL_ERROR("Bi-Tangents: Expected two curves in the list");
	    return NULL;
        }
    }
    else {
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes all lines that are tangent to SrfList at three locations.       M
*                                                                            *
* PARAMETERS:                                                                M
*   SrfList:      Three surfaces to compute tri-tangenties.		     M
*   StepSize:     Tolerance for the univariate curve tracing.		     M
*		  or similar normal orientation bi tangencies only.          M
*   SubdivTol:    The accuracy of the subdivision computation.               M
*   NumericTol:   The accuracy of the numeric computation.                   M
*   Euclidean:    TRUE result in Euclidean space.  If FALSE, Tv param space. M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  A list object with all parameter value pairs of the   M
*		       tangents.  Each point object in list will contain     M
...
*   SrfThreeTangents                                                         M
*****************************************************************************/
IPObjectStruct *SrfThreeTangentsLine(IPObjectStruct *SrfList,
				     IrtRType *StepSize,
				     IrtRType *SubdivTol,
				     IrtRType *NumericTol,
				     IrtRType *Euclidean)
{
	...
	*PObj2 = IPListObjectGet(SrfList, 1),
	*PObj3 = IPListObjectGet(SrfList, 2);
    CagdCrvStruct *Crv1, *Crv2, *Crv3, *Crvs;
    CagdSrfStruct *Srf1, *Srf2, *Srf3;
    IPObjectStruct
        *RetVal = IPGenLISTObject(NULL);

    if (PObj1 == NULL || !IP_IS_SRF_OBJ(PObj1) ||
	PObj2 == NULL || !IP_IS_SRF_OBJ(PObj2) ||
	PObj3 == NULL || !IP_IS_SRF_OBJ(PObj3)) {
	IRIT_WNDW_PUT_STR("SRF3TANS: Expected a list of surfaces.");
	return NULL;
    }

    Srf1 = CagdCnvrtBsp2OpenSrf(PObj1 -> U.Srfs);
    Srf2 = CagdCnvrtBsp2OpenSrf(PObj2 -> U.Srfs);
    Srf3 = CagdCnvrtBsp2OpenSrf(PObj3 -> U.Srfs);

    Crv1 = Crvs = MvarMVTriTangentLine(Srf1, Srf2, Srf3,
				       *StepSize, *SubdivTol, *NumericTol,
				       IRIT_REAL_PTR_TO_INT(Euclidean));
    CagdSrfFree(Srf1);
    CagdSrfFree(Srf2);
    CagdSrfFree(Srf3);
	...
	/* Push ruled surfaces between the curves on the result object. */
	Srf = CagdRuledSrf(Crv1, Crv2, 2, 2);
	SrfObj = IPGenSRFObject(Srf);
	IPListObjectInsert(RetVal, Count++, SrfObj);
	Srf = CagdRuledSrf(Crv2, Crv3, 2, 2);
	SrfObj = IPGenSRFObject(Srf);
	IPListObjectInsert(RetVal, Count++, SrfObj);
