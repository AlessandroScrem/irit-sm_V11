...
IPObjectStruct *GenSURFPREVObject(IPObjectStruct *Cross)
{
	...
	if (!IP_ATTR_IS_BAD_REAL(StartAngle) &&
	    ...
	    return IPGenSRFObject(CagdSurfaceRev2(Cross -> U.Crvs,
						  !IsRational,
						  StartAngle,
						  EndAngle));
	}
	    ...
	    return IPGenSRFObject(CagdSurfaceRev(Cross -> U.Crvs));
    }
    else
        return NULL;
}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Routine to subdivide a surface into two along a specified curve in the     M
* parameter space of SrfObj.    					     M
*                                                                            *
* PARAMETERS:                                                                M
*   SrfObj:       Surface (possibly a trimmed surface) to subdivide.         M
*   DivCrvObj:    A curve in the parameter space of SrfObj that splits the   M
*                 domain of SrfObj into two.                                 M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:  A list object of two surface objects, result of the   M
*		       subdivision.					     M
*                                                                            *
* SEE ALSO:                                                                  M
*   DivideSurfaceObject, SymbSrfSubdivAtCurve                                M
*                                                                            *
* KEYWORDS:                                                                  M
*   DivideSurfaceAtCurve                                                     M
*****************************************************************************/
IPObjectStruct *DivideSurfaceAtCurve(IPObjectStruct *SrfObj,
				     IPObjectStruct *DivCrvObj)
{
    IPObjectStruct *SrfList,
	*Srf1 = NULL,
	*Srf2 = NULL;
    ...
    if ((Srf = BzrSrfSubdivAtCurve(SrfObj -> U.Srfs,
				   DivCrvObj -> U.Crvs)) == NULL)
        return NULL;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Extract the knot lines of the control mesh/polygon of given freeform.    M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:         A freeform geometry to extract its knot lines.             M
*   DoKnotLines:  TRUE for knot polylines,				     M
*                 FALSE for knots isoparametric curves.			     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:    Geometry representing the knot lines/curves.        M
*                                                                            *
* KEYWORDS:                                                                  M
*   Freeform2KnotLinesCurves                                                 M
*****************************************************************************/
IPObjectStruct *Freeform2KnotLinesCurves(IPObjectStruct *PObj,
					 CagdRType *DoKnotLines)
{
    CagdPolylineStruct
	*KnotLines = NULL;
    CagdCrvStruct 
    ...
    switch (PObj -> ObjType) {
	case IP_OBJ_SURFACE:
	    if (IRIT_REAL_PTR_TO_INT(DoKnotLines))
	        KnotLines = CagdSrf2KnotLines(PObj -> U.Srfs);
	    else
	        KnotCurves = CagdSrf2KnotCurves(PObj -> U.Srfs);
	    break;
	case IP_OBJ_TRIVAR:
	    break;
        default:
    ...
    if (KnotLines != NULL) {
	IPPolygonStruct
	    *IritKnots = IPCagdPllns2IritPllns(KnotLines);

	return IPGenPOLYLINEObject(IritKnots);
    }
    else if (KnotCurves != NULL) {
	return IPGenCRVObject(KnotCurves);
    }
    else {
	IRIT_NON_FATAL_ERROR("FFKntLns: unable to compute knot lines for given object");
	return NULL;
    }
}
