				...
				IrtRType *Euclidean)
{
	...
	*Srf = SrfObj -> U.Srfs;
	...
	IP_IS_OLST_OBJ(LinesPts)) {
		...
		if ((PCntrObj = ContourFreeform(PZeroObj, Plane, &Resolution,
						NULL, NULL)) != NULL) {
...
static IrtPtType *ConvertListOfPtsToArray(IPObjectStruct *PtSet, int *n)
{
    ...
    *n = NumVertices;
    ...
    for (i = 0; i < NumVertices; i++) {
        PtObj = IPCoerceObjectPtTypeTo(IPListObjectGet(PtSet, i), IP_OBJ_POINT);
			      ...
			      IrtRType *LclUVs)
{
    if (IP_IS_SRF_OBJ(Srf)) {
        return UserDDMPolysOverSrf(Srf -> U.Srfs, Texture, *DuDup, *DvDup,
				   IRIT_REAL_PTR_TO_INT(LclUVs), FALSE);
    }
    if (IP_IS_TRIMSRF_OBJ(Srf)) {
        return UserDDMPolysOverTrimmedSrf(Srf -> U.TrimSrfs, Texture,
					  *DuDup, *DvDup,
					  IRIT_REAL_PTR_TO_INT(LclUVs), FALSE);
    }
        ...
        IRIT_NON_FATAL_ERROR("Expecting either a polygonal mesh or a (trimmed) surface.");
        return NULL;
    }
}
...
IPObjectStruct *EvalCurveCurvature(IPObjectStruct *PCrv, IrtRType *t)
{
    ...
    if (AttrGetRefPtrAttrib(PCrv -> U.Crvs -> Attr, "_EvalCurv") == NULL)
				     ...
				     IrtRType *REuclidean)
{
    ...
    if (AttrGetRefPtrAttrib(PSrf -> U.Srfs -> Attr, "_EvalCurv") == NULL)
...
*                                                                            *
* PARAMETERS:                                                                M
*   PSrfObj:    Surface to compute its fixed curvature lines, if any.        M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   UserSrfFixedCurvatureLines                                               M
*                                                                            *
* KEYWORDS:                                                                  M
*   SrfFixedCurvatureLines                                                   M
*****************************************************************************/
IPObjectStruct *SrfFixedCurvatureLines(IPObjectStruct *PSrfObj,
				       IrtRType *k1,
				       IrtRType *Step,
				       IrtRType *SubdivTol,
				       IrtRType *NumericTol,
...
{
    IPObjectStruct
        *PRetVal = UserSrfFixedCurvatureLines(PSrfObj -> U.Srfs, *k1,
					      *Step, *SubdivTol, *NumericTol,
				  ...
				  IrtRType *Euclidean)
{
	...
	*CtlPts = NULL;
	...
	if (*MinNrmlDeviation < 0.0)
	    MVPls = MvarSrfSelfInterDiagFactor(FF -> U.Srfs, *SubdivTol,
					       *NumerTol);
	else
	    MVPls = MvarSrfSelfInterNrmlDev(FF -> U.Srfs, *SubdivTol,
					    *NumerTol, *MinNrmlDeviation);

	CtlPts = MvarCnvrtMVPolysToCtlPts(MVPls);
				...
				IrtRType *Eps)
{
	...
	case IP_OBJ_CTLPT:
	    if (PObj1 -> ObjType == IP_OBJ_CTLPT) {
	        PTmp = IPCoerceObjectPtTypeTo(PObj1, IP_OBJ_POINT);
	    ...
	    switch (PObj2 -> ObjType) {
		    ...
		    if (PObj2 -> ObjType == IP_OBJ_CTLPT) {
		        PTmp = IPCoerceObjectPtTypeTo(PObj2, IP_OBJ_POINT);
    ...
    /* Convert Dist, Param1, Param2 to IPObjectStructs. */
...
*   MvarHFDistSrfSrfC1, MinimalDistance, HausdorffDistByPts	             M
*                                                                            *
* KEYWORDS:                                                                  M
*   HausdorffDistance                                                        M
*****************************************************************************/
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Estimate the Hausdorff distance between freeforms by sampling points.    M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj1:     First point/curve/surface.                                    M
*   PObj2:     Second point/curve/surface.                                   M
*   Samples:   Number of samples to take, uniformly in each parameter. For   M
*              example, for srf (Samples x Samples) samples will be taken.   M
*   HDistSide: 1 for HD from V1 to V2, 2 for HD from V2 to V1, 3 for a       M
*              symmetric HD.                                                 M
*                                                                            *
* RETURN VALUE:                                                              M
*   IrtRType:    The estimate (bound from above) on the Hausdorff distance.  M
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbHausDistBySamplesCrvCrv, SymbHausDistBySamplesCrvSrf		     M
*   SymbHausDistBySamplesSrfSrf, HausdorffDistance		             M
*                                                                            *
* KEYWORDS:                                                                  M
*   HausdorffDistByPts                                                       M
*****************************************************************************/
IrtRType HausdorffDistByPts(IPObjectStruct *PObj1,
			    IPObjectStruct *PObj2,
			    IrtRType *Samples,
			    IrtRType *HDistSide)
{
    int ISamples = IRIT_REAL_PTR_TO_INT(Samples),
        IHDistSide = IRIT_REAL_PTR_TO_INT(HDistSide);
    IrtRType
        HD = -1.0;
	...
	case IP_OBJ_CURVE:
	    switch (PObj2 -> ObjType) {
		case IP_OBJ_CURVE:
		    HD = SymbHausDistBySamplesCrvCrv(PObj1 -> U.Crvs,
						     PObj2 -> U.Crvs,
						     ISamples,
						     IHDistSide);
		    break;
		case IP_OBJ_SURFACE:
		    HD = SymbHausDistBySamplesCrvSrf(PObj1 -> U.Crvs,
						     PObj2 -> U.Srfs,
						     ISamples,
						     IHDistSide);
		    break;
	        default:
		    IRIT_WNDW_PUT_STR("Hausdorff: Expecting curves or surfaces only.");
		    break;
	    }
	    break;
	case IP_OBJ_SURFACE:
	    switch (PObj2 -> ObjType) {
		case IP_OBJ_CURVE:
		    HD = SymbHausDistBySamplesCrvSrf(PObj2 -> U.Crvs,
						     PObj1 -> U.Srfs,
						     ISamples,
						     IHDistSide);
		    break;
		case IP_OBJ_SURFACE:
		    HD = SymbHausDistBySamplesSrfSrf(PObj1 -> U.Srfs,
						     PObj2 -> U.Srfs,
						     ISamples,
						     IHDistSide);
		    break;
			     ...
			     IrtRType *Euclidean)
{
    ...
    MvarPolylineStruct *MVPl,
        *MVPlls = MvarSrfSrfInter(Srf1, Srf2, *Step, *SubdivTol, *NumerTol);
    IPPolygonStruct
        *Plls = NULL;
    IPObjectStruct
        *PtsObj = NULL,
