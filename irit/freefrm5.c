...
*   SubdivTol:  Accuracy of piecewise linear bisector approximation, or	     M
*               scaling of the ruling direction in the curve/point bisector. M
*                                                                            *
* RETURN VALUE:                                                              M
...
static int Skel2D2PrimsCnvrt(IPObjectStruct *Obj, MvarSkel2DPrimStruct *Prim)
{
	...
	IP_IS_CTLPT_OBJ(Obj)) {
	IPObjectStruct
	    *TmpObj = IPCoerceObjectPtTypeTo(Obj, IP_OBJ_POINT);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the MAT (Medial axis transform) of a given closed freeform      M
* planar curve.								     M
*                                                                            *
* PARAMETERS:                                                                M
*   CrvObj:        The closed curve to compute the MAT for.                  M
...
{
    CagdCrvStruct
        *MatCrvs = MvarCrv2DMAT(CrvObj -> U.Crvs, *SubdivTol, *NumericTol,
			        FALSE);
    IPObjectStruct 
        *PRetVal = IPLnkListToListObject(MatCrvs, IP_OBJ_CURVE);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Approximates a given freeform curve using piecewise cubics.              M
*                                                                            *
* PARAMETERS:                                                                M
*   PCrv:      The curve to piecewise cubic approximate.                     M
*   Tolerance: Of approximation, in Hausdorff distance sense.                M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:    List of C^1 cubics approximting PCrv.               M
*                                                                            *
* SEE ALSO:                                                                  M
*   CrvCubicApprox, CrvBiArcApprox                                           M
*                                                                            *
* KEYWORDS:                                                                  M
*   CrvCubicApprox                                                           M
*****************************************************************************/
...
{
    CagdCrvStruct
	*Cubics = SymbCrvCubicApprox(PCrv -> U.Crvs, *Tolerance);

    if (Cubics == NULL)
        return NULL;

    return IPLnkListToListObject(Cubics, IP_OBJ_CURVE);
}
