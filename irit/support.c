...
IRIT_STATIC_DATA
    IritFatalMsgFuncType GlblSetNonFatalErrorFunc = NULL;
...
IRIT_GLOBAL_DATA BspMultComputationMethodType
    GlblBspProdMethod = BSP_MULT_BEZ_DECOMP;
...
IRIT_GLOBAL_DATA int
    ...
    GlblPropagateNames = FALSE,
    GlblMakeAllHierarchyVisible = FALSE,
...
IPObjectStruct *IritSetIritState(const char *Name, IPObjectStruct *Data)
{
	...
	*OldVal = NULL;
    ...
    else if (stricmp(Name, "HierarchyVisible") == 0) {
	if (IP_IS_NUM_OBJ(Data)) {
	    OldVal = IPGenNUMValObject(GlblMakeAllHierarchyVisible);
	    GlblMakeAllHierarchyVisible = IRIT_REAL_TO_INT(Data -> U.R);
	}
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
    ...
    else if (stricmp(Name, "MVBivarOutPllns") == 0) {
	if (IP_IS_NUM_OBJ(Data))
	    OldVal = IPGenNUMValObject(MvarMVsZeros2DPolylines(IRIT_REAL_TO_INT(Data -> U.R)));
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
    else if (stricmp(Name, "MvDmnExt") == 0) {
	if (IP_IS_NUM_OBJ(Data))
	    OldVal = IPGenNUMValObject(MvarMVsZerosDmnExt(Data -> U.R));
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
    else if (stricmp(Name, "MVSbdvTolAction") == 0) {
	if (IP_IS_NUM_OBJ(Data))
	    OldVal = IPGenNUMValObject(MvarZerosSubdivTolAction(IRIT_REAL_TO_INT(Data -> U.R)));
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
    ...
    else if (stricmp(Name, "MvSnglrPts") == 0) {
	if (IP_IS_NUM_OBJ(Data))
	    OldVal = IPGenNUMValObject(MvarMVsZeros1DMergeSingularPts(IRIT_REAL_TO_INT(Data -> U.R)));
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
    ...
    else if (stricmp(Name, "PropagateNames") == 0) {
	if (IP_IS_NUM_OBJ(Data)) {
	    OldVal = IPGenNUMValObject(GlblPropagateNames);
	    GlblPropagateNames = IRIT_REAL_TO_INT(Data -> U.R);
	}
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
    ...
    else if (stricmp(Name, "TCrvs2Lin") == 0) {
	if (IP_IS_NUM_OBJ(Data)) {
	    OldVal = IPGenNUMValObject(
	       TrimSrfSubdivAtParamForcePiecwiseLinearTrimCrvs(
					      IRIT_REAL_TO_INT(Data -> U.R)));
	}
	else
	    IRIT_WNDW_PUT_STR("Numeric state value expected");
    }
...
*                                                                            *
* RETURN VALUE:                                                              M
*   IritFatalMsgFuncType:  Old error function reference.                     M
*                                                                            *
* KEYWORDS:                                                                  M
...
IritFatalMsgFuncType IritSetNonFatalErrorFunc(IritFatalMsgFuncType ErrorFunc)
{
	...
	OldNonFatalErrorFunc = GlblSetNonFatalErrorFunc;

    GlblSetNonFatalErrorFunc = ErrorFunc;

    return OldNonFatalErrorFunc;
}
...
void IritNonFatalError(const char *Format, ...)
{
    ...
    if (GlblSetNonFatalErrorFunc) {
        GlblSetNonFatalErrorFunc(p);
