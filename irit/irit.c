...
#elif NO_CONCAT_STR
     "(C) Copyright 1989-2015 Gershon Elber,  Unix  Version 10";
#else
...
/* GERSHON
    {
	IrtRType Start = 1.0, KeepStackStep = 0.0, TrackAllocID = 4;
	void *p;
	IritCheckMarkDynMemory(&Start, &KeepStackStep, &TrackAllocID);
	IritMalloc(5);
	p = IritMalloc(10);
	IritFree(p);
	p =IritMalloc(20);
...
* standard input and execute it "forever" (using Input Parser).		     *
*   Note exit from this program is controlled by input parser itself.	     *
*                                                                            *
* PARAMETERS:                                                                *
*   None                                                                     *
*                                                                            *
...
static void Interact(void)
{
    ...
    /* recoverable, 2 if cannot continue - must quit the program now.	    */
    switch (setjmp(IritGlblLongJumpBuffer)) {    /* Used in error recovery. */
	case 1:
	    IritDBValidateVariables();
	case 0:
	    /* Trap floating point errors & use the above setjmp to return. */
	    signal(SIGFPE, IritDefaultFPEHandler);
#	    ifdef WNT_SET_FPE_CONTROL
	    _controlfp(0,
		       _EM_INVALID |
		       _EM_UNDERFLOW |
		       _EM_OVERFLOW |
		       _EM_ZERODIVIDE);
#	    endif /* WNT_SET_FPE_CONTROL */
...
#   ifdef __WINNT__
#	ifdef DEBUG_DUMP_LEAKY_BLOCKS
	    _CrtDumpMemoryLeaks();
#	endif /* DEBUG_DUMP_LEAKY_BLOCKS */
