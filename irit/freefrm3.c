...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes intersection properties of two curves:			     M
* 1. The intersecting regions,						     M
* 2. The intersting regions' areas,					     M
* 3. The differential of the areas as a functin of the control points, as a  M
* chained list of both curves' XY coordinates:				     M
*     dAdPt = (P1x, P1y, P2x, P2y,...Q1x, Q1y, Q2x, Q2y, ...).		     M
*   Returned is a list of the form (dAdPt, Rgn1, Rgn2, ...) and each Rngi    M
* has an area attribute on it, if any.					     M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj1, PObj2:  Two curves to intersect.                                  M
*   Eps:           Accuracy control of the intersection.                     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:   List object of all intersection properties.          M
*                                                                            *
* SEE ALSO:                                                                  M
*   SymbGet2CrvsIntersectionRegions, SymbGet2CrvsIntersectionAreas           M
*   SymbGet2CrvsInterDAreaDCtlPt.					     M
*                                                                            *
* KEYWORDS:                                                                  M
*   CrvCrvInterProps                                                         M
*****************************************************************************/
IPObjectStruct *CrvCrvInterProps(IPObjectStruct *PObj1,
				 IPObjectStruct *PObj2,
				 IrtRType *Eps)
{
    int i, n;
    CagdRType R, *InterDomains, *dAreadPts;
    CagdCrvStruct *ICrv,
        *InterCrvs = SymbGet2CrvsIntersectionRegions(PObj1 -> U.Crvs,
						     PObj2 -> U.Crvs, *Eps);
    IPObjectStruct *PObj, *PdAdPt,
    ...
    /* Compute area differentials. */
    SymbGet2CrvsInterDAreaDCtlPts(PObj1 -> U.Crvs, PObj2 -> U.Crvs, *Eps,
				  &InterDomains, &dAreadPts);
    PdAdPt = IPGenLISTObject(NULL);
    n = (PObj1 -> U.Crvs -> Length + PObj2 -> U.Crvs -> Length) * 2;
    for (i = 0; i < n; i++)
        IPListObjectAppend(PdAdPt, IPGenNUMValObject(dAreadPts[i]));
    IPListObjectAppend(RetList, PdAdPt);

    /* Compute the areas. */
    while (InterCrvs != NULL) {
        IRIT_LIST_POP(ICrv, InterCrvs);

	R = SymbCrvEnclosedAreaEval(ICrv);

	PObj = IPGenCRVObject(ICrv);
	AttrSetObjectRealAttrib(PObj, "area", IRIT_ABS(R));
	IPListObjectAppend(RetList, PObj);
    }
				       ...
				       IPObjectStruct *Params)
{
	...
	case USER_CA_OPER_CREATE:
	    ...
	    if ((NewCA = UserCrvArngmnt(Operation, NULL,
	        ...
	        IRIT_WNDW_PUT_STR("Input is singular, colinear, or not plannar");
	    }
	    break;
	...
	*PGeom2 = IPListObjectGet(PGeom, 1);
	...
	if (PGeom3 == NULL) {
		...
		CAGD_NUM_OF_PT_COORD(PGeom2 -> U.Srfs -> PType) >= 3) {
	        if (IRIT_REAL_PTR_TO_INT(UseExprTree))
	            IRIT_NON_FATAL_ERROR("MVInter: Surface-Surface univariate intersection does not support expression trees - using regular multivariates.");

	        MVPls = MvarSrfSrfInter(PGeom1 -> U.Srfs, PGeom2 -> U.Srfs,
					*SubdivTol * 0.1, *SubdivTol,
					*NumerTol);
	    }
	    else {
	        IRIT_NON_FATAL_ERROR("MVInter: Incompatible geometry.");
	...
	MvarPtFreeList(MVPts);
        ...
        NewPObj = MvarCnvrtMVPolysToIritPolys(MVPls);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Composes a curve/surface on a curve/surface/trivar.			     M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj1:    A curve or a surface or a trivar to compose.                   M
*   PObj2:    A curve or a surface in the parametric space of PObj1.         M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:   A composed curve/surface of the form PObj1(Pobj2).   M
*                                                                            *
* KEYWORDS:                                                                  M
*   FreeformComposition                                                      M
*****************************************************************************/
IPObjectStruct *FreeformComposition(IPObjectStruct *PObj1,
				    IPObjectStruct *PObj2)
{
    IPObjectStruct
	*Obj = NULL;
	...
	if (IP_IS_SRF_OBJ(PObj2)) {
	    IRIT_FATAL_ERROR("Curve as first and surface as second composition parameter is not allowed.");
	    return NULL;
	}
	assert(IP_IS_CRV_OBJ(PObj2));

	Crv = SymbComposePeriodicCrvCrv(PObj1 -> U.Crvs, PObj2 -> U.Crvs,
					IRIT_EPS);
	...
	if (IP_IS_SRF_OBJ(PObj2)) {
	    Srf = SymbComposeSrfSrf(PObj1 -> U.Srfs, PObj2 -> U.Srfs);
	    ...
	    Crv = SymbComposePeriodicSrfCrv(PObj1 -> U.Srfs, PObj2 -> U.Crvs,
					    IRIT_EPS);

	    Obj = Crv ? IPGenCRVObject(Crv) : NULL;
	}
    }
    else if (IP_IS_TRIVAR_OBJ(PObj1)) {
	if (IP_IS_SRF_OBJ(PObj2)) {
	    Srf = TrivComposeTVSrf(PObj1 -> U.Trivars, PObj2 -> U.Srfs);
	    ...
	    Crv = TrivComposeTVCrv(PObj1 -> U.Trivars, PObj2 -> U.Crvs);

	    Obj = Crv ? IPGenCRVObject(Crv) : NULL;
	}
    }
    else {
	IRIT_FATAL_ERROR("Curve or surface or trivar as first parameter only.");
    }
...
{
    IPObjectStruct *NewPObj;
    int OrigMinSubdivLevel,
        MinSubdivLevel = AttrGetObjectIntAttrib(Srf, "AdapIsoMinSubdivLevel");

    if (!IP_ATTR_IS_BAD_INT(MinSubdivLevel))
        OrigMinSubdivLevel = SymbSetAdapIsoExtractMinLevel(MinSubdivLevel);
    else
        OrigMinSubdivLevel = 0;
    ...
    if (!IP_ATTR_IS_BAD_INT(MinSubdivLevel))
        SymbSetAdapIsoExtractMinLevel(OrigMinSubdivLevel);/* Recover value. */
...
IPObjectStruct *LineLeastSquarePtData(IPObjectStruct *PtObjList)
{
	...
	*PtList = NULL;
	...
	PtObj = IPCoerceObjectPtTypeTo(PtObj, IP_OBJ_POINT);
	if (PtList == NULL)
	    PtList = Pt = CagdPtNew();
	else {
	    Pt -> Pnext = CagdPtNew();
				     ...
				     CagdRType *RPeriodic)
{
	...
	*Crv = NULL;
	...
	if ((Crv = BspCrvFitLstSqr(PtObjList -> U.Crvs, Order, CrvSize,
				   Periodic, ParamType, FALSE, TRUE,
				   &Err)) == NULL)
	    return NULL;

	RetObj = IPGenCRVObject(Crv);
	AttrSetObjectRealAttrib(RetObj, "LSError", Err);
	...
	CtlPtObj = IPCoerceObjectPtTypeTo(PtObj, IP_OBJ_CTLPT);
	if (CtlPtList == NULL)
