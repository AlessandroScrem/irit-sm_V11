				   ...
				   IrtRType *K)
{
	...
	*PObj = NULL;
	...
	IRIT_FATAL_ERROR("ORTHOTOMC: Invalid freeform to check for orthotomics!");
	return NULL;
    }
...
*   RSubdivTol:   Subdivision tolerance of freeforms silhouette approx.      M
...
*   SubdivTol:  Subdivision tolerance.					     M
*                                                                            *
* KEYWORDS:                                                                  M
					...
					IrtRType *SubdivTol)
{
    ...
    PPoly = UserSrfTopoAspectGraph(PSrf -> U.Srfs, *SubdivTol);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Splits the given rational curve or surfaces at its poles.                M
*                                                                            *
* PARAMETERS:                                                                M
*   FreeForm:    A rational curve or surface to split at its poles.          M
*   SubdivTol:   Tolerance of the subdivision stage.                         M
*   NumerTol:    Tolerance of the numerical improvement stage.               M
*   OutReach:    Offset to extend the trimming at the poles.                 M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *: List of trimmed surfaces or split curves at the poles. M
*                                                                            *
* KEYWORDS:                                                                  M
*   FreeFormSplitAtPoles                                                     M
*****************************************************************************/
IPObjectStruct *FreeFormSplitAtPoles(IPObjectStruct *FreeForm,
				     IrtRType *SubdivTol,
				     IrtRType *NumericTol,
				     IrtRType *OutReach)
{
	...
	if (*OutReach < 0.0) {
	    CagdPtStruct *Pole,
		*Poles = SymbSplitRationalCrvsPoles(Crv, *NumericTol);

	    RetObj = IPGenLISTObject(NULL);

	    for (Pole = Poles, i = 0; Pole != NULL; Pole = Pole -> Pnext) {
		IPListObjectInsert(RetObj, i++,
				   IPGenNUMValObject(Pole -> Pt[0]));
	    }
	    IPListObjectInsert(RetObj, i, NULL);

	    CagdPtFreeList(Poles);
	}
	else {
	    CagdCrvStruct
	        *Crvs = SymbCrvSplitPoleParams(Crv, *NumericTol, *OutReach);

	    RetObj = IPLnkListToListObject(Crvs, IP_OBJ_CURVE);
	}
    }
    else if (IP_IS_SRF_OBJ(FreeForm)) {
        CagdSrfStruct
	...
	if (*OutReach < 0.0) {
	    ...
	    RetObj = MvarCnvrtMVPolysToIritPolys2(Poles, TRUE);
	    MvarPolylineFreeList(Poles);
	}
	else {
	    TrimSrfStruct
	        *TSrfs = MvarSrfSplitPoleParams(Srf, *SubdivTol, *NumericTol,
						*OutReach);

	    RetObj = IPLnkListToListObject(TSrfs, IP_OBJ_TRIMSRF);	    
	}
    }
    else {
	IRIT_FATAL_ERROR("FFSpltPoles: Invalid freeform to split at poles!");
	return NULL;
    }
...
*   Axis1, Axis2:  Two axis to flip around.  If Axis2 is negative, the MV is M
*                  reversed along Axis1.                                     M
*                                                                            *
* RETURN VALUE:                                                              M
...
{
    MvarMVStruct *RevMV;
    IPObjectStruct *RevMVObj;

    if (IRIT_REAL_PTR_TO_INT(Axis2) < 0) 
	RevMV = MvarMVReverseDir(MVObj -> U.MultiVars,
				  ...
				  IrtRType *NumerTol)
{
	...
	**MVs = (MvarMVStruct **) IritMalloc(sizeof(MvarMVStruct *)
    ...
    MvarPtStruct *MVPts;
    MvarPolylineStruct *MVPllns;
    MvarPolyStruct *MVPoly;
    IPObjectStruct *PObj,
	*ObjList = NULL;
    ...
    switch (MVs[0] -> Dim - l) {
	case 0:
	    MVPts = MvarMVsZeros0D(MVs, NULL, l, *SubdivTol, *NumerTol);

	    ObjList = MvarCnvrtMVPtsToCtlPts(MVPts, -1.0);
	    MvarPtFreeList(MVPts);
	    IritFree(MVs);
	    break;
	case 1:
	    MVPllns = MvarMVsZeros1D(MVs, NULL, l, *SubdivTol * 0.1, 
				     *SubdivTol, *NumerTol);

	    ObjList = MvarCnvrtMVPolysToIritPolys(MVPllns);
	    MvarPolylineFreeList(MVPllns);
	    IritFree(MVs);
	    break;
	case 2:
	    MVPoly = MvarMVsZeros2D(MVs, NULL, l, *SubdivTol * 0.1, 
				    *SubdivTol, *NumerTol, NULL); 

	    /* Converting to IRIT objects for display (polylines/polygons).  */
	    /* If the solution made its way back here with Dim > 3 for some  */
	    /* reason, it is simply projected on the first three coords.     */
	    if (MVPoly != NULL) {
		if (MVPoly -> PlType == 0) { 
		    /* Polyline loops output. */
		    if (MVPoly -> U.PllnList -> Pl -> Dim > 3) {
			int Coords[3] = { 0, 1, 2 };
			MvarPolylineStruct
			    *ProjPolys = MvarZeroSolverPolyProject(
							  MVPoly -> U.PllnList,
							  Coords, 3);

			    ObjList = MvarCnvrtMVPolysToIritPolys2(ProjPolys, 
								   TRUE);
			    printf("\nWARNING: Polyline solution has Dim > 3. Projecting solution on first three\n\tcoordinates.\n");
			    MvarPolylineFreeList(ProjPolys);
		    }
		    else {
			ObjList = MvarCnvrtMVPolysToIritPolys2(
							  MVPoly -> U.PllnList,
							  TRUE);
		    }
		}
		else { /* Triangles output. */
		    if (MVPoly -> U.TriList -> Dim > 3) {
			printf("\nWARNING: Solution triangles have Dim > 3. Projecting solution on first three\n\tcoordinates, and no normals are computed\n.");
		    }
		    ObjList = MvarCnvrtMVTrsToIritPolygons(MVPoly -> U.TriList,
							   NULL);
		}
		MvarPolyFree(MVPoly);
	    }
	    break;
	default:
	    IRIT_WNDW_PUT_STR("Invalid number of constaints vs. degrees of freedom");
	    IritFree(MVs);
	    break;
    }
					...
					IrtRType *NumerTol)
{
	...
	**MVs = (MvarMVStruct **) IritMalloc(sizeof(MvarMVStruct *)
    ...
    MVPlls = MvarMVsZeros1D(MVs, NULL, l, *Step, *SubdivTol, *NumerTol);
    IritFree(MVs);
				 ...
				 IrtRType *NumerTol)
{
	...
	if (MV1Obj -> U.MultiVars -> Dim == 2 &&
	    ...
	    Poly = MvarSrfSrfBisectorApprox(MV1Obj -> U.MultiVars,
					    MV2Obj -> U.MultiVars,
				            *SubdivTol, *NumerTol);
	    if (Poly != NULL) {
		if (Poly -> PlType == 0) {	       /* Polylines output. */
		    OutPoly = MvarCnvrtMVPolysToIritPolys2(Poly -> U.PllnList, 
							   TRUE);
		    MvarPolyFree(Poly);
		    return OutPoly;
		}
		    ...
		    OutPoly = MvarCnvrtMVTrsToIritPolygons(Poly -> U.TriList, 
							   NULL);
		    MvarPolyFree(Poly);
		    return OutPoly;
		}
		 ...
		 MV2Obj -> U.MultiVars -> Dim == 2)
	    Poly = MvarCrvSrfBisectorApprox(MV1Obj -> U.MultiVars,
					    MV2Obj -> U.MultiVars,
					    *SubdivTol, *NumerTol);
	else if (MV1Obj -> U.MultiVars -> Dim == 2 &&
		 MV2Obj -> U.MultiVars -> Dim == 1)
	    Poly = MvarCrvSrfBisectorApprox(MV2Obj -> U.MultiVars,
					    MV1Obj -> U.MultiVars,
					    *SubdivTol, *NumerTol);
	...
	if (OutputType == 1) {
		...
		*Mv = (MvarMVStruct *) RetVal,
	        *MvEuclid = AttrGetRefPtrAttrib(Mv -> Attr, "MVEuclid");
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Compute the bisector of two multivariates - old version not using the    M
* 2D solver.							             M
*                                                                            *
* PARAMETERS:                                                                M
*   MV1Obj, MV2Obj:   Two multivariates to consider.                         M
...
*   MultivarBisector                                                         M
*****************************************************************************/
IPObjectStruct *MultivarBisector2(IPObjectStruct *MV1Obj,
				  IPObjectStruct *MV2Obj,
				  IrtRType *ROutputType,
				  IrtRType *SubdivTol,
				  IrtRType *NumerTol)
{
	...
	if (MV1Obj -> U.MultiVars -> Dim == 2 &&
	    MV2Obj -> U.MultiVars -> Dim == 2)
	    RetVal = MvarSrfSrfBisectorApprox2(MV1Obj -> U.MultiVars,
					      MV2Obj -> U.MultiVars,
					      OutputType,
					      *SubdivTol, *NumerTol);
	else if (MV1Obj -> U.MultiVars -> Dim == 1 &&
		 MV2Obj -> U.MultiVars -> Dim == 2)
	    RetVal = MvarCrvSrfBisectorApprox2(MV1Obj -> U.MultiVars,
					       MV2Obj -> U.MultiVars,
					       OutputType,
					       *SubdivTol, *NumerTol);
	else if (MV1Obj -> U.MultiVars -> Dim == 2 &&
		 MV2Obj -> U.MultiVars -> Dim == 1)
	    RetVal = MvarCrvSrfBisectorApprox2(MV2Obj -> U.MultiVars,
					       MV1Obj -> U.MultiVars,
					       OutputType,
					       *SubdivTol, *NumerTol);
	else
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Compute a cyclic path of length Cycle over the given (control) function. M
*                                                                            *
* PARAMETERS:                                                                M
*   FuncObj:              Object to compute the cyclic path for.             M
*   RLength:              Desired length of cyclic path.                     M
*   SubdivTol, NumerTol:  Tolerances of computation.	                     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:   List of locations of cyclic paths of desired length, M
*			if any.						     M
*                                                                            *
* KEYWORDS:                                                                  M
*   MultivarCtrlCycle                                                        M
*****************************************************************************/
IPObjectStruct *MultivarCtrlCycle(IPObjectStruct *FuncObj,
				  IrtRType *RLength,
				  IrtRType *SubdivTol,
				  IrtRType *NumerTol)
{
    if (IP_IS_CRV_OBJ(FuncObj)) {
        IPObjectStruct *ObjPtsPaths;
        MvarPtStruct
	    *MVPaths = MvarCtrlComputeCrvNCycle(FuncObj -> U.Crvs,
						IRIT_REAL_PTR_TO_INT(RLength),
						*SubdivTol,
						*NumerTol);
	...
	ObjPtsPaths = MvarCnvrtMVPtsToCtlPts(MVPaths, -1.0);
	
	MvarPtFreeList(MVPaths);

	return ObjPtsPaths;

    }
    else if (IP_IS_SRF_OBJ(FuncObj)) {
        IPObjectStruct *ObjPtsPaths;
        MvarPtStruct
	    *MVPaths = MvarCtrlComputeSrfNCycle(FuncObj -> U.Srfs,
						IRIT_REAL_PTR_TO_INT(RLength),
						*SubdivTol,
						*NumerTol);
	...
	return ObjPtsPaths;
        ...
        IRIT_WNDW_PUT_STR("Only curve or surface expected as (control) function.\n");
	return NULL;
    }
}
...
{
    MvarPolylineStruct
        *MVPlls = MvarSrfAccessibility(PosSrf -> U.Srfs,
				       IP_IS_SRF_OBJ(OrientSrf) ?
				           OrientSrf -> U.Srfs :
				           NULL,
				       CheckSrf -> U.Srfs,
				       *SubdivTol, *NumerTol);
				  ...
				  IrtRType *RContactOrder)
{
    ...
    if (ContactOrder == 3) {
        MvarPolylineStruct
	    *MVPlls = MvarSrfFlecnodalCrvs(Srf -> U.Srfs,
	...
	ObjPtsList = MvarCnvrtMVPtsToCtlPts(MVPts, -1.0);
...
*                                                                            *
* PARAMETERS:                                                                M
*   Srf1, Srf2:           Surfaces to analyze.		                     M
...
*   SurfaceBiTangents                                                        M
*****************************************************************************/
IPObjectStruct *SurfaceBiTangents(IPObjectStruct *Srf1,
				  IPObjectStruct *Srf2,
				  IrtRType *SubdivTol,
				  IrtRType *NumerTol)
{
    IPObjectStruct *ObjPlList;
    MvarPolylineStruct *MVPllns;
    MvarMVStruct
	*MVSrf1 = MvarSrfToMV(Srf1 -> U.Srfs),		  
	*MVSrf2 = MvarSrfToMV(Srf2 -> U.Srfs);

    MVPllns = MvarMVBiTangents(MVSrf1, MVSrf2, *SubdivTol, *NumerTol);
    MvarMVFree(MVSrf1);
    MvarMVFree(MVSrf2);
    ...
    ObjPlList = MvarCnvrtMVPolysToIritPolys(MVPllns);
...
*   SubdivTol, NumericTol:  Tolerances of computation.	                     M
...
IPObjectStruct *ParabolicCurvesOnSrf(IPObjectStruct *PObjSrf,
				     ...
				     IrtRType *NumericTol,
				     IrtRType *REuclidean,
				     IrtRType *RDecompSrfs)
{
    int Euclidean = IRIT_REAL_PTR_TO_INT(REuclidean),
        DecompSrfs =  IRIT_REAL_PTR_TO_INT(RDecompSrfs);
    IPObjectStruct
        *ParabolicObjs = UserSrfParabolicLines(PObjSrf -> U.Srfs,
					       *SubdivTol / 10.0,
					       *SubdivTol, *NumericTol,
					       Euclidean, DecompSrfs);

    return ParabolicObjs;
}
				...
				IrtRType *RCrvEval)
{
	...
	IP_IS_CRV_OBJ(PrmObj2)) {
        ...
        IrtRType TMin1, TMax1, TMin2, TMax2, Resolution;
	CagdCrvStruct
	    *Crv1 = PrmObj1 -> U.Crvs,
	    *Crv2 = PrmObj2 -> U.Crvs;
	CagdSrfStruct *Srf;
	...
	if ((PCntrObj = ContourFreeform(PObj, Plane, &Resolution,
					NULL, NULL)) == NULL)
	    return NULL;
