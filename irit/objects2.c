...
*****************************************************************************/
IPObjectStruct *CoerceIritObjectTo(const IPObjectStruct *PObj,
				   IrtRType *RNewType)
{
	...
	case KV_UNIFORM_OPEN:
	    NewType = IP_COERCE_UNIFORM_OPEN;
	    break;
	case KV_UNIFORM_FLOAT:
	    NewType = IP_COERCE_UNIFORM_FLOAT;
	    break;
	case KV_UNIFORM_PERIODIC:
	    NewType = IP_COERCE_UNIFORM_PERIODIC;
	    break;
	default:
	    break;
    }

    return IPCoerceObjectTo(PObj, NewType);
}
...
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdExtrudeSrf, GenTwistEXTRUDEObject				     M
*                                                                            *
* KEYWORDS:                                                                  M
*   GenEXTRUDEObject                                                         M
*****************************************************************************/
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Constructs a full circular twisted/rotated extrusion in the +Z direction   M
* for the given profile curve or surface. Input curve/surface can be either  M
* a Bspline or a Bezier.						     M
*                                                                            *
* PARAMETERS:                                                                M
*   Cross:     To twist extrude in +Z Dir.   A curve or a surface.           M
...
*              be a planar (degenerated) surface.	                     M
*                                                                            *
* RETURN VALUE:                                                              M
*   IPObjectStruct *:    An EXTRUDE primitive.  A surface or a trivariate.   M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdZTwistExtrudeSrf, TrivZTwistExtrudeSrf, GenEXTRUDEObject	     M
*                                                                            *
* KEYWORDS:                                                                  M
*   GenZTwistEXTRUDEObject                                                   M
*****************************************************************************/
IPObjectStruct *GenZTwistEXTRUDEObject(IPObjectStruct *Cross,
				       IrtRType *Rational,
				       IrtRType *ZPitch)
{
    if (IP_IS_CRV_OBJ(Cross)) {
	CagdSrfStruct
	    *ExtSrf = CagdZTwistExtrudeSrf(Cross -> U.Crvs,
					   IRIT_REAL_PTR_TO_INT(Rational),
					   *ZPitch);

	return ExtSrf == NULL ? NULL : IPGenSRFObject(ExtSrf);
    }
    else if (IP_IS_SRF_OBJ(Cross)) {
	TrivTVStruct
	    *ExtTV = TrivZTwistExtrudeSrf(Cross -> U.Srfs,
					  IRIT_REAL_PTR_TO_INT(Rational),
					  *ZPitch);

	return ExtTV == NULL ? NULL : IPGenTRIVARObject(ExtTV);
    }
