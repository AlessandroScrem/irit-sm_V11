/*****************************************************************************
* Conversion routines from freeform to polygons and polylines.               *
******************************************************************************
...
IPPolygonStruct *IPCagdPllns2IritPllns(CagdPolylineStruct *Polys)
{
	...
	*PHead = NULL;
	...
	if ((Params = AttrGetRefPtrAttrib(CagdPoly -> Attr, "SaveParamVals"))
							           != NULL) {
				   ...
				   SymbCrvApproxMethodType Method)
{
	...
	*TCrv = NULL;
	...
	/* Construct a two point polyline with two end points of curve. */
					 ...
					 CagdBType *GenPoly)
{
	...
	*GenPoly = TRUE;
	...
	if (Nl != NULL) {
	    ...
	    IRIT_PT_ADD(Pin, Pt1, Nl);
	    Pl = PrimGenPolygon4Vrtx2(Pt1, Pt2, Pt3, Pt4, Pin,
				     &Rvrsd, &Singular, NULL);
	}
	else
	    Pl = PrimGenPolygon4Vrtx2(Pt1, Pt2, Pt3, Pt4, NULL,
				     &Rvrsd, &Singular, NULL);
    }
    else
        Pl = PrimGenPolygon4Vrtx2(Pt1, Pt2, Pt3, Pt4, NULL,
    ...
    if (Singular >= 0) {
        /* The polygon is in fact degenerate - create as a triangle. */
        IPFreePolygon(Pl);
	switch (Singular) {
	     case 1:
	         return IPGenTriangle(ComputeNormals, ComputeUV,
				      Pt2, Pt3, Pt4,
				      Nl2, Nl3, Nl4,
				      UV2, UV3, UV4, GenPoly);
	     case 2:
	     ...
	     case 4:
	         return IPGenTriangle(ComputeNormals, ComputeUV,
				      ...
				      UV1, UV2, UV3, GenPoly);
	     default:
	         assert(0); /* Expected only vertex 1 to 4. */
	}
    }
    ...
    if (!GlblPolyListCirc) {
	if (V4 == NULL)			      /* Only 3 vertices in Polygon. */
	    V3 -> Pnext = NULL;
	else
	    V4 -> Pnext = NULL;
    }
				    ...
				    int Optimal)
{
    ...
    return GMCleanUpPolygonList(&RetVal, IRIT_EPS);
}
				    ...
				    int Optimal)
{
    ...
    IPStderrObject(IPGenPOLYLINEObject(RetVal));           /* A mem leak... */
				   ...
				   int Optimal)
{
	...
	*Polys = NULL;
    ...
    return GMCleanUpPolygonList(&Polys, IRIT_EPS);
}
				   ...
				   int Optimal)
{
    ...
    return GMCleanUpPolygonList(&PHead, IRIT_EPS);
}
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPOpenPolysToClosed, GMVrtxListToCircOrLin                               M
*                                                                            *
* KEYWORDS:                                                                  M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPClosedPolysToOpen, GMVrtxListToCircOrLin                               M
*                                                                            *
* KEYWORDS:                                                                  M
