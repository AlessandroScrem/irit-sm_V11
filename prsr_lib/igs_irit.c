...
#define IGS_PROJ_PT_ON_SRF_SUBDIV_TOL	1e-2
#define IGS_PROJ_PT_ON_SRF_NUMER_TOL	1e-10
...
static void IgesRecurseAllObjects(IgesInfoStruct *IgesInfo,
				  const IPObjectStruct *PObjIn);
static void IgesRecurseListObject(IgesInfoStruct *IgesInfo,
...
static IPObjectStruct *IgesProcessFaceEntity(int SeqNum,
					     IgesInfoStruct *IgesInfo,
					     IPObjectStruct *SrfObj,
					     IPObjectStruct *TrimmingLoops);
static IrtRType IgsDeviseBestZRotation(const IPObjectStruct *TrimmingLoops,
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPIgesSaveFile, IPIgesLoadFileSetDefaultParameters                       M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPIgesLoadFile                                                           M
*****************************************************************************/
			       ...
			       int Messages)
{
	...
	if ((IgesInfo.DumpAll || IgesInfo.DirEntries[i] -> ObjUseCount == 1) &&
	    ...
	    IgesRecurseAllObjects(&IgesInfo, IgesInfo.DirEntries[i] -> PObj);
	}
    }

    if (IgesInfo.RetIritObjs == NULL ||
	IgesInfo.RetIritObjs -> Pnext == NULL) {
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Recursively traverse the (instances of the) objects and fetch them in,   *
* for the return value. 						     *
*                                                                            *
* PARAMETERS:                                                                *
*   IgesInfo:  All information one ever needs to process IGES file.          *
*   PObjIn:    Object to process.					     *
*                                                                            *
* RETURN VALUE:                                                              *
*   void                                                                     *
*****************************************************************************/
				  ...
				  const IPObjectStruct *PObjIn)
{
    if (PObjIn != NULL) {
        IPObjectStruct
	    *PObj = IPCopyObject(NULL, PObjIn, TRUE);

	if (IP_IS_INSTNC_OBJ(PObj)) {
	    IPObjectStruct
	        *PTmp = AttrGetObjectRefPtrAttrib(PObjIn, "IgesIrtOpParam1");
	    ...
	    if (PTmp == NULL) {
		    ...
		    Iges2IritWarning(IgesInfo, -1,
				     "IGES instance \"%s\" not detected",
				     PObj -> U.Instance -> Name);
	    }
	    else if (AttrGetObjectIntAttrib(PTmp,
					    "DumpedInstObj") != TRUE) {
	        IgesRecurseAllObjects(IgesInfo, PTmp);
		AttrSetObjectIntAttrib(PTmp, "DumpedInstObj", TRUE);
	    }
	}
	else if (IP_IS_OLST_OBJ(PObj)) {
	    IgesRecurseListObject(IgesInfo, PObj);
	}
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Recursively traverse a list object and dump leaves under instances.      *
*                                                                            *
* PARAMETERS:                                                                *
*   IgesInfo:  All information one ever needs to process IGES file.          *
*   PObjIn:    List object to process.					     *
*                                                                            *
* RETURN VALUE:                                                              *
*   void                                                                     *
*****************************************************************************/
				  ...
				  const IPObjectStruct *PObj)
{
	...
	if (IP_IS_INSTNC_OBJ(PObj)) {
	    IPObjectStruct
	        *PTmp = AttrGetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1");
		...
		IgesRecurseListObject(IgesInfo, PTmp);
	}
    }
}
				       ...
				       IgesInfoStruct *IgesInfo)
{
    ...
    Tol = IRIT_MAX(IRIT_FABS(TMax), IRIT_FABS(TMin)) * IP_KNOT_SEQ_VALIDITY_TOL;
    if (TMax - TMin < Tol) {
        if (IgesInfo -> MoreFlag >= 2) {
    ...
    if (!BspKnotVerifyKVValidity(Crv -> KnotVector, Crv -> Order,
				 Crv -> Length, IP_KNOT_SEQ_VALIDITY_TOL)) {
        if (IgesInfo -> MoreFlag >= 2) {
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "Invalid knot sequence detected in curve.\n");
	    if (IgesInfo -> MoreFlag >= 3)
	        CagdDbg(Crv);
	}
				       ...
				       IgesInfoStruct *IgesInfo)
{
    ...
    Tol = IRIT_MAX(IRIT_FABS(UMax), IRIT_FABS(UMin)) * IP_KNOT_SEQ_VALIDITY_TOL;
    if (UMax - UMin < Tol) {
        if (IgesInfo -> MoreFlag >= 2) {
	    Iges2IritWarning(IgesInfo, SeqNum,
    ...
    Tol = IRIT_MAX(IRIT_FABS(VMax), IRIT_FABS(VMin)) * IP_KNOT_SEQ_VALIDITY_TOL;
    if (VMax - VMin < Tol) {
        if (IgesInfo -> MoreFlag >= 2) {
	    Iges2IritWarning(IgesInfo, SeqNum,
    ...
    if (!BspKnotVerifyKVValidity(Srf -> UKnotVector, Srf -> UOrder,
				 Srf -> ULength, IP_KNOT_SEQ_VALIDITY_TOL)) {
        if (IgesInfo -> MoreFlag >= 2) {
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "Invalid U knot sequence detected in surface.\n");
	    if (IgesInfo -> MoreFlag >= 3)
	        CagdDbg(Srf);
	}
    ...
    if (!BspKnotVerifyKVValidity(Srf -> VKnotVector, Srf -> VOrder,
				 Srf -> VLength, IP_KNOT_SEQ_VALIDITY_TOL)) {
        if (IgesInfo -> MoreFlag) {
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "Invalid V knot sequence detected in surface.\n");
	    if (IgesInfo -> MoreFlag >= 2)
	        CagdDbg(Srf);
	}
...
static IPObjectStruct *HandleIgesDirEntry(int SeqNum, IgesInfoStruct *IgesInfo)
{
	...
	*V = NULL;
    char *Name;
    int i, j, n, m, IP, ReadSeqNum, Len1, Len2, Order1, Order2, OuterLoopFlag,
	Rat, Closed;
    ...
    IPObjectStruct *PTmp, *PTmp1, *PTmp2, *PTmp3, *UVObj,
	*PObj = NULL;
	...
	*IgesDirEntry = DirEntries[SeqNum];
	...
	case IGS_ENTYPE_CIRCULAR_ARC:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_CIRC_ARC);
	    PTmp = IPGenVECObject(&CagdPt.Pt[0], &CagdPt.Pt[1], &CagdPt.Pt[2]);
	    AttrSetObjectObjAttrib(PObj, "IgesIrtOpParam1", PTmp, FALSE);
	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam2",
				    sqrt(IRIT_SQR(P1[0]) + IRIT_SQR(P1[1])));
	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam3", t1);
	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam4", t2);
	    break;
	case IGS_ENTYPE_COMPOSITE_CURVE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_CURVE);
	    break;
	case IGS_ENTYPE_CONIC_ARC:
		...
		AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_CONIC_ARC);
	    }
	    break;
	case IGS_ENTYPE_COPIOUS_DATA:
		...
		switch (IP) {
			...
			IP_SET_OBJ_NAME(PObj, "COPIOUS_POLY%d",
					IgesInfo -> ObjCount++);
			AttrSetObjectIntAttrib(PObj, "IgesIrtOp",
					       IGS_IRT_OP_POLY);
			break;
		    case 11:
		    case 12:
	...
	case IGS_ENTYPE_PLANE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_PLANE);
	...
	case IGS_ENTYPE_LINE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_LINE);
	    break;
	case IGS_ENTYPE_DIRECTION:
	    GetParamsVector(3, V, SeqNum, IgesInfo);
	    PObj = IPGenVECObject(&V[0], &V[1], &V[2]);
	    IP_SET_OBJ_NAME(PObj, "DIR%d", IgesInfo -> ObjCount++);
	    break;
	...
	case IGS_ENTYPE_PARAM_SPLINE_CURVE:
	    ...
	    PObj = IgsGenCRVObject(Crv, FALSE, SeqNum, IgesInfo);
	    IP_SET_OBJ_NAME(PObj, "SPLINE_CRV%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_SPLINE_CRV);
	    break;
	...
	case IGS_ENTYPE_POINT:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_POINT);
	...
	case IGS_ENTYPE_RULED_SRF:
		...
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp2);
		AttrSetObjectIntAttrib(PObj, "IgesIrtOpParam3", Reverse);
	    }
	    else {
	        if (IgesInfo -> MoreFlag >= 2)
		    Iges2IritWarning(IgesInfo, SeqNum,
	...
	case IGS_ENTYPE_SRF_OF_REV:
		...
		if (IRIT_APX_EQ(V[1] - V[0], M_PI_MUL_2))
		    ...
		    Srf = CagdSurfaceRev2Axis(Crv, FALSE,
					      IRIT_RAD2DEG(V[0]),
		...
		if (IRIT_APX_EQ(V[1] - V[0], M_PI_MUL_2))
		    ...
		    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam4", V[0]);
		    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam5", V[1]);
		}
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp2);
		PTmp = IPGenVECObject(&Dir[0], &Dir[1], &Dir[2]);
		IP_SET_OBJ_NAME(PTmp, "DIR%d", IgesInfo -> ObjCount++);
		AttrSetObjectObjAttrib(PObj, "IgesIrtOpParam2", PTmp, FALSE);
		PTmp = IPGenPTObject(&P1[0], &P1[1], &P1[2]);
		IP_SET_OBJ_NAME(PTmp, "POS%d", IgesInfo -> ObjCount++);
		AttrSetObjectObjAttrib(PObj, "IgesIrtOpParam3", PTmp, FALSE);
	    }
	    else {
	        if (IgesInfo -> MoreFlag >= 2)
		    Iges2IritWarning(IgesInfo, SeqNum,
	...
	case IGS_ENTYPE_TABULAR_CYL:
		...
		CagdCrvFree(Crv);
		AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_EXTRUDE);
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	...
	case IGS_ENTYPE_RATIONAL_BSPLINE_CURVE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_RAT_BSP_CRV);
	    break;
	case IGS_ENTYPE_RATIONAL_BSPLINE_SURFACE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_RAT_BSP_SRF);
	    break;
	case IGS_ENTYPE_CURVE_ON_PARAM_SRF:
		...
		if (IP_IS_PLANE_OBJ(PTmp)) {
		    ...
		    AttrSetObjectRefPtrAttrib(PTmp, "IgesIrtOpParam2", PTmp2);
		}  
		else
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find uv curve seq. number");
		...
		AttrSetObjectRefPtrAttrib(PObj, "CompUVCrv", PTmp);
		IP_SET_OBJ_NAME(PObj, "CRV_ON_SRF%d", IgesInfo -> ObjCount++);
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp2);
		AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_CRV_ON_SRF);
	    }
	    break;
	...
	case IGS_ENTYPE_TRIMMED_PARAM_SRF:
		...
		if (IP_IS_PLANE_OBJ(PTmp)) {
		    ...
		    PObj = AttrGetObjectRefPtrAttrib(PTmp, "IgesIrtOpParam2");
		    PObj = IPCopyObject(NULL, PObj, FALSE);

		    IP_SET_OBJ_NAME(PObj, "TRIM_POLY%d", IgesInfo -> ObjCount++);
		    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_POLY);
		    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
		    break;
		}
		else
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_TRIM_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    ...
	    if ((ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo)) != 0) {
		    ...
		    (UVObj = (IPObjectStruct *)
		           AttrGetObjectRefPtrAttrib(PTmp, "CompUVCrv")) == NULL)
		    Iges2IritWarning(IgesInfo, SeqNum,
		...
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp);
	    }
		...
		char AttrName[IRIT_LINE_LEN];
		    ...
		    !IP_IS_CRV_OBJ(PTmp) ||
		    (UVObj = (IPObjectStruct *)
		            AttrGetObjectRefPtrAttrib(PTmp,
						      "CompUVCrv")) == NULL)
		    Iges2IritWarning(IgesInfo, SeqNum,
		...
		sprintf(AttrName, "IgesIrtOpParam%d", i + 3);
		AttrSetObjectRefPtrAttrib(PObj, AttrName, PTmp);
	    }
	...
	case IGS_ENTYPE_OFFSET_CURVE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_OFFSET_CRV);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam2", V[0]);
	    break;
	case IGS_ENTYPE_OFFSET_SURFACE:
	    ...
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_OFFSET_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam2", V[0]);
	    break;
	case IGS_ENTYPE_SUBFIG_DEF:
	    ...
	    AttrSetObjectIntAttrib(PObj, "Depth", i);
	    for (i = j = 0; i < n; i++) {
	        ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		...
		AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_LIST);
	    }
	    else {
	        IPFreeObject(PObj);
		PObj = NULL;
	...
	case IGS_ENTYPE_SNGL_SUBFIG_INSTNCE:
		...
		AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_INSTANCE);
		AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	...
	case IGS_ENTYPE_PLANE_SRF:
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_POINT_OBJ(PTmp))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find point of plane surface");
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_VEC_OBJ(PTmp2))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find normal of plane surface");
	    if (IgesDirEntry -> FormNum == 1) {
		ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		if ((PTmp3 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		    !IP_IS_VEC_OBJ(PTmp3))
		    Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find ref. vector of plane surface");
	    }
	    else
		PTmp3 = NULL;

	    t1 = -IRIT_DOT_PROD(PTmp2 -> U.Vec, PTmp -> U.Pt);
	    PObj = IPGenPLANEObject(&PTmp2 -> U.Vec[0],
				    &PTmp2 -> U.Vec[1],
				    &PTmp2 -> U.Vec[2],
				    &t1);

	    IP_SET_OBJ_NAME(PObj, "PLANE_SRF%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_PLANE_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp2);
	    if (PTmp3 != NULL)
	        AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam3", PTmp3);
	    break;

	case IGS_ENTYPE_RGT_CIRC_CYLIN_SRF:
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_POINT_OBJ(PTmp))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find location of cylinder surface");
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_VEC_OBJ(PTmp2))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find axis of cylinder surface");
	    t1 = GetParamsReal(SeqNum, IgesInfo);               /* Radius */
		...
		if ((PTmp3 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		    !IP_IS_VEC_OBJ(PTmp3))
		    Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find ref. vector of cylinder surface");
	    }
	    else
		PTmp3 = NULL;

	    PObj = IPGenNUMValObject(t1);  /* Save infinite cyl. info ... */
	    IP_SET_OBJ_NAME(PObj, "CYLIN_SRF%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_RGT_CYL_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp2);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam3", IPGenNUMValObject(t1));
	    if (PTmp3 != NULL)
	        AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam4", PTmp3);
	    break;

	case IGS_ENTYPE_RGT_CIRC_CONE_SRF:
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_POINT_OBJ(PTmp))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find location of cone surface");
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_VEC_OBJ(PTmp2))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find axis of cone surface");
	    t1 = GetParamsReal(SeqNum, IgesInfo);               /* Radius */
	    t2 = GetParamsReal(SeqNum, IgesInfo);           /* Semi angle */
		...
		if ((PTmp3 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		    !IP_IS_VEC_OBJ(PTmp3))
		    Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find ref. vector of cone surface");
	    }
	    else
		PTmp3 = NULL;

	    PObj = IPGenNUMValObject(t1);  /* Save infinite cone info ... */
	    IP_SET_OBJ_NAME(PObj, "CONE_SRF%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_RGT_CONE_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp2);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam3",
				      IPGenNUMValObject(t1));
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam4",
				      IPGenNUMValObject(t2));
	    if (PTmp3 != NULL)
	        AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam5", PTmp3);
	    break;

	case IGS_ENTYPE_SPHERE_SRF:
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_POINT_OBJ(PTmp))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find location of sphere surface");
	    t1 = GetParamsReal(SeqNum, IgesInfo);               /* Radius */
		...
		if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		    !IP_IS_VEC_OBJ(PTmp2))
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find axis direction of sphere surface");
	    
		ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		if ((PTmp3 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		    !IP_IS_VEC_OBJ(PTmp3))
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find ref. vector of sphere surface");
	    }
	    else
		PTmp2 = PTmp3 = NULL;

	    Srf = CagdPrimSphereSrf(PTmp -> U.Pt, t1, FALSE);
	    PObj = IPGenSRFObject(Srf);
	    IP_SET_OBJ_NAME(PObj, "SPHERE_SRF%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_SPHERE_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2",
				      IPGenNUMValObject(t1));
	    if (PTmp3 != NULL)
	        AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam3", PTmp2);
	    if (PTmp3 != NULL)
	        AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam4", PTmp3);
	    break;
	case IGS_ENTYPE_TORUS_SRF:
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "IGES torus surface - never tested!");
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_POINT_OBJ(PTmp))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find location of torus surface");
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		!IP_IS_VEC_OBJ(PTmp2))
	        Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find axis of torus surface");
	    t1 = GetParamsReal(SeqNum, IgesInfo);          /* Major radius */
	    t2 = GetParamsReal(SeqNum, IgesInfo);          /* Minor radius */
		...
		if ((PTmp3 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL ||
		    !IP_IS_VEC_OBJ(PTmp3))
		    Iges2IritWarning(IgesInfo, SeqNum,
				 "Failed to find ref. vector of torus surface");
	    }
	    else
		PTmp3 = NULL;

	    Srf = CagdPrimTorusSrf(PTmp -> U.Pt, t1, t2, FALSE);
	    PObj = IPGenSRFObject(Srf);
	    IP_SET_OBJ_NAME(PObj, "TORUS_SRF%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_TORUS_SRF);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam1", PTmp);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam2", PTmp2);
	    AttrSetObjectRefPtrAttrib(PObj, "IgesIrtOpParam3",
				      IPGenNUMValObject(t1));
	...
	case IGS_ENTYPE_RIGHT_CIRC_CYL:
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "IGES cylinder - never tested!");
	    GetParamsVector(8, V, SeqNum, IgesInfo);

	    /* Embed the height into the axis vector. */
	    for (i = 5; i < 8; i++)
	        V[i] *= V[0];

	    PObj = PrimGenCONEObject(&V[2], &V[5], V[1], 3);
    	    IP_SET_OBJ_NAME(PObj, "RGT_CYL%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_RGT_CYL);

	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam1", V[0]);
	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam2", V[1]);
	    break;

	case IGS_ENTYPE_RIGHT_CIRC_CONE:
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "IGES cone - never tested!");
	    GetParamsVector(8, V, SeqNum, IgesInfo);
	    ...
	    if (V[2] == 0)
	        PObj = PrimGenCONEObject(&V[3], &V[6], V[1], 3);
	    else
	        PObj = PrimGenCONE2Object(&V[3], &V[6], V[1], V[2], 3);
    	    IP_SET_OBJ_NAME(PObj, "RGT_CONE%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_RGT_CONE);
	...
	case IGS_ENTYPE_SPHERE:
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "IGES sphere - never tested!");
	    GetParamsVector(4, V, SeqNum, IgesInfo);

	    PObj = PrimGenSPHEREObject(&V[1], V[0]);
    	    IP_SET_OBJ_NAME(PObj, "SPHERE%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_SPHERE);
	...
	case IGS_ENTYPE_TORUS:
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "IGES torus - never tested!");
	    GetParamsVector(8, V, SeqNum, IgesInfo);

	    PObj = PrimGenTORUSObject(&V[2], &V[5], V[1], V[0]);
    	    IP_SET_OBJ_NAME(PObj, "TORUS%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp", IGS_IRT_OP_TORUS);

	    AttrSetObjectRealAttrib(PObj, "IgesIrtOpParam1", V[0]);
	    break;

	case IGS_ENTYPE_VERTEX_ENTITY:
	    n = GetParamsInteger(SeqNum, IgesInfo);
	    PObj = IPGenLISTObject(NULL);
	    for (i = 0; i < n; i++) {
	        GetParamsVector(3, V, SeqNum, IgesInfo);
		PTmp = IPGenPTObject(&V[0], &V[1], &V[2]);
		IPListObjectInsert(PObj, i, PTmp);
	    }
	    IPListObjectInsert(PObj, n, NULL);

	    IP_SET_OBJ_NAME(PObj, "VERTEX%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp",
				   IGS_ENTYPE_VERTEX_ENTITY);
	    break;

	case IGS_ENTYPE_EDGE_ENTITY:
	    n = GetParamsInteger(SeqNum, IgesInfo);
	    PObj = IPGenLISTObject(NULL);
	    for (i = 0; i < n; i++) {
	        int Idx1, Idx2;
		...
		if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL) {
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find model space curve in seq. number in edge entity");
		    break;
		}

	        ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		if ((PTmp1 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL) {
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find start vertex in seq. number in edge entity");
		    break;
		}
		Idx1 = GetParamsInteger(SeqNum, IgesInfo);

	        ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL) {
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find terminating vertex in seq. number in edge entity");
		    break;
		}
		Idx2 = GetParamsInteger(SeqNum, IgesInfo);

		IPListObjectInsert(PObj, i, IPCopyObject(NULL, PTmp, TRUE));
	    }
	    IPListObjectInsert(PObj, n, NULL);

	    IP_SET_OBJ_NAME(PObj, "EDGE%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp",
				   IGS_ENTYPE_EDGE_ENTITY);
	    break;

	case IGS_ENTYPE_LOOP_ENTITY:
	    n = GetParamsInteger(SeqNum, IgesInfo);
	    PObj = IPGenLISTObject(NULL);
	    IP_SET_OBJ_NAME(PObj, "LIST%d", IgesInfo -> ObjCount++);
	    for (i = 0; i < n; i++) {
	        int Ndx, k, OrientFlag, Isoparam, EdgeType;

		/* Edge type is not used for the moment - read and skip. */
		EdgeType = GetParamsInteger(SeqNum, IgesInfo);

	        ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL) {
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find vertex/edge list seq. number in loop entity");
		    break;
		}
		Ndx = GetParamsInteger(SeqNum, IgesInfo) - 1;
		OrientFlag = GetParamsInteger(SeqNum, IgesInfo);

		PTmp2 = IPListObjectGet(PTmp, Ndx);
		if (!IP_IS_CRV_OBJ(PTmp2)) {
		    if (IP_IS_POLY_OBJ(PTmp2)) {
			PTmp2 = IPGenCRVObject(
			     UserPolyline2LinBsplineCrv(PTmp2 -> U.Pl, TRUE));
		    }
		    else {
			Iges2IritWarning(IgesInfo, SeqNum,
					 "Unidentified entity type in loop entity");
			break;
		    }
		}
		else if (OrientFlag)		      /* Reverse the curve. */
		    PTmp2 = IPGenCRVObject(CagdCrvReverse(PTmp2 -> U.Crvs));
		else
		    PTmp2 = IPGenCRVObject(CagdCrvCopy(PTmp2 -> U.Crvs));

		k = GetParamsInteger(SeqNum, IgesInfo);
		if (k == 0) {
		    /* For some reason in practice the list is reversed... */
		    IPListObjectInsert(PObj, n - i - 1, PTmp2);
		}
		else {
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "IGES LOOP ENTITY - curves (k > 0) - never tested!");
		    for (j = 0; j < k; j++) {
			Isoparam = GetParamsInteger(SeqNum, IgesInfo);
			ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
			if ((PTmp2 = GetObjRefBySeqNum(ReadSeqNum,
						      IgesInfo)) == NULL) {
			    Iges2IritWarning(IgesInfo, SeqNum,
					     "Failed to find curve seq. number in loop entity");
			    break;
			}
			IPListObjectAppend(PObj, PTmp2);
		    }
		}
	    }
	    assert(i == n);
	    ...
	    IP_SET_OBJ_NAME(PObj, "LOOP%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp",
				   IGS_ENTYPE_LOOP_ENTITY);
	    break;

	case IGS_ENTYPE_FACE_ENTITY:
	    ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
	    if ((PTmp = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL) {
	        Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find underlying surface seq. number in face entity");
		break;
	    }

	    n = GetParamsInteger(SeqNum, IgesInfo);
	    OuterLoopFlag = GetParamsInteger(SeqNum, IgesInfo);
	    PTmp2 = IPGenLISTObject(NULL);

	    for (i = 0; i < n; i++) {
	        ReadSeqNum = GetParamsInteger(SeqNum, IgesInfo);
		if ((PTmp3 = GetObjRefBySeqNum(ReadSeqNum, IgesInfo)) == NULL) {
		    Iges2IritWarning(IgesInfo, SeqNum,
				     "Failed to find loop seq. number in loop entity");
		    break;
		}
		IPListObjectInsert(PTmp2, i, PTmp3);
	    }
	    IPListObjectInsert(PTmp2, n, NULL);

	    PObj = IgesProcessFaceEntity(SeqNum, IgesInfo, PTmp, PTmp2);
	    IP_SET_OBJ_NAME(PObj, "FACE%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp",
				   IGS_ENTYPE_FACE_ENTITY);
	    break;

	case IGS_ENTYPE_SHELL_ENTITY:
	    n = GetParamsInteger(SeqNum, IgesInfo);
	    PObj = IPGenLISTObject(NULL);
	    for (i = 0; i < n; i++) {
	        int OrientFlag;
		...
		OrientFlag = GetParamsInteger(SeqNum, IgesInfo);
		if (!OrientFlag)
		    PTmp2 = IPReverseObject(PTmp);
		else
		    PTmp2 = IPCopyObject(NULL, PTmp, TRUE);

		IPListObjectInsert(PObj, i, PTmp2);
	    }
	    assert(i == n);
	    IPListObjectInsert(PObj, n, NULL);
	    IP_SET_OBJ_NAME(PObj, "SHELL%d", IgesInfo -> ObjCount++);
	    AttrSetObjectIntAttrib(PObj, "IgesIrtOp",
				   IGS_ENTYPE_SHELL_ENTITY);
	    break;
	...
	if (IgesDirEntry -> ColorNumber < 0) {
		...
		!IP_IS_POINT_OBJ(PTmp)) {
		    ...
		    AttrSetObjectIntAttrib(PObj, "IGESResetColor",
	...
	AttrSetObjectRefPtrAttrib(PObj, "DirEntry", NewIgesDirEntry);
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Construct a face from a parametric description of the face and the       *
* trimming loops.                                                            *
*                                                                            *
* PARAMETERS:                                                                *
*   SeqNum:        Entry index to process.	  	                     *
*   IgesInfo:      All information one ever needs to process IGES file.      *
*   SrfObj:        Underlying surface.                                       *
*   TrimmingLoops: Trimming loops.                                           *
*                                                                            *
* RETURN VALUE:                                                              *
*   IPObjectStruct *:   Constructed face as a trimmed loop or NULL if error. *
*****************************************************************************/
static IPObjectStruct *IgesProcessFaceEntity(int SeqNum,
					     IgesInfoStruct *IgesInfo,
					     IPObjectStruct *SrfObj,
					     IPObjectStruct *TrimmingLoops)
{
    IRIT_STATIC_DATA const IrtVecType
        Origin = { 0.0, 0.0, 0.0 };
    int SrfType = AttrGetObjectIntAttrib(SrfObj, "IgesIrtOp"),
        OldSurfaceRational = PrimSetSurfacePrimitiveRational(FALSE),
        OldPrimGenType = PrimSetGeneratePrimType(GM_GEN_PRIM_SRFS);
    IrtRType Size, R1, R2, ZRot;
    IrtPtType Center, Pt;
    IrtVecType Nrml, Vec, Vec2;
    CagdMType RndmMat, Mat, Mat2, ZRotMat;
    CagdPtStruct Pt00, Pt01, Pt10, Pt11;
    CagdSrfStruct *TmpSrf,
	*Srf = NULL;
    TrimSrfStruct *TrimSrf;
    IPObjectStruct *Prm1, *Prm2, *Prm3, *Prm4, *Prm5,
        *PObj = NULL;
    GMBBBboxStruct
        *BBox = GMBBComputeBboxObject(TrimmingLoops);

    /* Create a 'random' rotation matrix. */
    MatGenMatRotY1(3.01060, Mat);
    MatGenMatRotY1(1.61188, Mat2);
    MatMultTwo4by4(Mat, Mat, Mat2);
    MatGenMatRotZ1(2.80791,  Mat2);
    MatMultTwo4by4(RndmMat, Mat, Mat2);

    /* Estimate center & size of needed surface from trimming information. */
    IRIT_PT_ADD(Center, BBox -> Min, BBox -> Max);
    IRIT_PT_SCALE(Center, 0.5);

    Size = (BBox -> Max[0] - BBox -> Min[0]) +
           (BBox -> Max[1] - BBox -> Min[1]) +
           (BBox -> Max[2] - BBox -> Min[2]);
 
    /* Get the parameters of the primitive surfaces. */
    Prm1 = AttrGetObjectRefPtrAttrib(SrfObj, "IgesIrtOpParam1");
    Prm2 = AttrGetObjectRefPtrAttrib(SrfObj, "IgesIrtOpParam2");
    Prm3 = AttrGetObjectRefPtrAttrib(SrfObj, "IgesIrtOpParam3");
    Prm4 = AttrGetObjectRefPtrAttrib(SrfObj, "IgesIrtOpParam4");
    Prm5 = AttrGetObjectRefPtrAttrib(SrfObj, "IgesIrtOpParam5");

#ifdef DEBUG_IGS2IRIT_DUMP_FACE_PARAMS
    IPSetFloatFormat("%10.8f");
    ...
    if (Prm2 != NULL) {
        fprintf(stderr, "Param2:\n");
	IPStderrObject(Prm2);
    }
    if (Prm3 != NULL) {
        fprintf(stderr, "Param3:\n");
	IPStderrObject(Prm3);
    }
    if (Prm4 != NULL) {
        fprintf(stderr, "Param4:\n");
	IPStderrObject(Prm4);
    }
    if (Prm5 != NULL) {
        fprintf(stderr, "Param5:\n");
	IPStderrObject(Prm5);
    }
#endif /* DEBUG_IGS2IRIT_DUMP_FACE_PARAMS */

    switch (SrfType) {
	case IGS_IRT_OP_PLANE:
	    fprintf(stderr, "IGS_IRT_OP_PLANE - never tested\n");
	    break;
	case IGS_IRT_OP_RGT_CYL:
	    fprintf(stderr, "IGS_IRT_OP_RGT_CYL - never tested\n");
	    break;
	case IGS_IRT_OP_RGT_CONE:
	    fprintf(stderr, "IGS_IRT_OP_RGT_CONE - never tested\n");
	    break;
	case IGS_IRT_OP_SPHERE:
	    fprintf(stderr, "IGS_IRT_OP_SPHERE - never tested\n");
	    break;
	case IGS_IRT_OP_TORUS:
	    fprintf(stderr, "IGS_IRT_OP_TORUS - never tested\n");
	    break;
	case IGS_IRT_OP_PLANE_SRF:
	    /* center point and surface normal. */
	    assert(Prm1 != NULL && IP_IS_POINT_OBJ(Prm1) &&
		   Prm2 != NULL && IP_IS_VEC_OBJ(Prm2));

	    /* Make sure our cylinder will span all the trimming size. */
	    Size += IRIT_PT_PT_DIST(Prm1 -> U.Pt, Center);

	    IRIT_VEC_COPY(Nrml, Prm2 -> U.Vec);       /* The plane Normal. */
	    IRIT_VEC_NORMALIZE(Nrml);

	    /* Compute two vectors that spans the plane, using the normal. */
	    if (Prm3 != NULL && IP_IS_VEC_OBJ(Prm3)) {
	        IRIT_VEC_COPY(Vec, Prm3 -> U.Vec);
	    }
	    else {
	        IRIT_VEC_RESET(Vec);
	        if (IRIT_ABS(Nrml[0]) < IRIT_ABS(Nrml[1]) &&
		    IRIT_ABS(Nrml[0]) < IRIT_ABS(Nrml[2]))
		    Vec[0] = 1.0;
		else if (IRIT_ABS(Nrml[1]) < IRIT_ABS(Nrml[0]) &&
			 IRIT_ABS(Nrml[1]) < IRIT_ABS(Nrml[2]))
		    Vec[1] = 1.0;
		else
		    Vec[2] = 1.0;
	    }

	    IRIT_CROSS_PROD(Vec2, Vec, Nrml);
	    IRIT_CROSS_PROD(Vec, Vec2, Nrml);

	    IRIT_VEC_NORMALIZE(Vec);
	    IRIT_VEC_NORMALIZE(Vec2);

	    IRIT_VEC_SCALE(Vec, Size);
	    IRIT_VEC_SCALE(Vec2, Size);

	    /* Build the four plane corners from Pt and the two vectors. */
	    IRIT_PT_SUB(Pt00.Pt, Prm1 -> U.Pt, Vec);
	    IRIT_PT_ADD(Pt01.Pt, Pt00.Pt, Vec2);
	    IRIT_PT_SUB(Pt00.Pt, Pt00.Pt, Vec2);

	    IRIT_PT_ADD(Pt10.Pt, Prm1 -> U.Pt, Vec);
	    IRIT_PT_ADD(Pt11.Pt, Pt10.Pt, Vec2);
	    IRIT_PT_SUB(Pt10.Pt, Pt10.Pt, Vec2);

	    Srf = CagdBilinearSrf(&Pt00, &Pt01, &Pt10, &Pt11);
	    break;
	case IGS_IRT_OP_RGT_CYL_SRF:
	    /* Base-center, Axis-Vec and Radius. */
	    assert(Prm1 != NULL && IP_IS_POINT_OBJ(Prm1) &&
		   Prm2 != NULL && IP_IS_VEC_OBJ(Prm2) &&
		   Prm3 != NULL && IP_IS_NUM_OBJ(Prm3));

	    /* Make sure our cylinder will span all the trimming size. */
	    Size += IRIT_PT_PT_DIST(Prm1 -> U.Pt, Center);

	    IRIT_VEC_COPY(Vec, Prm2 -> U.Vec);
	    IRIT_VEC_NORMALIZE(Vec);
	    IRIT_VEC_SCALE(Vec, Size);
	    IRIT_PT_SUB(Pt, Prm1 -> U.Pt, Vec);
	    IRIT_VEC_SCALE(Vec, 2.0);

	    Srf = CagdPrimCylinderSrf(Origin, Prm3 -> U.R,
				      IRIT_VEC_LENGTH(Vec),
				      FALSE, CAGD_PRIM_CAPS_NONE);

	    /* Compute the bast Z rotation of the canonical surface that    */
	    /* minimizes possible seams collision with the trimming curves. */
	    /*   Then orient the cone at the prorer direction/position.     */
	    GMGenMatrixZ2Dir(ZRotMat, Vec);
	    ZRot = IgsDeviseBestZRotation(TrimmingLoops, Pt, ZRotMat);
	    MatGenMatRotZ1(IRIT_DEG2RAD(ZRot), Mat);

	    MatMultTwo4by4(Mat, Mat, ZRotMat);
	    TmpSrf = CagdSrfMatTransform(Srf, Mat);
	    CagdSrfFree(Srf);
	    Srf = TmpSrf;

	    CagdSrfTransform(Srf, Pt, 1.0);
	    break;
	case IGS_IRT_OP_RGT_CONE_SRF:
	    /* Base-center, Axis-Vec and Base-Radius and semi-angle. */
	    assert(Prm1 != NULL && IP_IS_POINT_OBJ(Prm1) &&
		   Prm2 != NULL && IP_IS_VEC_OBJ(Prm2) &&
		   Prm3 != NULL && IP_IS_NUM_OBJ(Prm3) &&
		   Prm4 != NULL && IP_IS_NUM_OBJ(Prm4));

	    /* Make sure our cone will span all the trimming size. */
	    Size += IRIT_PT_PT_DIST(Prm1 -> U.Pt, Center);

	    IRIT_VEC_COPY(Vec, Prm2 -> U.Vec);
	    IRIT_VEC_NORMALIZE(Vec);
	    IRIT_VEC_SCALE(Vec, Size);
	    IRIT_PT_ADD(Pt, Prm1 -> U.Pt, Vec);

	    R1 = Prm3 -> U.R + Size * tan(IRIT_DEG2RAD(Prm4 -> U.R));

	    R2 = R1 - Size * 2.0 * tan(IRIT_DEG2RAD(Prm4 -> U.R));
	    if (R2 <= 0.0) {
	        IRIT_VEC_SCALE(Vec, -2.0 * R1 / (R1 - R2));
	        R2 = IRIT_UEPS;
	    }
	    else
	        IRIT_VEC_SCALE(Vec, -2.0);

	    Srf = CagdPrimCone2Srf(Origin, R1, R2, IRIT_VEC_LENGTH(Vec),
				   FALSE, CAGD_PRIM_CAPS_NONE);
	    ...
	    CagdSrfTransform(Srf, Pt, 1.0);
	    break;
	case IGS_IRT_OP_SPHERE_SRF:
	    /* Center and radius. */
	    assert(Prm1 != NULL && IP_IS_POINT_OBJ(Prm1) &&
		   Prm2 != NULL && IP_IS_NUM_OBJ(Prm2));

	    Srf = CagdPrimSphereSrf(Origin, Prm2 -> U.R, FALSE);

	    /* Randomly rotate the sphere so it will not be vertical, to    */
	    /* reduce chances of a trimming curve going through the poles.  */
	    TmpSrf = CagdSrfMatTransform(Srf, RndmMat);
	    CagdSrfFree(Srf);
	    Srf = TmpSrf;

	    CagdSrfTransform(Srf, Prm1 -> U.Pt, 1.0);
	    break;
	case IGS_IRT_OP_TORUS_SRF:
	    fprintf(stderr, "IGS_IRT_OP_TORUS_SRF - never tested\n");
	    break;
        default:
	    Iges2IritWarning(IgesInfo, SeqNum,
			     "Unsupported surface type in face entity");
	    break;
    }

    if (Srf != NULL) {
        int i = 0;
        IPObjectStruct *TLoopObj, *TCrvObj;
	TrimCrvStruct *TLoop,
	    *TrimLoops = NULL;

#	ifdef DEBUG_IGS2IRIT_DUMP_FACE_PARAMS
	    IPSetFloatFormat("%10.8f");
	    IPStderrObject(IPGenSrfObject("Srf", Srf, NULL));
            IPStderrObject(TrimmingLoops);
#	endif /* DEBUG_IGS2IRIT_DUMP_FACE_PARAMS */

	while ((TLoopObj = IPListObjectGet(TrimmingLoops, i++)) != NULL) {
	    int k,
	        j = 0;
	    TrimCrvSegStruct *TSeg,
		*TrimSegs = NULL;

	    assert(IP_IS_OLST_OBJ(TLoopObj));

	    while ((TCrvObj = IPListObjectGet(TLoopObj, j++)) != NULL) {
	        CagdCrvStruct *E3Crv,
		    *TCrv = TCrvObj -> U.Crvs;
	      
		assert(IP_IS_CRV_OBJ(TCrvObj) &&
		       CAGD_NUM_OF_PT_COORD(TCrv -> PType) >= 2);

		if (CAGD_IS_RATIONAL_CRV(TCrv)) {
		    /* Convert to piecewise linear representation in hope   */
		    /* it will work out accurate enough.		    */
		    CagdPolylineStruct
		        *Poly = SymbCrv2Polyline(TCrv, IGS_TRIM_CRV_TOL,
						 SYMB_CRV_APPROX_TOLERANCE,
						 TRUE);

		    TCrv = CagdCnvrtPolyline2LinBspCrv(Poly);
		    CagdPolylineFree(Poly);
		}
		else
		    TCrv = CagdCrvCopy(TCrv);

		E3Crv = CagdCrvCopy(TCrv);

		/* Compute the inverse projection of the E3 points to UV. */
		for (k = 0; k < TCrv -> Length; k++) {
		    CagdRType *UV;
		    CagdPType PtE3;

		    CagdCoerceToE3(PtE3, TCrv -> Points, k, TCrv -> PType);

		    UV = MvarDistSrfPoint(Srf, PtE3, TRUE,
					  IGS_PROJ_PT_ON_SRF_SUBDIV_TOL,
					  IGS_PROJ_PT_ON_SRF_NUMER_TOL);
#		    ifdef DEBUG
		    {
		        CagdRType
			...
			CagdCoerceToE3(EvalPtE3, &R, -1, Srf -> PType);
			if (IRIT_PT_PT_DIST_SQR(EvalPtE3, PtE3) >
			                                    IGS_TRIM_CRV_TOL)
			    Iges2IritWarning(IgesInfo, SeqNum,
					     "UV point pojection on surface failed.");
		    }
#		    endif /* DEBUG */

		    /* Update the curve, in place, with the UV values. */
		    if (CAGD_IS_RATIONAL_CRV(TCrv)) {
		        TCrv -> Points[1][k] = UV[0] * TCrv -> Points[0][k];
			TCrv -> Points[2][k] = UV[1] * TCrv -> Points[0][k];
		    }
		    else {
		        TCrv -> Points[1][k] = UV[0];
			TCrv -> Points[2][k] = UV[1];
		    }
		}
		if (CAGD_NUM_OF_PT_COORD(TCrv -> PType) != 2) {
		    CagdCrvStruct
			*TmpCrv = CagdCoerceCrvTo(TCrv,
						  CAGD_IS_RATIONAL_CRV(TCrv) ?
						      CAGD_PT_P2_TYPE :
						      CAGD_PT_E2_TYPE,
						  FALSE);
		...
		TSeg = TrimCrvSegNew(TCrv, E3Crv);
		IRIT_LIST_PUSH(TSeg, TrimSegs);
	    }

	    TLoop = TrimCrvNew(CagdListReverse(TrimSegs));

	    IRIT_LIST_PUSH(TLoop, TrimLoops);
	}

	TrimSrf = TrimSrfNew(Srf, CagdListReverse(TrimLoops), TRUE);

#	ifdef DEBUG_IGS2IRIT_DUMP_FACE_PARAMS
	    TrimDbg(TrimSrf);
#	endif /* DEBUG_IGS2IRIT_DUMP_FACE_PARAMS */

	PObj = IPGenTRIMSRFObject(TrimSrf);
    }

    PrimSetSurfacePrimitiveRational(OldSurfaceRational);
    PrimSetGeneratePrimType(OldPrimGenType);
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Find the orientation of the trimming loops that is farthest from the     *
* of the cone/cylinder, now canonically oriented along the Z axis.           *
*                                                                            *
* PARAMETERS:                                                                *
*   TrimmingLoops:   Trimming curves in Euclidean space in general position. *
*   Center:          Center in Euclidean space general posision coordinates. *
*   Mat:             Matrix to map orientation to be for a vectical surface, *
*                                                                            *
* RETURN VALUE:                                                              *
*   IrtRType:       Preferred rotation is Z, that minimizes chances of       *
*		    collision between trimming curves and seem.              *
*****************************************************************************/
static IrtRType IgsDeviseBestZRotation(const IPObjectStruct *TrimmingLoops,
				       const IrtPtType Center,
				       CagdMType Mat)
{
    int i = 0;
    IrtPtType CanonicCenter, CanonicPt;
    const IPObjectStruct *TLoopObj, *TCrvObj;
    GMBBBboxStruct BBox;

    BBox.Min[0] = BBox.Min[1] = IRIT_INFNTY;
    BBox.Max[0] = BBox.Max[1] = -IRIT_INFNTY;

    MatMultPtby4by4(CanonicCenter, Center, Mat);
	...
	while ((TCrvObj = IPListObjectGet(TLoopObj, j++)) != NULL) {
	    ...
	    for (k = 0; k < TCrv -> Length; k++) {
	        CagdCoerceToE3(CanonicPt, TCrv -> Points, k, TCrv -> PType);
		MatMultPtby4by4(CanonicPt, CanonicPt, Mat);

		IRIT_PT2D_SUB(CanonicPt, CanonicPt, CanonicCenter);
		if (BBox.Min[0] > CanonicPt[0])
		    BBox.Min[0] = CanonicPt[0];
		if (BBox.Max[0] < CanonicPt[0])
		    BBox.Max[0] = CanonicPt[0];
		if (BBox.Min[1] > CanonicPt[1])
		    BBox.Min[1] = CanonicPt[1];
		if (BBox.Max[1] < CanonicPt[1])
		    BBox.Max[1] = CanonicPt[1];
	    }
	}
    }

    /* Pick the direction that is closest to the origin (or beyond). */
    if (BBox.Max[0] <= 0.0)
        return 0.0; /* Place seam at +X. */
    else if (BBox.Max[1] <= 0.0)
        return 90.0; /* Place seam at +Y. */
    else if (BBox.Min[0] >= 0.0)
        return 180.0; /* Place seam at -X. */
    else if (BBox.Min[1] >= 0.0)
        return 270.0; /* Place seam at -Y. */
    else {
        CagdRType
	    Min = IRIT_MIN(IRIT_MIN(-BBox.Min[0], BBox.Max[0]),
			   IRIT_MIN(-BBox.Min[1], BBox.Max[1]));
	if (IRIT_APX_EQ(BBox.Max[0], Min))
	    return 0.0; /* Place seam at +X. */
	else if (IRIT_APX_EQ(BBox.Max[1], Min))
	    return 90.0; /* Place seam at +Y. */
	else if (IRIT_APX_EQ(-BBox.Min[0], Min))
	    return 180.0; /* Place seam at -X. */
	else if (IRIT_APX_EQ(-BBox.Min[1], Min))
	    return 270.0; /* Place seam at -Y. */
    }	   

    assert(0);

    return 0.0; /* Something is wrong if we are here... */   
}
...
static IPObjectStruct *IgsMergeListObject(IPObjectStruct *PObj)
{
	...
	*PTmp = IPListObjectGet(PObj, 0);
	...
	AttrSetObjectIntAttrib(POut, "IgesIrtOp", IGS_IRT_OP_POLY);
	i = 1;
	do {
	    IPPolygonStruct *Pl;
					    ...
					    IgesInfoStruct *IgesInfo)
{
	...
	if (LastPCrv != NULL) {
		...
		IP_SET_OBJ_NAME(PObjTCrv, "TRIM_CRV%d",
				IgesInfo -> ObjCount++);
		sprintf(AttrName, "IgesIrtOpParam%d", l++);
		AttrSetObjectRefPtrAttrib(PObj, AttrName, PObjTCrv);
					      ...
					      IgesInfoStruct *IgesInfo)
{
	...
	j = 2;
	while (TRUE) {
	    sprintf(AttrName, "IgesIrtOpParam%d", j++);
	    if ((PObjTCrv = AttrGetObjectRefPtrAttrib(PTmp, AttrName)) == NULL)
		break;
					    ...
					    IgesInfoStruct *IgesInfo)
{
	...
	*TrimSrf = PObj -> U.TrimSrfs;
	...
	sprintf(AttrName, "IgesIrtOpParam%d", l++);
	if ((PObjTCrv = AttrGetObjectRefPtrAttrib(PObj, AttrName)) == NULL) {
