...
typedef enum IPO2IPolyKindType {
    IP_O2I_POLY_NONE = 0,
    IP_O2I_POINTS,
    IP_O2I_POLYLINES,
    IP_O2I_POLYGONS
...
typedef struct IPO2ILoadFileDataStruct {
    ...
    int ConNextLine; /* If true, IP_O2I_CONTINUE_NEXT_LINE appeared at the   */
                     /* end of this line and we continue read tokens from    */
                     /* next line (Only for IPO2INextToken).                 */
    int EndOfLine;   /* We are at end of line. If ConNextLine is TRUE this   */
                     /* flag is FALSE.                                       */
...
/* The name of the obj file we are processing. */
IRIT_STATIC_DATA const char *IPO2IOBJFileName;
...
static char *IPO2IStrToIritName(char *Str);
...
#endif /* USE_VARARGS */
static IPObjectStruct *IPO2ISetName(IPObjectStruct *PObj);
...
{   
    ...
    {"g", IPO2IParseGO},
    {"group", IPO2IParseGO}, /* Not according to standard but treat as "g". */
    {"s", IPO2IParseS},
...
*                        RGB (1,1,1).                                        M
*   IgnoreFullTransp:    Full transparency of material is ignored.           M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPOBJSaveFile, IPOBJLoadFileSetDefaultParameters                         M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPOBJLoadFile                                                            M
*****************************************************************************/
                                        ...
                                        int ForceSmoothing)
{
    ...
    if (Res != NULL &&
	IP_IS_OLST_OBJ(Res) &&
	IPListObjectLength(Res) == 1) {
        PTmp = IPListObjectGet(Res, 0);
        IPListObjectInsert(Res, 0, NULL);
	IPFreeObject(Res);
	return PTmp;
    }
    else {
...
static void IPO2IParseV(void)
{
    ...
    if (IPO2IData -> Stage != IP_O2I_VECTORS_STAGE)
        return;
    /* A vertex with 6 numeric entries is treated as a vertex with XYZ.    */
    /* While 3D systems' scanner dumps 6 numeric values for a vertex, it   */
    /* is not really documented in the obj specs.			   */
    if (IPO2IReadTokens(3, 6, Temp, NULL, IPO2IInterpReal) == 4)
...
*   1. Remove empty objects.                                                 *
*   2. Replace IP_O2I_MTL_ATTRIB attribute with the attribute of the         *
*      relevant material.                                                    *
*   3. Do smoothing of polygons in smoothing groups.                         *
*   4. Add all groups to the root group IPO2IData -> Geom.                   *
*                                                                            *
* PARAMETERS:                                                                *
*   None                                                                     *
*                                                                            *
...
static void IPO2IFinalizeGeometry(void) 
{
    ...
    for (i = 0; i <= List -> Len - 1; i++) {
	    ...
	    *PObj = List -> U.Groups[i];
        ...
        for (PObjList = PObj -> U.Lst.PObjList;
	     (*PObjList) != NULL;
	     PObjList++) {
            IPObjectStruct **PObj2, 
                **PObj3 = (*PObjList) -> U.Lst.PObjList;

            /* Replacing material index with material's attributes. */
...
*   of the same type of geometric objects (polygons, polylines, polypoints,  *
*   surfaces, trimmed surfaces or curves) and the same material.             *
...
static void IPO2IAddToGeom(IPObjectStruct *PObj) 
{
    ...
    /* 1. IPObjectStruct with list of polygons with all the polygons         */
    /*    polylines and points which require smoothing. We put here only the */
    /*    inner geometric of PObj and only for polygons/polylines/points.    */
    ...
    /*    curves/points are held in this level as separate objects and not   */
    /*    inside a list.                                                     */
    ...
    /*    The tags for polygon/polyline/points are held in the forth level. */
	...
	*PList2 = NULL,
    ...
    /* (if it's polygon we need to check for polygon/polyline/pointslist)    */
    ...
    if (i == Len) {                              /* Need to create new list. */
        PList2 = IPO2ISetName(IPGenLISTObject(NULL));
...
static void IPO2IParsePLF(void) 
{
    ...
    for (i = 0; i <= List.Len - 1; i++) {
        ...
        if ((PolyType == IP_O2I_POLYGONS) && (Pattern[0] >= 3)) {
    ...
    PObj = IPO2ISetName(IPGenPOLYObject(Poly));
...
static void IPO2IParseCurvCurv2(void)
{
   ...
   CagdCrvStruct *Crv;
    ...
    switch (IPO2IData -> CrvSrfType) {
	    ...
	    if (v.Len <= IPO2IData -> DegU + 1)
                Crv = BzrCrvNew(v.Len, CAGD_MAKE_PT_TYPE(IPO2IData -> Rat, Dim));
	    else {
		int i, j,
		    k = 0;

		Crv = BspCrvNew(v.Len, IPO2IData -> DegU + 1, 
			        CAGD_MAKE_PT_TYPE(IPO2IData -> Rat, Dim));

		Crv -> KnotVector[k++] = Parm.U.r[0];
		for (i = 0; i < Parm.Len; i++) {
		    for (j = 0; j < IPO2IData -> DegU; j++)
			Crv -> KnotVector[k++] = Parm.U.r[i];
		}
		Crv -> KnotVector[k++] = Parm.U.r[i - 1];
		assert(k == Crv -> Length + Crv -> Order);
	    }
            break;
        case IP_O2I_BSPLINE:            
	...
	AttrSetRealAttrib(&Crv -> Attr, "BspDomainMax", u[1]);
        ...
        IPO2IAddToGeom(IPO2ISetName(IPGenCRVObject(Crv)));
    }
    else {
        /* Add control points to curve. */
...
{    
    *HasTopLvlTrim = -1;
    while (TRUE) {
		...
		CrvList = IPO2ISetName(IPGenLISTObject(NULL));

		sprintf(Name, "O2I_%d", IPO2IUniqueNum++);
		IP_SET_OBJ_NAME2(CrvList, Name);
	    }
		    ...
		    u[2] = { 0, 1 };
                ...
                AttrSetRealAttrib(&Crv -> Attr, "BspDomainMin", u[0]);
                AttrSetRealAttrib(&Crv -> Attr, "BspDomainMax", u[1]);
                IPListObjectAppend(CrvList, IPO2ISetName(IPGenCRVObject(Crv)));
            }
            while (!IPO2IData -> EndOfLine);
...
static void IPO2IParseSurf(void)
{
        ...
        case IP_O2I_BEZIER:
	{
	    ...
	    if (!IRIT_APX_EQ(St[0], 0.0) || !IRIT_APX_EQ(St[1], 1.0) ||
		!IRIT_APX_EQ(St[2], 0.0) || !IRIT_APX_EQ(St[3], 1.0)) {
		CagdSrfStruct *TSrf;

		/* Convert to a B-spline surface and set proper domain. */
		TSrf = CagdCnvrtBzr2BspSrf(Srf);
		CagdSrfFree(Srf);
		CagdSrfSetDomain(TSrf, St[0], St[1], St[2],St[3]);
		Srf = TSrf;
	    }
            break;
        }
    ...
    if (TrimCrvList == NULL) 
        IPO2IAddToGeom(IPO2ISetName(IPGenSRFObject(Srf)));
    else 
        IPO2IAddToGeom(IPO2ISetName(IPGenTRIMSRFObject(TrimSrfNew(Srf,
							     TrimCrvList,
							     HasTopLvlTrim))));
}
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Print warning that option of that statement isn't supported and return.  *
*                                                                            *
* PARAMETERS:                                                                *
*   None                                                                     *
*                                                                            *
...
static void IPO2IParseUsemtl(void)
{
    ...
    for (i = 0; i <= IPO2IData -> Mtls.Len - 1; i++)
	    ...
	    IPO2IListStruct
	        *MLst = &IPO2IData -> Mtls,
	        *GLst = &IPO2IData -> GroupsList;

            MLst -> Curr = i;
	    if ((PObj = GLst -> U.Groups[GLst -> Curr]) != NULL &&
		(Mtl = &MLst -> U.Mtl[i]) != NULL) {
		/* Apply the material info to this PObj group. */
		if (PObj -> Attr == NULL)
	            PObj -> Attr = AttrCopyAttributes(Mtl -> Attr);
		else {
		    IPAttributeStruct *A,
		        *Attr2 = AttrCopyAttributes(Mtl -> Attr);

		    while (Attr2 != NULL) {
		        IRIT_LIST_POP(A, Attr2);     /* Get the attribute. */

			/* Delete existing one if any (of same name).      */
			AttrFreeObjectAttribute(PObj, AttrGetAttribName(A));

			IRIT_LIST_PUSH(A, PObj -> Attr);
		    }
		}
	    }
            return;
...
*                                                                            *
* RETURN VALUE:                                                              *
*   char *: The combined path or NULL if FileName is NULL. Allocated dynam.  *
*****************************************************************************/
static char *IPO2ICombinePaths(const char *Path1, const char *Path2)
{
    char *Ret;
    ...
    Ret = IritMalloc((int) (strlen(Path1) + strlen(Path2) + 10));
    strcpy(Ret, Path1);
    ...
    if ((Path1[0] == '/') || (Path1[1] == ':') || ((Len = strlen(Path1)) == 0))
        return IritStrdup(Path2);
    Ret = IritMalloc(strlen(Path1) + strlen(Path2) + 10);
    strcpy(Ret, Path1);
    if (Path1[Len - 1] != '/')
        strcat(Ret, "/");
#endif
...
*        mtllib statement (or in the same name as the obj file name).        *
*****************************************************************************/
static int IPO2IInitMtlFile(IPO2ILoadFileDataStruct *Data) 
{
    ...
    if ((Data -> File = fopen(FilePath, "r")) == NULL) {
        char *p,
	    *FilePath2 = (char *) IritMalloc(strlen(IPO2IOBJFileName) + 10);

	/* Try mtl file with same name as obj file. */
	strcpy(FilePath2, IPO2IOBJFileName);
	if ((p = strrchr(FilePath2, '.')) != NULL)
	    *p = 0;
	strcat(FilePath2, ".mtl");

	if (IPO2IWarningMsgs)
	    IRIT_WARNING_MSG_PRINTF("Cannot open MTL file \"%s\", trying \"%s\" instead.\n",
				    FilePath, FilePath2);
        IritFree(FilePath);
	FilePath = FilePath2;
	if ((Data -> File = fopen(FilePath, "r")) == NULL) {
	    if (IPO2IWarningMsgs)
		IRIT_WARNING_MSG_PRINTF("Cannot open MTL file \"%s\"; Ignoring mtl.\n",
				        FilePath);
            IritFree(FilePath);
	    longjmp(IPO2ILongJumpBuffer, 1);
	}
	else {
	    IritFree(Data -> FileName);
	    Data -> FileName = IritStrdup(FilePath);
	    IritFree(Data -> DirPath);
	    Data -> DirPath = IritStrdup(FilePath);
	}
    }

    IRIT_INFO_MSG_PRINTF("Parsing MTL file \"%s\".\n", FilePath);
    IritFree(FilePath);
    return TRUE;
}
...
static void IPO2IParseMtllib(void)
{
    ...
    do {
	    ...
	    *SaveWarningMsg = IPO2IOtherWarningMsg;
        ...
        IPO2IOtherErrorMsg = "Error in mtl file \"%s\" line %d. %s. Stop parsing the file.\n";
        ...
        if (setjmp(IPO2ILongJumpBuffer) == 0) {
	        ...
	        char Token[IRIT_LINE_LEN + 1], Aux[IRIT_LINE_LEN + 3];
                ...
                if (i == Len)
                    IPO2IWarning("Can't recognize the statement \"%s\"", Token);
            }
        }
...
static char *IPO2IStrToIritName(char *Str) 
{
    ...
    if (Str == NULL || strlen(Str) == 0 || IritStrIStr(Str, "null") != NULL) {
        sprintf(Str, "O2I_%d", IPO2IUniqueNum++);
        return Str;
    }
...
static void IPO2IParseGO(void)
{
    char Token[IRIT_LINE_LEN + 1], OtherGroups[IRIT_LINE_LEN_LONG];
    ...
    /* If we have more groups' associations in the line - add as attribute. */
    OtherGroups[0] = 0;
    while (IPO2INextToken(Token) != IP_O2I_LINE_END) {
	sprintf(&OtherGroups[strlen(OtherGroups)], " %s", Token);
    }
    AttrSetObjectStrAttrib(Group, "OtherGroups", IritStrdup(OtherGroups));
}
...
static void IPO2IParseCall(void)
{
	...
	*SaveMsg = IPO2IOtherErrorMsg;
    ...
    IPO2IOtherErrorMsg = "Error in obj file \"%s\" line %d. %s. Ignoring the file.\n";
...
static int IPO2IInitLoadFile(const char *OBJFileName)
{
    ...
    IPO2IData -> Geom = IPGenListObject("O2IRoot", NULL, NULL);
...
static int IPO2ICheckReference(IPO2IListStruct *List, int *Number)
{
    ...
    if (*Number < 0) {
        ...
        if (Temp <= 0) 
            IPO2IErrorAndExit("The reference \"%d\" points to non-positive index (%d).\n",
			      *Number, Temp);
        *Number = Temp;
    }
    return *Number;
...
static int IPO2IReadLine(void) 
{
    ...
    while (TRUE) {
	    ...
	    int Len;
            ...
            else if ((Len = Line[strlen(Line) - 1]) == '\n' ||
		     Line[Len - 1] == '\r') {
	        while ((Len = (int) strlen(Line)) > 0 &&
		       (Line[Len - 1] == '\n' || Line[Len - 1] == '\r')) {
...
*           values were in that pattern and only that pattern is accepted.   *
*           If unaccepted pattern is received call IPO2IErrorAndExit.        *
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Interpret the given Token as one integer. Ignore additional "garbage"    *
    ...
    if (stricmp(Token1, "spectral") * stricmp(Token1, "xyz") == 0) {
        if (stricmp(Token1, "spectral") == 0) {
            ...
            if (IPO2INextToken(Token2) == IP_O2I_LINE_END)
                IPO2IErrorAndExit("Expecting more parameters");
            IPO2IReadTokens(0, 1, &Factor, NULL, IPO2IInterpReal);
...
static void IPO2IParseBoost(char *MapToken) 
{
    ...
    sprintf(AttrName, "%s%s_boost", IP_O2I_NOT_SUPPORTED_PREFIX, MapToken); 
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Sets a unique name to the given object.                                  *
*                                                                            *
* PARAMETERS:                                                                *
*   PObj:   Object to set a unique name for, in place.                       *
*                                                                            *
* RETURN VALUE:                                                              *
*   IPObjectStruct *:    PObj, but with a unique name set.                   *
*****************************************************************************/
static IPObjectStruct *IPO2ISetName(IPObjectStruct *PObj)
{
    char Name[IRIT_LINE_LEN_VLONG];

    assert(PObj -> ObjName == NULL);
