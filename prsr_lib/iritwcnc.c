...
static void DumpDataForNCGCode(IPObjectStruct *PObj, IrtHmgnMatType DummyMat)
{
	...
	*Pllns = NULL;
	...
	case IP_OBJ_CURVE:
	    ...
	    Pllns = PPObj -> U.Pl = IPReversePlList(PPObj -> U.Pl);
	    break;
    ...
    for (Pl = Pllns; Pl != NULL; Pl = Pl -> Pnext) {
        IPVertexStruct *V;
	int SimilarVrtxToLast = FALSE,
	    RecoverFeedrate = FALSE;
	IrtRType
	    RelPlFeedRate = AttrGetRealAttrib(Pl -> Attr, "RelFeedRate");

	if (IP_ATTR_IS_BAD_REAL(RelPlFeedRate))
	    RelPlFeedRate = 1.0;
	assert(RelPlFeedRate <= 1.0);
	if (RelPlFeedRate < 0.1) {
	    IRIT_INFO_MSG_PRINTF("WARNING: Relative feedrate smaller than 0.1 at %7f %7f %7f\n",
				 Pl -> PVertex -> Coord[0],
				 Pl -> PVertex -> Coord[1],
				 Pl -> PVertex -> Coord[2]); 
	}
	...
	for (V = Pl -> PVertex; V != NULL; V = V -> Pnext) {
	    if (V == Pl -> PVertex || SimilarVrtxToLast) {
	        /* First location in polyline. */
	        IrtRType Dz;

		if (IRIT_PT_PT_DIST(GlblNCGcodePos, V -> Coord) < IRIT_EPS) {
		    /* First location is same as last location of prev. pl. */
		    SimilarVrtxToLast = TRUE;
		    continue;
		}
		if (V != Pl -> PVertex ) {
		    /* Entered because SimilarVrtxToLast was on. */
		    SimilarVrtxToLast = FALSE;
		}
		...
		fprintf(GlblNCFile, _IPNCGcodeFloatFormat,
			GlblNCGCodeFeedRate *
			    IRIT_MIN(GlblNCGCodeBridgeRelFeedRate,
				     RelPlFeedRate));
	    }
	    else {
		if (RecoverFeedrate) {
		    ...
		    fprintf(GlblNCFile, _IPNCGcodeFloatFormat,
			    GlblNCGCodeFeedRate * RelPlFeedRate);
		    RecoverFeedrate = FALSE;
		}
		else
