...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPCnvPolyToPolyVrtxIdxStruct, IPCnvFindAdjacentEdge,		     M
*   IPCnvFindAdjacentEdge, IPCnvIsVertexBoundary                             M
*                                                                            *
* KEYWORDS:                                                                  M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Find out if the given vertex is a boudnary vertex.  A boundary vertex is M
* a vertex that does not have polygons completely around it, forming a       M
* closed ring.								     M
*                                                                            *
* PARAMETERS:                                                                M
*   PVIdx:         A mesh to search the adjacent edge in.		     M
*   VertexIndex:   The vertex index to examine if a boudnary vertex.	     M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:   TRUE if examined vertex is a boundary vertex, FALSE otherwise.    M
*                                                                            *
* SEE ALSO:                                                                  M
*   IPCnvFindAdjacentPoly, IPCnvFindAdjacentEdge, IPCnvPolyVrtxNeighbors,    M
*   IPCnvPolyToPolyVrtxIdxStruct					     M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPCnvIsVertexBoundary  						     M
*****************************************************************************/
int IPCnvIsVertexBoundary(const IPPolyVrtxIdxStruct *PVIdx, int VertexIndex)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrPIdxID = ATTRIB_NAME_BAD_NUMBER;
    const IPPolyPtrStruct
        *PPoly = PVIdx -> PPolys[VertexIndex];
    int i;
    int * const 
        *Polygons = PVIdx -> Polygons;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrPIdxID, "_PIdx");

    /* Go over all polygons sharing this VertexIndex. */
    for ( ; PPoly != NULL; PPoly = PPoly -> Pnext) {
	int j, Num; 
	IPVertexStruct 
	    *V = PPoly -> Poly -> PVertex;
	
	i = AttrGetIntAttrib2(PPoly -> Poly -> Attr, AttrPIdxID);
	...
	for (j = 0; Polygons[i][j] != -1; j++) {
	    if (Polygons[i][j] == VertexIndex) {
	        /* We are on an edge that holds our vertex as first one -   */
	        /* make sure it also has an adjacent edge.     		    */
	        if (IPCnvFindAdjacentEdge(PVIdx, Polygons[i][j],
					  Polygons[i][(j + 1) % Num]) == NULL)
		    return TRUE;              /* This is a boundary vertex. */
	    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Find out the adjacent edge of the given edge defined by vertices	     M 
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPCnvFindAdjacentPoly, IPCnvPolyVrtxNeighbors, IPCnvIsVertexBoundary,    M
*   IPCnvPolyToPolyVrtxIdxStruct					     M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPCnvFindAdjacentEdge  						     M
*****************************************************************************/
				      ...
				      int SecondVertexIndex)
{
    ...
    /* Go over all polygons sharing SecondVertexIndex. */
    for ( ; PPoly != NULL; PPoly = PPoly -> Pnext) {
	int j, Num; 
	IPVertexStruct 
	    *V = PPoly -> Poly -> PVertex;
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPCnvFindAdjacentEdge, IPCnvPolyVrtxNeighbors, IPCnvIsVertexBoundary     M
*   IPCnvPolyToPolyVrtxIdxStruct					     M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPCnvFindAdjacentPoly  						     M
*****************************************************************************/
...
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrVIdxID = ATTRIB_NAME_BAD_NUMBER;
    int VIdx;
    IPPolyPtrStruct *PolyPtr, *PolyArray;
    IPPolygonStruct *Poly;
    IPVertexStruct *Ver;
    
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrVIdxID, "_VIdx");

    VIdx = AttrGetIntAttrib2(VNext -> Attr, AttrVIdxID);
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPCnvFindAdjacentEdge, IPCnvFindAdjacentPoly, IPCnvPolyVrtxNeighbors,    M
*   IPCnvIsVertexBoundary						     M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPCnvPolyToPolyVrtxIdxStruct                                             M
