...
*   Overwriting existing item if was any.				     M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPListObjectInsert2, IPListObjectLength, IPListObjectFind,               M
*   IPListObjectAppend, IPListObjectDelete, IPListObjectGet	             M
*                                                                            *
* KEYWORDS:                                                                  M
...
* DESCRIPTION:                                                               M
* Insert an object PObjItem at index Index into a list of objects, PObj.     M
*   Same as IPListObjectInsert, but make sure to first make space in the     M
* list at index Index, by moving all items above or equal to index Index one M
* position up, expanding the length of the list by one.			     M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:       A list of objects to insert PObjItem into.                   M
*   Index:      Index where PObjItem should enter PObj.                      M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPListObjectInsert, IPListObjectLength, IPListObjectFind,                M
*   IPListObjectAppend, IPListObjectDelete, IPListObjectGet	             M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPListObjectInsert2, lists, insert                                       M
*****************************************************************************/
...
{
    int i,
      Len = IPListObjectLength(PObj);

    /* Move the end of list (and also make space if necessary). */
    IPListObjectInsert(PObj, Len + 1, NULL);
    for (i = Len; i > Index; i--)		        /* Shift all items. */
...
*   If input name is ObjName, the hierarchy will be assigned names of the    M
* form RootName{_%d}*.				                             M
*                                                                            *
* PARAMETERS:                                                                M
*   Obj:      To propagate object names from.  Root of hierarchy.            M
*   ObjName:  Object name to propagate, NULL if to be picked up fron Obj.    M
*                                                                            *
* RETURN VALUE:                                                              M
*   void		                                                     M
*                                                                            *
...
void IPPropagateObjectName(IPObjectStruct *Obj, const char *ObjName)
{
    ...
    if (ObjName == NULL) {
	if (IP_VALID_OBJ_NAME(Obj) &&
	    strcmp(IP_GET_OBJ_NAME(Obj), "NONE") != 0)
	    ObjName = IP_GET_OBJ_NAME(Obj);
	else
	    return;				  /* No name to propagate. */
    }
    strncpy(Name, ObjName, IRIT_LINE_LEN_VLONG);
...
#   endif /* DEBUG */
	...
	for (Index = 0;
	     ...
	     Index++) {
	    char NameSon[IRIT_LINE_LEN_VLONG];

	    sprintf(NameSon, "%s_%d", Name, Index + 1);
	    IPPropagateObjectName(PObjTmp, NameSon);
	}
    }
}
			     ...
			     int CopyAll)
{
    ...
    if (Dest -> ObjType == IP_OBJ_LIST_OBJ && Dest -> U.Lst.PObjList != NULL)
	IritFree(Dest -> U.Lst.PObjList);/*IPCopyObjectGeomData mallocs it. */
