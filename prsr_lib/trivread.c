				     ...
				     int *ErrLine)
{
	...
	*ErrLine = _IPStream[Handler].LineNum;
	...
	if (CAGD_IS_RATIONAL_PT(PType)) {
	    if ((Token = _IPGetToken(Handler, StringToken)) != IP_TOKEN_OTHER ||
	        !IPGetRealNumber(StringToken, &NewTV -> Points[0][i])) {
	...
	for (j = 1; j <= MaxCoord; j++) {
	    if ((Token = _IPGetToken(Handler, StringToken)) != IP_TOKEN_OTHER ||
	        !IPGetRealNumber(StringToken, &NewTV -> Points[j][i])) {
		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		TrivTVFree(NewTV);
		return NULL;
				     ...
				     int *ErrLine)
{
    ...
    *ErrLine = _IPStream[Handler].LineNum;
    ...
    assert(NewTV != NULL);
    if (!BspKnotVerifyKVValidity(NewTV -> UKnotVector, NewTV -> UOrder,
				 NewTV -> ULength, IP_KNOT_SEQ_VALIDITY_TOL))
        IRIT_WARNING_MSG_PRINTF("Invalid U knot sequence detected in trivariate.\n");
    if (!BspKnotVerifyKVValidity(NewTV -> VKnotVector, NewTV -> VOrder,
				 NewTV -> VLength, IP_KNOT_SEQ_VALIDITY_TOL))
        IRIT_WARNING_MSG_PRINTF("Invalid V knot sequence detected in trivariate.\n");
    if (!BspKnotVerifyKVValidity(NewTV -> WKnotVector, NewTV -> WOrder,
				 NewTV -> WLength, IP_KNOT_SEQ_VALIDITY_TOL))
        IRIT_WARNING_MSG_PRINTF("Invalid W knot sequence detected in trivariate.\n");
