...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarMVStruct *: The read multi-variate, or NULL if an error occurred.    M
*                                                                            *
* KEYWORDS:                                                                  M
				     ...
				     int *ErrLine)
{
	...
	*ErrLine = _IPStream[Handler].LineNum;
	...
	if (CAGD_IS_RATIONAL_PT(PType)) {
	    if ((Token = _IPGetToken(Handler, StringToken)) != IP_TOKEN_OTHER ||
	        !IPGetRealNumber(StringToken, &NewMV -> Points[0][i])) {
	...
	for (j = 1; j <= MaxCoord; j++) {
	    if ((Token = _IPGetToken(Handler, StringToken)) != IP_TOKEN_OTHER ||
	        !IPGetRealNumber(StringToken, &NewMV -> Points[j][i])) {
		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		MvarMVFree(NewMV);
		return NULL;
				     ...
				     int *ErrLine)
{
    ...
    *ErrLine = _IPStream[Handler].LineNum;
    ...
    for (i = 0; i < Dim; i++) {
        if (!BspKnotVerifyKVValidity(NewMV -> KnotVectors[i],
				     NewMV -> Orders[i], NewMV -> Lengths[i],
				     IP_KNOT_SEQ_VALIDITY_TOL))
	    IRIT_WARNING_MSG_PRINTF("Invalid knot sequence detected in multivariate in direction %d.\n", i);
    }
