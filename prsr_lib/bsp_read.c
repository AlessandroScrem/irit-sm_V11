				   ...
				   int *ErrLine)
{
	...
	*ErrLine = _IPStream[Handler].LineNum;
	...
	if ((Token = _IPGetToken(Handler, StringToken)) != IP_TOKEN_OTHER ||
	    !IPGetRealNumber(StringToken, &NewCrv -> KnotVector[i])) {
	...
	*ErrLine = _IPStream[Handler].LineNum;
	...
	if (CAGD_IS_RATIONAL_PT(PType)) {
	    if ((Token = _IPGetToken(Handler, StringToken)) !=
							IP_TOKEN_OTHER ||
		!IPGetRealNumber(StringToken, &NewCrv -> Points[0][i])) {
		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		CagdCrvFree(NewCrv);
		return NULL;
	...
	for (j = 1; j <= MaxCoord; j++) {
	    if ((Token = _IPGetToken(Handler, StringToken)) !=
							IP_TOKEN_OTHER ||
		!IPGetRealNumber(StringToken, &NewCrv -> Points[j][i])) {
		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		CagdCrvFree(NewCrv);
		return NULL;
    ...
    *ErrStr = NULL;
    ...
    /* Validate the knot sequence values. */
    if (!BspKnotVerifyKVValidity(NewCrv -> KnotVector, NewCrv -> Order,
				 NewCrv -> Length, IP_KNOT_SEQ_VALIDITY_TOL))
        IRIT_WARNING_MSG_PRINTF("Invalid knot sequence detected in curve.\n");
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Reads from a file B-spline surface(s).				     M
*   If error is detected in reading the file, ErrStr is set to a string      M
				   ...
				   int *ErrLine)
{
	...
	*ErrLine = _IPStream[Handler].LineNum;
	...
	for (i = 0; i < KVLen; i++) {
	    if ((Token = _IPGetToken(Handler, StringToken)) !=
							     IP_TOKEN_OTHER ||
    		!IPGetRealNumber(StringToken, &KnotVector[i])) {
    		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		CagdSrfFree(NewSrf);
		return NULL;
	...
	if (CAGD_IS_RATIONAL_PT(PType)) {
	    if ((Token = _IPGetToken(Handler, StringToken)) !=
							     IP_TOKEN_OTHER ||
	        !IPGetRealNumber(StringToken, &NewSrf -> Points[0][i])) {
		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		CagdSrfFree(NewSrf);
		return NULL;
	...
	for (j = 1; j <= MaxCoord; j++) {
	    if ((Token = _IPGetToken(Handler, StringToken)) !=
							     IP_TOKEN_OTHER ||
	        !IPGetRealNumber(StringToken, &NewSrf -> Points[j][i])) {
		*ErrStr = IRIT_EXP_STR("Numeric data expected");
		*ErrLine = _IPStream[Handler].LineNum;
		CagdSrfFree(NewSrf);
		return NULL;
    ...
    *ErrLine = _IPStream[Handler].LineNum;
    ...
    /* Validate the knot sequence values. */
    if (!BspKnotVerifyKVValidity(NewSrf -> UKnotVector, NewSrf -> UOrder,
				 NewSrf -> ULength,
				 IP_KNOT_SEQ_VALIDITY_TOL))
        IRIT_WARNING_MSG_PRINTF("Invalid U knot sequence detected in surface.\n");
    if (!BspKnotVerifyKVValidity(NewSrf -> VKnotVector, NewSrf -> VOrder,
				 NewSrf -> VLength, IP_KNOT_SEQ_VALIDITY_TOL))
        IRIT_WARNING_MSG_PRINTF("Invalid V knot sequence detected in surface.\n");
