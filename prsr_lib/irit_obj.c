                           ...
                           const IPObjectStruct *PObj)
{
    ...
    for (i = v; i < 0; i++) {
#	ifdef  IRIT_OBJ_DUMP_NEW_LINES
            /* Starting new line each 10 vectors. */
            if ((((i - v) % 10 ) == 0) && (v != i))
	        fprintf(IPI2OData -> ObjFile, "\\\n\t");
#	endif /* IRIT_OBJ_DUMP_NEW_LINES */
        ...
        if ((TrimSrfs -> TrimCrvList == NULL) || 
	        ...
	        *TrimCrv = TrimSrfs -> TrimCrvList;
            ...
            for(i = 0; TrimCrv != NULL; TrimCrv = TrimCrv -> Pnext, i++) {
                ...
                SubTrimCrv = (TrimCrvStruct *) 
		    AttrGetRefPtrAttrib(TrimCrv -> Attr, IP_I2O_SUB_TRIM_ATTRIB);
...
static void IPI2OWriteData(const IPObjectStruct *PObj)
{
    switch (PObj -> ObjType) {
        ...
        case IP_OBJ_POLY: {
	        ...
	        IPI2OWarning("Encountered unsupported element \"%s\" of type POLYSTRIP",
			     IP_GET_OBJ_NAME(PObj));
                break;
            }
        ...
        case IP_OBJ_POINT: {
            ...
            if (ATTR_OBJ_ATTR_EXIST(PObj, "LIGHT_SOURCE")) {
                IPI2OWarning("Encountered unsupported element \"%s\" of type POINT as LIGHT_SOURCE",
			     IP_GET_OBJ_NAME(PObj));
                break;
            }
        ...
        case IP_OBJ_NUMERIC: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type NUMBER",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_PLANE: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type PLANE",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_MATRIX: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type MATRIX",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_STRING: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type STRING",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_TRIVAR: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type TRIVAR",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_INSTANCE: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type INSTANCE",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_TRISRF: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type TRISRF",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        case IP_OBJ_MODEL: {
        ...
        case IP_OBJ_MULTIVAR: {
            IPI2OWarning("Encountered unsupported element \"%s\" of type MULTIVAR",
			 IP_GET_OBJ_NAME(PObj));
            break;
        }
        default:
            IPI2OWarning("Encountered unknown element \"%s\" recognized as ObjType %d", 
...
static void IPI2OUsemtl(void)
{
    ...
    if (IPI2OData -> Map_Kd != NULL) {
        IrtImgParsePTextureString(IPI2OData -> Map_Kd, Texture, Scale,
				  &Flip, &NewImage);
...
static int IPI2OCmpTextures(const char *Text1, const char* Text2)
{
    ...
    IrtImgParsePTextureString(Text1, Name1, Scale, &Flip, &NewImage);
    IrtImgParsePTextureString(Text2, Name2, Scale, &Flip, &NewImage);
