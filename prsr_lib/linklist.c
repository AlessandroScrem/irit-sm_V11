...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Reverses a list.  Original input list is used in place.                    M
*                                                                            *
* PARAMETERS:                                                                M
*   ListObj:    List object to reverses its entries, in place.               M
*                                                                            *
* RETURN VALUE:                                                              M
...
IPObjectStruct *IPReverseListObj(IPObjectStruct *ListObj)
{
    ...
    for (i = IPListObjectLength(ListObj) - 1, j = 0; i > j; i--, j++) {
        IRIT_SWAP(IPObjectStruct *, ListObj -> U.Lst.PObjList[i],
			            ListObj -> U.Lst.PObjList[j]);
    }
...
void IPReverseVrtxList(IPPolygonStruct *Pl)
{
	...
	*VNext = V -> Pnext;
    ...
    assert(!_IPPolyListCirc || IPGetLastVrtx(V) -> Pnext != NULL);
