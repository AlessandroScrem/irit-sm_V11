...
IRIT_STATIC_DATA IPIgesLoadDfltFileParamsStruct
    IPIgesLoadDfltParams = { FALSE, FALSE, FALSE, FALSE };

IRIT_STATIC_DATA IPSTLLoadDfltFileParamsStruct
    IPSTLLoadDfltParams = { FALSE, FALSE, FALSE, FALSE };

IRIT_STATIC_DATA IPOBJLoadDfltFileParamsStruct
    IPOBJLoadDfltParams = { FALSE, TRUE, TRUE, TRUE };

IRIT_STATIC_DATA IPGcodeLoadDfltFileParamsStruct
    IPGcodeLoadDfltParams = { FALSE, FALSE };
			  ...
			  int IsPipe)
{
	...
	_setmode(fileno(f), O_BINARY);    /* Make sure it is in binary mode. */
#endif /* __OS2GCC__ */
...
IPObjectStruct *IPGetObjects(int Handler)
{
	...
	*PObj = NULL;
	...
	case IP_IGS_FILE:
	        ...
	        PObj = IPIgesLoadFile(_IPStream[Handler].FileName,
				      IPIgesLoadDfltParams.ClipTrimmedSrf,
				      IPIgesLoadDfltParams.DumpAll,
				      IPIgesLoadDfltParams.IgnoreGrouping,
				      IPIgesLoadDfltParams.Messages);
            break;
	case IP_STL_FILE:
            if (!IRT_STR_ZERO_LEN(_IPStream[Handler].FileName))
	        PObj = IPSTLLoadFile(_IPStream[Handler].FileName,
				     _IPStream[Handler].FileType
							    == IP_FILE_BINARY,
				     IPSTLLoadDfltParams.EndianSwap,
				     IPSTLLoadDfltParams.NormalFlip,
				     IPSTLLoadDfltParams.Messages);
            break;
	case IP_OBJ_FILE:
            if (!IRT_STR_ZERO_LEN(_IPStream[Handler].FileName))
	        PObj = IPOBJLoadFile(_IPStream[Handler].FileName,
				     IPOBJLoadDfltParams.WarningMsgs,
				     IPOBJLoadDfltParams.WhiteDiffuseTexture,
				     IPOBJLoadDfltParams.IgnoreFullTransp,
				     IPOBJLoadDfltParams.ForceSmoothing);
            break;
	case IP_GCODE_FILE:
            if (!IRT_STR_ZERO_LEN(_IPStream[Handler].FileName))
	        PObj = IPNCGCodeLoadFile(
				  _IPStream[Handler].FileName,
				  IPGcodeLoadDfltParams.ArcCentersRelative,
				  IPGcodeLoadDfltParams.Messages);
            break;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Sets default loading parameters for Iges files.	                     M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPGetObjects, IPIgesLoadFile                                             M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPIgesLoadFileSetDefaultParameters                                       M
*****************************************************************************/
void IPIgesLoadFileSetDefaultParameters(int ClipTrimmedSrf,
					...
					int Messages)
{
    IPIgesLoadDfltParams.ClipTrimmedSrf = ClipTrimmedSrf;
    IPIgesLoadDfltParams.DumpAll = DumpAll;
    IPIgesLoadDfltParams.IgnoreGrouping = IgnoreGrouping;
    IPIgesLoadDfltParams.Messages = Messages;
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Sets default loading parameters for STL files.                           M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPGetObjects, IPSTLLoadFile                                              M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPSTLLoadFileSetDefaultParameters                                        M
*****************************************************************************/
void IPSTLLoadFileSetDefaultParameters(int BinarySTL,
				       ...
				       int Messages)
{
    IPSTLLoadDfltParams.BinarySTL = BinarySTL;
    IPSTLLoadDfltParams.EndianSwap = EndianSwap;
    IPSTLLoadDfltParams.NormalFlip = NormalFlip;
    IPSTLLoadDfltParams.Messages = Messages;
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Sets default loading parameters for OBJ files.                           M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPGetObjects, IPOBJLoadFile                                              M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPOBJLoadFileSetDefaultParameters                                        M
*****************************************************************************/
void IPOBJLoadFileSetDefaultParameters(int WarningMsgs,
				       ...
				       int ForceSmoothing)
{
    IPOBJLoadDfltParams.WarningMsgs = WarningMsgs;
    IPOBJLoadDfltParams.WhiteDiffuseTexture = WhiteDiffuseTexture;
    IPOBJLoadDfltParams.IgnoreFullTransp = IgnoreFullTransp;
    IPOBJLoadDfltParams.ForceSmoothing = ForceSmoothing;
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Sets default loading parameters for GCode NC files.                      M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* SEE ALSO:                                                                  M
*   IPGetObjects, IPNCGCodeLoadFile                                          M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPNCGCodeLoadFileSetDefaultParameters                                    M
*****************************************************************************/
void IPNCGCodeLoadFileSetDefaultParameters(int ArcCentersRelative,
					   int Messages)
{
    IPGcodeLoadDfltParams.ArcCentersRelative = ArcCentersRelative;
    IPGcodeLoadDfltParams.Messages = Messages;
}
...
int IPProcessModel2TrimSrfs(IPFreeFormStruct *FreeForms)
{
	...
	/* Re-assign the trimmed surfaces into the model object, in place. */
	IPFreeObjectGeomData(PObj);
	PObj -> ObjType = IP_OBJ_TRIMSRF;
	PObj -> U.TrimSrfs = TrimSrfs;
    }

    /* And move the in-place trimmed surfaces to the proper place. */
    FreeForms -> TrimSrfObjs = FreeForms -> ModelObjs;
...
static void GetNumericToken(int Handler, IrtRType *r)
{
    ...
    if (!IPGetRealNumber(StringToken, r))
...
static void IPGetAuxObject(int Handler, IPObjectStruct *PObj)
{
	...
	Token = IP_TOKEN_NONE;
    char *ErrStr,
        *StringToken = IritMalloc(IP_MAX_ATTR_LENGTH);
    CagdRType *Coords;
    IPPolygonStruct *PPolygon;
...
void _IPUnGetToken(int Handler, char *StringToken)
{
    ...
    if (strlen(StringToken) > IRIT_LINE_LEN - 2)
        IP_FATAL_ERROR_EX(IP_ERR_STR_TOO_LONG, _IPStream[Handler].LineNum,
			  StringToken);
		...
		*Quoted = TRUE;
		while ((!InputEOF(Handler)) &&
		    ...
		    *LocalStringToken++ = c;      /* Copy the quoted string. */
		    if (Len++ >= IP_MAX_ATTR_LENGTH) {
			StringToken[IP_MAX_ATTR_LENGTH - 1] = 0;
				...
				char *Name)
{
	...
	if (i < 0 && IPGetRealNumber(StringToken, &d)) {
	    if (d == (int) d)
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Fetchs a real number fro ma given string.                                M
*                                                                            *
* PARAMETERS:                                                                M
*   StrNum:    A string representing a real number.			     M
*   RealNum:   The fetched real number.					     M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:   TRUE if number was parsed correctly, FALSE otherwise.             M
*                                                                            *
* KEYWORDS:                                                                  M
*   IPGetRealNumber, files                                                   M
*****************************************************************************/
int IPGetRealNumber(const char *StrNum, IrtRType *RealNum)
{
    ...
    if (strchr(StrNum, ',') != NULL && strchr(StrNum, '.') == NULL) {
        char
	    *StrNum2 = IritStrdup(StrNum);

	for (i = 0; i < strlen(StrNum2); i++) {
	    if (StrNum2[i] == ',')
	        StrNum2[i] = '.';
	}
        i = sscanf(StrNum2, IP_IRIT_FLOAT_READ, RealNum);
	IritFree(StrNum2);
    }
    else
        i = sscanf(StrNum, IP_IRIT_FLOAT_READ, RealNum);
...
#	if defined(__WINNT__) && defined(_WIN64)
	    if ((Program = getenv("IRIT_DISPLAY64")) == NULL)
