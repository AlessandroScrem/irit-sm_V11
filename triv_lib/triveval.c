...
IRIT_STATIC_DATA CagdRType
    GlblEval3W = IRIT_INFNTY;
IRIT_STATIC_DATA const TrivTVStruct
    *GlblEval3TV = NULL;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   This function is the same as TrivTVEval/TrivTVEval2 above.               M
*   A preparation routine for TrivTVEval3.			             M
*   This function is geared toward massive evaluation fo the same TV, where  M
* u is changing the most, w the least.					     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To keep for massive evaluation using TrivTVEval3. This trivar  M
*             must be valid during all subsequent TrivTVEval3 calls.         M
*                                                                            *
* RETURN VALUE:                                                              M
*   void								     M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivTVEval, TrivTVEval2, TrivTVEval3                                     M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivTVEval3Prep, evaluation, trivariates                                 M
*****************************************************************************/
void TrivTVEval3Prep(const TrivTVStruct *TV)
{
    if (GlblEval3Srf != NULL)
	CagdSrfFree(GlblEval3Srf);
...
* DESCRIPTION:                                                               M
*   This function is the same as TrivTVEval/TrivTVEval2 above.               M
*   The evaluation function once TrivTVEval3Prep is called.	             M
*   This function is geared toward massive evaluation fo the same TV, where  M
* u is changing the most, w the least.					     M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivTVEval3, evaluation, trivariates                                     M
*****************************************************************************/
CagdRType *TrivTVEval3(CagdRType u,
		       CagdRType v,
		       CagdRType w)
{
    if (!TrivParamsInDomain(GlblEval3TV, u, v, w)) {
	TRIV_FATAL_ERROR(TRIV_ERR_WRONG_DOMAIN);
	return NULL;
    }

    if (!IRIT_APX_UEQ(GlblEval3W, w)) {
        if (GlblEval3Srf != NULL)
	    CagdSrfFree(GlblEval3Srf);

        GlblEval3W = w;
	GlblEval3Srf = TrivSrfFromTV(GlblEval3TV, w, TRIV_CONST_W_DIR, FALSE);
    }

    return CagdSrfEval(GlblEval3Srf, u, v);
}
	...
	*OrientN = NULL;
	...
	case TRIV_CONST_U_DIR:
		...
		for (i = IsNotRational; i <= MaxCoord; i++) {
	        ...
	        CagdRType
		    *BasisFuncs = BzrCrvEvalBasisFuncs(TV -> ULength, t);
	...
	case TRIV_CONST_V_DIR:
		...
		for (k = 0, i = IsNotRational; i <= MaxCoord; i++) {
	        ...
	        CagdRType
		    *BasisFuncs = BzrCrvEvalBasisFuncs(TV -> VLength, t);
	...
	case TRIV_CONST_W_DIR:
		...
		for (i = IsNotRational; i <= MaxCoord; i++) {
	        ...
	        CagdRType
		    *BasisFuncs = BzrCrvEvalBasisFuncs(TV -> WLength, t);
...
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivSrfFromTV, TrivBndryEdgesFromTV, TrivBndryCrnrsFromTV                M
*                                                                            *
* KEYWORDS:                                                                  M
...
{
    IRIT_STATIC_DATA CagdSrfStruct *Srfs[6];
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Extracts the twelve boundary curves of the (topological cube of) given   M
* tensor product trivariate.						     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To extract the twelve boundary curves from.                    M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdCrvStruct *:  A linked list of twelve curves, representing the       M
*                     twelve boundary edges of trivariate TV.                M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivSrfFromTV, TrivBndryEdgesFromTV, TrivBndryCrnrsFromTV                M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivBndryEdgesFromTV, trivariates                                        M
*****************************************************************************/
CagdCrvStruct *TrivBndryEdgesFromTV(const TrivTVStruct *TV)
{
    ...
    /* Extract the four bottom curves. */
    Srf = TrivSrfFromTV(TV, WMin, TRIV_CONST_W_DIR, TRUE);
    Crvs = CagdBndryCrvsFromSrf(Srf);
    for (i = 0; i < 4; i++)
        IRIT_LIST_PUSH(Crvs[i], AllCrvs);
    CagdSrfFree(Srf);

    /* Extract the four vertical curves. */
    Srf = TrivSrfFromTV(TV, VMin, TRIV_CONST_V_DIR, TRUE);
    Crv = CagdBndryCrvFromSrf(Srf, CAGD_U_MIN_BNDRY);
    IRIT_LIST_PUSH(Crv, AllCrvs);
    Crv = CagdBndryCrvFromSrf(Srf, CAGD_U_MAX_BNDRY);
    IRIT_LIST_PUSH(Crv, AllCrvs);
    CagdSrfFree(Srf);

    Srf = TrivSrfFromTV(TV, VMax, TRIV_CONST_V_DIR, TRUE);
    Crv = CagdBndryCrvFromSrf(Srf, CAGD_U_MIN_BNDRY);
    IRIT_LIST_PUSH(Crv, AllCrvs);
    Crv = CagdBndryCrvFromSrf(Srf, CAGD_U_MAX_BNDRY);
    IRIT_LIST_PUSH(Crv, AllCrvs);
    ...
    /* Extract the four top curves. */
    Srf = TrivSrfFromTV(TV, WMax, TRIV_CONST_W_DIR, TRUE);
    Crvs = CagdBndryCrvsFromSrf(Srf);
    for (i = 0; i < 4; i++)
        IRIT_LIST_PUSH(Crvs[i], AllCrvs);
    CagdSrfFree(Srf);

    return CagdListReverse(AllCrvs);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Extracts the eight boundary corners of the (topological cube of) given   M
* tensor product trivariate.						     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To extract the eight boundary corners from.                    M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdPtStruct *:  A linked list of eight corners of trivariate TV.        M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivSrfFromTV, TrivBndryEdgesFromTV, TrivBndrySrfsFromTV                 M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivBndryCrnrsFromTV, trivariates                                        M
*****************************************************************************/
CagdPtStruct *TrivBndryCrnrsFromTV(const TrivTVStruct *TV)
{
    int i;
    CagdRType UMin, UMax, VMin, VMax, WMin, WMax;
    CagdSrfStruct *Srfs[2];
    CagdPtStruct *Pt,
        *AllPts = NULL;
    ...
    /* Extract the four bottom curves. */
    Srfs[0] = TrivSrfFromTV(TV, WMin, TRIV_CONST_W_DIR, TRUE);
    Srfs[1] = TrivSrfFromTV(TV, WMax, TRIV_CONST_W_DIR, TRUE);

    for (i = 0; i < 2; i++) {
        CagdRType *R;

	Pt = CagdPtNew();
	R = CagdSrfEval(Srfs[i], UMin, VMin);
	CagdCoerceToE3(Pt -> Pt, &R, -1, Srfs[i] -> PType);
        IRIT_LIST_PUSH(Pt, AllPts);

	Pt = CagdPtNew();
	R = CagdSrfEval(Srfs[i], UMin, VMax);
	CagdCoerceToE3(Pt -> Pt, &R, -1, Srfs[i] -> PType);
        IRIT_LIST_PUSH(Pt, AllPts);

	Pt = CagdPtNew();
	R = CagdSrfEval(Srfs[i], UMax, VMin);
	CagdCoerceToE3(Pt -> Pt, &R, -1, Srfs[i] -> PType);
        IRIT_LIST_PUSH(Pt, AllPts);

	Pt = CagdPtNew();
	R = CagdSrfEval(Srfs[i], UMax, VMax);
	CagdCoerceToE3(Pt -> Pt, &R, -1, Srfs[i] -> PType);
        IRIT_LIST_PUSH(Pt, AllPts);
    }
