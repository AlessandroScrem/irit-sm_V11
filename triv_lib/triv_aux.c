				 ...
				 TrivTVDirType Dir)
{
    ...
    if (!IRIT_APX_EQ_EPS(t1, R1, EPS_ROUND_KNOT) || !OpenEnd) {
    ...
    if (IRIT_APX_EQ_EPS(t2, R2, EPS_ROUND_KNOT) && OpenEnd)
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Computes the volume enclosed in the given trivariate.			     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To compute its volume.		                             M
...
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdRType:  The computed volume (can be negative if TV reversed).        M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivTVVolume, bbox, bounding box                                         M
*****************************************************************************/
CagdRType TrivTVVolume(const TrivTVStruct *TV, CagdBType VolType)
{
    ...
    CagdRType
        Vol = 0.0;
    CagdSrfStruct
        **Bndries = TrivBndrySrfsFromTV(TV);

    for (i = 0; i < 6; i++) {
        Vol += VolType ? SymbSrfVolume1(Bndries[i])
		       : SymbSrfVolume2(Bndries[i]);
	CagdSrfFree(Bndries[i]);
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Preperation step for testing for point inclusions in a trivariate.  Must M
* be called before TrivTVPointInclusion is called.			     M
*   Optimized for many queries of point inclusions over this TV.	     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To make the necessary steps for point inclusion test.          M
*   n:        Sampling rate.  The large this number the better the accuracy  M
*             (near the boundaries) at the additional computational cost.    M
*               Roughly the number of samples of the TV in each parametric   M
*	      direction and can vary between ~10 to ~100.		     M
*                                                                            *
* RETURN VALUE:                                                              M
*   void			                                             M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivTVPointInclusion, TrivTVPointInclusionFree                           M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivTVPointInclusionPrep, point inclusion                                M
*****************************************************************************/
void TrivTVPointInclusionPrep(TrivTVStruct *TV, int n)
{
    ...
    for (i = 0; i < 6; i++) {
        if (i && 0x0) {
	    CagdSrfStruct
	        *TSrf = CagdSrfReverse(Bndries[i]);
	    
	    CagdPl = CagdSrf2PolygonsN(TSrf, n, n, FALSE, FALSE, FALSE);
	    CagdSrfFree(TSrf);
	}
	else
	    CagdPl = CagdSrf2PolygonsN(Bndries[i], n, n, FALSE, FALSE, FALSE);
	AllCagdPl = CagdListAppend(CagdPl, AllCagdPl);

	CagdSrfFree(Bndries[i]);
    }

    Pl = IPCagdPlgns2IritPlgns(AllCagdPl, FALSE);

    PlObj = IPGenPOLYObject(Pl);
    AttrSetObjAttrib(&TV -> Attr, "_TVPl", PlObj, FALSE);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Point inclusion test in a trivariate.  TrivTVPointInclusionPrep must be  M
* called before this function is invoked for a valid result.		     M
*   Optimized for many queries of point inclusions over this TV.	     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To compute point inclusion for.	                             M
*   Pt:       Point to test if inside TV or not.                             M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdBType:  TRUE if inside, FALSE otherwise.			     M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivTVPointInclusionPrep, TrivTVPointInclusionFree                       M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivTVPointInclusion, point inclusion                                    M
*****************************************************************************/
CagdBType TrivTVPointInclusion(TrivTVStruct *TV, const IrtPtType Pt)
{
    const IPObjectStruct
        *PlObj = AttrGetObjAttrib(TV -> Attr, "_TVPl");

    if (PlObj == NULL) {
        /* Do the preprocessing on the fly. */
        TrivTVPointInclusionPrep(TV, 30);
	if ((PlObj = AttrGetObjAttrib(TV -> Attr, "_TVPl")) == NULL)
	    return FALSE;
    }

    return GMPolygonPointInclusion3D(PlObj -> U.Pl, Pt);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Freeing step for testing for point inclusions in a trivariate.  Should   M
* be called after all TrivTVPointInclusion calls are done.		     M
*                                                                            *
* PARAMETERS:                                                                M
*   TV:       To free the auxiliary data used for TV point inclusion test.   M
*                                                                            *
* RETURN VALUE:                                                              M
*   void			                                             M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrivTVPointInclusion, TrivTVPointInclusionPrep                           M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrivTVPointInclusionFree, point inclusion                                M
*****************************************************************************/
void TrivTVPointInclusionFree(TrivTVStruct *TV)
{
    AttrFreeOneAttribute(&TV -> Attr, "_TVPl");
}
