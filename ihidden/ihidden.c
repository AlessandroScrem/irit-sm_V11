...
{    
	...
	*NewSrf = NULL,
	...
	AttrSetRefPtrAttrib(&Crv1 -> Attr, "_Uv", Crv);
					      ...
					      IHidCrvType CType)
{
	...
	*E3Crvs = NULL;
	...
	AttrSetRefPtrAttrib(&CrvE3 -> Attr, "_Uv", CrvUV);
				    ...
				    TrimIsoInterStruct *IsoInters)
{
	...
	*Crv = BspSrfCrvFromSrf(Srf, t, Dir);
	...
	AttrSetRefPtrAttrib(&Crv -> Attr, "_Uv", UVCrv1);
    }
...
static void UpdateAttributesAndDumpOut(CagdCrvStruct *Crvs, int OutStream)
{
	...
	int Hidden = AttrGetIntAttrib(Crv -> Attr, "hidden") == TRUE;
        IPObjectStruct
	    *OrigObj = (IPObjectStruct *) AttrGetRefPtrAttrib(Crv -> Attr,
							      "_OrigObj"),
