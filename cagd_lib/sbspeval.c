...
CagdCrvStruct *BspSrfC1DiscontCrvs(const CagdSrfStruct *Srf)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrC1DiscDirID = ATTRIB_NAME_BAD_NUMBER,
        AttrC1DiscValID = ATTRIB_NAME_BAD_NUMBER;
    int i, n;
    CagdRType *C1PotDisc;
    CagdCrvStruct *Crv,
	*C1DiscCrvs = NULL;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrC1DiscDirID, "C1DiscDir");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrC1DiscValID, "C1DiscVal");
	...
	for (i = 0; i < n; i++) {
		...
		AttrSetIntAttrib2(&Crv -> Attr, AttrC1DiscDirID,
				  CAGD_CONST_U_DIR);
		AttrSetRealAttrib2(&Crv -> Attr, AttrC1DiscValID,
				   C1PotDisc[i]);
