	...
	*PolyList = NULL;
	...
	for (i = 0; i < NumOfIsos; i++) {
	    ...
	    for (p = Poly; p != NULL; p = p -> Pnext)
	        AttrSetRealAttrib2(&p -> Attr, AttrUIsoParamID, u);
	...
	for (i = 0; i < NumOfIsos; i++) {
	    ...
	    Poly = BspCrv2Polyline(Crv, SamplesPerCurve, A, TRUE);
	    for (p = Poly; p != NULL; p = p -> Pnext)
	        AttrSetRealAttrib2(&p -> Attr, AttrVIsoParamID, v);
	    ((CagdPolylineStruct * ) CagdListLast(Poly)) -> Pnext = PolyList;
	    PolyList = Poly;
	    CagdCrvFree(Crv);
	}
			     ...
			     int NumOfIsocurves[2])
{
	...
	*CrvList = NULL;
	...
	Crv = BspSrfCrvFromSrf(Srf, u, CAGD_CONST_U_DIR);
	AttrSetRealAttrib2(&Crv -> Attr, AttrUIsoParamID, u);
	Crv -> Pnext = CrvList;
	CrvList = Crv;
    }
    if (IsoParams != NULL)
	...
	Crv = BspSrfCrvFromSrf(Srf, v, CAGD_CONST_V_DIR);
	AttrSetRealAttrib2(&Crv -> Attr, AttrVIsoParamID, v);
	Crv -> Pnext = CrvList;
	CrvList = Crv;
    }
    if (IsoParams != NULL)
