			   ...
			   int *FineNessV)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrUResID = ATTRIB_NAME_BAD_NUMBER,
        AttrVResID = ATTRIB_NAME_BAD_NUMBER;
    CagdRType t, RealFineNessU, RealFineNessV;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrUResID, "u_resolution");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrVResID, "v_resolution");
    ...
    t = AttrGetRealAttrib2(Srf -> Attr, AttrUResID);
    if (!IP_ATTR_IS_BAD_REAL(t))
        RealFineNessU *= t;
    t = AttrGetRealAttrib2(Srf -> Attr, AttrVResID);
    if (!IP_ATTR_IS_BAD_REAL(t))
        RealFineNessV *= t;
				     ...
				     int SamplesPerCurve)
{
    ...
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrUIsoParamID = ATTRIB_NAME_BAD_NUMBER,
        AttrVIsoParamID = ATTRIB_NAME_BAD_NUMBER;
    int i;
    CagdRType t;
    CagdCrvStruct *Crv;
	...
	*PolyList = NULL;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrUIsoParamID, "UIsoParam");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrVIsoParamID, "VIsoParam");
	...
	AttrSetRealAttrib2(&Poly -> Attr, AttrUIsoParamID, t);
	Poly -> Pnext = PolyList;
	PolyList = Poly;
	CagdCrvFree(Crv);
    }
	...
	Poly = BzrCrv2Polyline(Crv, SamplesPerCurve);
	AttrSetRealAttrib2(&Poly -> Attr, AttrVIsoParamID, t);
	Poly -> Pnext = PolyList;
	PolyList = Poly;
	CagdCrvFree(Crv);
    }
...
CagdCrvStruct *BzrSrf2Curves(const CagdSrfStruct *Srf, int NumOfIsocurves[2])
{
	...
	*CrvList = NULL;
	...
	AttrSetRealAttrib2(&Crv -> Attr, AttrUIsoParamID, t);
	Crv -> Pnext = CrvList;
	CrvList = Crv;
    }
	...
	AttrSetRealAttrib2(&Crv -> Attr, AttrVIsoParamID, t);
	Crv -> Pnext = CrvList;
	CrvList = Crv;
    }
