...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Examines if the given curve's control polygon is monotone in given Axis. M
*                                                                            *
* PARAMETERS:                                                                M
*   Crv:   To examine if monotone in axis Axis.                              M
*   Axis:  1 for X, 2 for Y, etc.                                            M
*   Eps:   For monotonicity epsilon test.  Zero for strictly monotone.       M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:   1 if increasingly monotone in axis Axis, -1 if decreasingly       M
*          monotone and 0 otherwise.			                     M
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdCrvIsCtlPolyMonotone                                                 M
*****************************************************************************/
int CagdCrvIsCtlPolyMonotone(const CagdCrvStruct *Crv, int Axis, CagdRType Eps)
{
    ...
    if (CAGD_IS_RATIONAL_CRV(Crv)) {
        Crv = RCrv = CagdCoerceCrvTo(Crv,
		       CAGD_MAKE_PT_TYPE(FALSE,
    ...
    /* Examine if increasingly monotone. */
    for (i = 1; i < Crv -> Length; i++) {
        if (Pts[i] < Pts[i - 1] && !IRIT_APX_EQ_EPS(Pts[i], Pts[i - 1], Eps))
	    break;
    }
    ...
    /* Examine if decreasingly monotone. */
    for (i = 1; i < Crv -> Length; i++) {
        if (Pts[i] > Pts[i - 1] && !IRIT_APX_EQ_EPS(Pts[i], Pts[i - 1], Eps))
	    break;
    }
...
*   EuclideanC1Discont:  TRUE to compute the C1 discontinuities and verify   M
*              them in the Euclidean space.  FALSE to only consider C1       M
*              discontinuities by knot multiplicity in parametric space.     M
*   Tolerance: Of parametric C1 discontinuity that is also a Euclidean       M
*              discontinuity - deviation from inner product of unit tangents M
*              before and after discontinuity by less than Tolerance (1.0 if M
*              tangent identical, 0.0 if orthogonal, -1.0 if opposite).      M
*                Ignored if < -1.0 or EuclideanC1Discont is FALSE.	     M
*                                                                            *
* RETURN VALUE:                                                              M
					   ...
					   IrtRType Tolerance)
{
	...
	*Pts = NULL;
    ...
    for (i = n - 1; i >= 0; i--) {
        CagdRType CosAngle;
        int IndxFirst = BspKnotLastIndexL(Crv -> KnotVector,
					  Crv -> Order + Crv -> Length,
					  C1Disconts[i]);

        if (!EuclideanC1Discont ||
	    (!BspCrvMeshC1Continuous(Crv, IndxFirst, &CosAngle) &&
	     Tolerance > CosAngle)) {
	    Pt = CagdPtNew();
