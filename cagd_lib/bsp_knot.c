...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Returns TRUE iff the given B-spline curve has open end conditions.	     M
*                                                                            *
* PARAMETERS:                                                                M
...
{
    return !Crv -> Periodic &&
	   BspKnotHasOpenEC(Crv -> KnotVector, Crv -> Length, Crv -> Order);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
* Returns TRUE iff the given B-spline surface has open end conditions.	     M
*                                                                            *
* PARAMETERS:                                                                M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Returns TRUE iff the given B-spline surface has open end conditions in the M
...
CagdBType BspSrfHasOpenECDir(const CagdSrfStruct *Srf, CagdSrfDirType Dir)
{
	...
	case CAGD_CONST_U_DIR:
	    return !Srf -> UPeriodic &&
		   BspKnotHasOpenEC(Srf -> UKnotVector, Srf -> ULength,
				    Srf -> UOrder);
	case CAGD_CONST_V_DIR:
	    return !Srf -> VPeriodic &&
		   BspKnotHasOpenEC(Srf -> VKnotVector, Srf -> VLength,
				    Srf -> VOrder);
	default:
...
CagdBType BspKnotHasOpenEC(const CagdRType *KnotVector, int Len, int Order)
{
	...
	default:
	    for (i = 0; ++i < Order; )
		    ...
		    return FALSE;
	    for (i = l; --i >= Len; )
	        if (!IRIT_APX_EQ_EPS(KnotVector[l], KnotVector[i], IRIT_UEPS))
		    return FALSE;
	    break;
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the multiplicity of given knot index Idx in KnotVector,         M
*                                                                            *
* PARAMETERS:                                                                M
*   KnotVector:    To compute the multiplicity of knot index Idx.            M
*   Len:           Of knot vector. This is not the length of the             M
*                  curve/surface using this KnotVector.                      M
*   Idx:           Index of knot to compute its multiplicity.                M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:       Multiplicity of the knot. At least one.                       M
*                                                                            *
* KEYWORDS:                                                                  M
*   BspKnotMultiplicity                                                      M
*****************************************************************************/
int BspKnotMultiplicity(const CagdRType *KnotVector, int Len, int Idx)
{
    ...
    for (i = Idx; IRIT_APX_EQ(KnotVector[i], KnotVector[Idx]) && --i >= 0; );
    Mult += Idx - i - 1;
    for (i = Idx; IRIT_APX_EQ(KnotVector[i], KnotVector[Idx]) && ++i < Len; );
    Mult += i - Idx - 1;
...
#ifdef __WINNT__
#if _MSC_VER >= 1800  /* Visual 2013 optimization bug. */
#pragma optimize( "", off )
#endif /* _MSC_VER >= 1800 */
#endif /* __WINNT__ */
...
#ifdef __WINNT__
#if _MSC_VER >= 1800  /* Visual 2013 - end optimization bug. */
#pragma optimize( "", on )
#endif /* _MSC_VER >= 1800 */
#endif /* __WINNT__ */
...
*                                                                            *
* SEE ALSO:                                                                  M
*   BspKnotTranslate, BspKnotAffineTrans, BspKnotAffineTrans2                M
*                                                                            *
* KEYWORDS:                                                                  M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Applies a translation transformation to the given knot vector. Note        M
* translation transformation on the knot vector does not change the Bspline  M
* curve.								     M
*   Knot vector is translated by Trans as KV[i] = KV[i] + Trans.             M
*   Translation is taken place in place.				     M
*                                                                            *
* PARAMETERS:                                                                M
*   KnotVector:     To translate.	                                     M
*   Len:            Of knot vector. This is not the length of the curve or   M
*                   surface using this knot vector.                          M
*   Trans:          Amount to translate the knot vector.                     M
*                                                                            *
* RETURN VALUE:                                                              M
*   void                                                                     M
*                                                                            *
* SEE ALSO:                                                                  M
*   BspKnotScale, BspKnotAffineTrans, BspKnotAffineTrans2                    M
*                                                                            *
* KEYWORDS:                                                                  M
*   BspKnotTranslate, knot vectors, affine transformation                    M
*****************************************************************************/
...
*   If a knot equal to t (up to IRIT_APX_EQ) already exists with	     M
* multiplicity i only (Mult - i) knot are being inserted into the new knot   M
* vector.								     M
		    ...
		    CagdRType t)
{
    ...
    for (LastIndex = FirstIndex;
	 LastIndex < Len + Order &&
	     IRIT_APX_EQ_EPS(KnotVector[LastIndex], t, IRIT_UEPS);
	 LastIndex++);
...
int BspKnotVerifyKVValidity(CagdRType *KV, int Order, int Len, CagdRType Tol)
{
    ...
    /* Make sure we do not have multiplicities more than the order. */
    KnotValues = (CagdRType *) IritMalloc((Len + Order) * sizeof(CagdRType));
    KnotMultiplicities = (int *) IritMalloc((Len + Order) * sizeof(int));
    i = BspKnotsMultiplicityVector(KV, Len + Order,
			           KnotValues, KnotMultiplicities);
    IritFree(KnotValues);
    while (--i >= 0) {
	if (KnotMultiplicities[i] > Order) {
	    IritFree(KnotMultiplicities);
	    return FALSE;
	}
    }
    IritFree(KnotMultiplicities);
