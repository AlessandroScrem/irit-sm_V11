...
*		  invocation of this function will overwrite the first.      M
*                                                                            *
* SEE ALSO:                                                                  M
			     ...
			     CagdRType v)
{
	...
	SrfEvalCache -> UIndexFirst = -1;
	SrfEvalCache -> VIndexFirst = -1;
	SrfEvalCache -> IsoSubCrv = NULL;
	SrfEvalCache -> VBasisFunc = NULL;
	SrfEvalCache -> v = IRIT_INFNTY;
    }
    else {
    ...
    if (SrfEvalCache -> v != v) {
	if (SrfEvalCache -> IsoSubCrv != NULL)
	    CagdCrvFree(SrfEvalCache -> IsoSubCrv);
	SrfEvalCache -> IsoSubCrv = BzrSrfCrvFromSrf(Srf, v, CAGD_CONST_V_DIR);

	SrfEvalCache -> v = v;
    }

    return BzrCrvEvalAtParam(SrfEvalCache -> IsoSubCrv, u);
}
	...
	*Crv = NULL;
	...
	case CAGD_CONST_U_DIR:
	    ...
	    for (i = IsNotRational; i <= MaxCoord; i++) {
	        CagdRType
		    *BasisFuncs = BzrCrvEvalBasisFuncs(VecLen, t);
