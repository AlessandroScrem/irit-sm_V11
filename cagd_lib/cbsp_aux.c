			 ...
			 int Mult)
{
    ...
    if (k == 2) {
        /* Do subdiv. directly as the Alpha matrix can be quite large if    */
        /* curve is a very long polyline.                                   */
        int Idx = BspKnotLastIndexLE(Crv -> KnotVector,
				     Crv -> Length + Crv -> Order, t);

	/* Copy the respective control points. */
	for (j = IsNotRational; j <= MaxCoord; j++) {
	    IRIT_GEN_COPY(LPoints[j], Points[j], sizeof(CagdRType) * LLength);
	    IRIT_GEN_COPY(RPoints[j], &Points[j][Idx - 1],
			  sizeof(CagdRType) * RLength);
	}

	if (!IRIT_APX_EQ(Crv -> KnotVector[Idx], t)) {
	    t = (t - Crv -> KnotVector[Idx]) /
	        (Crv -> KnotVector[Idx + 1] - Crv -> KnotVector[Idx]);

	    /* Needs to linearly blend point at the subdivision location. */
	    for (j = IsNotRational; j <= MaxCoord; j++) {
	        LPoints[j][LLength - 1] =
		    RPoints[j][0] = IRIT_BLEND(Points[j][Idx],
					       Points[j][Idx - 1], t);
	    }
	}
					     ...
					     CagdPolylineStruct **Params)
{
    ...
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrSaveParamValsID = ATTRIB_NAME_BAD_NUMBER;
    int PtsLen = CagdListLength(Pts);
    CagdBType
        SaveParamVals = Params != NULL &&
                        !IP_ATTR_IS_BAD_REAL(AttrGetRealAttrib2(Pts -> Attr,
						        AttrSaveParamValsID));

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrSaveParamValsID, "SaveParamVals");
	...
	for (Pt = Pts, i = 0; Pt != NULL; Pt = Pt -> Pnext, i++) {
		...
		(*Params) -> Polyline[i].Pt[0] =
		    AttrGetRealAttrib2(Pt -> Attr, AttrSaveParamValsID);
	    }
	}
