...
* interpolate, curve type and the parameterization type desired.	     M
*                                                                            *
* PARAMETERS:                                                                M
*   PtList:     List of point to interpolate.			             M
*   Order:      Order of interpolating curve.			             M
			  ...
			  CagdRType **RetKV)
{
    ...
    CagdCtlPtStruct PrdcLastPt, *PrdcPrevLastPt;

    if (Periodic) {
	/* Add the first point as last as well. */
	PrdcLastPt = *PtList;
	PrdcLastPt.Pnext = NULL;
	PrdcPrevLastPt = CagdListLast((CagdCtlPtStruct *) PtList);
	PrdcPrevLastPt -> Pnext = &PrdcLastPt;
	NumPts++;
    }
    else
        PrdcPrevLastPt = NULL;
    ...
    /* Normalize the domain of the parameters to be [0, 1].		    */
    /* However, if periodic, normalize to [0, (NumPts - 1) / NumPts].	    */
    assert(PtKnots[0] == 0.0);
    t = 1.0 / PtKnots[NumPts - 1];
    for (i = 1; i < NumPts; i++)
        PtKnots[i] *= t;

    /* Construct the knot vector of the B-spline curve. */
    if (Periodic) {
        int FirstIdx, LastIdx;
        CagdRType *KVTmp;

        PrdcPrevLastPt -> Pnext = NULL;    /* Remove last periodic dup. pt. */
	NumPts--;

	KVTmp = BspPtSamplesToKV(PtKnots, NumPts, Order, CrvSize);

        KV = (CagdRType *) IritMalloc(sizeof(CagdRType) *
				      (CrvPerSize + Order));

	/* Update the central portion. */
	CAGD_GEN_COPY(KV, KVTmp, sizeof(CagdRType) * (CrvSize + Order));
	FirstIdx = Order1;
	LastIdx = CrvSize + 1;
	KV[LastIdx] = 1.0;
	IritFree(KVTmp);

	/* Update periodic end-conditions in the beginning. */
	for (i = Order1 - 1; i >= 0; i--)
	    KV[i] = KV[i + 1] - 
	         (KV[LastIdx + i + 1 - Order1] - KV[LastIdx + i + 1 - Order]);
	/* Update periodic end-conditions in the end. */
	for (i = 1; i < Order1 + Order1; i++)
	    KV[LastIdx + i] = KV[LastIdx + i - 1] + (KV[i + 1] - KV[i]);

	/* Normalize the KV to be [0, 1]. */
	for (i = 0, t = KV[CrvPerSize]; i < CrvPerSize + Order; i++)
	    KV[i] /= t;
    }
...
#ifdef DEBUG_DUMP_PT_KNOTS
    fprintf(stderr, "PtKnots=");
    for (i = 0; i < NumPts; i++)
        fprintf(stderr, " %7.4f", PtKnots[i]);
    fprintf(stderr, "\nKnots=  ");
    for (i = 0; i < (Periodic ? CrvPerSize : CrvSize) + Order; i++)
        fprintf(stderr, " %7.4f", KV[i]);
    fprintf(stderr, "\n");
#endif /* DEBUG_DUMP_PT_KNOTS */
				 ...
				 CagdBType Periodic)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrDerivativeID = ATTRIB_NAME_BAD_NUMBER,
        AttrCtlPtID = ATTRIB_NAME_BAD_NUMBER,
        AttrSymmetryID = ATTRIB_NAME_BAD_NUMBER,
        AttrAreaID  = ATTRIB_NAME_BAD_NUMBER,
        AttrSymStateID = ATTRIB_NAME_BAD_NUMBER, 
        AttrAreaInnerProdsID = ATTRIB_NAME_BAD_NUMBER, 
        AttrAreaPtsID = ATTRIB_NAME_BAD_NUMBER;
    CagdBType
	...
	**AreaPts = NULL;
    ...
    IRIT_ZAP_MEM(Symmetry, sizeof(int) * 3);
    IRIT_ZAP_MEM(Area, sizeof(int) * 3);

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrDerivativeID, "Derivative");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrCtlPtID, "CtlPt");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrSymmetryID, "Symmetry");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAreaID, "Area");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrSymStateID, "SymState");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAreaInnerProdsID, "AreaInnerProds");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrAreaPtsID, "AreaPts");
    ...
    for (l = 0, R2 = Params, Pt = PtList;
	 l < NumPts;
	 l++, R2++, Pt = Pt -> Pnext) {
	if ((i = AttrGetIntAttrib2(Pt -> Attr,
				   AttrDerivativeID)) != IP_ATTR_BAD_INT) {
	    int Index;
	    CagdRType *Line;
	...
	else if ((i = AttrGetIntAttrib2(Pt -> Attr,
				        AttrCtlPtID)) != IP_ATTR_BAD_INT) {
		...
		MiddleCtlPtConstrained = TRUE;
				        ...
				        AttrSymmetryID)) != IP_ATTR_BAD_INT &&
		 (SymState = (CagdRType **) AttrGetRefPtrAttrib2(Pt -> Attr,
						    AttrSymStateID)) != NULL) {
	    ...
	    SymmetrySum = Symmetry[0] + Symmetry[1] + Symmetry[2] + CircSym;
				        ...
				        AttrAreaID)) != IP_ATTR_BAD_INT &&
		 (AreaInnerProds = (CagdRType **) AttrGetRefPtrAttrib2(Pt -> Attr,
					      AttrAreaInnerProdsID)) != NULL &&
		 (AreaPts = (CagdRType **) AttrGetRefPtrAttrib2(Pt -> Attr,
						     AttrAreaPtsID)) != NULL) {
	...
	for (i = StartAxis; i != EndAxis; i += StepAxis) {
	    ...
	    for (Pt = PtList, R = InterpPts, j = NumPts, NumCnstns = 0;
		 j > 0;
		 Pt = Pt -> Pnext, j--) {
	        if (AttrGetIntAttrib2(Pt -> Attr,
				      AttrSymmetryID) == IP_ATTR_BAD_INT &&
		    AttrGetIntAttrib2(Pt -> Attr,
				      AttrAreaID) == IP_ATTR_BAD_INT) {
		    *R++ = Pt -> Coords[i];
		    NumCnstns++;
		}
			       ...
			       CagdRType *Err)
{
	...
	PtType = Crv -> PType;
    CagdBType
        Rational = CAGD_IS_RATIONAL_CRV(Crv);
    int OldInterp,
	...
	*CtlPtList = NULL;
	...
	for (i = 0; i < Crv -> Length; i++) {
	    ...
	    for (j = !Rational; j <= MaxDimension; j++)
    ...
    if (NewCrv != NULL) {
        ...
        /* Make sure both curves are not periodic. */
	if (CAGD_IS_PERIODIC_CRV(NewCrv))
	    NC = CagdCnvrtPeriodic2FloatCrv(NewCrv);
	else
	    NC = CagdCrvCopy(NewCrv);  

	if (CAGD_IS_PERIODIC_CRV(Crv))
	    C = CagdCnvrtPeriodic2FloatCrv(Crv);
	else
	    C = CagdCrvCopy(Crv);  

        /* Match the domains. */
        BspKnotAffineTransOrder2(NC -> KnotVector, NC -> Order,
				 CAGD_CRV_PT_LST_LEN(NC) + NC -> Order,
				 TMin, TMax);

	/* Measure the error. */
	OldInterp = BspMultComputationMethod(BSP_MULT_BEZ_DECOMP);

	DCrv = SymbCrvSub(C, NC);
	CagdCrvFree(C);
	CagdCrvFree(NC);
...
{
    int i;
    CagdRType w, t, TMin, TMax, Dt, *R, **Points,
        Weights = 0.0;
    CagdPType LastPt;
    CagdPtStruct *Pt,
	*PtList = NULL;
	...
	w = IRIT_PT_PT_DIST(LastPt, Pt -> Pt);
	IRIT_PT_SCALE_AND_ADD(Loc, Loc, Pt -> Pt, w);
	Weights += w;


	IRIT_PT_COPY(LastPt, Pt -> Pt);
	IRIT_LIST_PUSH(Pt, PtList);
    }

    IRIT_PT_SCALE(Loc, 1.0 / Weights);
