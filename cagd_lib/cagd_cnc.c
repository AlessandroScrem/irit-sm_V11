				    ...
				    CagdBType RationalEllipses)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrConicID = ATTRIB_NAME_BAD_NUMBER;
    IrtVecType Trans;
    CagdRType **Points, 
	...
	*Crv = NULL;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrConicID, "conic");
    ...
    if (A1 * C1 > 0.0) {
	    ...
	    AttrSetStrAttrib2(&Crv -> Attr, AttrConicID, "Ellipse");
	}
	else {
	    CAGD_FATAL_ERROR(CAGD_ERR_INVALID_CONIC_COEF);
	    return NULL;
	...
	AttrSetStrAttrib2(&Crv -> Attr, AttrConicID, "Hyper");
    }
				     ...
				     CagdBType RationalEllipses)
{
	...
	*Crv = NULL;
	...
	Crv = CagdCnvrtPwr2BzrCrv(PwrCrv);
	AttrSetStrAttrib2(&Crv -> Attr, AttrConicID, "Parab");
