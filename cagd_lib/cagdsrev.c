...
CagdSrfStruct *CagdSurfaceRev(const CagdCrvStruct *CCrv)
{
	...
	*Srf = BspPeriodicSrfNew(9, Len, 3, CCrv -> Order,
    ...
    /* For each control point in the original curve - generate 9 points     */
    /* that form a circle perpendicular to the Z axis.		            */
			       ...
			       CagdRType EndAngle)
{
	...
	*Circ = PolyApprox ? BspCrvCreateUnitPCircle()
    ...
    TSrf = PolyApprox ? CagdSurfaceRevPolynomialApprox(Crv)
                      : CagdSurfaceRev(Crv);
				   ...
				   const CagdVType Axis)
{
    ...
    TCrv = CagdCrvMatTransform(Crv, InvMat); /* Bring crv to Z axis of rot. */
    Srf = CagdSurfaceRev2(TCrv, PolyApprox, StartAngle, EndAngle);
    CagdCrvFree(TCrv);
