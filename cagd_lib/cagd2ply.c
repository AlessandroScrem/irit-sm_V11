				     ...
				     VoidPtr AuxSrfData)
{
	...
	*AuxSrf2Data = NULL;
	...
	if (CagdSrfA2PGridProcessUV(A2PGrid)) {
	    /*   Traverse rectangle domains and convert to polys while      */
	    /* closing cracks.						    */
	    for (Rect = GlblRectList; Rect != NULL; Rect = Rect -> Pnext) {
		GlblPolys = (CagdPolygonStruct *) 
		    CagdListAppend(GlblPolys,
				   CagdSrfAdapRect2Polys(Srf, A2PGrid, Rect));
	    }
	}
				     ...
				     VoidPtr AuxSrfData)
{
    ...
    if (UMax - UMin < IRIT_EPS || VMax - VMin < IRIT_EPS) {
	IRIT_INFO_MSG("tessellation surface domain too small and is ignored.\n");
	return;
    }
...
#	ifdef DEBUG
	    ...
	    IRIT_SET_IF_DEBUG_ON_PARAMETER(_DebugCagd2PlyGridRes, TRUE) {
		    ...
		    UMax - UMin < IRIT_EPS ||
		    VMax - VMin < IRIT_EPS)
		    IRIT_INFO_MSG("tessellation max grid resolution or domain resolution reached.\n");
	    }
	}
#	endif /* DEBUG */
					  ...
					  const CagdSrfAdapRectStruct *Rect)
{
	...
	*Polys = NULL;
    ...
    while (ListLen-- > 3) {
	/* Look for 3 or more collinear vertices along a parametric line    */
	/* and cut the end corner of it, at the last collinear vertex.      */
	if (Has3ColinearPts) {
