...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Checks the matching with the tangents' inner product norm, < T1, T2 >.   *
*                                                                            *
* PARAMETERS:                                                                *
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*  Copies the two Src integer vectors to Dests.                              *
*                                                                            *
* PARAMETERS:                                                                *
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*  Find the angle between the two input vectors, in radians.                 *
*                                                                            *
* PARAMETERS:                                                                *
...
*                                                                            *
* RETURN VALUE:                                                              *
*  CagdRType: The angle between the two input vectors.                       *
*****************************************************************************/
static CagdRType AngleBetweenVectors(CagdVType Vector1, CagdVType Vector2)
{
    CagdRType
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*  Rotate SrcVec vector in Angle angles around Z.                            *
*                                                                            *
* PARAMETERS:                                                                *
			 ...
			 int NewIndex2[2 * CAGD_CM_MAX_SAMPLE_NUM])
{
     ...
     NewIndex1[*Cnt] = i - 1;
     NewIndex2[(*Cnt)++] = j - 1;
     NewIndex1[*Cnt] = NewIndex2[*Cnt] = -1;
									     }

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the distance norm to the matching, || P1 - P2 ||.               M
*                                                                            *
* PARAMETERS:                                                                M
*   T1: A pointer to unit tangent to the first curve at i-th point.          M
*   T2: A pointer to unit tangent to the second curve at j-th point.         M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the bisector norm to the matching.                              M
*                                                                            *
* PARAMETERS:                                                                M
*   T1: A pointer to unit tangent to the first curve at i-th point.          M
*   T2: A pointer to unit tangent to the second curve at j-th point.         M
...
*									     *
* KEYWORDS:                                                                  M
*   CagdMatchBisectorNorm, correspondence, matching                          M
*****************************************************************************/
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the default morphing norm to the matching, 1.0 - < T1, T2 >.    M
*                                                                            *
* PARAMETERS:                                                                M
*   T1: A pointer to unit tangent to the first curve at i-th point.          M
*   T2: A pointer to unit tangent to the second curve at j-th point.         M
...
*									     *
* KEYWORDS:                                                                  M
*   CagdMatchMorphNorm, correspondence, matching                             M
*****************************************************************************/
...
* < T1 x (P2 - P1), T2 x (P2 - P1) > must be non negative and then the norm  M
* is 1.0 - < T1, T2 >.                                                       M
*                                                                            *
* PARAMETERS:                                                                M
*   T1: A pointer to unit tangent to the first curve at i-th point.          M
...
* RETURN VALUE:                                                              M
*   CagdRType: -1 for no matching or the cost of the matching for the point  M
*		between zero and one, the minimum (non negative) the better. M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdMatchDistNorm, CagdMatchBisectorNorm, CagdMatchMorphNorm             M
*   CagdMatchingTwoCurves, CagdMatchRuledNorm				     M
*									     *
* KEYWORDS:                                                                  M
*   CagdMatchRuledNorm, correspondence, matching                             M
*****************************************************************************/
...
*   Computes the default ruled norm to the matching,			     M
* < T1 x (P2 - P1), T2 x (P2 - P1) > must be non negative and then the norm  M
* is 1.0 - MIN( || T1 x (P2 - P1) ||^2, || T2 x (P2 - P1) ||^2 )             M
*                                                         / || P2 - P1 ||^2. M
*                                                                            *
* PARAMETERS:                                                                M
*   T1: A pointer to unit tangent to the first curve at i-th point.          M
...
*									     *
* KEYWORDS:                                                                  M
*   CagdMatchRuled2Norm, correspondence, matching                            M
*****************************************************************************/
			      ...
			      const CagdVType P2)
{
    ...
    IRIT_PT2D_NORMALIZE(RuledVec);
    CrossVec1 = IRIT_CROSS_PROD_2D(T1, RuledVec);
    CrossVec2 = IRIT_CROSS_PROD_2D(T2, RuledVec);

    if (!GlblAllowNegativeNorm && CrossVec1 * CrossVec2 < -IRIT_UEPS)  
        return -1.0;
...
*   NewIndex1:  The vector of the new matching to the first curve.           *
*   NewIndex2:  The vector of the new matching to the second curve.          *
*   MinimizeMaxError:  TRUE for minimizing maximal error, FALSE to minimize  *
*               the error's sum over the entire domain.			     *
   MatchNormFunc: A pointer to the matching norm function.                   *
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdRType:   The matching cost over all the all sample point.            *
*****************************************************************************/
static CagdRType FindNewMatching(CagdVType TanVecCrv1[CAGD_CM_MAX_SAMPLE_NUM],
				 CagdVType TanVecCrv2[CAGD_CM_MAX_SAMPLE_NUM],
				 CagdVType Crv1Eval[CAGD_CM_MAX_SAMPLE_NUM],
				 ...
				 CagdMatchNormFuncType MatchNormFunc)
{
	...
	LinkStruct
	    *MatchVec = MatchMat[i - 1];
	    ...
	    if (MinimizeMaxError) {
		*MaxVec = IRIT_MAX(*DotProdVec, Min);
		DotProdVec++;
	    }
	    else
...
*               the error's sum over the entire domain.			     *
*   MatchNormFunc: A pointer to the matching norm function.                  *
*   ResultError:  Returns the error of resulting match, if found any.        *
*                                                                            *
* RETURN VALUE:                                                              *
*   int:   -1 for no matching between curves, or Shift idx with best result. *
*****************************************************************************/
static int FindBestMatching(CagdVType TanVecCrv1[CAGD_CM_MAX_SAMPLE_NUM],
			    CagdVType TanVecCrv2[CAGD_CM_MAX_SAMPLE_NUM],
			    CagdVType Crv1Eval[CAGD_CM_MAX_SAMPLE_NUM],
			    ...
			    CagdRType *ResultError)
{
	...
	TmpSum = FindNewMatching(TanVecCrv1,
				 ...
				 MinimizeMaxError, MatchNormFunc);
...
*   Order:  Of the reparameterization curves.                                *
*   Len:    The length of Vect and Vecr.                                     *
*   Reduce: The degree of freedom.                                           *
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdCrvStruct *:   The reparameterization curve.                         *
*****************************************************************************/
static CagdCrvStruct *CreateReparamCrv(int *Vect,
				       int *Vecr,
				       int Order,
				       ...
				       int Reduce)
{
	...
	*PtList = NULL;
    ...
    /* Interpolate a B-spline curve from the points. */
...
*   MinimizeMaxError:  TRUE for minimizing maximal error, FALSE to minimize  M
*               the error's sum over the entire domain.			     M
...
*                    Error of returned result is saved in an "_Error" attr.  M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdMatchDistNorm, CagdMatchBisectorNorm, CagdMatchMorphNorm             M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdMatchingTwoCurves, correspondence, matching                          M
*****************************************************************************/
				     ...
				     CagdMatchNormFuncType MatchNormFunc)
{
    ...
    CagdRType StartCrv2Par, StartCrv1Par, EndCrv2Par, EndCrv1Par, ShiftPar,
        ResultError, TangentsDotProdRes[CAGD_CM_MAX_SAMPLE_NUM];
    CagdCrvStruct *TCrv, *Crv2c, *NewParaCrv;

    Crv2c = CagdCrvCopy(Crv2);

    GlblNumberOfSamples =
        (SampleSet > CAGD_CM_MAX_SAMPLE_NUM) ? CAGD_CM_MAX_SAMPLE_NUM
					     : SampleSet;
    GlblRotationFlag = RotateFlag;
    GlblShiftFlag = CagdIsClosedCrv(Crv2c) ? GlblNumberOfSamples : 1;
    ...
    if (FindTangentsVec(Crv1, TanVecCrv1, Crv1Eval) == 0 ||
        FindTangentsVec(Crv2c, TanVecCrv2, Crv2Eval) == 0) {
	...
	CagdCrvDomain(Crv2c, &StartCrv2Par, &EndCrv2Par);
	ShiftPar = StartCrv2Par + ((CagdRType) (GlblNumberOfSamples - Shift))
					* (EndCrv2Par -  StartCrv2Par)
					/ ((CagdRType) GlblNumberOfSamples);
	H2Crv2 = CagdCrvRegionFromCrv(Crv2c, StartCrv2Par , ShiftPar);
	H1Crv2 = CagdCrvRegionFromCrv(Crv2c, ShiftPar, EndCrv2Par);
	CagdCrvFree(Crv2c);
	Crv2c = CagdMergeCrvCrv(H1Crv2, H2Crv2, TRUE);
	CagdCrvFree(H1Crv2);
	CagdCrvFree(H2Crv2);

	BspKnotAffineTrans2(Crv2c -> KnotVector,
			    Crv2c -> Length + Crv2c -> Order,  
	...
	AttrSetRealAttrib(&NewParaCrv -> Attr, "_Error", ResultError);

	return NewParaCrv;
    }
    else {
	TCrv = SymbComposeCrvCrv(Crv2c, NewParaCrv);
	CagdCrvFree(NewParaCrv);
	CagdCrvFree(Crv2c);

	AttrSetRealAttrib(&TCrv -> Attr, "_Error", ResultError);
