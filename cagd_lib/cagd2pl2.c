			    ...
			    CagdRType v)
{
	...
	if (!IRIT_APX_UEQ(A2PGrid -> UGridVec[UIndex] -> SrfPt.Uv[0], u))
	    return;
...
int CagdSrfA2PGridProcessUV(struct CagdA2PGridStruct *A2PGrid)
{
	...
	for (Pt = A2PGrid -> UGridVec[i], n = 0;
	     ...
	     Pt = Pt -> VPnext) {
	    PtsVec[n++] = Pt;
	    if (!IRIT_APX_UEQ(PtsVec[0] -> SrfPt.Uv[0], Pt -> SrfPt.Uv[0]) ||
	...
	for (j = 1; j < n; j++) {
	    ...
	    if (!IRIT_APX_UEQ(Pt -> SrfPt.Uv[0], Pt2 -> SrfPt.Uv[0])) {
	        IritFree(PtsVec);
	        return FALSE;
	    }
    ...
    /* Re-sort the same points, now along U, from the VGridVec.  Also eval. */
	...
	for (Pt = A2PGrid -> VGridVec[i], n = 0;
	     ...
	     Pt = Pt -> UPnext) {
	    PtsVec[n++] = Pt;
	    if (!IRIT_APX_UEQ(PtsVec[0] -> SrfPt.Uv[1], Pt -> SrfPt.Uv[1]) ||
		n >= 4 * (CAGD2PLY_MAX_SUBDIV_INDEX + 1)) {
	        IritFree(PtsVec);
	        return FALSE;
	    }
	...
	for (j = 0; j < n; j++) {
	    ...
	    if (Pt != NULL &&
		IRIT_APX_UEQ(Pt -> SrfPt.Uv[0], Pt2 -> SrfPt.Uv[0])) {
	        IritFree(PtsVec);
	        return FALSE;
	    }
