...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Returns a new curve, usually similar to the original but with one degree   M
* smaller.								     M
...
CagdCrvStruct *CagdCnvrtBsp2BzrCrv(const CagdCrvStruct *CCrv)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrBspDomainMinID = ATTRIB_NAME_BAD_NUMBER,
        AttrBspDomainMaxID = ATTRIB_NAME_BAD_NUMBER;
    int i, Order, Length;
    CagdRType LastT;
	...
	*BezierCrvs = NULL;
    ...
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrBspDomainMinID, "BspDomainMin");
    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrBspDomainMaxID, "BspDomainMax");
	...
	AttrSetRealAttrib2(&Crv -> Attr, AttrBspDomainMinID, TMin);
	AttrSetRealAttrib2(&Crv -> Attr, AttrBspDomainMaxID, TMax);
    }
