...
#define CAGD_POLE_SPLIT_OUTREACH	1e-4
				...
				CagdRType *AngularSpan)
{
	...
	*YPoints = Crv -> Points[2];
	...
	if ((R = IRIT_DOT_PROD_2D(V, Dir)) < 0.0)
	    return FALSE;
	else if (IRIT_APX_UEQ(R, 1.0))
	    R = 1.0;	     /* Filter precisely the exact same angle case. */
			       ...
			       CagdRType Eps)
{
	...
	CagdCrvTanAngularSpan(Crv2, Dir2, &Angle2)) {
        CagdBType
	    LineSegs = Crv1 -> Length == 2 &&
		       Crv2 -> Length == 2;
        CagdRType
	    d = IRIT_DOT_PROD_2D(Dir1, Dir2);

	/* A curve with two control points must be a line segment. */
	assert((Crv1 -> Length != 2 || IRIT_APX_UEQ(Angle1, 0.0)) &&
	       (Crv2 -> Length != 2 || IRIT_APX_UEQ(Angle2, 0.0)));

	if (LineSegs || acos(IRIT_FABS(d)) > Angle1 + Angle2) {
	    /* Tangent field's cones do not overlap - only one intersection */
	    /* occur between the two curves - try to find it numerically.   */
	    if (CagdCrvCrvInterNumer(Crv1, Crv2, LineSegs, Eps))
		return;
	}
...
* PARAMETERS:                                                                *
*   Crv1, Crv2:  Two curves to compute their intersection points.            *
*   LineSegs:    TRUE if both curves are straight lines.		     *
*   Eps:         Accuracy of computation.                                    *
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdBType:   TRUE if found intersection or no intersection can exist,    *
*                FALSE otherwise.				             *
*****************************************************************************/
static CagdBType CagdCrvCrvInterNumer(const CagdCrvStruct *Crv1,
				      const CagdCrvStruct *Crv2,
				      ...
				      CagdRType Eps)
{
    CagdRType t1, t2, TMin1, TMax1, TMin2, TMax2, Inter1Param, Inter2Param, *R,
	CrntDist = IRIT_INFNTY;
    CagdPType Pt1, Pt2, Inter1, Inter2;
    CagdVType Tan1, Tan2;
	...
	CagdCoerceToE2(Tan1, &R, -1, Crv1 -> PType);
	IRIT_PT2D_SUB(Tan1, Tan1, Pt1);

	R = CagdCrvEval(Crv2, TMin2);
	CagdCoerceToE2(Pt2, &R, -1, Crv2 -> PType);

	R = CagdCrvEval(Crv2, TMax2);
	CagdCoerceToE2(Tan2, &R, -1, Crv2 -> PType);
	IRIT_PT2D_SUB(Tan2, Tan2, Pt2);
	...
	if (GM2PointsFromLineLine(Pt1, Tan1, Pt2, Tan2,
	    ...
	    Inter2Param >= 0.0 &&
	    Inter2Param <= 1.0) {
	    /* Found an intersection points. */
	    t1 = Inter1Param * (TMax1 - TMin1) + TMin1;
	    t2 = Inter2Param * (TMax2 - TMin2) + TMin2;
...
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdCrvStruct *:  (Sub) curves as split due to cci's, or identical       M
					 ...
					 CagdRType Eps)
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrInterPtsID = ATTRIB_NAME_BAD_NUMBER;
    CagdCrvStruct *Crv1, *Crv2,
	*Crvs = CagdCrvCopyList(ArngCrvs);
    CagdPtStruct *IPts, *IPts1, *IPts2;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrInterPtsID, "InterPts");
    ...
    for (Crv1 = Crvs; Crv1 != NULL; Crv1 = Crv1 -> Pnext) {
		...
		AttrSetRefPtrAttrib2(&Crv1 -> Attr, AttrInterPtsID,
			CagdListAppend(IPts1,
				       AttrGetRefPtrAttrib2(Crv1 -> Attr,
							    AttrInterPtsID)));
		AttrSetRefPtrAttrib2(&Crv2 -> Attr, AttrInterPtsID,
			CagdListAppend(IPts2,
				       AttrGetRefPtrAttrib2(Crv2 -> Attr,
							    AttrInterPtsID)));
	    }
	}
    }
    ...
    if (SplitCrvs) {
	    ...
	    *CrvsSplitted = NULL;
	    ...
	    if ((IPts = (CagdPtStruct *) AttrGetRefPtrAttrib2(Crv1 -> Attr,
						   AttrInterPtsID)) != NULL) {
	        int Proximity;
