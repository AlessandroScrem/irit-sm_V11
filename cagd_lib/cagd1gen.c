...
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdAreClosedCrvs, CagdIsClosedSrf, CagdIsZeroLenCrv                     M
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdIsClosedCrv                                                          M
*****************************************************************************/
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Returns TRUE if the curves form a closed loop.                           M
*                                                                            *
* PARAMETERS:                                                                M
*   Crvs:       To test if form a closed loop.                               M
*   Srf:        If not NULL, Crvs are assumed in the parameteric space of    M
*               Srf, and crossing a boundary of Srf can still be valid loop. M
*               Otherwise, if NULL, curves end points are compared directly. M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdBType:    TRUE if closed, FALSE otherwise.                           M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdIsClosedCrv, CagdIsClosedSrf, CagdIsZeroLenCrv                       M
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdAreClosedCrvs                                                        M
*****************************************************************************/
...
{
    CagdRType *R;
    CagdPType P1, P2, FirstP, LastP;
    CagdCrvStruct *Crv;

    if (Crvs -> Pnext == NULL)
        return CagdIsClosedCrv(Crvs);

    /* Evaluate teh last point of the first curve. */
    if (CAGD_IS_BEZIER_CRV(Crvs)) {
        CagdCoerceToE3(FirstP, Crvs -> Points, 0, Crvs -> PType);
        CagdCoerceToE3(LastP, Crvs -> Points, Crvs -> Length - 1,
		       Crvs -> PType);
    }
    else if (CAGD_IS_BSPLINE_CRV(Crvs)) {
	if (CAGD_IS_PERIODIC_CRV(Crvs))
	    return FALSE;
	else {
	    if (!BspCrvHasOpenEC(Crvs)) {
		CagdCrvStruct
		    *TCrv = BspCrvOpenEnd(Crvs);

		CagdCoerceToE3(FirstP, TCrv -> Points, 0, TCrv -> PType);
		CagdCoerceToE3(LastP, TCrv -> Points, TCrv -> Length - 1,
			       TCrv -> PType);
    ...
    if (Srf != NULL) {
        R = CagdSrfEval(Srf, LastP[0], LastP[1]);
	CagdCoerceToE3(LastP, &R, -1, Srf -> PType);
        R = CagdSrfEval(Srf, FirstP[0], FirstP[1]);
	CagdCoerceToE3(FirstP, &R, -1, Srf -> PType);
    }

    /* Make sure all curves ends where next curve starts. */
    for (Crv = Crvs -> Pnext; Crv != NULL; Crv = Crv -> Pnext) {
        if (CAGD_IS_BEZIER_CRV(Crv)) {
	    CagdCoerceToE3(P1, Crv -> Points, 0, Crv -> PType);
	    CagdCoerceToE3(P2, Crv -> Points, Crv -> Length - 1, Crv -> PType);
	}
	...
	if (Srf != NULL) {
	    R = CagdSrfEval(Srf, P1[0], P1[1]);
	    CagdCoerceToE3(P1, &R, -1, Srf -> PType);
	    R = CagdSrfEval(Srf, P2[0], P2[1]);
	    CagdCoerceToE3(P2, &R, -1, Srf -> PType);
	}

	if (!IRIT_PT_APX_EQ(LastP, P1))
	    return FALSE;
	IRIT_PT_COPY(LastP, P2);
    }

    return IRIT_PT_APX_EQ(LastP, FirstP);
}
...
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdCtlMeshsSame, BspKnotVectorsSame, CagdSrfsSame, CagdCrvsSame2,	     M
		       ...
		       CagdRType Eps)
{
	...
	if ((Crv1 -> KnotVector != NULL && Crv2 -> KnotVector != NULL) &&
	    !BspKnotVectorsSame(Crv1 -> KnotVector, Crv2 -> KnotVector,
				CAGD_CRV_PT_LST_LEN(Crv1) + Crv1 -> Order,
				Eps))
	    return FALSE;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Compare the two curves for similarity, after bringing them to a common     M
* function space, by degree raising and refinment.                           M
*                                                                            *
* PARAMETERS:                                                                M
*   Crv1, Crv2:     The two curves to compare.                               M
*   Eps:            Tolerance of equality.		                     M
			...
			CagdRType Eps)
{
    CagdBType RetVal;
    CagdCrvStruct
	*Crv1t = CagdCrvCopy(Crv1),
        *Crv2t = CagdCrvCopy(Crv2);

    RetVal = CagdMakeCrvsCompatible(&Crv1t, &Crv2t, TRUE, TRUE) &&
             CagdCrvsSame(Crv1t, Crv2t, Eps);
...
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdBType:  TRUE if surfaces are the same, FALSE otherwise.              M
*                                                                            *
* SEE ALSO:                                                                  M
*   CagdCtlMeshsSame, BspKnotVectorsSame, CagdCrvsSame, CagdSrfsSame2        M
*   CagdCrvsSameUptoRigidScl2D, CagdSrfsSameUptoRigidScl2D		     M
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdSrfsSame                                                             M
		       ...
		       CagdRType Eps)
{
	...
	if ((Srf1 -> UKnotVector != NULL && Srf2 -> UKnotVector != NULL &&
	     !BspKnotVectorsSame(Srf1 -> UKnotVector, Srf2 -> UKnotVector,
				 CAGD_SRF_UPT_LST_LEN(Srf1) + Srf1 -> UOrder,
				 Eps)) ||
	    (Srf1 -> VKnotVector != NULL && Srf2 -> VKnotVector != NULL &&
	     !BspKnotVectorsSame(Srf1 -> VKnotVector, Srf2 -> VKnotVector,
				 CAGD_SRF_VPT_LST_LEN(Srf1) + Srf1 -> VOrder,
				 Eps)))
	    return FALSE;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Compare the two surfaces for similarity, after bringing them to a common   M
* function space, by degree raising and refinement.                          M
*                                                                            *
* PARAMETERS:                                                                M
*   Srf1, Srf2:     The two surfaces to compare.                             M
*   Eps:            Tolerance of equality.		                     M
*   Modified:	    0 if no surface was refined/degree raised,               M
*                   1 if Srf1 was refined/degree raised,                     M
*                   2 if Srf2 was refined/degree raised,                     M
*                   3 if both Srf1 and Srf2 were refined/degree raised.      M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdBType:  TRUE if surfaces are the same, FALSE otherwise.              M
*                                                                            *
...
*   CagdSrfsSameUptoRigidScl2D, CagdSrfsSameUptoRigidScl2D	             M
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdSrfsSame2                                                            M
*****************************************************************************/
			...
			int *Modified)
{
    CagdBType RetVal;
    CagdSrfStruct
	*Srf1t = CagdSrfCopy(Srf1),
        *Srf2t = CagdSrfCopy(Srf2);

    RetVal = CagdMakeSrfsCompatible(&Srf1t, &Srf2t, TRUE, TRUE, TRUE, TRUE) &&
             CagdSrfsSame(Srf1t, Srf2t, Eps);

    *Modified = (CagdSrfsSame(Srf1, Srf1t, Eps) ? 0 : 1) +
	        (CagdSrfsSame(Srf2, Srf2t, Eps) ? 0 : 2);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Resize the mesh length of the surface, in place. The new surface is not  M
* the same as the original while a minimal effort is invested to keep the    M
* surface similar.							     M
*                                                                            *
* PARAMETERS:                                                                M
*   Srf:        Surface to update its mesh length.                           M
*   NewLength:	New length to reallocate for the surface.                    M
*   Dir:	Direction to resize the mesh length, U or V.		     M
*                                                                            *
* RETURN VALUE:                                                              M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Checks if the given control point index (in the Points vector of control M
* points in CagdSrfStruct) is of a boundary control point.		     M
*                                                                            *
* PARAMETERS:                                                                M
*   Srf:        Surface to examine if PtIdx is of a boundary control point.  M
*   PtIdx:	Index into Points vectors to examine if of a boundary point. M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdSrfBndryType:    Boundary type, or CAGD_NO_BNDRY if interior.        M
*                                                                            *
* KEYWORDS:                                                                  M
*   CagdSrfIsPtIndexBoundary                                                 M
*****************************************************************************/
CagdSrfBndryType CagdSrfIsPtIndexBoundary(CagdSrfStruct *Srf, int PtIdx)
{
    int PtIdx1 = PtIdx / Srf -> ULength;

    if (PtIdx1 == 0)
        return CAGD_V_MIN_BNDRY;
    else if (PtIdx1 == Srf -> VLength - 1)
        return CAGD_V_MAX_BNDRY;
    else {
        int PtIdx2 = PtIdx - PtIdx1 * Srf -> ULength;

	if (PtIdx2 == 0)
	    return CAGD_U_MIN_BNDRY;
	else if (PtIdx2 == Srf -> ULength - 1)
	    return CAGD_U_MAX_BNDRY;
    }

    return CAGD_NO_BNDRY;
}
