...
typedef struct FastAllocStruct {
    ...
    unsigned int Offset;             /* Allocation size (aligned type size). */
    unsigned int Count;    /* Number of allocations made so far (statistic). */
    BlkStruct *CurrBlk;              /* Pointer to current block descriptor. */
    BlkStruct *FirstBlk;               /* Pointer to first block descriptor. */
    unsigned int TypeSize;            /* Copies of initialization variables. */
    unsigned int BlkSize;
    unsigned int AllgnBits;
    unsigned int Verbose;
} FastAllocStruct;

#define FLOOR2(n, p) ((n) & ~((((IritIntPtrSizeType) 1) << (p)) - 1))
#define CEIL2(n, p)  (FLOOR2(n - 1, p) + (((IritIntPtrSizeType) 1) << (p)))
                            ...
                            unsigned Verbose)
{
    ...
    Alloc -> Offset = (unsigned int) CEIL2(TypeSz, AllgnBits);
