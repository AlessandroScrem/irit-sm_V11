		      ...
		      IrtHmgnMatType ScrnMat)
{
    ...
    MatGenUnitMat(Matrices -> ViewMat);
    MatGenUnitMat(Matrices -> PrspMat);
    ...
    if (ViewMat) {
        IRIT_HMGN_MAT_COPY(Matrices -> ViewMat, ViewMat);
        MatMultTwo4by4(Matrices -> TransMat, Matrices -> TransMat, ViewMat);
    }
    if (PrspMat) {
        IRIT_HMGN_MAT_COPY(Matrices -> PrspMat, PrspMat);
    ...
    MatMultTwo4by4(Matrices -> TotalViewMat, Matrices -> TransMat,
		   Matrices -> ScreenMat);

    if (!MatInverseMatrix(Matrices -> TotalViewMat, Matrices -> ViewInvMat))
        _IRndrReportFatal(IRIT_EXP_STR("Error: non-invertable matrix.\n"));
