...
/* This was visibility map code. It was replaced by another algorithm. */
		    ...
		    /*   Therefore, XYZ holds the xyz values while x and y */
                    ...
                    /* In perspective mode, the rendering is done over the */
                    /* coordinates before the perspective and screen       */
                    /* transformation, therefore when I get the rendered   */
                    /* XYZ I need to apply on it the perspective and screen*/
                    /* transformations.                                    */
                    if (!Buffer -> Scene -> Matrices.ParallelProjection) {
                        XYZ[0] = (-Buffer -> Scene -> Matrices.TransMat[0][0] * Val.u / Val.z) *
                            Buffer -> Scene -> Matrices.ScreenMat[0][0] +
			    Buffer -> Scene -> Matrices.ScreenMat[3][0];
                        XYZ[1] = (-Buffer -> Scene -> Matrices.TransMat[1][1] * Val.v / Val.z) *
                            Buffer -> Scene -> Matrices.ScreenMat[1][1] +
			    Buffer -> Scene -> Matrices.ScreenMat[3][1];
                        XYZ[2] = -(Buffer -> Scene -> Matrices.TransMat[2][2] + 
                            Buffer -> Scene -> Matrices.TransMat[3][2]/Val.z) *
                            Buffer -> Scene -> Matrices.ScreenMat[2][2] +
			    Buffer -> Scene -> Matrices.ScreenMat[3][2];
                    }
                    else {
...
void ZBufferScanVMTri(IRndrZBufferStruct *Buffer,
                      ...
                      VoidPtr ClbkData)
{
    IrtHmgnMatType ViewMat, PrspMat, ScrnMat;

    /* In perspective mode the rendering is done over the coordinates before*/
    /* the perspective and screen transformation, therefore for each        */
    /* rendered pixel those transformations are done separately. For that   */
    /* we set the matrices to do only the perspective and screen            */
    /* transformation without the view transformations.                     */
    if (!Buffer -> Scene -> Matrices.ParallelProjection) {
        IRIT_HMGN_MAT_COPY(ViewMat, Buffer -> Scene -> Matrices.ViewMat);
        IRIT_HMGN_MAT_COPY(PrspMat, Buffer -> Scene -> Matrices.PrspMat);
        IRIT_HMGN_MAT_COPY(ScrnMat, Buffer -> Scene -> Matrices.ScreenMat);
        SceneSetMatrices(Buffer -> Scene, NULL, PrspMat, ScrnMat);
    }
    Buffer -> DoVisMapScan = TRUE;
    ZBufferScanTri(Buffer, Tri, ClbkData);
    Buffer -> DoVisMapScan = FALSE;
    if (!Buffer -> Scene -> Matrices.ParallelProjection) 
        SceneSetMatrices(Buffer -> Scene, ViewMat, PrspMat, ScrnMat);
}
                ...
                if (Validity != IRNDR_VISMAP_VALID_OK) {
                    ...
                    switch (Validity) {
                        ...
                        case IRNDR_VISMAP_VALID_DEGEN:
                            ...
                            break;
                        case IRNDR_VISMAP_VALID_NOT_RENDERED:
                            Color = &RNDR_VISMAP_NOT_RENDERED_COLOR;
                            break;
                        default:
                            Color = NULL;
                            assert(0);
