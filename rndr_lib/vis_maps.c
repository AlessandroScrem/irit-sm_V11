...
const IrtImgPixelStruct
    ...
    RNDR_VISMAP_DEGEN_COLOR        = { 255, 255, 255 }, /* White. If empty. */
    RNDR_VISMAP_NOT_RENDERED_COLOR = { 255, 0, 0 };     /* Red, it's mapped. */
...
*   UVBackfaceCulling: IN, use backface culling during the UV scan           M
*                      coversion.                                            M
*                                                                            *
* RETURN VALUE:                                                              M
                ...
                int UVBackfaceCulling)
{
    ...
    VisMap -> IsScanOnUV = TRUE;
    VisMap -> UVBackfaceCulling = UVBackfaceCulling;
    return 0;
}
...
void IRndrVMSetLimits(IRndrVMStruct *VisMap, IPObjectStruct* Objects)
{
    ...
    if (VisMap -> Scene -> Matrices.ParallelProjection)
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* The method gets a scanned triangle polygon and scans it into visibility    M
* map using the regular scan conversion                                      M
*                                                                            *
* PARAMETERS:                                                                M
*   VisMap:      IN,OUT, pointer to the visibility map.                      M
...
#endif
    /* If returns false, there is no UV value which means we are handling    */
    /* obstalces and therefore no need to scan on uvz space.                 */
    if (!VisMapCheckValidity(VisMap, Triangle, &RendTri -> Validity)) 
        return FALSE;

    /* I change the rendered coordinates to be the ones before the           */
    /* perspective and screen transformations.                               */
    /* Notice that this must be done before VisMapSwitchTriangleSpaces.      */
    if (!Scene -> Matrices.ParallelProjection) {
        for (v = Triangle -> PVertex, i = 0; v != NULL; v = v -> Pnext, i++) {
            IRIT_PT_COPY(Pt[i], v -> Coord);
            v -> Coord[RNDR_X_AXIS] = 
                AttrGetRealAttrib(v -> Attr, VIS_MAP_BEFORE_PRSP_X_ATTRIB);
            v -> Coord[RNDR_Y_AXIS] = 
                AttrGetRealAttrib(v -> Attr, VIS_MAP_BEFORE_PRSP_Y_ATTRIB);
            v -> Coord[RNDR_Z_AXIS] = 
                AttrGetRealAttrib(v -> Attr, VIS_MAP_BEFORE_PRSP_Z_ATTRIB);
        }
    }
...
void IRndrVMRelocatePtIntoTriangle(IPPolygonStruct *Triangle, IrtPtType Pt) 
{
    ...
    if (IRIT_PT_LENGTH(BestVector) < IRIT_PT_LENGTH(VectorFromCOM)) {
        ...
        /* Cases when original point is on the perimeter of the triangle.  */
        if (IRIT_APX_EQ(Scale, 0))
            Scale = 10*IRIT_EPS;
        ...
        for (j = x - k; j <= x + k; j++) {
            ...
            for (; z != NULL; z = z -> Next) {                
                ...
                if (IRndrVMIsPointInTriangle(z -> Triangle, Pt, TRUE, 
                                             &TriangleZ, NULL, NULL) &&
                    ((VisMap -> Scene -> Matrices.ParallelProjection &&  
                                            (TriangleZ > Pt[2] + IRIT_UEPS)) ||
                    (!VisMap -> Scene -> Matrices.ParallelProjection &&  
                                            (TriangleZ < Pt[2] - IRIT_UEPS)))) {
                    return TRUE;
                }
...
void IRndrVMScan(IRndrVMStruct *VisMap, IRndrZBufferStruct *Buff)
{
    ...
    for (v = 0; v < VisMap -> SizeV; ++v) {
        for (u = 0; u < VisMap -> SizeU; ++u) {
	        ...
	        /* This is old visibility map code. It was replaced by     */
	        /* another algorithm.					   */
#ifdef VM_USE_OLD_CODE 
                int x, y;
                IrtRType z, ZBufZ;
                ...
                Pixel -> Value = ((Total == 0) || 
                    (SumCovered + SumNotRendered > Total / 2)) ? 
                     ...
                     IPPolygonStruct *Triangle)
{
    ...
    /* If true then the current vertex should be left as it is without     */
    /* considering the Vertex -> Value itself.                             */
    GetCurrentVertex = 
        /* If the current is degenerated then any new value will better and*/
        /* will replace it.                                                */
        (Vertex -> Validity == IRNDR_VISMAP_VALID_DEGEN) ||
        /* If the current is not rendered then anything except degenerated */
        /* will be better and will replace it.                             */
        ((Vertex -> Validity == IRNDR_VISMAP_VALID_NOT_RENDERED) && 
         (Validity != IRNDR_VISMAP_VALID_DEGEN)) ||
        /* If the current is backfaced then anything except degenerated or */
        /* not rendered will be better and will replace it.                */
        ((Vertex -> BackFaced) && (Validity != IRNDR_VISMAP_VALID_DEGEN) &&
        (Validity != IRNDR_VISMAP_VALID_NOT_RENDERED));
...
*   Result:      OUT, the visibility values of the line. When super sampling M
*                is disabled 0 For not visible and 1 for visible. When super M
*                sampling is enable the values are in [0,1] and reflacts the M
...
*                undefined.                                                  M
*   Validity:    OUT, The validity of the pixel.                             M
*                                                                            *
* RETURN VALUE:                                                              M
    ...
    if (VisMap -> SuperSize == 1) {
        ...
        for (u = u0; u <= u1; ++u, ++Result, ++Validity) {
	        ...
	        *Result = 0;
                ...
                if (Count == 0)
                    *Result = -1; /*Result gets negative number to mark     */
                                  /*that the result is empty (the UV pixel  */
                                  /*isn't mapped at all).                   */
                                  /*If Count isn't 0 it means that Validity */
                                  /*isn't IRNDR_VISMAP_VALID_OK in which    */
                                  /*case the value of Result should be      */
                                  /*undefined.                              */
            }
        }
    }
...
*                        Transformations and scaling as element i in PObj.   M
*                        Objects which aren't surfaces, trimmed surfaces or  M
*                        polygons are ignored.                               M
*                                                                            *
* RETURN VALUE:                                                              M
*   int: FALSE if PObj doesn't contain any objects.                          M
*                                                                            *
* KEYWORDS:                                                                  M
                                   ...
                                   IPObjectStruct *PObj2)
{
    ...
    IPObjectStruct *PObj1, **Objects;

    /* 1. Calculates values for each object. */
    Length = IPObjListLen(PObj);
    if (Length == 0)
        return FALSE;
    Indexes = (int*) IritMalloc(Length * sizeof(int));
    UMin = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    UMax = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    VMin = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    VMax = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    SizeU = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    SizeV = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    Area = (IrtRType *) IritMalloc(Length * sizeof(IrtRType));
    Objects = (IPObjectStruct **) IritMalloc(Length *
					     sizeof(IPObjectStruct *));
    MinWidth = IRIT_INFNTY;
    MaxWidth = -IRIT_INFNTY;
    SumArea = 0;

    for (PObj1 = PObj, i = 0; PObj1 != NULL; PObj1 = PObj1 -> Pnext, i++) {
        IrtRType CurScale;
        GMBBBboxStruct *BBox;

        Indexes[i] = i;
        if (!IRndrVMGetObjDomain(PObj1, &UMin[i], &UMax[i], &VMin[i], 
				 &VMax[i])) {
	    Length--;
	    i--;
            if (Length == 0)
        ...
        Area[i] = GMPolyObjectArea(PObj1) +
                                IRIT_PT_PT_DIST_SQR(BBox -> Max, BBox -> Min);
        SumArea += Area[i];
        CurScale = sqrt(Area[i]/((UMax[i] - UMin[i])*(VMax[i] - VMin[i])));
        /* Here we give the uv domain the same area as the euclidean area.  */
        SizeU[i] = (UMax[i] - UMin[i])*CurScale;
        SizeV[i] = (VMax[i] - VMin[i])*CurScale;
        Objects[i] = PObj1;
    ...
    for (i = 0; i <= Length - 1; i++) {
        int Index = Indexes[i];
        int CurWidthPixels = 
                IRIT_REAL_TO_INT(SumPrevWidth + SizeU[Index]*ScaleWidth) -
                IRIT_REAL_TO_INT(SumPrevWidth),
            CurHeightPixels = 
                IRIT_REAL_TO_INT(SumPrevHeight + SizeV[Index]*ScaleHeight) -
        ...
        /* Next line. */
        if (SumPrevWidth +  SizeU[Index]*ScaleWidth - MapWidth > IRIT_EPS) {
            SumPrevHeight += MaxLineHeight;
            CurWidthPixels = IRIT_REAL_TO_INT(SizeU[Index]*ScaleWidth);
            CurHeightPixels =
                IRIT_REAL_TO_INT(SumPrevHeight + SizeV[Index]*ScaleHeight) 
                - IRIT_REAL_TO_INT(SumPrevHeight);
            SumPrevWidth = 0;
        ...
        /* If we aren't at the really end of width, we have right margins. */
        if (fabs(SumPrevWidth + SizeU[Index] *
		 ScaleWidth - MapWidth) > IRIT_EPS) {
            WidthMargins++;
        }
        if (SumPrevHeight != 0)
            HeightMargins++;
        ...
        /* If we aren't at the really end of width, we have right margins. */
        if (fabs((double)(IRIT_REAL_TO_INT(SumPrevHeight + 
            SizeV[Index]*ScaleHeight) - MapHeight)) > IRIT_EPS) {
            HeightMargins++;
        }
        ...
        Scale[0] = (CurWidthPixels - WidthMargins * MARGIN) /
	           (UMax[Index] - UMin[Index]);
        Scale[1] = (CurHeightPixels - HeightMargins * MARGIN) /
	           (VMax[Index] - VMin[Index]);

        /* Translate to origin and then translate to the begining of the    */
        /* calculated section (SumPrevWidth).                               */
        Trans[0] = -UMin[Index] * Scale[0] + 
            (IRIT_REAL_TO_INT(SumPrevWidth) + (SumPrevWidth == 0 ? 0 : MARGIN) 
            + IRIT_UEPS);
        Trans[1] = -VMin[Index] * Scale[1] +
                   (IRIT_REAL_TO_INT(SumPrevHeight) +
	                       (SumPrevHeight == 0 ? 0 : MARGIN) + IRIT_UEPS);

        /* Doing scaling and then translation. */
        GMAffineTransUVVals(Objects[Index], Scale, Trans);

        /* The list of objects in PObj2 will go through the same uv changes */
        /* as those in PObj. Here the object in PObj2 corresponding to the  */
        /* current object in PObj is going through the transformation. The  */
        /* concept here is that PObj2 has the same objects as in PObj but   */
        /* before triangulations. So the results are the original not       */
        /* triangulated objects but with the requried UV changes.           */
        if (PObj2 != NULL) {
            int j;
            IPObjectStruct *PObj3;
            IrtRType NewMin[2], NewMax[2], OldMin[2], OldMax[2];

            for (j = 0, PObj3 = PObj2; j < Index; PObj3 = PObj3 -> Pnext, j++);

            if (IP_IS_TRIMSRF_OBJ(PObj3)) {
                TrimSrfStruct *TrimSrf;

                CagdSrfDomain(PObj3 -> U.TrimSrfs -> Srf,
			      &OldMin[0],  &OldMax[0], &OldMin[1], &OldMax[1]);
                for (TrimSrf = PObj3 -> U.TrimSrfs; 
                    TrimSrf != NULL;
		    TrimSrf = TrimSrf -> Pnext) {
                    IrtRType Min[2], Max[2];

                    CagdSrfDomain(TrimSrf -> Srf,
				  &Min[0], &Max[0], &Min[1], &Max[1]);
                    RNDR_MAXM(OldMax[0], Max[0]);
                    RNDR_MAXM(OldMax[1], Max[1]);
                    RNDR_MINM(OldMin[0], Min[0]);
                    RNDR_MINM(OldMin[1], Min[1]);
                }
            }
            else if (IP_IS_SRF_OBJ(PObj3)) {
                CagdSrfStruct *Srf;

                CagdSrfDomain(PObj3 -> U.Srfs,
			      &OldMin[0], &OldMax[0], &OldMin[1], &OldMax[1]);
                for (Srf = PObj3 -> U.Srfs; Srf != NULL; Srf = Srf -> Pnext) {
                    IrtRType Min[2], Max[2];
            ...
            Trans[0] *= 1.0 / MapWidth;
            Trans[1] *= 1.0 / MapHeight;
            Scale[0] *= 1.0 / MapWidth;
            Scale[1] *= 1.0 / MapHeight;
            NewMin[0] = OldMin[0] * Scale[0] + Trans[0];
            NewMax[0] = NewMin[0] + (OldMax[0] - OldMin[0]) * Scale[0];
            NewMin[1] = OldMin[1] * Scale[1] + Trans[1];
            NewMax[1] = NewMin[1] + (OldMax[1] - OldMin[1]) * Scale[1];
            if (IP_IS_TRIMSRF_OBJ(PObj3)) {
                TrimSrfStruct **TrimSrf;
		     ...
		     *TrimSrf != NULL;
		     TrimSrf = &((*TrimSrf) -> Pnext)) {
                    ...
                    TrimSrfTemp = TrimAffineTransTrimSrf(*TrimSrf,
			         NewMin[0], NewMax[0], NewMin[1], NewMax[1]);
                    TrimSrfFree(*TrimSrf);
                    *TrimSrf = TrimSrfTemp;
                }
            }
                ...
                for (Srf = PObj3 -> U.Srfs; Srf != NULL; Srf = Srf -> Pnext) {
                    CagdSrfSetDomain(Srf, NewMin[0], NewMax[0],
				          NewMin[1], NewMax[1]);
                }
            }
            else
                GMAffineTransUVVals(PObj3, Scale, Trans);
        }
        /* Updating data as result of new section. */
        MaxLineHeight = IRIT_MAX(MaxLineHeight, SizeV[Index]*ScaleHeight);
        SumPrevWidth += SizeU[Index]*ScaleWidth;
    }
...
*   Obj:               IN, pointer to Object which contains a Triangle and   M
*                      stores various characteristics common to every        M
*                      polygon in the object.                                M
                             ...
                             int UVBackfaceCulling)
{
    ...
    BackFace = Scene -> BackFace;
    Scene -> BackFace = UVBackfaceCulling;
    retval = TriangleSet(RendTri, Triangle, Obj, Scene);
    Scene -> BackFace = BackFace;
...
*   Assumes that XY values weren't yet switched with UVZ in the triangle.    M
*                                                                            *
* PARAMETERS:                                                                M
*   VisMap:   IN, pointer to the visibility map.                             M
                               ...
                               IRndrVisibleValidityType *Validity)
{
    ...
    *Validity = IRNDR_VISMAP_VALID_OK;
    ...
    for (v = Triangle -> PVertex, i = 0;
	 ...
	 v = v -> Pnext, ++i) {
        float
            *TempUv = AttrGetUVAttrib(v -> Attr, VIS_MAP_UV_VALUES);
	...
	VisMapIsPoorAR(VisMap, uvyVec[0], uvyVec[1]) ) {
        ...
        *Validity = IRNDR_VISMAP_VALID_POOR_AR;
        ...
        /* Those commented lines doesn't seems reasonable,   */
...
*   uses VisMap -> CosTanAng, as cos of angle between triangle normal and    M
*   z axis.                                                                  M
*                                                                            *
* PARAMETERS:                                                                M
*   VisMap:   IN, pointer to the visibility map.                             M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Set whether to scan the following triangles on UV space or just on XYZ   M
* space.                                                                     M
*                                                                            *
* PARAMETERS:                                                                M
*   VisMap:        Pointer to the visibility map.                            M
*   IsScanOnUV:    TRUE in order to scan on XYZ and UV spaces. FALSE to scan M
*                  only on XYZ space.                                        M
*                                                                            *
* RETURN VALUE:                                                              M
*   void                                                                     M
