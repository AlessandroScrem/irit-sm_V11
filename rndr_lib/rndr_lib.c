...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Gets the view, perspective and screen matrices.                          M
*                                                                            *
* PARAMETERS:                                                                M
*   Rend:     IN,OUT, the rendering context.                                 M
*   ViewMat:  OUT, the view matrix. If NULL, it's ignored.                   M
*   PrspMat:  OUT, the perspective matrix. If NULL, it's ignored.            M
*   ScrnMat:  OUT, the mapping to the screen. If NULL, it's ignored.         M
*                                                                            *
* RETURN VALUE:                                                              M
*   void                                                                     M
*                                                                            *
* KEYWORDS:                                                                  M
*   IRndrGetViewPrsp, view, perspective, matrix context                      M
*****************************************************************************/
		      ...
		      IrtHmgnMatType ScrnMat)
{
    ...
    Matrices = SceneGetMatrices(&Rend -> Scene);
    if (ViewMat != NULL)
        IRIT_HMGN_MAT_COPY(ViewMat, Matrices -> ViewMat);
    if (PrspMat != NULL)
        IRIT_HMGN_MAT_COPY(PrspMat, Matrices -> PrspMat);
    if (ScrnMat != NULL)
        IRIT_HMGN_MAT_COPY(ScrnMat, Matrices -> ScreenMat);
}
...
void IRndrPutTriangle(IRndrPtrType Rend, IPPolygonStruct *Triangle)
{
    ...
    /* VisMap section: scan conversion of a triangle to visibility map.      */
    /* IRndrVMPutTriangle set xy values with uv values (vertex -> Coord[1,2] */
    /* are given the values vertex -> Attr => "uvvals", vertex -> Coord[1,2] */
    /* are backuped as attributes and restored later) and initializes        */
    /* triangle structrure.                                                  */
    ...
    /* value over the u and v values (for this purpose we set xy with the    */
    /* uv values). The xyz value for each uv value is stored in VisMap and   */
    /* also validity (IRndrVisibleValidityType) and fill value               */
	...
	(Rend -> ZBuf.VisMap -> IsScanOnUV) &&
        IRndrVMPutTriangle(&Rend -> VisMap, &Rend -> Tri,
			   &Rend -> Scene, &Rend -> Obj, Triangle)) {
        if ((!IsTriangleRendered)&&
            (Rend -> Tri.Validity == IRNDR_VISMAP_VALID_OK))
            Rend -> Tri.Validity = IRNDR_VISMAP_VALID_NOT_RENDERED;
...
{
    return IRndrVMPrepareUVValuesOfGeoObj(PObj, MapWidth, MapHeight, PObj2);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*   Wrapper function for setting whether to scan the following triangles on  M
* both UV space and XYZ space or only on XYZ space.                          M
*                                                                            *
* PARAMETERS:                                                                M
*   Rend:          The rendering context.                                    M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   IRndrVisMapSetScanOnUV                                                   M
*****************************************************************************/
void IRndrVisMapSetScanOnUV(IRndrPtrType Rend, int IsScanOnUV)
{
    IRndrVMSetScanOnUV(&Rend -> VisMap, IsScanOnUV);
}

/*****************************************************************************
* DESCRIPTION:                                                               M
*    Wrapper function for calculating the transformed vertex coordinate.     M
*                                                                            *
* PARAMETERS:                                                                M
*   Rend:         The rendering context.                                     M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   IRndrVertexTransform                                                     M
*****************************************************************************/
void IRndrVertexTransform(IRndrPtrType Rend,
                          IPVertexStruct *Vertex,                          
                          IrtRType *Result)
{
    VertexTransform(Vertex, &Rend -> Scene.Matrices, &Rend -> Obj, Result);
}
