                     ...
                     IrtRType *Result)
{
    ...
    /* VertexTransform is required to set Result to be with w == 1 and      */
    /* the result of MatMultPtby4by4 is indeed with w == 1.                 */
                ...
                IRndrSceneStruct *Scene)
{
    ...
    /* In visibility map while doing scan conversion on the UV space we don't*/
    /* exit on negative w since we need to mark the uv area of the triangle  */
    /* as mapped. In xyz scan coversion for the visibility map we do exit    */
    /* with 0 which signal us to set Tri -> Validity to                      */
    /* IRNDR_VISMAP_VALID_NOT_RENDERED which so this triangle will treated   */
    /* later as not rendered triangle.                                       */
    if (o -> DoVisMapCalcs == FALSE) {
        for (i = 0; i < 3; i++) {
            if (TVertex[i][RNDR_W_AXIS] < 0) {
                ...
                if (!PrintIt) {
                    _IRndrReportWarning(IRIT_EXP_STR("Negative w coordinate\n"));
                    PrintIt = TRUE;
                }
                return 0;
            }
	...
	IsPolyBackfaced(Poly, Viewer,
        ...
        /* In visibility map creation while scanning in UV space I need to   */
        /* know when it's backface so I can treat it as hidden, but for that */
        /* I also need to scan convert it so to mark all its pixels as       */
        /* hidden so I don't return here. I continue to the scan convertion. */
        if (o -> DoVisMapCalcs == FALSE) {
