...
MvarVoronoiCrvStruct *MvarVoronoiCrvNew(void)
{
    ...
    NewVoronoiCrv -> Type = MVAR_BSCT_NONE;
...
MvarVoronoiCrvStruct *MvarVoronoiCrvCopy(MvarVoronoiCrvStruct *Crv)
{
    ...
    if (Crv -> Type == MVAR_BSCT_CV_CV) {
        NewCrv -> F3 = CagdSrfCopy(Crv -> F3);
        NewCrv -> Type = MVAR_BSCT_CV_CV;
    }
    else {
        /* Snaity check.*/
        if (Crv -> Type != MVAR_BSCT_CV_PT)
            MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);

        NewCrv -> F3 = NULL;
        NewCrv -> Type = MVAR_BSCT_CV_PT;
...
{
    if (Cv -> Type == MVAR_BSCT_CV_CV) {
        MvarBsctCCCurveLeft(Cv, Res);
    }
    else {
        /* Sanity check.*/
        if (Cv -> Type != MVAR_BSCT_CV_PT)
            MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);
						 ...
						 MvarPtStruct **Points)
{
    ...
    /* Sanity check.*/
    if (!((Cv1 -> Type == MVAR_BSCT_CV_PT) && (Cv2 -> Type == MVAR_BSCT_CV_PT)))
        MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);
    ...
    CagdCrvFree(DistSq1W);
    CagdCrvFree(DistSq1X);
    CagdCrvFree(DistSq2W);
    CagdCrvFree(DistSq2X);
    ...
    MVPts = MvarMVsZeros0D(MVVec, NULL, 1, MvarBsctSubdivTol,
			   -IRIT_FABS(MvarBsctNumerTol));
    MvarMVFree(MVVec[0]);
						 ...
						 MvarPtStruct **Points)
{
    ...
    MVPts = MvarMVsZeros0D(MVVec, NULL, 2, MvarBsctSubdivTol,
			   -IRIT_FABS(MvarBsctNumerTol));
    MvarMVFree(MVVec[0]);
    MvarMVFree(MVVec[1]);
						 ...
						 MvarPtStruct **Points)
{
    ...
    if (!((Cv1 -> Type == MVAR_BSCT_CV_CV) && (Cv2 -> Type == MVAR_BSCT_CV_CV)))
        MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);
				      ...
				      MvarPtStruct **Points)
{
    ...
    else if (Cv1 -> Type == MVAR_BSCT_CV_PT) {
        /* Sanity check.*/
        if (Cv2 -> Type != MVAR_BSCT_CV_CV)
            MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);
        MvarBsctCPCCGetAllIntersectionPoints(Cv1, Cv2, Points);
    }
    else if (Cv2 -> Type == MVAR_BSCT_CV_PT) {
        /* Sanity check.*/
        if (Cv1 -> Type != MVAR_BSCT_CV_CV)
            MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);
