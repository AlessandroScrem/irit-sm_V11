...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Reverses the direction of the mesh of the given MV in direction Axis (and  M
* also reverse the knot vector if B-spline.				     M
*                                                                            *
* PARAMETERS:                                                                M
*   MV:        Multi-Variate to reverse.	                             M
*   Axis:      Direction to reverse.				             M
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarMVStruct *:   Reversed multi-variate.				     M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarPromoteMVToMV, MvarMVShiftAxes, MvarMVReverse			     M
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarMVReverseDir, multi-variates                                         M
*****************************************************************************/
MvarMVStruct *MvarMVReverseDir(const MvarMVStruct *MV, int Axis)
{
    CagdBType
	IsNotRational = !MVAR_IS_RATIONAL_MV(MV);
    int i, k, *Indices, Index,
        LengthAxis = MV -> Lengths[Axis],
        MaxCoord = MVAR_NUM_OF_MV_COORD(MV);
    CagdRType **RevPoints;
    MvarMVStruct *MVRev;
    ...
    /* Flip knot vector if has any. */
    if (MVAR_IS_BSPLINE_MV(MVRev)) {
        CagdRType
	    *KVRev = BspKnotReverse(MVRev -> KnotVectors[Axis],
				    MVAR_MVAR_ITH_PT_LST_LEN(MVRev, Axis) +
				        MVRev -> Orders[Axis]);

	IritFree(MVRev -> KnotVectors[Axis]);
	MVRev -> KnotVectors[Axis] = KVRev;
    }

    /* Flip the control points. */
    Indices = (int *) IritMalloc(sizeof(int) * MVRev -> Dim);
    IRIT_ZAP_MEM(Indices, sizeof(int) * MVRev -> Dim);

    Index = 0;
    RevPoints = MVRev -> Points;
    do {
	/* Swap the elements long this row in direction Axis. */
	for (k = 0; k < LengthAxis / 2; k++) {
	    int Idx1 = Index + k * MVAR_NEXT_DIM(MV, Axis), 
	        Idx2 = Index + (LengthAxis - k - 1) * MVAR_NEXT_DIM(MV, Axis);

	    for (i = IsNotRational; i <= MaxCoord; i++)
	        IRIT_SWAP(IrtRType, RevPoints[i][Idx1], RevPoints[i][Idx2]);
	}
    }
    while (MVAR_INC_SKIP_MESH_INDICES(MVRev, Indices, Axis, Index));
