...
IRIT_GLOBAL_DATA MvarMVStruct 
    *Param2BisectorMVs = NULL;

static MvarMVStruct **MvarCrvCrvBisector2DCreateMVs(CagdCrvStruct *Crv1,
						    CagdCrvStruct *Crv2, 
						    CagdRType *BBoxMin,
						    CagdRType *BBoxMax);
static const CagdRType *MvarCrvSrfBisectParam2Euclid(CagdRType *InParam, 
					             int Dim);
static const CagdRType *MvarSrfSrfBisectParam2Euclid(CagdRType *InParam, 
					             int Dim);
...
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarCrvSrfBisectorApprox2, bisectors                                     M
*****************************************************************************/
				  ...
				  CagdRType NumericTol)
{
	...
	*DMV1, *NrmlMV2, *Final, *MV1, *MV2;
    ...
    if (OutputType == 1) {
        ...
        AttrSetRefPtrAttrib(&Final -> Attr, "MVEuclid", P);
	return Final;
    }

    /* Compute the Zero set for Final. */
    Pts = MvarMVsZeros2DBy0D(&Final, &Constraint, 1, SubdivTol, NumericTol);
    MvarMVFree(Final);
...
*                    < Alpha(u, v, t) n(u, v), Alpha(u, v, t) n(u, v) > = 0  V
*									     M
* with simple algebraic manipulation, the following equivalent form is       M
* is obtained, and is a polynomial (not rational) function, assuming the     M
* the input is not rational:                                                 M
*                                                                            M
*  < S(u, v) - C(t), S(u, v) - C(t) > < n(u, v), T(t) > +                    V
*             2 < C(t) - S(u, v), T(t) > <S(u, v) - C(t), n(u, v) > = 0      V
*                                                                            M
*   Finding the zero set of the last equation provides the solution in       M
* (t, u, v) space, and the bisector point is given by:                       M
*                                                                            M
*           (x, y, z) = S(u, v) + Alpha(t, u, v) n(u, v)                     V
*                                                                            M
* which is implemented as a post-process mapping.                            M
*                                                                            M
* PARAMETERS:                                                                M
*   CMV1:	The univariate (curve) in R^3.                               M
*   CMV2:	The bivariate (surface) in R^3.                              M
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolyStruct *: An approximation of the bisector surface, either by    M
*		      polyline loops or triangles.                           M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarMVsBisector, MvarSrfSrfBisector, MvarCrvSrfBisector                  M
*   MvarSrfSrfBisectorApprox						     M
				         ...
				         CagdRType NumericTol)
{
    IRIT_STATIC_DATA MvarConstraintType
	Constraint = MVAR_CNSTRNT_ZERO;
    MvarMVStruct *MVTmp1, *MVTmp2, *MVTmp3, *MVTmp4, *MVTmp5,
	*DMV1, *NrmlMV2, *Final, *MV1, *MV2, *MVAlphaDenom, *MVAlphaEnum;
    MvarPolyStruct
	*Poly = NULL;
    MvarMapPrm2EucCallBackFuncType 
	PostProcessMap = NULL;

#ifndef MVBISECT_CRV_SRF_PARAM_SOL
    PostProcessMap = MvarCrvSrfBisectParam2Euclid;
#endif /* MVBISECT_CRV_SRF_PARAM_SOL */
    ...
    if (MVAR_IS_RATIONAL_MV(CMV1) || MVAR_IS_RATIONAL_MV(CMV2)) {
	MVAR_FATAL_ERROR(MVAR_ERR_GEOM_NO_SUPPORT);
	return NULL;
    }

    /* Bring both the curve and the surface to a (t, u, v) trivariate. */
    if (CMV1 -> Dim == 1 && CMV2 -> Dim == 2) {
	CagdRType Min, Max;
    ...
    /* Compute Alpha(t, u, v). */
    MVTmp1 = MvarMVSub(MV1, MV2);
    MVAlphaEnum = MvarMVDotProd(MVTmp1, DMV1);         /* Alpha's numerator. */
    MvarMVFree(MVTmp1);

    MVAlphaDenom = MvarMVDotProd(NrmlMV2, DMV1);     /* Alpha's denominator. */
    
    /* The mapping S(u, v) + Alpha(t, u, v) n(u, v) from the parameter space */
    /* (t,u,v) to the bisector points (x,y,z) is used in post-process:       */
    Param2BisectorMVs = MV2;
    MV2 -> Pnext = NrmlMV2;
    NrmlMV2 -> Pnext = MVAlphaEnum;
    MVAlphaEnum -> Pnext = MVAlphaDenom;

    /* Compute the final function, we seek its zero set. */
    MVTmp1 = MvarMVSub(MV2, MV1);
    MVTmp2 = MvarMVDotProd(MVTmp1, MVTmp1);
    MvarMVFree(MVTmp1);

    MVTmp1 = MvarMVMult(MVAlphaDenom, MVTmp2);
    MvarMVFree(MVTmp2);

    MVTmp2 = MvarMVSub(MV2, MV1);
    MVTmp5 = MvarMVDotProd(MVTmp2, NrmlMV2);
    MVTmp3 = MvarMVMultScalar(MVAlphaEnum, MVTmp5);
    MVTmp4 = MvarMVScalarScale(MVTmp3, 2.0);

    Final = MvarMVAdd(MVTmp1, MVTmp4);
    /* Compute the Zero set for Final. */
    Poly = MvarMVsZeros2D(&Final, &Constraint, 1, SubdivTol * 0.1, SubdivTol, 
	                  NumericTol, PostProcessMap);
    MvarMVFree(MVTmp1);
    MvarMVFree(MVTmp2);
    MvarMVFree(MVTmp3);
    MvarMVFree(MVTmp4);
    ...
    MvarMVFree(MVAlphaEnum);
    MvarMVFree(MVAlphaDenom);
    MvarMVFree(Final);
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolyStruct *:    Either a list of polyline loops or a list of        M
*			 triangles.					     M
*                                                                            *
* SEE ALSO:                                                                  M
					 ...
					 CagdRType NumericTol)
{
	...
	*NrmlMV1, *NrmlMV2, *AlphaNumer, *AlphaDenom, *Finals[2];
	...
	if (MV1 -> GType == MVAR_BSPLINE_TYPE) {
	    MvarMVDomain(MV1, &Min, &Max, 0);
	    BspKnotAffineTrans2(MV2 -> KnotVectors[0],
			        MV2 -> Lengths[0] + MV2 -> Orders[0], Min, Max);
	    MvarMVDomain(MV1, &Min, &Max, 1);
	    BspKnotAffineTrans2(MV2 -> KnotVectors[1],
				MV2 -> Lengths[1] + MV2 -> Orders[1], Min, Max);
	    MvarMVDomain(MV2, &Min, &Max, 2);
	    BspKnotAffineTrans2(MV1 -> KnotVectors[2],
				MV1 -> Lengths[2] + MV1 -> Orders[2], Min, Max);
	    MvarMVDomain(MV2, &Min, &Max, 3);
	    BspKnotAffineTrans2(MV1 -> KnotVectors[3],
				MV1 -> Lengths[3] + MV1 -> Orders[3], Min, Max);
	}
    }
    else {
	MVAR_FATAL_ERROR(MVAR_ERR_GEOM_NO_SUPPORT);
    ...
    /* The multivariates that will be used for mapping back the solution     */
    /* the parameter space to the Euclidean space are arranged in a list.    */
    Param2BisectorMVs = AlphaNumer; 
    Param2BisectorMVs -> Pnext = AlphaDenom;
    Param2BisectorMVs -> Pnext -> Pnext = MV1;
    Param2BisectorMVs -> Pnext -> Pnext -> Pnext = NrmlMV1;

    /* Compute the Zero set for Finals. */
    Poly = MvarMVsZeros2D(Finals, Constraints, 2, 0.1 * SubdivTol, SubdivTol,
			  NumericTol, MvarSrfSrfBisectParam2Euclid);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes an approximation to the bisector of two surfaces - old version  M
* that does not use the 2D solver.					     M
* Let S1(u, v) and S2(r, s) be two parametric surfaces and let n1(u, v) and  M
* n2(r, s) be their unnormalized normal fields.				     M
*   Becuase the two normals of the two surfaces must be coplanar we          M
* introduce the following constraint, forcing the three vectors n1, n2, and  M
...
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarSrfSrfBisectorApprox2, bisectors                                     M
*****************************************************************************/
				  ...
				  CagdRType NumericTol)
{
	...
	*NrmlMV1, *NrmlMV2, *AlphaNumer, *AlphaDenom, *Finals[2];
    ...
    /* Compute the Zero set for Finals. */
    Pts = MvarMVsZeros0D(Finals, Constraints, 2, SubdivTol, NumericTol);
...
*		      the bisector curve, or NULL if none.		     M
*									     *
* KEYWORDS:                                                                  M
				    ...
				    CagdBType SupportPrms)
{
	...
	**MVs = MvarCrvCrvBisector2DCreateMVs(Crv1, Crv2, BBoxMin, BBoxMax);
    MvarPolylineStruct
	*BisecPls = MvarMVsZeros1D(MVs, NULL, 3, Step, SubdivTol, NumericTol);
    IPPolygonStruct *Pl;
    IPObjectStruct *PBis;
    CagdCrvStruct
        *RetCrvs = NULL;
...
*									     *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *:  A (list of) polyline(s) that approximates	     M
...
{
    MvarMVStruct **MVs;
    MvarPolylineStruct *TrisecCrv;
    int i, Eqns;

    MVs = MvarTrisector3DCreateMVs(FF1, FF2, FF3, BBoxMin,
				   BBoxMax, &Eqns);
    TrisecCrv = MvarMVsZeros1D(MVs, NULL, Eqns, Step, SubdivTol, NumericTol);
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Mapping a single point from the parameter space of the curve-surface     *
* bisector problem to the Euclidean space. This callback function is invoked *
* by the post process phase of the equation solver.                          *
*                                                                            *
* PARAMETERS:                                                                *
*   InParam:	The solution point in the problem's parameter space.         *
*   Dim:	The dimension of InParam.			             *
*                                                                            *
* RETURN VALUE:                                                              *
*   CagdRType *:    The bisector point in Euclidean 3D space.                *
*****************************************************************************/
static const CagdRType *MvarCrvSrfBisectParam2Euclid(CagdRType *InParam, 
					             int Dim)
{
    IRIT_STATIC_DATA CagdRType OutParam[3];
    CagdRType AlphaVal, ValAlphaEnum, ValAlphaDenum, *Tmp, 
	*SrfPt = (CagdRType *) IritMalloc(3 * sizeof(CagdRType)), 
	*SrfNrml = (CagdRType *) IritMalloc(3 * sizeof(CagdRType));
    MvarMVStruct
	*MV2 = Param2BisectorMVs,
	*NrmlMV2 = Param2BisectorMVs -> Pnext, 
	*MVAlphaEnum = Param2BisectorMVs -> Pnext -> Pnext,
	*MVAlphaDenom = Param2BisectorMVs -> Pnext -> Pnext -> Pnext;

    assert(Dim == Param2BisectorMVs -> Dim);

    /* The mapping from the parameter space (t,u,v) to the bisector points   */ 
    /* (x,y,z) is given by S(u, v) + Alpha(t, u, v) n(u, v).                 */
    Tmp = MvarMVEval(MV2, InParam);
    CAGD_GEN_COPY(SrfPt, &(Tmp[1]), 3 * sizeof(CagdRType));

    Tmp = MvarMVEval(MVAlphaEnum, InParam);
    ValAlphaEnum = Tmp[1];

    Tmp = MvarMVEval(MVAlphaDenom, InParam);
    ValAlphaDenum = Tmp[1];

    if (IRIT_FABS(ValAlphaDenum) > IRIT_UEPS)
	AlphaVal = ValAlphaEnum / ValAlphaDenum;
    else 
	AlphaVal = IRIT_INFNTY;

    Tmp = MvarMVEval(NrmlMV2, InParam);
    CAGD_GEN_COPY(SrfNrml, &(Tmp[1]), 3 * sizeof(CagdRType));

    OutParam[0] = SrfPt[0] + AlphaVal * SrfNrml[0];
    OutParam[1] = SrfPt[1] + AlphaVal * SrfNrml[1];
    OutParam[2] = SrfPt[2] + AlphaVal * SrfNrml[2];
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Mapping a single point from the parameter space of the surface-surface   *
* bisector problem to the Euclidean space. This callback function is invoked *
* by the post process phase of the equation solver.                          *
*                                                                            *
* PARAMETERS:                                                                *
					             ...
					             int Dim)
{
    ...
    IrtVecType Nrml1E3;
    const MvarMVStruct
	*AlphaNumer = Param2BisectorMVs,
	*AlphaDenom = AlphaNumer -> Pnext, 
	*MV1 = AlphaDenom -> Pnext, 
	*NrmlMV1 = MV1 -> Pnext;

    assert(Dim == AlphaNumer -> Dim);
    R = MvarMVEval(MV1, InParam);
    CagdCoerceToE3(Pt1E3, &R, -1, (CagdPointType) MV1 -> PType);

    R = MvarMVEval(AlphaNumer, InParam);
    t1 = R[1];
    R = MvarMVEval(AlphaDenom, InParam);
    t1 /= R[1];

    R = MvarMVEval(NrmlMV1, InParam);
    CagdCoerceToE3(Nrml1E3, &R, -1, (CagdPointType) NrmlMV1 -> PType);

    IRIT_PT_SCALE(Nrml1E3, t1);
    IRIT_PT_ADD(Pt1E3, Pt1E3, Nrml1E3);

    IRIT_GEN_COPY(OutParam, Pt1E3, 3 * sizeof(CagdRType));
