			       ...
			       int *HasOrig)
{
    ...
    if ((MVGrad = AttrGetRefPtrAttrib(MV -> Attr, "Gradient")) != NULL) {
			   ...
			   MvarMVDirType Dir)
{
	...
	*Orders = MV -> Orders,
	...
	if (MV -> GType == MVAR_BSPLINE_TYPE)
	    ...
	    BasisFuncs = BzrCrvEvalBasisFuncs(MV -> Lengths[Dir], t);
			     ...
			     MvarMVDirType Dir)
{
	...
	*Lengths = MV -> Lengths,
	...
	for (i = 0; i < NewDim; i++) {
	    int l = i < Dir ? i : i + 1,
		KVLen = MVAR_MVAR_ITH_PT_LST_LEN(MV, l) + MV -> Orders[l];

	    if ((NewMV -> Periodic[i] = MV -> Periodic[l]) != FALSE) {
	        IritFree(NewMV -> KnotVectors[l]);
		NewMV -> KnotVectors[i] = IritMalloc(sizeof(CagdRType) * KVLen);
	    }

	    CAGD_GEN_COPY(NewMV -> KnotVectors[i], MV -> KnotVectors[l],
			  sizeof(CagdRType) * KVLen);
	}
    }
    else if (MVAR_IS_BEZIER_MV(MV)) {
	NewMV = MvarBzrMVNew(NewDim, NewLengths, MV -> PType);
    ...
    /* Original indices follow all indices except in Dir which follows       */
    /* only zero!							     */
    OldIndex = NewIndex = 0;
    do {
	for (i = IsNotRational; i <= MaxCoord; i++) {
