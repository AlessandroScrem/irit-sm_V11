...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Gets one aux. domain range in the given Direction. The Min/Max arrays    M
* are assumed to be of sufficiently large enough space to hold all           M
* dimensions, if Axis == -1.						     M
*                                                                            *
* PARAMETERS:                                                                M
			   ...
			   int Dir)
{
    ...
    if (Dir == -1) {
        for (i = 0; i < MV -> Dim; i++) {
	    Min[i] = MV -> AuxDomain[i][0];
	    Max[i] = MV -> AuxDomain[i][1];
	}
    }
    else {
...
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarMVExtension, MvarBzrMVRegionFromMV				     M
*                                                                            *
* KEYWORDS:                                                                  M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Changing the domain of a multivariate, in direction Dir, such that:      M
*   The values of the output multivariate over [0,1] in direction Dir, are   M
* those of the input multivariate over [NewMinDmn,NewMaxDmn].                M
*   If required, the user of this function should map back point x from      M
* [0,1] to [NewMinDmn,NewMaxDmn] by the affine domain change:		     M
* y[dir] = (1 - x[dir]) * NewMinDmn + x[dir] * NewMaxDmn.                    M
*   NOTE: when the new domain is always known to be contained in the old     M
* domain, if possible use MvarMVRegionFromMV, which suits B-splines as       M
* well and supports the same operation. This function is made for cases      M
* where a new region is not contained in [0,1].                              M
*									     *
* PARAMETERS:                                                                M
*   MV:  The bezier multivariate, values of which are required over a domain M
*	 other than [0,1].						     M
*   t1:	 The required min' domain.					     M
*   t2:	 The required max' domain.					     M
*   Dir: The direction to extract the domain.                                M
*									     *
* RETURN VALUE:                                                              M
*   MvarMVStruct *:    The new MV with the required new domain.	             M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarMVRegionFromMV, MvarMVExtension					     M
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarBzrMVRegionFromMV						     M
*****************************************************************************/
MvarMVStruct *MvarBzrMVRegionFromMV(const MvarMVStruct *MV,
				    ...
				    MvarMVDirType Dir)
{
    CagdRType Relative2ndDmnVal;
    MvarMVStruct *TmpMV1, *TmpMV2, *DummyMV, 
	*FinalMV = MvarMVCopy(MV);

#ifdef DEBUG
    /* Allow subdivision outside input domain only for Bezier. */
    assert(MV -> GType == MVAR_BEZIER_TYPE);
#endif /* DEBUG */

    if (IRIT_ABS(t2) > IRIT_EPS) {
	TmpMV1 = MvarMVSubdivAtParam(FinalMV, t2, Dir);
	MvarMVFree(TmpMV1 -> Pnext);
	/* The next subdivision param' is relative to [0,1] again. */
	Relative2ndDmnVal = t1 / t2;
	TmpMV2 = MvarMVSubdivAtParam(TmpMV1, Relative2ndDmnVal, Dir);
	MvarMVFree(TmpMV1);
	MvarMVFree(FinalMV);
	FinalMV = TmpMV2 -> Pnext;
	MvarMVFree(TmpMV2);
    }
    else { /* The new right end point is near zero. Different treatment. */
	DummyMV = MvarMVSubdivAtParam(FinalMV, t1, Dir);
	TmpMV1 = DummyMV -> Pnext;
	MvarMVFree(DummyMV);
	/* The next subdivision param' is relative to [0,1] again. */
	Relative2ndDmnVal = (t2 - t1) / (1.0 - t1);
	FinalMV = MvarMVSubdivAtParam(TmpMV1, Relative2ndDmnVal, Dir);
	MvarMVFree(TmpMV1);
	MvarMVFree(FinalMV -> Pnext);
    }
    FinalMV -> Pnext = NULL;
    return FinalMV;
}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Converts a B-spline multivariate into a B-spline multivariate with         M
* floating end conditions.                                                   M
*                                                                            *
* PARAMETERS:                                                                M
*   MV:       B-spline multivariate to convert to floating end conditions.   M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Converts a float B-spline multivariate to a B-spline multivariate with     M
* open end conditions.							     M
*                                                                            *
* PARAMETERS:                                                                M
*   MV:       B-spline multivariate to convert to open end conditions.       M
*                                                                            *
* RETURN VALUE:                                                              M
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Returns TRUE iff the given B-spline multivariate has open end conditions   M
...
CagdBType MvarBspMVIsOpenInDir(const MvarMVStruct *MV, MvarMVDirType Dir)
{
    ...
    return !MV -> Periodic[Dir] &&
	   BspKnotHasOpenEC(MV -> KnotVectors[Dir],
			    MV -> Lengths[Dir],
			    MV -> Orders[Dir]);
}
...
CagdBType MvarBspMVIsOpen(const MvarMVStruct *MV)
{
	...
	Open = (Open &&
	        !MV -> Periodic[i] &&
	        BspKnotHasOpenEC(MV -> KnotVectors[i], MV -> Lengths[i],
			         MV -> Orders[i]));
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Returns TRUE iff the given B-spline multivariate has periodic end          M
* conditions in the specified direction Dir.				     M
*                                                                            *
* PARAMETERS:                                                                M
