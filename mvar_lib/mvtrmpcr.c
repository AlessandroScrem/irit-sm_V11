				      ...
				      CagdRType Alpha) 
{
	...
	while (LeftPtCrvTmp != NULL) {
	    ...
	    MVPts2 = MvarMVsZeros0D(MVVec1, NULL, 1, MvarBsctSubdivTol, 
				    MVAR_BSCT_NUMER_TOL);
...
#       endif
	...
	if (CagdListLength(CCPtCrv) == 1) {
	    ...
	    UMin = UMin + 10 * MVAR_BSCT_NUMER_TOL;
	    UMax = UMax - 10 * MVAR_BSCT_NUMER_TOL;
	    TrimmedBis = CagdCrvRegionFromCrv(CCPtCrv, UMin, UMax);
	}
	else
				 ...
				 CagdCrvStruct *BaseCrv) 
{
	...
	TrimmedBis1 = MvarBsctTrimCrvPtPair(Crv1, Pt, 0.5);
	    ...
	    while (TrimmedBisCpy != NULL) {
	        CagdCrvStruct
		    *TrimmedBisCpy1 = CagdCrvCopy(TrimmedBisCpy);
...
#       endif
	...
	if (CagdListLength(TrimmedBis) > 1) {
	    CagdCrvStruct *TrimmedBisCpy, *TrimmedBisCpyOrig, *TmpCrv,
	        *TrimmedRLE = NULL; 
	    CagdRType UMin, UMax;
		...
		if ((IRIT_APX_EQ_EPS(Pt2[0], Pt1[0],
				     10 * MVAR_BSCT_NUMER_TOL)) &&
		    (IRIT_APX_EQ_EPS(Pt2[1], Pt1[1],
				     10 * MVAR_BSCT_NUMER_TOL))) {
		    RadialLE = CagdCrvCopyList(TrimmedBis);
		}
		else if ((IRIT_APX_EQ_EPS(Pt4[0], Pt3[0],
				     10 * MVAR_BSCT_NUMER_TOL)) && 
			 (IRIT_APX_EQ_EPS(Pt4[1], Pt3[1],
	    ...
	    TrimmedBisCpy = TrimmedBisCpyOrig = RadialLE;
...
#                   endif 
	    ...
	    CagdCrvFreeList(TrimmedBis);
	    CagdCrvFreeList(TrimmedBisCpyOrig);
	    ...
	    /* Given the Line is certainly a problem. It is not a vector  */
	    ...
	    /* which can be identified using the dot product of the two.  */
