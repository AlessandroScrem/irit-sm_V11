...
static int MvarExprTreeDomain(const MvarExprTreeStruct *ET,
			      CagdRType *Min,
			      CagdRType *Max,
			      int Axis);
static int MvarETUpdateConstDegDomainsAux(MvarExprTreeStruct *MVET,
					  CagdRType Min,
					  CagdRType Max,
					  int Dir);
					...
					const MvarBBoxStruct *MVBBox)
{
	...
	*ET = (MvarExprTreeStruct *) IritMalloc(sizeof(MvarExprTreeStruct));
	...
	ET -> MVBCone = MVBCone == NULL ? NULL
	                                : MvarNormalConeCopyList(MVBCone);
	ET -> IsRef = FALSE;
    }
    else {
	ET -> MV = MV;
				     ...
				     CagdBType DuplicateMVs)
{
	...
	*NewET = (MvarExprTreeStruct *) IritMalloc(sizeof(MvarExprTreeStruct));
	...
	NewET -> MVBCone = ET -> MVBCone != NULL ?
	                	MvarNormalConeCopyList(ET -> MVBCone):
				NULL;
...
void MvarExprTreeFreeSlots(MvarExprTreeStruct *ET, CagdBType ThisNodeOnly)
{
    ...
    if (ET -> MVBCone != NULL) {
	MvarNormalConeFreeList(ET -> MVBCone);
	ET -> MVBCone = NULL;
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Square of subexpression tree into a new expression tree.                 M
*                                                                            *
* PARAMETERS:                                                                M
*   Left:  The expression tree to take square of.  The sub tree              M
*	          is used in place, so duplicate before calling this         M
*	 	  function if you like to keep the original Left.            M
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarExprTreeStruct *:   Square expression tree, "Square(Left)".          M
*                                                                            *
* SEE ALSO:                                                                  M
*                                                                            M
*                                                                            *
...
MvarExprTreeStruct *MvarExprTreeSqr(MvarExprTreeStruct *Left)
{
    ...
    ET = MvarExprTreeIntrnlNew(MVAR_ET_NODE_SQR, Left, NULL, NULL);
    ET -> Val = 2;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Subexpression tree to a power of Power.		                     M
*                                                                            *
* PARAMETERS:                                                                M
*   Left:  The expression tree to to raise to power.  The sub tree           M
*	          is used in place, so duplicate before calling this         M
*	 	  function if you like to keep the original Left.            M
*   Power: The power to raise the expression to.			     M
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarExprTreeStruct *:   Square expression tree, "Left^power".            M
*                                                                            *
* SEE ALSO:                                                                  M
*                                                                            M
*                                                                            *
...
MvarExprTreeStruct *MvarExprTreeNPow(MvarExprTreeStruct *Left, int Power)
{
    ...
    ET = MvarExprTreeIntrnlNew(MVAR_ET_NODE_NPOW, Left, NULL, NULL);
    ET -> Val = Power;
	...
	case MVAR_ET_NODE_LEAF:
		...
		*Right = MvarExprTreeLeafNew(FALSE, MV2, MV2 -> Dim, 0,
		...
		if ((MVGrad = AttrGetRefPtrAttrib(MV -> Attr,
			...
			LGrad -> IsRational =
			    RGrad -> IsRational = MVGrad -> IsRational;
			LGrad -> HasOrig =
			    RGrad -> HasOrig = MVGrad -> HasOrig;
			...
			if (MVGrad -> MVGrad != NULL) {
			    ...
			    LGrad -> MVGrad =
			        MvarMVSubdivAtParam(MVGrad -> MVGrad, t, Dir);
...
const MvarBBoxStruct *MvarExprTreeBBox(MvarExprTreeStruct *ET)
{
	...
	case MVAR_ET_NODE_RECIP:
	    ...
	    for (i = 0; i < Dim; i++) {
			...
			break;
		    case MVAR_ET_NODE_SQRT:
		        assert(LBBox -> Max[i] >= 0.0);
			/* Allow errors in estimation. */
			if (LBBox -> Min[i] < 0.0)
			    B -> Min[i] = 0.0;
			else
		    ...
		    case MVAR_ET_NODE_SQR:
		        if (LBBox -> Min[i] > 0.0) {
			    B -> Min[i] = IRIT_SQR(LBBox -> Min[i]);
			    B -> Max[i] = IRIT_SQR(LBBox -> Max[i]);
			}
			else if (LBBox -> Max[i] < 0.0) {
			    B -> Min[i] = IRIT_SQR(LBBox -> Max[i]);
			    B -> Max[i] = IRIT_SQR(LBBox -> Min[i]);
			}
			else {
			    B -> Min[i] = 0.0;
			    B -> Max[i] = IRIT_MAX(IRIT_SQR(LBBox -> Min[i]),
						   IRIT_SQR(LBBox -> Max[i]));
			}
			break;
		    case MVAR_ET_NODE_NPOW:
		        if (ET -> Val & 1) {
			    B -> Min[i] = pow(LBBox -> Min[i], ET -> Val);
			    B -> Max[i] = pow(LBBox -> Max[i], ET -> Val);
			    break;
			}
			else {
			    if (LBBox -> Min[i] > 0.0) {
				...
				B -> Max[i] = pow(LBBox -> Max[i], ET -> Val);
			        ...
			        B -> Min[i] = pow(LBBox -> Max[i], ET -> Val);
				B -> Max[i] = pow(LBBox -> Min[i], ET -> Val);
			    }
			    else {
			        B -> Min[i] = 0.0;
				B -> Max[i] = IRIT_MAX(pow(LBBox -> Min[i],
							   ET -> Val),
						       pow(LBBox -> Max[i],
							   ET -> Val));
			    }
...
*   Axis:      The direction along with to compute the domain of ET, or -1   M
*              to evaluate the domain in all direction (Min and Max should   M
*              be vectors of size ET -> Dim).				     m
*                                                                            *
* RETURN VALUE:                                                              M
		 ...
		 int Axis)
{
	...
	int i;
        ...
        for (i = 0; i < ET -> Dim; i++) {
	    if (!MvarExprTreeDomain(ET, &Min[i], &Max[i], i))
	        return FALSE;
	}
	return TRUE;
    }
			      ...
			      int Axis)
{
	...
	case MVAR_ET_NODE_DOT_PROD:
	case MVAR_ET_NODE_CROSS_PROD:
	    IL = MvarExprTreeDomain(ET -> Left, Min, Max, Axis);
	    if (IL) {
#		ifdef DEBUG
	        CagdRType Min2, Max2;
	        int IR = MvarExprTreeDomain(ET -> Right, &Min2, &Max2, Axis);

		if (IR) { /* If we have both domains - verify the same. */
		    if (!IRIT_APX_EQ(*Min, Min2) ||
			!IRIT_APX_EQ(*Max, Max2)) {
		        assert(0);
		    }
		}
#		endif /* DEBUG */
		return TRUE;
	        ...
	        return MvarExprTreeDomain(ET -> Right, Min, Max, Axis);
	case MVAR_ET_NODE_EXP:
	case MVAR_ET_NODE_COS:
	case MVAR_ET_NODE_LOG:
	case MVAR_ET_NODE_SQRT:
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Computes the domain of the expression tree variable (leaf) along         M
*   direction Axis.                                                          M
*   The assumption is that any MV in ET with a non trivial degree in         M
* direction Axis possess the same domain.				     M
*                                                                            *
			   ...
			   int Axis)
{
    ...
    switch (ET -> NodeType) {
		...
		*Max = ET->MVBBox.Max[0];
        ...
        case MVAR_ET_NODE_DOT_PROD:
        case MVAR_ET_NODE_CROSS_PROD:
	    return MvarExprTreeLeafDomain(ET -> Left, Min, Max, Axis) ||
	           MvarExprTreeLeafDomain(ET -> Right, Min, Max, Axis);
        case MVAR_ET_NODE_EXP:
        case MVAR_ET_NODE_COS:
        case MVAR_ET_NODE_LOG:
        case MVAR_ET_NODE_SQRT:
        ...
        case MVAR_ET_NODE_NPOW:
        case MVAR_ET_NODE_RECIP:
	    /* Some func domain isn't trivial & rather hard to compute. */
	    return MvarExprTreeLeafDomain(ET -> Left, Min, Max, Axis);
        case MVAR_ET_NODE_COMMON_EXPR:
	    if ((ET = MVAR_EXPR_TREE_GET_COMMON_EXPR(ET)) != NULL)
	        return MvarExprTreeLeafDomain(ET, Min, Max, Axis);
	    return TRUE;
        default:
	    assert(0);
	    return FALSE;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Compute the shared domain of all given expression trees and update all   M
* expression trees leafs to that domain, in their constant directions.       M
*   Only constant degree directions can be updated.  Higher order degree of  M
* similar directions in two different (sub) expression trees must be sharing M
* the same domain (or otherwise an error will be returned).		     M
*                                                                            *
* PARAMETERS:                                                                M
*   MVETs:   vectors of expression tree MVs to update.                       M
*   n:       Size of vector MVETs.                                           M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:  TRUE if successful, FALSE otherwise.			             M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarMVUpdateConstDegDomains                                              M
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarETUpdateConstDegDomains                                              M
*****************************************************************************/
int MvarETUpdateConstDegDomains(MvarExprTreeStruct **MVETs, int n)
{
	...
	/* Find the domain in this l'th axis.  Note an ET can have improper */
	/* domain defined in some (constant) axis.			    */
	for (i = 0; i < n; i++)
	    if (MvarETDomain(MVETs[i], &Min, &Max, l))
	        break;

	for (i = 0; i < n; i++) {
	    if (!MvarETUpdateConstDegDomainsAux(MVETs[i], Min, Max, l))
		return FALSE;
	}
    }
    return TRUE;
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Traverses the given expression tree and update all leaf MVs to the       *
* [Min, Max] domain in the Dir direction.				     *
*   Only constrant order domains will be updated and if higher order domains *
* are detected with different domain size and error is returned.             *
*                                                                            *
* PARAMETERS:                                                                *
*   MVET:       An expression tree to parse and update is constant domains.  *
*   Min, Max:   New domain size to use for direction Dir.                    *
*   Dir:        Axis direction to update the domain.			     *
*                                                                            *
* RETURN VALUE:                                                              *
					  ...
					  int Dir)
{
    ...
    assert(Dir >= 0 && Dir < MVET -> Dim);

    switch (MVET -> NodeType) {
	case MVAR_ET_NODE_LEAF:
	    MV = MVET -> MV;
	    MvarMVDomain(MV, &Min2, &Max2, Dir);
	    if (!IRIT_APX_EQ(Min, Min2) || !IRIT_APX_EQ(Max, Max2)) {
		    ...
		    BspKnotAffineTransOrder2(MV -> KnotVectors[Dir],
					     MV -> Orders[Dir],
					     MV -> Orders[Dir] +
					         MV -> Lengths[Dir],
					     Min, Max);
	...
	case MVAR_ET_NODE_DOT_PROD:
	case MVAR_ET_NODE_CROSS_PROD:
	  return MvarETUpdateConstDegDomainsAux(MVET -> Right,
						Min, Max, Dir) &&
	         MvarETUpdateConstDegDomainsAux(MVET -> Left, Min, Max, Dir);
	case MVAR_ET_NODE_EXP:
	case MVAR_ET_NODE_LOG:
	case MVAR_ET_NODE_COS:
	case MVAR_ET_NODE_SQRT:
	...
	case MVAR_ET_NODE_NPOW:
	case MVAR_ET_NODE_RECIP:
	    return MvarETUpdateConstDegDomainsAux(MVET -> Left, Min, Max, Dir);
	case MVAR_ET_NODE_COMMON_EXPR:
	    break;
	default:
	    assert(0);
...
*   PrintMVInfo:    TRUE to print information on MV leafs.		     M
			   ...
			   MvarExprTreePrintFuncType PrintFunc)
{
	...
	case MVAR_ET_NODE_LEAF:
		...
		for (i = 0; i < MV -> Dim; i++)
		    sprintf(&Line[strlen(Line)], "%d%s",
			    MV -> Orders[i], i == MV -> Dim - 1 ? "}," : ",");
		PrintFunc(Line);

		sprintf(Line, "len{");
		for (i = 0; i < MV -> Dim; i++)
		    sprintf(&Line[strlen(Line)], "%d%s",
			    MV -> Lengths[i], i == MV -> Dim - 1 ? "}," : ",");
		PrintFunc(Line);
		sprintf(Line, "dmn{");
		for (i = 0; i < MV -> Dim; i++) {
		    CagdRType Min, Max;
		    ...
		    sprintf(&Line[strlen(Line)], "%.3f:%.3f%s",
			    Min, Max, i == MV -> Dim - 1 ? "}]" : ",");
		}
		PrintFunc(Line);
	    }
	...
	case MVAR_ET_NODE_CROSS_PROD:
	    PrintFunc("(");
	    MvarExprTreePrintInfo(ET -> Left, CommonExprIdx, PrintMVInfo,
				  PrintFunc);
	    ...
	    MvarExprTreePrintInfo(ET -> Right, CommonExprIdx, PrintMVInfo,
				  PrintFunc);
	    PrintFunc(")");
	    break;
	case MVAR_ET_NODE_EXP:
	...
	case MVAR_ET_NODE_COMMON_EXPR:
		...
		MvarExprTreePrintInfo(MVAR_EXPR_TREE_GET_COMMON_EXPR(ET),
				      CommonExprIdx, PrintMVInfo, PrintFunc);
	    PrintFunc("]");
	    break;
	default:
	    assert(0);
				...
				int *Dim)
{
	...
	case MVAR_ET_NODE_NPOW:
	case MVAR_ET_NODE_RECIP:
	    /* Fill with zeros so that gradient will automatically be       */
	    /* coerced to higher PtSize if needed.			    */
	    ...
	    *Dim = Dim1;
		...
		case MVAR_ET_NODE_RECIP:
		    ...
		    if (ET -> Right) {
		        CAGD_GEN_COPY(Pt2,
				      MvarExprTreeEval(ET -> Right, Params),
				      sizeof(CagdRType) *
				                  (ET -> Right -> PtSize + 1));
		    }
		    else {
			...
			case MVAR_ET_NODE_SQRT:
		        ...
		        case MVAR_ET_NODE_NPOW:
			    assert(ET -> Left -> PtSize == 1);
			    Pt1[1] = ET -> Val*pow(Pt1[1], ET -> Val - 1);
			    for (i = 0; i < Dim1; i++)
			        V[i] = V1[i] * Pt1[1];
			    break;
			case MVAR_ET_NODE_RECIP:
			...
			case MVAR_ET_NODE_CROSS_PROD:
		        ...
		        default:
			    assert(0);
			    IRIT_ZAP_MEM(V, sizeof(CagdRType) *
					                    MVAR_MAX_PT_COORD);
			    break;
		    }
		    break;
						 ...
						 MvarBBoxStruct *BBox)
{
	...
	case MVAR_ET_NODE_COS:
	    ...
	    break;
	case MVAR_ET_NODE_SQRT:
	    assert(ET -> MVBBox.Max[0] >= 0.0);
	    if (ET -> MVBBox.Min[0] < 0.0)     /* Allow errors in estimation */
		BBox -> Max[0] = IRIT_INFNTY;
	    else
		BBox -> Max[0] = 0.5 / sqrt(ET -> MVBBox.Min[0]);
	    BBox -> Min[0] = 0.5 / sqrt(ET -> MVBBox.Max[0]);
	    break;
	case MVAR_ET_NODE_SQR:
	    BBox -> Max[0] = 2.0 * ET -> MVBBox.Max[0];
	    BBox -> Min[0] = 2.0 * ET -> MVBBox.Min[0];
	    break;
	case MVAR_ET_NODE_NPOW:
	    if (ET -> Val & 1) {
		if (ET -> MVBBox.Min[0] >= 0.0) {
		    BBox -> Min[0] = pow(ET -> MVBBox.Min[0], Val-1) * Val;
		    BBox -> Max[0] = pow(ET -> MVBBox.Max[0], Val-1) * Val;
		}
		else if (ET -> MVBBox.Max[0] < 0.0) {
		    BBox -> Max[0] = pow(ET -> MVBBox.Min[0], Val-1) * Val;
		    BBox -> Min[0] = pow(ET -> MVBBox.Max[0], Val-1) * Val;
		}
		else {
		    BBox -> Min[0] = 0.0;
		    BBox -> Max[0] =
		        pow(IRIT_MAX(IRIT_FABS(ET -> MVBBox.Min[0]),
				     IRIT_FABS(ET -> MVBBox.Max[0])),
			    Val-1) * Val;
		}
	    }
	    else {
