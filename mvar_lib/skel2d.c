...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Sets the tolerances to be used in this module for the multivariate zero  M
* set solver.                                                                M
*                                                                            *
* PARAMETERS:                                                                M
						  ...
						  MvarSkel2DPrimStruct *Prim3)
{
	...
	*EquadistPts = NULL;
    ...
    MVPts = MvarCircTanTo3Crvs(Crv1, Crv2, Crv3,
			       Skel2DSubdivTol, Skel2DNumerTol, FALSE);

    /* Convert the computed points to our form. */
    for (MVPt = MVPts; MVPt != NULL; MVPt = MVPt -> Pnext) {
        CagdRType *R, d1, d2, d3, Radius;
	MvarSkel2DInter3PrimsStruct
	...
	if (GMCircleFrom3Points(EqPt -> EquadistPoint, &Radius,
			        EqPt -> PosPrim1, EqPt -> PosPrim2,
				EqPt -> PosPrim3)) {
	    /* Validate solutions - might get bogus answers if input        */
	    /* geometry intersect itself.				    */
	...
	if (EqPt != NULL) {
#	    define DEBUG_WARN_LOW_RES_SOL
#	    ifdef DEBUG_WARN_LOW_RES_SOL
		fprintf(stderr,
			"Low tri-tangent circle solution found and purged.\n");
#	    endif /* DEBUG_WARN_LOW_RES_SOL */
