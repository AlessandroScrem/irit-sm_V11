...
static CagdCrvStruct *SplitCrvAtDiscontinuity(CagdCrvStruct *Crv)
{
    ...
    if (CAGD_IS_BSPLINE_CRV(Crv) && BspCrvKnotC1Discont(Crv, &t))
...
IPObjectStruct *MvarComputeVoronoiCell(CagdCrvStruct *Crv)
{
    ...
    /* Check if only C0 is discontinuous. If yes, check for non-disc Ci's.   */
    ...
    /* Handling C1-discontinuous C0 points starts here.                      */
	...
	Tan1End = CagdVecCopy(CagdCrvTangent(SmoothCrvIter, UMax, TRUE)); 
	...
	if (CrossProd < -10 * IRIT_FABS(MVAR_BSCT_NUMER_TOL)) {
		...
		if (IsCiCrvDisc) 
		    MVAR_FATAL_ERROR(MVAR_ERR_GEOM_NO_SUPPORT);
		else {		       /* Check for end point discontinuity. */
					  ...
					  CagdCrvStruct *PtCrvBisector)
{
	...
	*PlHead = NULL;
    ...
    while (LEnvelopeTmp != NULL) {
        ...
        if (LEnvelopeTmp -> Type == MVAR_BSCT_CV_CV) {
       	    CagdSrfDomain(LEnvelopeTmp -> F3, &UMin, &UMax, &VMin, &VMax);
        }
        else {
            /* Sanity check. */
            if (LEnvelopeTmp -> Type != MVAR_BSCT_CV_PT)
                MVAR_FATAL_ERROR(MVAR_ERR_UNDEFINE_ERR);
