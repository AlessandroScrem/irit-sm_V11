...
static int MvarMVsZerosDelUMinusVCB(MvarZeroPrblmStruct *Problem, int Depth);
static void MVarMVAccumulate(MvarMVStruct **MVAccum, MvarMVStruct *MVNew);
static MvarPolylineStruct *MvarPtsMapAndAppend(MvarPolylineStruct *MVPllns,
					       CagdSrfStruct *BzrSrf1,
					       CagdSrfStruct *BzrSrf2,
					       MvarPolylineStruct *MVPllnsAll);
static MvarPolylineStruct *MvarBspSrfSelfInterPurgePt(MvarPolylineStruct *MVPls,
						      IrtRType U1,
						      IrtRType V1,
						      IrtRType U2,
						      IrtRType V2);
...
*                                                                            *
* PARAMETERS:                                                                *
*   problem:      The problem in hand we seek the zeros for.		     *
				     ...
				     CagdRType NumericTol)
{
    ...
    AntiPodalPoints = MvarMVsZeros0D(MVs, Constrs, 3, SubdivTol, NumericTol);
				     ...
				     CagdRType NumericTol)
{
	...
	*MVDsSrf, *MVDtSrf, *MVTemp, *MVTemp1, *MVTemp2, *MVs[5];
    ...
    AntiPodalPoints = MvarMVsZeros0D(MVs, Constrs, 5, SubdivTol, NumericTol);
    MvarMVFree(MVs[0]);
    MvarMVFree(MVs[1]);
    MvarMVFree(MVs[2]);
    MvarMVFree(MVs[3]);
				      ...
				      CagdRType MinNrmlDeviation)
{
    ...
    SelfInterPoints = MvarMVsZeros0D(MVs, Constrs, 3, SubdivTol, NumericTol);
					 ...
					 CagdRType NumericTol)
{
    ...
    MvarMVsZerosSubdivCallBackFuncType OldCallBack;
    ...
    SelfInterPoints = MvarMVsZeros0D(MVs, NULL, 2, SubdivTol, NumericTol);
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *:  Self intersection pllns, as points in E4          M
*							       (u, v, s, t). M
*                                                                            *
* SEE ALSO:                                                                  M
					    ...
					    CagdRType MinNrmlDeviation)
{
    ...
    MvarPolylineStruct *SelfInterPllns;
    ...
    SelfInterPllns = MvarMVsZeros1D(MVs, Constrs, 4, SubdivTol * 0.2,
    ...
    if (UMin != 0.0 || UMax != 1.0 || VMin != 0.0 || VMax != 1.0) {
        ...
        for (Pl = NULL; SelfInterPllns != NULL; Pl = Pl -> Pnext) {
	    MvarPtStruct *Pt;

	    /* Map solution back to original domain. */
	    for (Pt = Pl -> Pl; Pt != NULL; Pt = Pt -> Pnext) {
	        Pt -> Pt[0] = Pt -> Pt[0] * Du + UMin;
		Pt -> Pt[1] = Pt -> Pt[1] * Dv + VMin;
		Pt -> Pt[2] = Pt -> Pt[2] * Du + UMin;
		Pt -> Pt[3] = Pt -> Pt[3] * Dv + VMin;
...
*   Due to singularities in these equations, all six equations of the form:  M
*         Gx Hy - Gy Hx = 0,						     V
*         Gx Hz - Gz Hx = 0,						     V
*         Gy Hz - Gz Hy = 0,						     V
*         (u1 - u3) Gx + (u2 - u4) Hx = 0,				     V
...
*   Finally a 7th equations of the form "|| S - S ||^2 = 0 is added for      M
* faster pruning of far regions.					     M
*                                                                            *
* PARAMETERS:                                                                M
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *: List of self intersection pllns, as (u, v)         M
*		  parameter pairs into the surfaces' domain. Points in R^4.  M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarBzrSelfInter4VarDecomp                                               M
*                                                                            *
					          ...
					          CagdRType NumericTol)
{
	...
	*MVTmp1, *MVTmp2, *MVTmp3, *MVTmp4;
    ...
    MVPllns = MvarMVsZeros1D(MVs, Constraints, 6, SubdivTol * 0.2,
			     SubdivTol, NumericTol);
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *: List of intersection pllns, as (u, v) parameter    M
...
*                                                                            *
* PARAMETERS:                                                                *
*   MVPts:    List of pllns to map and merge.                                *
...
*                                                                            *
* RETURN VALUE:                                                              *
*   MvarPolylineStruct *:  Returned appended master list.                    *
*****************************************************************************/
static MvarPolylineStruct *MvarPtsMapAndAppend(MvarPolylineStruct *MVPllns,
					       CagdSrfStruct *BzrSrf1,
					       CagdSrfStruct *BzrSrf2,
					       MvarPolylineStruct *MVPllnsAll)
{
    ...
    for (MVPl = MVPllns -> Pl; MVPl != NULL; MVPl = MVPl -> Pnext) {
        MVPl -> Pt[0] = MVPl -> Pt[0] * DU1 + UMin1;
        MVPl -> Pt[1] = MVPl -> Pt[1] * DV1 + VMin1;
        MVPl -> Pt[2] = MVPl -> Pt[2] * DU2 + UMin2;
        MVPl -> Pt[3] = MVPl -> Pt[3] * DV2 + VMin2;
    }

    return CagdListAppend(MVPllns, MVPllnsAll);
}
...
*                                                                            *
* PARAMETERS:                                                                *
*   MVPts:           List of pllns to purge the given point from.            *
*   U1, V1, U2, V2:  The coordinate of the point to purge.                   *
*                                                                            *
* RETURN VALUE:                                                              *
*   MvarPolylineStruct *:  List of pllns, after the given point was purged.  *
*****************************************************************************/
static MvarPolylineStruct *MvarBspSrfSelfInterPurgePt(MvarPolylineStruct *MVPls,
						      IrtRType U1,
						      IrtRType V1,
						      ...
						      IrtRType V2)
{
    ...
    for (MVPl = MVPls; MVPl != NULL; MVPl = MVPl -> Pnext) {
        MvarPtStruct *MVPtNext,
	    *MVPt = MVPl -> Pl;

	if (IRIT_APX_EQ(MVPt -> Pt[0], U1) &&
	    IRIT_APX_EQ(MVPt -> Pt[1], V1) &&
	    IRIT_APX_EQ(MVPt -> Pt[2], U2) &&
	    IRIT_APX_EQ(MVPt -> Pt[3], V2)) {
	    MVPl -> Pl = MVPt -> Pnext;
	    MvarPtFree(MVPt);
	}
	else {
	    for (MVPtNext = MVPt -> Pnext;
		 MVPtNext != NULL;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Given a B-spline surface, S, compute its self intersections, if any, by  M
					          ...
					          CagdRType NumericTol)
{
	...
	/* Lets see if the surface is closed. */
	CagdSrfDomain(Srf, &UMin, &UMax, &VMin, &VMax);
	...
	while (Srfs != NULL) {
		...
		if (IRIT_APX_EQ(UMin1, UMin2) && IRIT_APX_EQ(UMax1, UMax2)) {
		    ...
		    if (IRIT_APX_EQ(VMax1, VMin2) ||/* TSrf1 V-below TSrf2. */
			(VClosed &&
			 IRIT_APX_EQ(VMax1, VMax) &&/* Same, for closed srf.*/
			 IRIT_APX_EQ(VMin2, VMin)))
		        MVPls = MvarAdjacentSrfSrfInter(TSrf1, TSrf2,
							CAGD_V_MAX_BNDRY,
							SubdivTol, NumericTol);
		    else if (IRIT_APX_EQ(VMax2, VMin1) ||/* TSrf1 V-above TSrf2. */
			     (VClosed &&
			      IRIT_APX_EQ(VMax2, VMax) &&/* Same, for closed srf. */
			      IRIT_APX_EQ(VMin1, VMin)))
		        MVPls = MvarAdjacentSrfSrfInter(TSrf1, TSrf2,
							CAGD_V_MIN_BNDRY,
							SubdivTol, NumericTol);
		    else {/* No common boundary curve between two surfaces. */
		        MVPls = MvarSrfSrfInter(TSrf1, TSrf2, SubdivTol * 0.1,
						SubdivTol, NumericTol);
		    }
		}
		else if (IRIT_APX_EQ(VMin1, VMin2) && IRIT_APX_EQ(VMax1, VMax2)) {
		    /* Surface share the same V Domain. */
		    if (IRIT_APX_EQ(UMax1, UMin2) ||/* TSrf1 U-below TSrf2. */
			(UClosed &&
			 IRIT_APX_EQ(UMax1, UMax) &&/* Same, for closed srf.*/
			 IRIT_APX_EQ(UMin2, UMin)))
		         MVPls = MvarAdjacentSrfSrfInter(TSrf1, TSrf2,
							 CAGD_U_MAX_BNDRY,
							 SubdivTol, NumericTol);
		    else if (IRIT_APX_EQ(UMax2, UMin1) ||/* TSrf1 U-above TSrf2. */
			     (UClosed &&
			      IRIT_APX_EQ(UMax2, UMax) &&/* Same, for closed srf. */
			...
			(UClosed &&
			 ...
			 IRIT_APX_EQ(UMax2, UMax))) {
			    ...
			    MVPls = MvarBspSrfSelfInterPurgePt(MVPls,
							       0, 0, 1, 1);
		...
		/* Points were computed in a Bezier space of [0, 1]^2.  We  */
		/* better map them to the B-spline domain of this patches   */
		MVPllnsAll = MvarPtsMapAndAppend(MVPls, TSrf1, TSrf2,
						 MVPllnsAll);
	    }
