...
#undef MvarPolylineFree
#undef MvarPolylineFreeList
#undef MvarVecFree
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *:    A new multi-variate polyline.	             M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarPolylineFree                                                         M
*                                                                            *
...
{
    MvarPolylineStruct
	*Poly = (MvarPolylineStruct *) IritMalloc(sizeof(MvarPolylineStruct));
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *:    Duplicated multi-variate polyline.              M
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarPolylineCopy			                                     M
*****************************************************************************/
...
{
    MvarPolylineStruct
	*NewPoly = MvarPolylineNew(MvarPtCopyList(Poly -> Pl));
...
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolylineStruct *:  Duplicated list of multi-variate polylines.       M
*                                                                            *
* KEYWORDS:                                                                  M
*   MvarPolylineCopyList, multi-variates                                     M
*****************************************************************************/
MvarPolylineStruct *MvarPolylineCopyList(MvarPolylineStruct *PolyList)
{
    MvarPolylineStruct *PolyTemp, *NewPolyList;

    if (PolyList == NULL)
	return NULL;
    PolyTemp = NewPolyList = MvarPolylineCopy(PolyList);
    PolyList = PolyList -> Pnext;
    while (PolyList) {
	PolyTemp -> Pnext = MvarPolylineCopy(PolyList);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
* Allocates the memory required for a new multi-variate poly container.      M
*                                                                            *
* PARAMETERS:                                                                M
*   PlList:      List of polylines. NULL if the required type is triangles.  M
*   TrList:      List of triangles. NULL if the required type is polylines.  M
*                                                                            *
* RETURN VALUE:                                                              M
*   MvarPolyStruct *:    A new (single) poly object, holding the list.	     M
*                                                                            *
* SEE ALSO:                                                                  M
*   MvarPolyFree                                                             M
*                                                                            *
...
#ifdef DEBUG
    assert(PlList == NULL || TrList == NULL);
#endif /* DEBUG */

    Poly -> Pnext = NULL;
    if (PlList != NULL) {
	Poly -> U.PllnList = PlList;
	Poly -> PlType = 0;
    }
    else {
	Poly -> U.TriList = TrList;
	Poly -> PlType = 1;
    }
    return Poly;
}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Deallocates and frees all slots of a multi-variate poly structure.	     M
*                                                                            *
* PARAMETERS:                                                                M
*   Poly:      Multivariate poly to free.                                    M
*                                                                            *
* RETURN VALUE:                                                              M
*   void                                                                     M
*                                                                            *
...
{
    if (Poly -> PlType == 0)
	MvarPolylineFreeList(Poly -> U.PllnList);
    else
	MvarTriangleFreeList(Poly -> U.TriList);
    IritFree(Poly);
}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Extension of a B-spline muiltivariate, in any of the 2 * Dim optional    M
* directions of the Dim dimensional domain. The extension is such that	     M
* the image coincides with the original image over the original domain.	     M
*   Assumes open end conditions (in all knot vectors).			     M
*									     *
* PARAMETERS:                                                                M
*   OrigMV:	The multivariate to be extended.			     M
*   ExtMins:	A vector of Dim Boolean values to set the extension          M
*               directions in the Min side. Direction i is extended in its   M
*		Min side if ExtMins[i] is TRUE, or if ExtMins is NULL.       M
*   ExtMaxs:	A vector of Dim Boolean values to set the extension          M
*               directions in the Max side. Direction i is extended in its   M
*		Max side if ExtMaxs[i] is TRUE, or if ExtMaxs is NULL. 	     M
*   Epsilons:   A vector of real numbers representing the length of the      M
*		extension in each direction (both Min and Max).		     M
*									     *
* RETURN VALUE:                                                              M
*   MvarMVStruct *:    The new extended MV.				     M
*                                                                            *
* SEE ALSO:                                                                  M
...
{
    CagdBType
	IsRational = MVAR_IS_RATIONAL_PT(OrigMV -> PType),
	AllMaxDirs = ExtMaxs == NULL,
	AllMinDirs = ExtMins == NULL;
    int i, d, Length, *Indices, Order, TmpInd,
	MaxAxis = MVAR_NUM_OF_PT_COORD(OrigMV -> PType),
	Dim = OrigMV -> Dim,
	Index = 0;
    MvarPointType 
	PType = OrigMV -> PType;
    MvarMVDirType Dir;
    MvarMVStruct *MV;
    CagdCrvStruct *IsoPrmCrv, *CTmp;

    if (!MVAR_IS_BSPLINE_MV(OrigMV)) {
	MVAR_FATAL_ERROR(MVAR_ERR_BSPLINE_EXPECTED);
	return NULL;
    }

    MV = MvarMVCopy(OrigMV);
    Indices = IritMalloc(sizeof(int) * OrigMV -> Dim); 
    
    for (Dir = 0; Dir < Dim; Dir++) {
	/* Update the params that are common to the current Dir, allocate   */
	/* once the curve to be temporarily used, since its size remains    */
	/* as long as we are in the same direction.                         */
	Length = OrigMV -> Lengths[Dir];
	Order = OrigMV -> Orders[Dir];
	IsoPrmCrv = BspCrvNew(Length, Order, PType);
	IRIT_ZAP_MEM(Indices, sizeof(int) * OrigMV -> Dim);

	do {
	    /* Update the ctrl pts of IsoPrmCrv. Beginning at Index,        */
	    /* traversing with TmpInd, copy from OrigMV to the curve:       */
	    for (d = !IsRational; d <= MaxAxis; d++) {
		TmpInd = Index;
		for (i = 0; i < Length; i++) {
		    IsoPrmCrv -> Points[d][i] = MV -> Points[d][TmpInd];
		    TmpInd += MVAR_NEXT_DIM(OrigMV, Dir);
		}
	    }

	    /* The knot vector in its non-extended version: */
	    CAGD_GEN_COPY(IsoPrmCrv -> KnotVector, OrigMV -> KnotVectors[Dir], 
			  sizeof(CagdRType) * (Length + Order));

	    /* Curve is ready, perform the extension: */
	    if (AllMinDirs || ExtMins[Dir]) {
		CTmp = BspCrvExtensionOneSide(IsoPrmCrv, TRUE,
					      Epsilons[Dir], TRUE);
		CagdCrvFree(IsoPrmCrv);
		IsoPrmCrv = CTmp;
	    }
	    if (AllMaxDirs || ExtMaxs[Dir]) {
		CTmp = BspCrvExtensionOneSide(IsoPrmCrv, FALSE,
					      Epsilons[Dir], TRUE);
		CagdCrvFree(IsoPrmCrv);
		IsoPrmCrv = CTmp;
	    }

	    /* Extension is done, copy to the new MV: */
	    for (d = !IsRational; d <= MaxAxis; d++) {
		TmpInd = Index;
		for (i = 0; i < Length; i++) {
		    MV -> Points[d][TmpInd] =  IsoPrmCrv -> Points[d][i];
		    TmpInd += MVAR_NEXT_DIM(OrigMV, Dir);
		}
	    }
	}
	...
	/* In the current Dir, mesh is ready. Update knot vector in MV and */
	/* free the curve, since it is not useful for other directions.    */
	CAGD_GEN_COPY(MV -> KnotVectors[Dir], IsoPrmCrv -> KnotVector, 
		      sizeof(CagdRType) * (Length + Order));
	CagdCrvFree(IsoPrmCrv);
    }
