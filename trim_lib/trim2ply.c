...
#define DEBUG_DUMP_TRIM_CRV_SEGS(TCrvs, Str, t, Dir) {	\
	TrimCrvStruct *TCrv; \
 \
	fprintf(stderr, "******* %s (divide at %f in Dir %d): ********\n", Str, t, Dir); \
	for (TCrv = TrimCrvs; TCrv != NULL; TCrv = TCrv -> Pnext) { \
	    TrimCrvSegStruct *TSegs; \
 \
	    for (TSegs = TCrv -> TrimCrvSegList; TSegs != NULL; TSegs = TSegs -> Pnext) \
		CagdDbg(CagdCrvCopy(TSegs -> UVCrv)); /* Mem leak but dumps only this curve. */ \
	} \
    }
...
*			     given trimming curves hierarchy.		     M
*                                                                            *
* SEE ALSO:                                                                  M
...
#   endif /* DEBUG */
	...
	if ((SubLoops = (TrimCrvStruct *)
	       AttrGetRefPtrAttrib(LoopVec[l] -> Attr, "_subTrims")) != NULL) {
	    LoopVec[i] -> Pnext = SubLoops;
	}
	AttrSetRefPtrAttrib(&LoopVec[l] -> Attr, "_subTrims", LoopVec[i]);
    }
