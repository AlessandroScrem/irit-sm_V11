...
TrimCrvStruct *TrimCrvCopy(const TrimCrvStruct *TrimCrv)
{
	...
	*NewTrimCrv = (TrimCrvStruct *)
    ...
    NewTrimCrv -> Attr = IP_ATTR_COPY_ATTRS(TrimCrv -> Attr);
...
*                                                                            *
* PARAMETERS:                                                                M
*   Srf:             Surface to make into a trimmed surface. Used in place.  M
*   TrimCrvList:     A list of trimming curves, used in place.		     M
...
* PARAMETERS:                                                                M
*   Srf:             Surface to make into a trimmed surface.  Used in place. M
*   TrimCrvList:     A list of trimming curves, as regular curves, used      M
*                    in place.						     M
*   HasTopLvlTrim:   Do we have a top level outer most trimming curve?	     M
*                                                                            *
* RETURN VALUE:                                                              M
	...
	for ( ; TrimCrvSegs != NULL; TrimCrvSegs = TrimCrvSegs -> Pnext) {
		...
		*UVCrv = TrimCrvSegs -> UVCrv;
		...
		case CAGD_PT_E1_TYPE:
		    IRIT_WARNING_MSG_PRINTF("Error: E1 or P1 trimming curves detected.  Can be fatal!\n");
		    return FALSE; /* Trim curves must have be at least 2D. */
		default:
		    break;
	    }
		...
		if (Pt[0] < UMin - IRIT_EPS ||
		    ...
		    Pt[1] < VMin - IRIT_EPS ||
		    Pt[1] > VMax + IRIT_EPS) {
		    /* Evaluate the curve at the node value, if non-linear. */
		    if (UVCrv -> Order > 2) {
		        CagdRType *R;

			if (Nodes == NULL)
			    Nodes = CagdCrvNodes(UVCrv);

			R = CagdCrvEval(UVCrv, Nodes[i]);
		         
			CagdCoerceToE2(Pt, &R, -1, UVCrv -> PType);

			if (Pt[0] < UMin - IRIT_EPS ||
			    Pt[0] > UMax + IRIT_EPS ||
			    Pt[1] < VMin - IRIT_EPS ||
			    Pt[1] > VMax + IRIT_EPS) {
			    /* Out of domain by too much - can not do much. */
			    IRIT_WARNING_MSG_PRINTF("Warning: trim curve's of order %d ctlpt %d out of domain detected.  Can be fatal!\n",
						    UVCrv -> Order, i);
			    return FALSE;
			}
		    }
		        ...
		        IRIT_WARNING_MSG_PRINTF("Warning: linear trim curve's, ctlpt %d, out of domain.  Likely to be fatal!\n", i);
			return FALSE;
		    }
		}
	    }
...
TrimSrfStruct *TrimSrfCopy(const TrimSrfStruct *TrimSrf)
{
	...
	*NewTrimSrf = (TrimSrfStruct *)
    ...
    NewTrimSrf -> Attr = IP_ATTR_COPY_ATTRS(TrimSrf -> Attr);
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Find the largest trimmed surface in the given list.  While there notion  M
* of 'largest' is not really well define, we simply use here the heuristics  M
* of largest == trimmed surface with longest trimming curves.                M
*                                                                            *
* PARAMETERS:                                                                M
*   TSrfs:     List of trimmed usrface to find the 'largest'.                M
*   Extract:   TRUE to extract and return the 'largest' trimmed surface from M
*              TSrfs,  FALSE to only find it.				     M
*                                                                            *
* RETURN VALUE:                                                              M
*   TrimSrfStruct *:  List of trimmed surface to search for 'largest'.       M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrimGetOuterTrimCrv, TrimGetFullDomainTrimCrv                            M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrimGetLargestTrimmedSrf                                                 M
*****************************************************************************/
TrimSrfStruct *TrimGetLargestTrimmedSrf(TrimSrfStruct **TSrfs, int Extract)
{
    ...
    for (TSrf = *TSrfs; TSrf != NULL; TSrf = TSrf -> Pnext) {
        CagdCrvStruct *Crv,
	    *Crvs = TrimGetTrimmingCurves(TSrf, TRUE, FALSE);

	for (Len = 0.0, Crv = Crvs; Crv != NULL; Crv = Crv -> Pnext)
	    Len += CagdCrvArcLenPoly(Crv);
    ...
    if (LargestTSrf != NULL && Extract) {
        if (LargestTSrf == *TSrfs)
	    *TSrfs = (*TSrfs) -> Pnext;
	else {
	    assert((*TSrfs) -> Pnext != NULL);
	    for (TSrf = *TSrfs; TSrf -> Pnext != NULL; TSrf = TSrf -> Pnext) {
	        if (TSrf -> Pnext == LargestTSrf)
		    break;
	    }
	    assert(TSrf -> Pnext != NULL);
	    TSrf -> Pnext = TSrf -> Pnext -> Pnext;
	}
    }
    return LargestTSrf;
}
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Returns the outer trimming curve found in TSrf.  If their is more than   M
* one outer curve, one of then is returned.				     M
*                                                                            *
* PARAMETERS:                                                                M
*   TSrf:  Trimmed surface to look for its outer loop.			     M
*                                                                            *
* RETURN VALUE:                                                              M
*   const TrimCrvSegStruct *:   Outer loop.                                  M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrimGetLargestTrimmedSrf, TrimGetFullDomainTrimCrv                       M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrimGetOuterTrimCrv                                                      M
*****************************************************************************/
const TrimCrvSegStruct *TrimGetOuterTrimCrv(const TrimSrfStruct *TSrf)
{
	...
	*TrimCrvList = TSrf -> TrimCrvList;
	...
	for (;
	    ...
	    CagdCrvBBox(TrimCrvSegList -> UVCrv, &BBox);
	    if (MinU > BBox.Min[0]) {
	        MinU = BBox.Min[0];
		OuterCrv = TrimCrvSegList;
	    }
	}
    }
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Identifies the outer loop of the trimmed surfaces and returns a          M
* reference to it if it covers all the tensor product surface domain.        M
*   A NULL is returned if outer loops is not entire tensor product domain.   M
*                                                                            *
* PARAMETERS:                                                                M
*   TSrf:  Trimmed surface to look for its outer full loop.		     M
*                                                                            *
* RETURN VALUE:                                                              M
*   const TrimCrvSegStruct *:  Return outer full loop if exists.             M
*                                                                            *
* SEE ALSO:                                                                  M
*   TrimGetLargestTrimmedSrf, TrimGetOuterTrimCrv                            M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrimGetFullDomainTrimCrv                                                 M
*****************************************************************************/
const TrimCrvSegStruct *TrimGetFullDomainTrimCrv(const TrimSrfStruct *TSrf)
{
    CagdRType UMin, UMax, VMin, VMax;
    const TrimCrvSegStruct
        *TCSeg = TrimGetOuterTrimCrv(TSrf);
    const CagdCrvStruct
        *UVCrv = TCSeg -> UVCrv;
    CagdRType
        * const *Points = UVCrv -> Points;

    if (UVCrv -> Order != 2 || UVCrv -> Length != 5)
        return NULL;

    CagdSrfDomain(TSrf -> Srf, &UMin, &UMax, &VMin, &VMax);
    if (IRIT_APX_EQ(Points[1][0], UMin) &&
	IRIT_APX_EQ(Points[2][0], VMin) &&
	IRIT_APX_EQ(Points[1][1], UMax) &&
	IRIT_APX_EQ(Points[2][1], VMin) &&
	IRIT_APX_EQ(Points[1][2], UMax) &&
	IRIT_APX_EQ(Points[2][2], VMax) &&
	IRIT_APX_EQ(Points[1][3], UMin) &&
	IRIT_APX_EQ(Points[2][3], VMax) &&
	IRIT_APX_EQ(Points[1][4], UMin) &&
	IRIT_APX_EQ(Points[2][4], VMin))
	return TCSeg;
    else
        return NULL;	  
}
