...
#define SUBDIV_TCRV_EPS_HIGHORDER	1e-10
#define SUBDIV_TCRV_EPS_LINEAR		1e-6
#define SUBDIV_PERTURB_PARAM		3.01060e-11

IRIT_STATIC_DATA CagdPtStruct
    *GlblPtList = NULL;

IRIT_STATIC_DATA CagdBType
    GlblForcePiecwiseLinearTrimCrvs = TRUE;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Controls the way future trimmed surfaces subdivisions are made:	     M
* If TRUE, all trimming curves are made piecewise linear as a side effect.   M
* If FALSE, trimming curves are left as is.			             M
*                                                                            M
* PARAMETERS:                                                                M
*   ForcePLCrvs: TRUE to make sure all trimming curves are piecewise linear. M
*                                                                            *
* RETURN VALUE:                                                              M
*   CagdBType:   Old state of this flag.                                     M
*                                                                            *
* SEE ALSO:                                                                  M
*                                                                            M
*                                                                            *
* KEYWORDS:                                                                  M
*   TrimSrfSubdivAtParamForcePiecwiseLinearTrimCrvs                          M
*****************************************************************************/
CagdBType TrimSrfSubdivAtParamForcePiecwiseLinearTrimCrvs(CagdBType
							          ForcePLCrvs)
{
    CagdBType
        OldVal = GlblForcePiecwiseLinearTrimCrvs;

    GlblForcePiecwiseLinearTrimCrvs = ForcePLCrvs;
    return OldVal;
}
				    ...
				    CagdSrfDirType Dir)
{
	...
	*Srf = TrimSrf -> Srf;
    ...
    assert(Dir == CAGD_CONST_U_DIR ? (t >= UMin && t <= UMax) : 
				     (t >= VMin && t <= VMax));
    ...
    if (GlblForcePiecwiseLinearTrimCrvs)
        TrimPiecewiseLinearTrimmingCurves(TrimSrf, FALSE);

    /* Coerce Bezier surfaces to B-spline so we do not have to scale the  */
...
* above the subdividing line in direction Dir at parameter t.		     M
*                                                                            *
* PARAMETERS:                                                                M
*   TrimCrvs:  To subdivide at the prescribed parameter value t.             M
*   t:         The parameter to subdivide the curve Crv at.                  M
*   Dir:       Direction of subdivision. Either U or V.                      M
	...
	for ( ; TrimCrvSeg != NULL; TrimCrvSeg = TrimCrvSeg -> Pnext) {
		...
		**Points = UVCrv -> Points;
	    ...
	    for (i = 0; i < UVCrv -> Length; i++)
	        if (IRIT_APX_EQ_EPS(Points[Axis + 1][i], t,
				    CAGD_MAX_DOMAIN_EPS * 100))
		    Points[Axis + 1][i] = t;
	}
    }
					...
					CagdRType t)
{
	...
	*Crvs = NULL;
	...
	CagdCoerceToE2(PtStart, Points, 0, Crv -> PType);
	CagdCoerceToE2(PtEnd, Points, Len1, Crv -> PType);

	if (IRIT_APX_UEQ(PtStart[Axis], t))
	    Inters[NumOfInters++] = PtStart[1 - Axis];
	if (IRIT_APX_UEQ(PtEnd[Axis], t))
	    Inters[NumOfInters++] = PtEnd[1 - Axis];
    }
	...
	if (!IRIT_APX_EQ_EPS(Inters[i], Inters[i + 1],
			     SUBDIV_TCRV_EPS_HIGHORDER)) {
			       ...
			       CagdRType *Inters)
{
	...
	OAxis = 1 - Axis;
    CagdBType
        LinearBSpline = CAGD_IS_BSPLINE_CRV(UVCrv) && UVCrv -> Order == 2;
    ...
    *UVCrv1 = *UVCrv2 = NULL;
	...
	/* Verify the curve is not too small. */
	CagdCrvDomain(Crv1, &TMin, &TMax);
	if (CagdCrvArcLenPoly(Crv1) < SUBDIV_TCRV_EPS_HIGHORDER ||
	    TMax - TMin < SUBDIV_TCRV_EPS_HIGHORDER) {
	    CagdCrvFree(Crv1);
	}
	else { /* Classify the segment. */
    ...
    /* Verify the curve is not too small. */
    CagdCrvDomain(UVCrv, &TMin, &TMax);
    if (CagdCrvArcLenPoly(UVCrv) < SUBDIV_TCRV_EPS_HIGHORDER ||
	TMax - TMin < SUBDIV_TCRV_EPS_HIGHORDER) {
        CagdCrvFree(UVCrv);
    }
    else { /* Classify the last segment. */
        if (ClassifyTrimCurve(UVCrv, t, Axis)) {
	    UVCrv -> Pnext = *UVCrv2;
	    *UVCrv2 = UVCrv;
	}
