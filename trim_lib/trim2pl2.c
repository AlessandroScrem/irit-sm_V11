...
static CagdPolygonStruct *CnvrtCellToPolygons2(TrimSrfStruct *TrimSrf,
					       ...
					       int IuMax,
					       int IvMax);
static CagdPolygonStruct *TrimGenCellRectangle(CagdBType ComputeNormals,
					       CagdBType ComputeUV,
					       const CagdRType *Pos0,
					       const CagdRType *Pos1,
					       const CagdRType *Pos2,
					       const CagdRType *Pos3,
					       const CagdRType *Nrml0,
					       const CagdRType *Nrml1,
					       const CagdRType *Nrml2,
					       const CagdRType *Nrml3,
					       const CagdRType *Uv0,
					       const CagdRType *Uv1,
					       const CagdRType *Uv2,
					       const CagdRType *Uv3);
					     ...
					     CagdBType ComputeUV)
{
	...
	*CagdPolygons = NULL;
...
#ifdef DEBUG_PRINT_TRIM_POLYS
    IPStderrObject(IPGenPOLYLINEObject(TrimPolys));        /* A mem leak... */
#endif /* DEBUG_PRINT_TRIM_POLYS */
    ...
    /* The first element should be NULL as it is outside boundary. */
    assert(IsoInters[0] == NULL);
    for (j = 0; j < FineNessV; j++) {
        IsoInters[j] = IsoInters[j + 1];
...
#   ifdef DEBUG_PRINT_SRF_POLYS
    IPStderrObject(IPGenPOLYLINEObject(
		         IPAllocPolygon(0, VHead, NULL))); /* A mem leak... */
#   endif /* DEBUG_PRINT_SRF_POLYS */
	...
	if (!SameU || !SameV) {
		...
		*VMid2 = IPAllocVertex2(NULL);
	    ...
	    /* Blend coordinates and make sure we are properly on boundary. */
...
* scans for cells with no trimming curves and merges neighboring cells, if   *
* possible.								     *
*                                                                            *
* PARAMETERS:                                                                *
*   TrimSrf:   processed trimmed surface.                                    *
*   Grid:          To process.                                               *
					       ...
					       int IvMax)
{
	...
	return TrimGenCellRectangle(Cell[0] -> Nrml != NULL,
				    Cell[0] -> Uv != NULL,
				    Cell[0] -> Pos,
				    Cell[1] -> Pos,
				    Cell[2] -> Pos,
...
* no trimming curves, is neither planar nor merged with neighboring cells.   *
*                                                                            *
* PARAMETERS:                                                                *
*   TrimSrf:   processed trimmed surface.                                    *
*   Grid:      To process.                                                   *
...
static CagdPolygonStruct *CnvrtCellToPolygons2(TrimSrfStruct *TrimSrf,
					       ...
					       int IvMax)
{
    IPPolygonStruct *Pll, *Pl,
        *InteriorPlls = NULL,
	*Plls = GMMergePolylines(Grid[Iv][Iu].Plls, IRIT_EPS);
    TrimBndrySortStruct *SInter, *SInter2,
	...
	*Plgs = NULL;
    ...
    if (Plls != NULL) {
        fprintf(stderr,
		"Detected trimming curves in domain [%d, %d] to [%d, %d]\n",
		Iu, Iv, Iu + 1, Iv + 1);
	IPStderrObject(IPGenPOLYLINEObject(Plls));          /* A mem leak... */
    }
#endif /* DEBUG_PRINT_SRF_POLYS */
    ...
    for (Pll = Plls; Pll != NULL; Pll = Pll -> Pnext) {
	    ...
	    *V2 = IPGetLastVrtx(V);
        ...
        /* Compute the starting/end point of the trimming curve. */
	SInter = (TrimBndrySortStruct *)
				      IritMalloc(sizeof(TrimBndrySortStruct));
	SInter2 = (TrimBndrySortStruct *)
				      IritMalloc(sizeof(TrimBndrySortStruct));

	/* Compute the two boundary location of the start/end point of the   */
	/* trimming curve - should be positive to be valid and both          */
	/* intersections should be different.				     */
	SInter -> Val = CnvrtToBndryIndex(V -> Coord, Iu, Iv);
        SInter2 -> Val = CnvrtToBndryIndex(V2 -> Coord, Iu, Iv);
	if (SInter -> Val >= 0.0 &&
	    SInter2 -> Val >= 0.0 &&
	    !IRIT_APX_EQ(SInter -> Val, SInter2 -> Val)) {
	    SInter -> V = V;
	    SInter -> StartOfCurve = TRUE;
	    SortedInterList = InsertSInterToList(SInter, SortedInterList);

	    SInter2 -> V = V;
	    SInter2 -> StartOfCurve = FALSE;
	    SortedInterList = InsertSInterToList(SInter2, SortedInterList);

	    Pll -> PVertex = NULL;
	}
	else {
	    if (SInter -> Val * SInter2 -> Val < 0.0) {
	        /* One points is on boundary and the other is not - error! */
#		ifdef DEBUG
...
#		endif /* DEBUG */
	        ...
	        /* Two interior end points - we have interior trim curve. */
	        Pl = IPAllocPolygon(0, V, NULL);

		IRIT_LIST_PUSH(Pl, InteriorPlls);
    ...
    if (InteriorPlls != NULL) {
        /* Need to process the interior polygons, but we simply ignore them */
        /* for now.  This will generate the WRONG result in low resolution! */
        IPFreePolygonList(InteriorPlls);
    }

    if (SortedInterList == NULL) { /* No bndry trimming curve in this cell. */
	...
	if (CornersOnBndryYetInside ||
	    ((Cell[0] -> Inside && !Cell[0] -> OnBndry) ||
	     (Cell[1] -> Inside && !Cell[1] -> OnBndry) ||
	     (Cell[2] -> Inside && !Cell[2] -> OnBndry) ||
	     (Cell[3] -> Inside && !Cell[3] -> OnBndry))) {
	    /* Cell is part of the valid domain of the trimmed surface. */
	    Plgs = TrimGenCellRectangle(Cell[0] -> Nrml != NULL,
					Cell[0] -> Uv != NULL,
					Cell[0] -> Pos,
					Cell[1] -> Pos,
					Cell[2] -> Pos,
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Generates a rectangle polygon for given four corners.                    *
*                                                                            *
* PARAMETERS:                                                                *
*   ComputeNormals:     If TRUE then use Nrml parameters, otherwise ignore.  *
*   ComputeUV:          If TRUE then use Uv parameters, otherwise ignore.    *
*   Pos1, Pos2, Pos3, Pos4:      Euclidean locations of vertices.            *
*   Nrml1, Nrml2, Nrml3, Nrml4:  Optional Normals of vertices.		     *
*   Uv1, Uv2, Uv3, Uv4:          Optional UV param. vals of vertices.	     *
*                                                                            *
* RETURN VALUE:                                                              *
*   IPPolygonStruct *:    Generated polygon                                  *
*****************************************************************************/
static CagdPolygonStruct *TrimGenCellRectangle(CagdBType ComputeNormals,
					       CagdBType ComputeUV,
					       const CagdRType *Pos0,
					       ...
					       const CagdRType *Uv3)
{
    CagdPolygonStruct *Plgs,
        *Plg = NULL;
    int GenPoly = FALSE;

#ifdef DEBUG_PRINT_TRIM_POLYS
    {
	fprintf(stderr, "Dumping out rectangle:\n");
	fprintf(stderr, "\t[%9.6f  %9.6f  %9.6f]\n",
		Pos0[0], Pos0[1], Pos0[2]);
	fprintf(stderr, "\t[%9.6f  %9.6f  %9.6f]\n",
		Pos1[0], Pos1[1], Pos1[2]);
	fprintf(stderr, "\t[%9.6f  %9.6f  %9.6f]\n",
		Pos2[0], Pos2[1], Pos3[2]);
	fprintf(stderr, "\t[%9.6f  %9.6f  %9.6f]\n",
    ...
    /* Create one planar polygon for all four corners. */
    if (GMCoplanar4Pts(Pos0, Pos1, Pos2, Pos3))
        Plg = _CagdSrfMakeRectFunc(ComputeNormals, ComputeUV,
				   Pos0, Pos1, Pos2, Pos3,
				   Nrml0, Nrml1, Nrml2, Nrml3,
				   Uv0, Uv1, Uv2, Uv3,
				   &GenPoly);
    if (GenPoly)
        return Plg;

    Plg = _CagdSrfMakeTriFunc(ComputeNormals, ComputeUV,
			      Pos0, Pos1, Pos2,
			      Nrml0, Nrml1, Nrml2,
			      Uv0, Uv1, Uv2,
			      &GenPoly);
    Plgs = _CagdSrfMakeTriFunc(ComputeNormals, ComputeUV,
			       Pos0, Pos2, Pos3,
			       Nrml0, Nrml2, Nrml3,
			       Uv0, Uv2, Uv3,
			       &GenPoly);
				...
				int Iv)
{
	...
	if (Len == 3)
	    ...
	    TrimGenCellRectangle(Cells[0] -> Nrml != NULL,
				 Cells[0] -> Uv != NULL,
				 CagdPl.U.Polygon[0].Pt,
				 CagdPl.U.Polygon[1].Pt,
				 CagdPl.U.Polygon[2].Pt,
...
/*****************************************************************************
* DESCRIPTION:                                                               *
*   Insert one trimming boundary item into its sorted list and return the    *
* updated list.                                                              *
*                                                                            *
* PARAMETERS:                                                                *
