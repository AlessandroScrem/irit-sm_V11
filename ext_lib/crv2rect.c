				 ...
				 const char *Name)
{
	...
	*ErrorMsg = "Crv2RectRgns: Edge size limit split failed.";
	...
	GMPolyMeshSmoothing(Os, NULL, FALSE, 0.0, NumSmoothingSteps, 1.0); 
...
*                                                                            *
* RETURN VALUE:                                                              *
*   IPPolygonStruct *: Return list of polygon after splitting.		     *
*****************************************************************************/
static IPPolygonStruct *IrtExtC2SConvertPolysToRectangles(IPPolygonStruct
							                *Pls)
{
						...
						IrtRType Side)
{
	...
	*V2 = VB2 -> Pnext;
	...
	!IRIT_APX_EQ(Len, 0.0) &&
	!GMIsInterLinePolygon2D(V1, V1 -> Coord, V2 -> Coord, NULL)) {
	IrtRType ValOrtho;
	...
	*Pl = PVIdx -> PObj -> U.Pl;
	...
	do {
		...
		AttrSetIntAttrib(&V -> Attr, "IrtExtC2SUniqueEdgeId", 
								    UniqueId);
		OtherV = IPCnvFindAdjacentEdge(PVIdx, 
