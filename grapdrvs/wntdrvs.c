		     ...
		     int nShow)
{
	...
	wndClass.lpfnWndProc   = (WNDPROC) SubViewWndProc;
	wndClass.hIcon         = NULL;
	wndClass.hbrBackground = CreateSolidBrush(IGBackGroundColor);
	...
	wndClass.lpfnWndProc   = (WNDPROC) TransWndProc;
		      ...
		      LONG lParam)
{
	...
	case WM_NCACTIVATE:
        ...
        default:
	  return (LONG) DefWindowProc(hWndFrame, wMsg, wParam, lParam);
    }
    return 0;
}
			   ...
			   LONG lParam)
{
	...
	case WM_RBUTTONDOWN:
	    ...
	    bShiftDown = (BOOL) (wParam & MK_SHIFT);
...
static void DrawTextLocal(char *Str, int PosX, int PosY)
{
  TextOut(IGCurrenthDC, PosX, PosY, Str, (int) strlen(Str));
}
