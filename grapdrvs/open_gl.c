...
/*	glActiveTextureARB(GL_TEXTURE0_ARB); */
	...
	IrtBType *Pwr2Image,
	    *Image = AttrGetObjectRefPtrAttrib(PObj, "_ImageTexture");
	int PwrX, PwrY,
...
#pragma message("Warning: gluScaleImage is not supported by this compiler and is bypassed.")
	...
	/* Get and update a unique texture ID of this texture. */
	glEnable(GL_TEXTURE_2D);
	GlblTextureActive = TRUE;
...
/*****************************************************************************
* DESCRIPTION:                                                               M
*   Prepares the movie texture mapping function of an object.                M
*                                                                            *
* PARAMETERS:                                                                M
*   PObj:    Object to apply texture movie to.                               M
*                                                                            *
* RETURN VALUE:                                                              M
*   int:    TRUE if successful, FALSE otherwise.                             M
*                                                                            *
...
*   IGSetMovie                                                               M
*****************************************************************************/
int IGSetMovie(IPObjectStruct *PObj)
{
  int *TexIDs;
    ...
    if ((TexIDs = AttrGetObjectRefPtrAttrib(PObj, "_MovieTexIDs")) > 0) {
        int n, MovieIdx;
	IrtRType RelT;
	IPObjectStruct *MovieTimes;			  /* TMin, TMax, Dt. */

	glEnable(GL_TEXTURE_2D);
	GlblTextureActive = TRUE;
#ifdef IRIT_HAVE_OGL_CG_LIBg
/*	glActiveTextureARB(GL_TEXTURE0_ARB); */
#endif /* IRIT_HAVE_OGL_CG_LIB */

	if (!IGGlblAnimation) {           /* IF no animation then no movies. */
	    glBindTexture(GL_TEXTURE_2D, TexIDs[0]);
	    AttrFreeObjectAttribute(PObj, "_MovieIdx");
	    return TRUE;
	}

	n = AttrGetObjectIntAttrib(PObj, "_MovieLength");

	MovieTimes = AttrGetObjectObjAttrib(PObj, "_MovieTimes");
	if (MovieTimes != NULL)
	    RelT = (IGAnimation.RunTime - MovieTimes -> U.Pt[0]) /
	           (MovieTimes -> U.Pt[1] - MovieTimes -> U.Pt[0]);
	else
	    RelT = (IGAnimation.RunTime - IGAnimation.StartT) /
	           (IGAnimation.FinalT - IGAnimation.StartT);

	if (AttrGetObjectIntAttrib(PObj, "_MovieRestart") == TRUE)
	    RelT -= ((int) RelT);
	if (RelT < 0.0)
	    RelT += 1.0;	       /* If was negative, it is off by one. */

	MovieIdx = (int) (RelT * (n - 1) + 0.5);
	MovieIdx = IRIT_BOUND(MovieIdx, 0, n - 1);

	glBindTexture(GL_TEXTURE_2D, TexIDs[MovieIdx]); 

      	return TRUE;
    }
    else {
        IrtBType *Pwr2Image, *Image,
	    **Movie = AttrGetObjectRefPtrAttrib(PObj, "_MovieTexture");
	int PwrX, PwrY,
	    i, n,
	    Width = AttrGetObjectIntAttrib(PObj, "_MovieWidth"),
	    Height = AttrGetObjectIntAttrib(PObj, "_MovieHeight"),
	    Alpha = AttrGetObjectIntAttrib(PObj, "_MovieAlpha");

	if (Movie == NULL)
	    return FALSE;

	/* Count how many images we have in the movie and allocate an      */
	/* array to hold these images' Open GL texture IDs.                */
	for (n = 0; (Image = Movie[n]) != NULL; n++);
	AttrSetObjectIntAttrib(PObj, "_MovieLength", n);

	TexIDs = (int *) IritMalloc(sizeof(int) * (n + 1));
	glGenTextures(n, (GLuint *) TexIDs);
        TexIDs[n] = -1; /* Close the list of IDs. */

	for (i = 0; (Image = Movie[i]) != NULL; i++) {
	    /* Find the smallest power of two larger/equal to both axes. */
	    for (PwrX = 1; PwrX <= Width; PwrX <<= 1);
	    if (PwrX != Width)
	        PwrX >>= 1;
...
#pragma message("Warning: gluScaleImage is not supported by this compiler and is bypassed.")
	    ...
	    /* Update the unique texture ID of this texture. */
	    glEnable(GL_TEXTURE_2D);
	    GlblTextureActive = TRUE;
	...
	AttrSetObjectRefPtrAttrib(PObj, "_MovieTexIDs", TexIDs);
