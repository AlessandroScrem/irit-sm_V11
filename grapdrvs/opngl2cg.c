...
static int IGCreateFFDTexture(IPObjectStruct *DefObj, IPObjectStruct *TvObj)
{
    ...
    AttrSetObjectRefPtrAttrib(TvObj, "_ffd_def_obj", DefObj); 
...
static int IGInitFfdDraw(IPObjectStruct *PObj)
{
    ...
    AttrSetObjectRefPtrAttrib(PObj, "_const_samples", ConstOffset);
#ifdef DEBUG_CG_FFD
...
static int RenderShaderSinglePhase(IPObjectStruct *PObj)
{
    ...
    if (DefObj == NULL)
	DefObj = (IPObjectStruct*)AttrGetObjectRefPtrAttrib(PObj, "_ffd_def_obj");
...
static int RenderShaderDoublePhase(IPObjectStruct *PObj)
{
    ...
    ConstOffset = AttrGetObjectRefPtrAttrib(PObj, "_const_samples");
    if (ConstOffset) {
...
static int PreProcessAnimation(IPObjectStruct * PObj)
{
	...
	*AnimObjList = NULL;
    ...
    DefObj = AttrGetObjectRefPtrAttrib(PObj, "_ffd_def_obj");
    if (DefObj == NULL)
	return FALSE;
    
    if ((AnimObjList = (IPObjectStruct*) 
	AttrGetRefPtrAttrib(PObj -> Attr, "_anim_object_list")) == NULL) {
	...
	AttrSetRefPtrAttrib(&PObj -> Attr, "_anim_object_list", AnimObjList);
	AttrSetRefPtrAttrib(&PObj -> Attr, "_anim_offset", AnimOffset);
...
static IPObjectStruct *SingleStepAnimation(IPObjectStruct * PObj)
{
    ...
    if ((AnimObjList = AttrGetObjectRefPtrAttrib(PObj,
					      "_anim_object_list")) == NULL) {
	return NULL;
    }

    if ((AnimOffset = AttrGetObjectRefPtrAttrib(PObj, "_anim_offset")) == NULL) {
	return NULL;
    }
...
static int IGCheckSysStatus(IPObjectStruct *PObj, int ForceRecompute)
{
	...
	if (CurrState == State || Init) {
	    ...
	    AttrSetObjectIntAttrib(PObj, "State", State);
	    RecomputeLists(PObj);
	    if ((DefObj = AttrGetObjectRefPtrAttrib(PObj,
						 "_ffd_def_obj")) != NULL)
...
static void SetControlPoint(IPObjectStruct *PObj)
{
	...
	*Tv = NULL;
    ...
    TexId = AttrGetObjectIntAttrib(PObj, "_ffd_tex_id");
    TvObj = (IPObjectStruct*)AttrGetObjectRefPtrAttrib(PObj, "_ffd_triv");
    Tv = TvObj -> U.Trivars;
