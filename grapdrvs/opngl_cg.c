...
#include <gl/glext.h>
#include "grap_loc.h"
#include "inc_irit/opngl_cg.h"
#include "inc_irit/time.h"
...
static int IGCreateDdmDisplayList(IPObjectStruct *PObj)
{
	...
	**DdmHandleList = NULL;
    ...
    if ((DdmHandleList = AttrGetObjectRefPtrAttrib(PObj, "_DdmHandleList")) != NULL
	    && DdmHandleList[0][0] > 0)
	return TRUE;
	...
	ObjPtrList = AttrGetObjectRefPtrAttrib(PObj, "_ObjPtrList");
	if (ObjPtrList == NULL){
	    ObjPtrList = IritMalloc(NumFiles + 1 * sizeof(IPObjectStruct*));
	    for (i = 0; i < NumFiles + 1; i++)
		ObjPtrList[i] = NULL;
	    AttrSetObjectRefPtrAttrib(PObj, "_ObjPtrList", ObjPtrList);
	}
	...
	if (ObjPtrList[0] != NULL) {
		...
		AttrSetObjectRefPtrAttrib(PObj, "_DdmHandleList", DdmHandleList);
		for (i = 0; i < (NumFiles + 1); i++)
...
void IGCGFreeDTexture(IPObjectStruct *PObj)
{
    ...
    DdmHandleList = AttrGetObjectRefPtrAttrib(PObj, "_DdmHandleList");
...
static int IGSetVertexTexture(IPObjectStruct *PObj, IPObjectStruct *PObj2)
{
	...
	*P2 = PObj2 -> U.Pl;
	...
	for ( ; V1 != NULL, V2 != NULL; V1 = V1 -> Pnext, V2 = V2 -> Pnext) {
	    ...
	    AttrSetRefPtrAttrib(&V1 -> Attr, "_CompatibleVertex", V2);
	}
    }
    return TRUE;
}
...
static int IGSetTextureCoord(IPVertexStruct *V)
{
    ...
    if ((V2 = (IPVertexStruct *) AttrGetRefPtrAttrib(V -> Attr,
...
static int IGInitDTexture(IPObjectStruct *PObj)
{
	...
	*TextureFiles = AttrGetObjectStrAttrib(PObj, "DTextureFiles");
    ...
    if ((DdmHandleList =
	   AttrGetObjectRefPtrAttrib(PObj, "_DdmHandleList")) != NULL &&
	   DdmHandleList[0][0] > 0) 
	return TRUE;
...
int IGCGDrawDTexture(IPObjectStruct *PObj)
{
	...
	IPObjectStruct** ObjPtrList = 
	    AttrGetObjectRefPtrAttrib(PObj, "_ObjPtrList");
    ...
    TileMap = (char *) AttrGetRefPtrAttrib(PObj -> Attr, "_TileDecisionMap");
			   ...
			   int YLen)
{
    ...
    AttrSetRefPtrAttrib(&PObj -> Attr, "_TileDecisionMap", TileMap);
}
