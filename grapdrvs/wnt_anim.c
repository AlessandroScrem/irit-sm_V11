...
#ifdef _WIN64
    SetWindowLongPtr(Button, GWLP_USERDATA, (LONG_PTR) Bitmap);
#else
...
{
    LONG Dummy,
	Value = (LONG) SendMessage(Scale, SBM_GETPOS, 0, 0);
    SCROLLINFO LPSi;
		    ...
		    PVOID ClientData)
{
    ...
    return (int) DialogBoxParam(NULL, 
					   ...
					   LPARAM lParam)
{
...
#ifdef _WIN64
    WNDPROC
	WndProc = (WNDPROC) GetClassLongPtr(hWnd, GCLP_WNDPROC);
#else
    WNDPROC
	...
	case WM_DESTROY:
#	    ifdef _WIN64
		DeleteBitmap(GetWindowLongPtr(hWnd, GWLP_USERDATA));
#	    else
