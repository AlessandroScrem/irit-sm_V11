...
static void BoolPropObjectID(IPObjectStruct *PObj)
{
    ...
    ID = AttrGetIntAttrib2(PObj -> Attr, AttrIDID);
	...
	while (Pl) {
	    AttrSetIntAttrib2(&Pl -> Attr, AttrIDID, ID);
...
{
    IRIT_STATIC_DATA AttribNumType /* Persistent store to unique Attrs IDs. */
        AttrPerturbedID = ATTRIB_NAME_BAD_NUMBER;
    IPObjectStruct *PObj;

    IP_ATTR_INIT_UNIQUE_ID_NUM(AttrPerturbedID, "_Perturbed");
	...
	if (setjmp(LclLongJumpBuffer) == 0) { /* Its the setjmp itself call! */
		...
		if (PObj1 -> U.Pl == NULL)
		    PObj = IPCopyObject(NULL, PObj2, FALSE);
			 ...
			 AttrGetObjectIntAttrib2(PObj2, AttrPerturbedID) != TRUE) {
		    ...
		    AttrSetObjectIntAttrib2(PObj2Perturbed, AttrPerturbedID, TRUE);
		    ...
		    if (AttrGetObjectIntAttrib2(PObj2, AttrPerturbedID) != TRUE) {
